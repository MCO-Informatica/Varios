...
User Function relfat02()
	Local oReport := nil
	Local cPerg:= Padr("relfat02",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Relatório de Notas Emitidas por Contrato (Geral)",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	oSection2:= TRSection():New(oReport, "Relatório de Notas Emitidas por Contrato (Geral)", {"SD2"}, NIL, .F., .T.)
...
Static Function ReportPrint(oReport)
	...
	cQuery += " D2_CLIENTE    <= '" + MV_PAR06 + "' AND  "
  	cQuery += "	SD2010.D_E_L_E_T_ <> '*' GROUP BY D2_DOC, D2_SERIE, D2_CF, D2_ITEMCC, D2_EMISSAO, D2_CLIENTE, A1_NOME,CTD_XVDCI,CTD_XVDSI ORDER BY 1, 2"
