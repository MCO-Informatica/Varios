...
Static Function RptDef(cNome)
	...
	TRCell():New(oSection2,"TMP_DOC"	,"TRBDC","Doc.Entr."		   		,"@!"				,10)  
	TRCell():New(oSection2,"TMP_OC"		,"TRBDC","OC"		   		,"@!"				,10)
...
Static Function ReportPrint(oReport)
	...
	// -- SELECT DE NF EMITIDAS REMOVENDO NFS CANCELADAS E DEVOLUCAO
	 cQuery := "SELECT D1_ITEMCTA AS 'TMP_CONTRATO', D1_DOC AS 'TMP_DOC', D1_SERIE AS 'TMP_SERIE',  D1_CF AS 'TMP_CFOP',  "  
	 cQuery += "	   CAST(D1_EMISSAO AS DATE) AS 'TMP_EMISSAO', D1_FORNECE AS 'TMP_FORNECE', A2_NOME AS 'TMP_NOME', " 
	 ...
	 cQuery += "		NOT IN (SELECT REPLACE(D2_NFORI+D2_SERIORI+D2_CLIENTE+D2_LOJA+D2_COD,' ','') FROM SD2010 WHERE D2_TIPO = 'D' AND D_E_L_E_T_ <> '*')  AND "
  	 cQuery += "		SD1010.D_E_L_E_T_ <> '*' AND SA2010.D_E_L_E_T_ <> '*' AND "
	 ...
	 cQuery += " D1_DOC    		<= '" + MV_PAR08 + "' "
  	 cQuery += "		GROUP BY D1_DOC, D1_SERIE, D1_CF, D1_ITEMCTA, D1_EMISSAO, D1_FORNECE, A2_NOME, A2_COD, A2_LOJA,D1_PEDIDO ORDER BY  D1_ITEMCTA, D1_EMISSAO, D1_DOC "
