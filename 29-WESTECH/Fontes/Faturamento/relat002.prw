...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Relatório Pedidos de Compra - Contrato x Fornecedor",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	TRCell():New(oSection1,"TMP_NOME"  ,	"TRBOC",	"Fornecedor"	,"@!"	,	80)
	...
	TRCell():New(oSection2,"TMP_GRUPO"	,"TRBOC","Grupo"			,""					,06)
	...
	oBreak1 := TRBreak():New(oSection2,{|| (TRBOC->TMP_FORNECE) },"Subtotal:",.F.)
...
Static Function ReportPrint(oReport)
	...
	cQuery += "	CAST(C7_EMISSAO as Date) AS 'TMP_EMISSAO', CAST(C7_DATPRF AS DATE) AS 'TMP_ENTREGA', C7_FORNECE AS 'TMP_FORNECE', SA2010.A2_NOME AS 'TMP_NOME', B1_GRUPO AS 'TMP_GRUPO', BM_DESC AS 'TMP_NGRUPO', " 
	cQuery += "	CASE "
    ...
    cQuery += "	GROUP BY C7_NUM, C7_ITEMCTA, C7_FORNECE, A2_NOME, C7_EMISSAO, B1_GRUPO, BM_DESC, C7_ENCER, C7_EMISSAO, C7_DATPRF, C7_MOEDA, M2_MOEDA2 "
    cQuery += "	ORDER BY 7, 1, 2 "
	...
	While !Eof()
		...
		cNcm 	:= TRBOC->TMP_FORNECE
		
		
		IncProc("Imprimindo OC "+alltrim(TRBOC->TMP_FORNECE))
		...
		While TRBOC->TMP_FORNECE == cNcm 
			...
			oSection2:Cell("TMP_GRUPO"):SetValue(TRBOC->TMP_GRUPO)
