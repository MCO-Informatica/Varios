...
*/
	...
	if MV_PAR10 = 1 .AND. QUERY->CTD_DTEXSF < DDATABASE
		QUERY->(dbskip())
		Loop
	endif
		
	if MV_PAR10 = 2 .AND. QUERY->CTD_DTEXSF >= DDATABASE
		QUERY->(dbskip())
		Loop
	endif
	...
	//if alltrim(QUERY->E5_TIPODOC) <> "CH"
		...
		*/
		...
		TRB1->VALOR		:= nTotSC7 + nTotSD1 + nTotSDE + nTotSE2 + nTotCV4 + nTotSZ4 + nTotZZA
		TRB1->COGS		:= nTotSC7 + nTotSD1 + nTotSDE + nTotSE2 + nTotCV4 + nTotSZ4 + nTotZZA
...
aadd(aHeader, {"COGS"				,"COGS"		,"@E 999,999,999.99",15,2,"","","N","TRB1","R"})
...
static function VALIDPERG()
	...
	putSX1(cPerg, "10", "Contratos?"					, "", "", "mv_ch10", "N", 01, 0, 0, "C", "", "", "", "", "mv_par10","Abertos","","","","Fechados","","","","Todos","","","","","","","")
