...
//Envia E-mail
User Function zCPCEnvM(cTo, cCc, cAssunto, cMsg, aAnexo)
    local _cQuery := ""
    ...
    //******************* base de dados
    _cQuery2 := " SELECT	CTD_XIDPM "
    _cQuery2 += " FROM SE2010  " 
    _cQuery2 += " INNER JOIN CTD010 ON SE2010.E2_XXIC=CTD010.CTD_ITEM " 
    _cQuery2 += " WHERE  SE2010.D_E_L_E_T_ <> '*'  AND CTD010.D_E_L_E_T_ <> '*' AND E2_SALDO > 0 AND E2_VENCTO < GETDATE() AND E2_TIPO = 'PR' "  
    _cQuery2 += " AND E2_XXIC NOT IN ('ADMINISTRACAO','PROPOSTA','QUALIDADE','ATIVO','ENGENHARIA','ZZZZZZZZZZZZZ','XXXXXX','ESTOQUE') "   
    _cQuery2 += " AND E2_XXIC NOT IN  (SELECT E2_XXIC FROM SE2010 WHERE E2_XXIC = ' ') "
    _cQuery2 += " GROUP BY CTD_XIDPM ORDER BY 1 "
    ...
    nI := 1
    while QUERY2->(!eof()) //for nI=1 to Len( "_cQuery" )
        cXIDPM := alltrim(QUERY2->CTD_XIDPM) //Posicione("CTD",1,xFilial("CTD")+QUERY2->E2_XXIC,"CTD_XIDPM")

        /**********/
        cEmail := UsrRetMail(cXIDPM) //UsrRetMail(aMatriz2[nI2])
        cIdCoord := Alltrim(cXIDPM) //  Alltrim(aMatriz2[nI2])
        ...
        _cQuery += " AND E2_XXIC NOT IN  (SELECT E2_XXIC FROM SE2010 WHERE E2_XXIC = ' ') AND "
        ...
        AAdd( aMatriz, cIdCoord ,nI ) // Posicione("CTD",1,xFilial("CTD") +  ALLTRIM(aCols[nI][3]),"CTD_XIDPM")
...
/*
        for nI2=1 to len(aMatriz2) 
            if cIdCoord == aMatriz2[nI2]
                alert("fim")
                RETURN .F.
            ENDIF
            alert(cIdCoord + " == " + aMatriz2[nI2])
