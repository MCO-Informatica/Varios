...
//Envia E-mail
User Function zEvRejOC(cTo, cCc, cAssunto, cMsg, aAnexo)
	Local cNum			:= TRB1->NUM
...
for nI=1 to Len( "TRB1" )
	AAdd( aMatriz, Posicione("CTD",1,xFilial("CTD") +  ALLTRIM(TRB1->ITEMCONTA),"CTD_XIDPM") ,nI ) // Posicione("CTD",1,xFilial("CTD") +  ALLTRIM(aCols[nI][3]),"CTD_XIDPM")
...
//////////////////////////////////////////
	...
	cMsg+='	<div id="titulo">'
	cMsg+='				<h1 class="titulo-topo"> PEDIDO DE COMPRA REJEITADO</h1>'
	cMsg+='				<h4 class="titulo-topo"> por ' + AllTrim(UsrFullName(Posicione("SC7",1,xFilial("SC7") + cNum,"C7_XAPRN1"))) + '</h4>'
	cMsg+='	</div>'
	...
	cMsg+='				<tr>'
	cMsg+='					<td> <b> Motivo: </b>'  + Posicione("SC7",1,xFilial("SC7") + cNum,"C7_XREJEIT") + '</td>'
	cMsg+='				</tr>'
	cMsg+='			</table>'
	...
	cMsg+='							REQUER APROVACAO <br>'
	cMsg+='							DA DIRETORIA'
				elseif EMPTY(SC7->C7_XAPRN2) .AND. nTotalCom > 1500.AND. alltrim(SC7->C7_XCTRVB) <> "4" // cRequer = "1"
	cMsg+='							REQUER APROVACAO <br>'
	cMsg+='							DA DIRETORIA'
	...
	for nI=1 to len(aMatriz2) 
		...
		//oMessage:cBcc             := ""
		oMessage:cSubject           := "Aprovação Pedido de Compra pelo coordenador" //cAssunto
		oMessage:cBody              := cMsg
...
//Desconecta do servidor
	...
	msginfo('Aviso de pedido REJEITADO enviado paro o e-mail ' + cEmail + '.','Westech')
