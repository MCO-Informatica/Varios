...
user function MT120OK()
		...
		if SC7->( dbSeek(xFilial("SC7")+cNum) ) // == .F. .OR. Empty(cNum)
			...
			if !empty(dDTIso) .AND. dDTIso < dDataBase
				Alert("Fornecedor " + alltrim(cFornece) + " - " + cDFornece + " está com data do ISO vencida (" + dtoc(dDTIso) + "). Pedido de Compra não pode ser cadastrado, atualize o data do certificado ISO.")
				return .F. 
			elseif  !empty(dDTIso) .AND. dDTIso >= dDataBase .AND. dDTIso <= dDataBase+15
				msgalert("Certificado ISO vencerá em " + cValToChar(dDTIso-dDataBase) + " dias. Por favor providenciar atualização do certificado ISO")
			endif
		endif
						...
						if CTD->( dbSeek( xFilial("CTD")+cItemICM) )
							 ...
							 //if round(nTotal,2) > round(nTotalOCA,2)
								 ...
								 if  round(nTotACPR,2) > round(nTotCUPR,2)
								 	...
								 	"Total Verba" + cValToChar(Transform(CTD->CTD_XVBAD+nTotCUPR,"@E 999,999,999.99")) + CRLF + CRLF +;
								 	"Custo Atual s/ Tributos: " + cValToChar(Transform(CTD->CTD_XACPR,"@E 999,999,999.99")) + CRLF + CRLF +  ;
								 	"Alteracao da OC está adicionando o valor de: " + cValToChar(Transform(nTotal-nTotalOLD,"@E 999,999,999.99")) +  CRLF + CRLF + "Ordem de Compra nï¿½o pode ser salva. " )
