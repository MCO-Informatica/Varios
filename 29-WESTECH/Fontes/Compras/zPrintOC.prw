	...
	For nP := 1 to len(aDados)
		...
		If lEmpCab
			...
			// Numero de Pagina / Paginas
		   	...
		   	oPrint:Say  (0360,3000,"Página " + Transform(StrZero(ncont,3),"") ,oFont8) // + " de " + Transform(StrZero(ncont1,3),"") ****
		...
		If nLinAtu <= nLinMax
			...
			IF MV_PAR12 = 1
				cProdIngles		:= Posicione("SB1",1,xFilial("SB1") + aDados[nP,7],"B1_XXDI") // descrição em ingles
...
IF MV_PAR12 == 2
		...
		oPrint:Box	(0050,0050,0510,3300) //Box Cabeça
		...
		//oPrint:Say  ( 0370,0060,"E-mail NF Eletrônica: notafiscal@westech.com.br ",oFont8n)

		// Ordem de Compra
		oPrint:Box	(0050,0740,0190,1340) // Titulo Pedido
		oPrint:Say  (0070,0800,"ORDEM DE COMPRA  ",oFont14)
		oPrint:Say  (0120,0900,"Nº " + cNumPed,oFont14)

		oPrint:Box	(0190,0740,0350,1050) // Data Emissao
		oPrint:Say  (0220,0780,"Data Emissão " ,oFont12)
		oPrint:Say  (0280,0780,DTOC(dDataEmi) ,oFont12)


		oPrint:Box	(0190,1050,0350,1340) // Revisao
		oPrint:Say  (0220,1100,"Revisão: Nº: " + Transform(nRevisao, "@E 999"),oFont9)
		If EMPTY(dRevisao)
		...
		//oPrint:Say  (0280,1090,"Nº: " + Transform(nRevisao, "@E 999"),oFont8n)
		...
		oPrint:Say  (0195,2410,"Local de Cobrança: ",oFont9n)
		oPrint:Say  (0235,2410, ALLTRIM( aDadosEmp[2]) ,oFont8)
		oPrint:Say  (0275,2410, ALLTRIM( aDadosEmp[3]) + " - " + ALLTRIM( aDadosEmp[4]),oFont8)
		oPrint:Say  (0315,2410,"E-mail NF Eletrônica: notafiscal@westech.com.br ",oFont8n)

		oPrint:Box	(0350,0740,0410,3300) // Local de Cobranca
		oPrint:Say  (0360,0790,"Autorizamos o fornecimento dos seguintes materiais / serviços, conforme condições estabelecidas nesta ordem de compra e seus anexos. ",oFont9n)

		oPrint:Box	(0350,2900,0410,3300) // Numero Pagina
		//oPrint:Say  (0360,2920,"Página",oFont9)
		...
		oPrint:Say  (0420,2350,"Preços com Impostos",oFont7n)
		oPrint:Box	(0410,2700,0460,2960)
		oPrint:Say  (0420,2730,"Impostos Inclusos",oFont7n)

		oPrint:Box	(2050,0050,2120,3300) // Cond. pag
		//oPrint:Say  (2020,1510,"Condição de Pagamento: ",oFont9n)
		oPrint:Say  (2060,0070,"Condição de Pagamento: " + cCondPagto, oFont9n,,,0)

		oPrint:Box	(1370,2400,1480,3300)
		oPrint:Say  (1400,2420,"SubTotal página " + varSimb + " : ",oFont9)
		...
		oPrint:Say  (0470,3130,"Item Contábil",oFont7n)
		...
		IF EMPTY(cFormula2) .AND. EMPTY(cXNotas)
			...
			oPrint:Say  (1410,0070,"As Condições Gerais de Compras - Anexo 3 - PQ-90-0784 revisão 05 - são parte integrante desta Ordem de compra: ",oFont7a)
			oPrint:Say  (1450,0070,"A Ordem de Compra e as Condições Gerais de Compra deverão ser assinadas e devolvidas em até três dias. A partir deste prazo serão considerados aprovadas.",oFont7a)
			oPrint:Say  (1490,0070,"Não serão aceitas notas fiscais de recebimento de materiais sem que nela constem número da Ordem de Compra.",oFont7a)
			oPrint:Say  (1530,0070,"A Westech se reserva o direito de efetuar testes na fábrica do fornecedor antes da liberação para entrega. ",oFont7a)
			oPrint:Say  (1570,0070,"A penalidadade por atraso de entrega será de 0,3% ao dia com teto máximo de 10%. Os valores correspondente serão glosados do pagamento a ser feito. ",oFont7a)
			oPrint:Say  (1610,0070,"Os preço informados incluem ICMS, PIS e COFINS.",oFont7a)
			oPrint:Say  (1650,0070,"Os pagamentos serão feitos através de depósito bancário.",oFont7a)
			oPrint:Say  (1690,0070,"Material destinado a industrialização.",oFont7a)
			...
			oPrint:Say  (1810,0070,"A Westech não aceita emissão de boletos para pagamentos, bem como, não aceita negociação de duplicata com terceiros.",oFont7an)
			oPrint:Say  (1850,0070,"Fornecer uma via física do Certificado de Matéria Prima / Procedência junto com envio do produto e uma via eletrônica (e-mail) junto com nota fiscal.",oFont7an)
		...
		oPrint:Say  (2130,0650,"Emissão",oFont8n)
		
				
		oPrint:Box	(2120,1290,2170,2450)
		oPrint:Say  (2130,1820,"Aprovação",oFont8n)
		
		oPrint:Box	(2120,2450,2170,3300)
		oPrint:Say  (2130,2560,"Aceitação desta Ordem de Compra Pelo Fornecedor",oFont9n)
		...
		oPrint:Box	(2170,1290,2350,1870) // Gerência
		if  EMPTY(SC7->C7_XAPRN1) .and. SC7->C7_ITEMCTA <> 'ADMINISTRACAO'//.OR. alltrim(SC7->C7_XCTRVB) <> "4" // 
			oPrint:Say  (2210,1300,"REQUER APROVAÇÃO",oFont20)
		...
		if nTotalCom > 1000 .AND. cRequer = "2" .and.  ALLTRIM(SC7->C7_XAPRN1) $ ("000071/000076/000078/000046")
				oPrint:Say  (2210,1950,"REQUER APROVAÇÃO",oFont20)
...
ELSE
		...
		oPrint:Box	(0190,1050,0350,1340) // Revisao
		oPrint:Say  (0220,1100,"Review: Nº: " + Transform(nRevisao, "@E 999"),oFont9)
		If EMPTY(dRevisao)
		...
		elseif EMPTY(SC7->C7_XAPRN2) .AND. nTotalCom > 5000 .AND. cRequer = "1" .or. EMPTY(SC7->C7_XAPRN2) .AND. nTotalCom > 1000 .AND. cRequer = "2"
			oPrint:Say  (2210,1920,"NÃO APROVADO",oFont20)
		...
		oPrint:Box	(2170,1490,2350,1970) // Gerência
