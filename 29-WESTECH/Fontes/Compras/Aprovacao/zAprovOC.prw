...
user function zAprovOC()
	...
	Private aTitles		:= {"Aprovação Ordem de Compra"}
	Private aOrdCrp	:= {{"","//","","","",""}}
	Private aRotina := {}
	...
	*/
	...
	oFld := tFolder():New(005,005,aTitles,{},oDlg,,,,.T.,.F.,468,228)
	
	fCarregaDados() 
	
	//===================================================================== Folder 1 - Cliente ======================================================================
	@ 005,005 LISTBOX oLbx1 FIELDS HEADER 	"No.OC","Emissao","Fornecedor ","Nome","Item Conta","Status" SIZE 456,205 PIXEL OF oFld:aDialogs[1]
	oLbx1:SetArray(aOrdCrp)
	oLbx1:bLine := { || {	aOrdCrp[oLbx1:nAt,01],	aOrdCrp[oLbx1:nAt,02],	aOrdCrp[oLbx1:nAt,03],	aOrdCrp[oLbx1:nAt,04],;
									aOrdCrp[oLbx1:nAt,05],	aOrdCrp[oLbx1:nAt,06]}}  
	
	oBtn := tButton():New(258,435,"Sair",oDlg,{||Sair()},036,012,,,,.T.,,"",,,,.F.)
	oBtn := tButton():New(258,380,"OK",oDlg,{||pAPC7Mt()},036,012,,,,.T.,,"",,,,.F.)
...
Local cAliasOC := GetNextAlias() 
Local cSql      := ""
                                  

//******************************************************************************

cSql :=" 		SELECT DISTINCT C7_NUM, C7_EMISSAO, C7_FORNECE, A2_NREDUZ, C7_ITEMCTA, C7_XCTRVB " 
cSql +="		FROM SC7010 "
cSql +="		INNER JOIN SA2010 ON C7_FORNECE = A2_COD AND C7_LOJA = A2_LOJA " 
cSql +="		WHERE SC7010.D_E_L_E_T_ <> '*' AND C7_ENCER <> 'E' "

cSql := ChangeQuery(cSql)

dbUseArea( .T.,"TOPCONN", TCGENQRY(,,cSql),(cAliasOC), .F., .T.)
...
While ! (cAliasOC)->( EOF() ) 
        ...
        AADD ( aOrdCrp, { (cAliasOC)->C7_NUM,;
 	  				      (cAliasOC)->C7_EMISSAO ,;
 	  					  (cAliasOC)->C7_FORNECE,;
 	  					  (cAliasOC)->A2_NREDUZ,;
 	  					  (cAliasOC)->C7_ITEMCTA,;
 	  				       IIF( (cAliasOC)->C7_XCTRVB <> "1" , "Não Aprovado","Aprovado" ) } )             
...
static Function pAPC7Mt(cAlias, nReg, nOpc)
