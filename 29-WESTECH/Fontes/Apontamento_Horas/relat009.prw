...
User Function relat009()
	Local oReport := nil
	Local cPerg:= Padr("RELAT009",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Relatório Apontamento de Horas",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	oReport:SetPortrait()    
	...
	TRCell():New(oSection1,"TMP_ID"			,	"TRBAPT",	"Id"  			,"@!"	,	13)
	...
	oSection2:= TRSection():New(oReport, "Documento de Entrada", {"SZ4"}, NIL, .F., .T.)
	TRCell():New(oSection2,"TMP_DATA"	,"TRBAPT","Data"				,""				,12)
	TRCell():New(oSection2,"TMP_CODTAREFA","TRBAPT","Código"			,"@!"			,03)
	TRCell():New(oSection2,"TMP_TAREFA"	,"TRBAPT","Tarefa"			,"@!"				,30)
	TRCell():New(oSection2,"TMP_QTDHRS"	,"TRBAPT","Qtd.Horas"		,"@E 999,999,999.99",14,,,,,"RIGHT")
	TRCell():New(oSection2,"TMP_DESCR"	,"TRBAPT","Descrição"		,""					,80)
	
		
	oBreak1 := TRBreak():New(oSection2,{|| (TRBAPT->TMP_CONTRATO) },"Subtotal:",.F.)
	TRFunction():New(oSection2:Cell("TMP_CODTAREFA"),NIL,"COUNT",oBreak1,,,,.F.,.T.)
...
Static Function ReportPrint(oReport)
	...
	cQuery += " Z4_DESCR AS 'TMP_DESCR', Z4_QTDHRS AS 'TMP_QTDHRS', Z4_NOMECLI "
	cQuery += " FROM SZ4010  "
	...
	cQuery += " Z4_COLAB	  >= '" + MV_PAR05 + "' AND  "
	cQuery += " Z4_COLAB	  <= '" + MV_PAR06 + "' " 
	cQuery += " ORDER BY Z4_COLAB, Z4_ITEMCTA,  Z4_DATA, Z4_NOMECLI   "
	...
	While !Eof()
		...
		IncProc("Imprimindo OC "+alltrim(TRBAPT->TMP_CONTRATO))
		
		//imprimo a primeira seção				
		oSection1:Cell("TMP_CONTRATO"):SetValue(TRBAPT->TMP_CONTRATO)
		oSection1:Cell("TMP_ID"):SetValue(TRBAPT->TMP_ID)
		...
		While TRBAPT->TMP_CONTRATO == cNcm
			...
			IncProc("Imprimindo Documento de Entrada "+alltrim(TRBAPT->TMP_CONTRATO))
			oSection2:Cell("TMP_DATA"):SetValue(TRBAPT->TMP_DATA)
			oSection2:Cell("TMP_CODTAREFA"):SetValue(TRBAPT->TMP_CODTAREFA)	
...
static function ajustaSx1(cPerg)
	...
	putSx1(cPerg, "05", "Colaborador de ?"	  , "", "", "mv_ch5", "C", 30, 0, 0, "G", "", "SZ4", "", "", "mv_par05")
	putSx1(cPerg, "06", "Colaborador até?"	  , "", "", "mv_ch6", "C", 30, 0, 0, "G", "", "SZ4", "", "", "mv_par06")
