...
// Estrutura da tabela temporaria
	...
	dbSelectArea("SZ4")
	dbSetOrder(1)
	SET FILTER TO SZ4->Z4_STATUS == "1" .AND. ! ALLTRIM(SZ4->Z4_ITEMCTA) $ "ADMINISTRACAO/PROPOSTA/QUALIDADE/ATIVO/ENGENHARIA/ZZZZZZZZZZZZZ/XXXXXX/ESTOQUE/OPERACOES" .AND. SUBSTR(DTOS(SZ4->Z4_DATA),5,2) == cPar01   //SZ4->Z4_COLAB >= MV_PAR01 .AND. SZ4->Z4_COLAB <= MV_PAR02 .AND. SZ4->Z4_ITEMCTA >= MV_PAR03 .AND. SZ4->Z4_ITEMCTA <= MV_PAR04   SZ4->Z4_ITEMCTA <> "ADMINISTRACAO" .AND.
...
SZ4->( DbCloseArea() )        				// fecha a tabela tempor�ria
...
//if cGrupo == "Diretoria" .OR. cGrupo == "Gerente_Operacoes" .OR. cGrupo == "ADMINISTRACAO" .OR. cColab == "Admin" .OR. cColab == "Administrador"
	...
	If MsgYesNo( "Deseja aprovar :" +  cValToChar(Len(aRecDel))+ " itens Selecionados? ", "ATEN��O !!! ")
	...
	oMark:oBrowse:Gotop()		// for�a o posicionamento do browse no primeiro registro
//Else
//	MsgInfo( "Recurso n�o dispon�vel." )
//	Return .F.
