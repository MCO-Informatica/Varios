...
�����������������������������������������������������������������������������
*/
user function PFIN04()

local aSays		:=	{}
local aButtons 	:= 	{}
local nOpca 	:= 	0
local cCadastro	:= 	"Geracao de planilha de Fluxo de Caixa"
local _cOldData	:= 	dDataBase // Grava a database
local cGrup := ""

private cPerg 	:= 	"PFIN01"
private _cArq	:= 	"PFIN04.XLS"
...
if !cGrup $ ("Diretoria/Administracao/TI/Contratos/Contratos(E)")
	MsgAlert("Recurso nao disponivel.")
	return
endif
...
AADD(aSays,"parametros fornecidos pelo usuario. O arquivo gerado pode ser aberto de forma ")
AADD(aSays,"automatica pelo Excel.")
AADD(aSays,"Movimentos apas a data de referancia sao considerados previstos e anteriores ")
...
if nOpcA == 1 
	...
	MSAguarde({|| PFIN01TIT()},"Titulos Contas a Receber")
	
	MSAguarde({|| PFIN01SE2()},"Titulos Conta a Pagar")
	...
	MSAguarde({||PFIN01SINT()},"Gerando arquivo sintetico.") // *** Funcao de gravacao do arquivo sintetico ***
...
while SA6->(!eof())
	...
	if SA6->A6_COD $ '117' //if SA6->A6_COD $ '115/117' '115/117/502'
		SA6->(dbSkip())
	end if
...
if _lSelBco
	...
	BANCOS->( dbGotop() )
	DEFINE MSDIALOG oDlg TITLE "Selecione os Bancos que deverao ser considerados" From 009,000 To 030,063 OF oMainWnd
...
while QUERY->(!eof()) //.and. SE2->E2_FILIAL == _cFilSE2 .and. QUERY->E2_VENCREA <= _dDataFim
	...
	if 	substr(QUERY->E2_TIPO,3,1) == "-" .or. ;
		...
		dtos(QUERY->E2_EMISSAO) > dtos(_dDataFim) .or. QUERY->E2_TIPO = "PA" .or. ALLTRIM(QUERY->E2_FLUXO) = "N"
		QUERY->(dbskip())
		loop
		//!(QUERY->E2_SALDO > 0 .OR. dtos(QUERY->E2_BAIXA) > dtos(_dDtRef)) .or. ; 
	endif
...
if _cAlias == "SE1"
	...
	_dDtRef,_dDtRef,SE2->E2_LOJA) // dDatabase-1,,SE2->E2_LOJA)                //_dDtRef,_dDtRef,SE2->E2_LOJA)
	
	dbselectarea("SE2")

	_nSaldo -= SomaAbat(SE2->E2_PREFIXO,SE2->E2_NUM,SE2->E2_PARCELA,"P",1,;
	_dDtRef,SE2->E2_FORNECE,SE2->E2_LOJA,SE2->E2_FILIAL)  // dDatabase-1            //_dDtRef,SE2->E2_FORNECE,SE2->E2_LOJA,SE2->E2_FILIAL)
...
while SZR->(!eof())
	...
	if alltrim(SZR->ZR_GRUPO) $ "9.XX.04/9.XX.05"
		SZR->(dbskip())
		loop
	endif
...
while TRB1->(!eof())
	...
	if _nPos > 0 .AND. ! TRB2->GRUPO $ "9.ZZ.02/9.ZZ.03/9.ZZ.04/9.ZZ.05/9.ZZ.06/9.ZZ.07/9.ZZ.08/9.ZZ.09/9.ZZ.10/9.ZZ.11/9.ZZ.12/9.ZZ.13/9.ZZ.14/9.ZZ.15/0.00.09/0.00.10"
...
for _ni := 1 to _nCampos
	...
	TRB2->(dbskip(-17))
	nTRecA := TRB2->(FieldGet(fieldpos(_aCpos[_ni])))
	TRB2->(dbskip(17))
	
	TRB2->(dbskip(-16))
	nTPagA := TRB2->(FieldGet(fieldpos(_aCpos[_ni])))
	TRB2->(dbskip(16))
	

	FieldPut(TRB2->(fieldpos(_aCpos[_ni])) , ((nSIni + nTRec + nTRecA ) - nTDes) + nTPagA )
...
for _ni := 1 to _nCampos-1
	...
	_nSaldo += (_aSaldos[_ni,2] + nTRecA) + nTPagA
...
if mv_par07 = 1
	cCadastro := "Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate " + dtoc(_dDataFim) + iif(_nDiasPer > 1, " - Periodo de " + alltrim(str(_nDiasPer,4,0)) + " dias", " - Diario")
else
...
aadd(aHeader, {"Descricao/Description","DESCRICAO","",50,0,"","","C","TRB2","R"})
...
TITLE "Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate " + dtoc(_dDataFim) + iif(_nDiasPer > 1, " - Periodo de " + alltrim(str(_nDiasPer,4,0)) + " dias", " - Diario") ;
...
aadd(aButton , { "BMPTABLE3" , { || zExFuA()}, "Analitico Geral " } )
aadd(aButton , { "BMPTABLE2" , { || zAtualizar()}, "Atualizar Sintetico " } )
...
Static Function SFMudaCor(nIOpcao)
   ...
   Private _cCor := ""
	...
	if !cGrup $ ("Diretoria/Administracao/TI")

		if nIOpcao == 1 // Cor da Fonte
			if ALLTRIM(TRB2->GRUPO) ==  "1.0X.01"; _cCor := CLR_LIGHTGRAY; endif
			if ALLTRIM(TRB2->GRUPO) ==  "3.XX.00"; _cCor := CLR_LIGHTGRAY; endif
			if ALLTRIM(TRB2->GRUPO) ==  "4.4X.00"; _cCor := CLR_LIGHTGRAY; endif
			if ALLTRIM(TRB2->GRUPO) ==  "9.EM.00"; _cCor := CLR_LIGHTGRAY; endif
			if ALLTRIM(TRB2->GRUPO) ==  "9.XX.00"; _cCor := CLR_LIGHTGRAY; endif
...
cCadastro := "Fluxo de caixa - Analitico - " + _cPeriodo
...
if cGrup $ ("Contratos/Contratos(E)")  .AND. ALLTRIM(TRB2->GRUPO) $ ("1.0X.01/3.XX.00/4.4X.00/9.EM.00/9.XX.00")
	MsgAlert("Voce nao tem permissao para visualizar detalhes.")
	return
endif
...
aadd(aHeader, {"Emissao"	,"EMISSAO"	,"",08,0,"","","D","TRB1","R"})
...
aadd(aHeader, {"Descricao"	,"DESC_NAT"	,"",30,0,"","","C","TRB1","R"})
...
@ aPosObj[2,1]+5 , aPosObj[2,2]+5 Say "LEGENDA ORIGEM: MB - Movimento Bancario / CR - Contas a Receber / CP - Contas a Pagar / OC - Ordem de Compras / PV - Pedido de Vendas "  COLORS 0, 16777215 PIXEL
...
cCadastro := "Fluxo de caixa - Analitico " 
...
DEFINE MSDIALOG _oDlgAnalit TITLE "Fluxo de caixa - Analitico Geral"  From aSize[7],0 to aSize[6],aSize[5] of oMainWnd PIXEL
...
Static Function zExpAnalitico()
    ...
    //Aba 01 - Teste
    oFWMsExcel:AddworkSheet("Fluxo de caixa - Analitico") 
        //Criando a Tabela
        oFWMsExcel:AddTable("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico")
        
        aAdd(aColunas, "Item Conta")							// 3 Item Conta
        aAdd(aColunas, "No.Titulo/OC")							// 5 No.Titulo/OC
        ...
        aAdd(aColunas, "Descricao")								// 9 Descri��o Natureza
        aAdd(aColunas, "Historico")								// 13 Hist�rico
        aAdd(aColunas, "Origem")								// 14 Origem
         
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Item Conta",1,2)			// 3 Item Conta  
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "No.Titulo/OC",1,2)			// 5 No.Titulo/OC
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Tipo",1,2)					// 7 Tipo
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Cod.Cli./For.",1,2)			// 10 Codigo Cliente / Fornecedor
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Cliente/Fornecedor",1,2)	// 11 Cliente / Fornecedor
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Loja",1,2)					// 12 Loja 
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Emissao",2,4)					// 1 Data
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Vencimento",2,4)					// 1 Data
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Data Real",2,4)					// 1 Data
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Valor",1,2)					// 13 Valor       
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Banco",1,2)					// 2 Banco
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Prefixo",1,2)				// 4 Prefixo
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Parcela",1,2)				// 6 Parcela
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Natureza",1,2)				// 8 Natureza
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Descricao",1,2)				// 9 Descri��o
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Historico",1,2)					// 13 Valor
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Origem",1,2)				// 14 Origem
        ...
        While  !(TRB1->(EoF()))
        	...
        	//else
        		...
        		oFWMsExcel:AddRow("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , aLinhaAux)
        	//endif
            TRB1->(DbSkip())
...
Static Function EditReg()
	...
	Local cITEMCTA  := alltrim(TRB1->ITEMCONTA)
    Local cOrigem	  := alltrim(TRB1->ORIGEM)
	local cGrup := ""
	local cUser		:= ALLTRIM(RetCodUsr())
	local cCoord 	:= ALLTRIM(Posicione("CTD",1,xFilial("CTD") + cITEMCTA,"CTD_XIDPM"))
	...
	if cGrup $ ("Diretoria/Administracao/TI") .OR. cUser = cCoord
		...
		MsgAlert("Somente usuarios da Diretoria, Administracao ou Coordenador do Contrado podem editar registro.")
		return
	endif
...
Static _oDlg
  ...
  DEFINE MSDIALOG _oDlg TITLE "Altera Titulo" FROM 000, 000  TO 200, 400 COLORS 0, 16777215 PIXEL
...
Static function zAtualizar()
	...
	DbSelectArea("TRB2")
	zap
	MSAguarde({||PFIN01SINT()},"Atualizando arquivo sintetico.")
	//MSAguarde({|| AtuSaldo()},"Atualizando Fluxo de Caixa")
...
Static _oDlg
  ...
  DEFINE MSDIALOG _oDlg TITLE "Altera Titulo" FROM 000, 000  TO 220, 400 COLORS 0, 16777215 PIXEL
...
private _nOpcoes 	:= 1
private _aOpcoes 	:= {"Simulacao a Pagar ","Simulacao a Receber"}
private _cGrupo	 	:= TRB2->DESCRICAO
private _cLegPer 	:= _aLegPer[ascan(_aCpos,_cCampo)]
private _nValor	 	:= 0
private _cHistorico	:= space(130)

@ 000,000 To 280,350 Dialog _oDlgSimul Title "Simulacao"
...
if _nOpca == 1 // Se confirmada a inclusao da simulacao
	...
	if msgyesno("Deseja guardar essa simulacao para consultas futuras?")
		RecLock("SZS",.T.)
	...
	DbSelectArea("TRB2")
	zap
	MSAguarde({||PFIN01SINT()},"Gerando arquivo sintetico.")
...
if TRB1->SIMULADO <> "S"
	...
	MsgStop("Apenas movimentos de simulacao podem ser excluidos.")
	
elseif msgyesno("Confirma exclusao do movimento de simulacao?")
	...
	if msgyesno("Deseja excluir essa simulacao de consultas futuras?")
...
Static Function zExportExc()
   ...
   //Local oExcel	:= YExcel():new()
    ...
    if mv_par07 = 1
    	...
    	cTabela 	:= "Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate " + dtoc(_dDataFim)
    else
    	cPlan 		:= "Cash Flow"
    ...
    oFWMsExcel:AddworkSheet(cPlan) //N�o utilizar n�mero junto com sinal de menos. Ex.: 1-
        ...
        aAdd(aColunas, "Descricao" + SPACE(25))
        for _ni := 1 to len(_aCpos) // Monta campos com as datas
...
if file(_cArq) .and. ferase(_cArq) == -1
	msgstop("Nao foi possivel abrir o arquivo PFIN01.XLS pois ele pode estar aberto por outro usuario.")
	return(.F.)
endif
...
if _dDataIni > _dDtRef // Data de inicio maior que data de referencia
	msgstop("A data de inicio do processamento deve ser menor ou igual a data de refer�ncia.")
	return(.F.)
endif
