...
AADD(aSays,"Movimentos apas a data de referencia sao considerados previstos e anteriores ")
...
if nOpcA == 1 
	...
	MSAguarde({|| PFIN01TIT02()},"Titulos Contas a Receber")
	
	MSAguarde({|| PFIN01SE202()},"Titulos Conta a Pagar")
...
Private aInfo   := { aSize[ 1 ], aSize[ 2 ]+100, aSize[ 3 ], aSize[ 4 ], 5, 5 }
...
cCadastro := "Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate " + dtoc(_dDataFim) + iif(_nDiasPer > 1, " - Periodo de " + alltrim(str(_nDiasPer,4,0)) + " dias", " - Diario - " + _ItemICIni )

// Monta aHeader do TRB2

aadd(aHeader, {"Descricao","DESCRICAO","",50,0,"","","C","TRB2","R"})
...
TITLE "Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate " + dtoc(_dDataFim) + iif(_nDiasPer > 1, " - Periodo de " + alltrim(str(_nDiasPer,4,0)) + " dias", " - Diario - " + _ItemICIni) ;
...
@ 0070,0830 Say  "Inicio " 	COLORS 0, 16777215 PIXEL
...
@ 0036,0200 Say  "Cod.Cliente: " 	 COLORS 0, 16777215 PIXEL
...
@ 0100,0060 Say  "Producao " 	COLORS 0, 16777215 PIXEL
...
@ 0100,0440 Say  "Producao " 	COLORS 0, 16777215 PIXEL
...
aadd(aButton , { "BMPTABLE3" , { || ShowAna02()}, "Analetico Geral " } )
...
Static Function zExportExc()
   ...
   //Local oExcel	:= YExcel():new()
    ...
    oFWMsExcel:AddworkSheet("Fluxo de Caixa") //N�o utilizar n�mero junto com sinal de menos. Ex.: 1-
        //Criando a Tabela
        oFWMsExcel:AddTable("Fluxo de Caixa","Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate" + dtoc(_dDataFim))
        ...
        For nAux := 1 To Len(aColunas)
            oFWMsExcel:AddColumn("Fluxo de Caixa", "Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate " + dtoc(_dDataFim), aColunas[nAux],1,2)
            aAdd(aColsMa,  nX1 )
            nX1++
        Next
        ...
        While  !(TRB2->(EoF()))
            ...
            if alltrim(aLinhaAux[1]) == "SALDO INICIAL" .OR. alltrim(aLinhaAux[1]) == "TOTAL RECEITAS" .OR. ;
            	...
            	oFWMsExcel:AddRow("Fluxo de Caixa", "Fluxo de caixa - Sintetico - de " + dtoc(_dDataIni) + " ate " + dtoc(_dDataFim), aLinhaAux,aColsMa)
...
cCadastro :=  "Fluxo de caixa - Analitico - " + _cPeriodo + " - " + _ItemICIni

DEFINE MSDIALOG _oDlgAnalit TITLE "Fluxo de caixa - Analitico - " + _cPeriodo From aSize[7],0 to aSize[6],aSize[5] of oMainWnd PIXEL
...
cCadastro :=  "Fluxo de caixa - Analitico - " + _ItemICIni

DEFINE MSDIALOG _oDlgAnalit TITLE "Fluxo de caixa - Analitico  "   From aSize[7],0 to aSize[6],aSize[5] of oMainWnd PIXEL
...
Static Function zExpAnalitico()
    ...
    oFWMsExcel:AddworkSheet("Fluxo de caixa - Analitico") 
        ...
        oFWMsExcel:AddColumn("Fluxo de caixa - Analitico","Fluxo de caixa - Analitico" , "Data",2,4)					// 1 Data
...
if _dDataIni > _dDtRef // Data de inicio maior que data de referencia
	msgstop("A data de inicio do processamento deve ser menor ou igual a data de referencia.")
	return(.F.)
endif

if _dDataFim < _dDtRef // Data de fim menor que data de referencia
	msgstop("A data de final do processamento deve ser maior ou igual a data de referencia.")
	return(.F.)
endif
...
//putSx1(cPerg, "07", "Item Conta at�?"	  , "", "", "mv_ch7", "C", 13, 0, 0, "G", "", "CTD", "", "", "mv_par07")
return
