...
User Function relat039()
	Local oReport := nil
	Local cPerg:= Padr("RELAT039",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Contas a Pagar - Geral Diario",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
...
Static Function ReportPrint(oReport)
	...
	cQuery += " E2_FORNECE    <= '" + MV_PAR10 + "' AND "
	IF cFluxo = "S"
		cQuery += " E2_FLUXO    = '" + cFluxo + "' "
	ELSEIF cFluxo = "N"
		cQuery += " E2_FLUXO    = '" + cFluxo + "' "
	ELSEIF cFluxo = ""
		cQuery += " E2_FLUXO    >= '' AND E2_FLUXO <= 'Z' "
	ENDIF
	cQuery += "	ORDER BY E2_VENCREA, E2_EMISSAO,E2_XXIC "
	...
	While !Eof()
		...
		While TRBFIN->TMP_VENCREAL == cNcm
			...
			IncProc("Imprimindo Contas a Pagar - Diario "+alltrim(TRBFIN->TMP_VENCREAL))
