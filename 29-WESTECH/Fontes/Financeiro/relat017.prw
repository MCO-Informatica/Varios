...
User Function relat017()
	Local oReport := nil
	Local cPerg:= Padr("RELAT017",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Contas a Receber em Atraso",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	oSection2:= TRSection():New(oReport, "Contas a Receber em Atraso", {"SE1"}, NIL, .F., .T.)
	...
	TRCell():New(oSection2,"TMP_CODCLI"		,"TRBFIN","Código"			,"@!"				,12)
	TRCell():New(oSection2,"TMP_CLIENTE"	,"TRBFIN","Cliente"			,"@!"				,40)
	TRCell():New(oSection2,"TMP_VENCTO"		,"TRBFIN","Vcto."			,""					,19)
	TRCell():New(oSection2,"TMP_VENCREAL"	,"TRBFIN","Vcto.Real"		,""					,19)
...
Static Function ReportPrint(oReport)
	...
	cQuery += "	FROM SE1010 "
	cQuery += "	WHERE SE1010.D_E_L_E_T_ <> '*' AND E1_SALDO > 0 AND E1_VENCTO < GETDATE() AND E1_TIPO <> 'RA' "
	cQuery += "	ORDER BY E1_XXIC, E1_VENCREA, E1_EMISSAO "
	...
	While !Eof()
		...
		IncProc("Imprimindo Contas a Receber em Atraso"+alltrim(TRBFIN->TMP_VENCREAL))
		...
		While TRBFIN->TMP_VENCREAL == cNcm
			...
			IncProc("Imprimindo Contas a Receber "+alltrim(TRBFIN->TMP_CONTRATO))
