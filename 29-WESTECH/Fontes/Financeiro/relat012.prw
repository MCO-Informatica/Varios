...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Contas a Pagar - Provisões",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	oSection2:= TRSection():New(oReport, "Contas a Pagar - Provisões", {"SE2"}, NIL, .F., .T.)
...
Static Function ReportPrint(oReport)
	...
	While !Eof()
		...
		IncProc("Imprimindo Contas a Pagar - Provisões"+alltrim(TRBFIN->TMP_VENCREAL))
