...
User Function relat013()
	Local oReport := nil
	Local cPerg:= Padr("RELAT013",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Contas a Receber - Provisões",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	oSection2:= TRSection():New(oReport, "Contas a Receber - Provisões", {"SE1"}, NIL, .F., .F.)
...
Static Function ReportPrint(oReport)
	...
	cQuery += "	FROM SE1010 "
	cQuery += "	WHERE SE1010.D_E_L_E_T_ <> '*' AND E1_TIPO = 'PR' AND "
	...
	While !Eof()
		...
		IncProc("Imprimindo Contas a Receber - Provisões"+alltrim(TRBFIN->TMP_VENCREAL))
