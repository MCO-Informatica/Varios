...
User Function relat016()
	Local oReport := nil
	Local cPerg:= Padr("RELAT016",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Contas a Pagar em Atraso",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	oSection2:= TRSection():New(oReport, "Contas a Pagar em Atraso", {"SE2"}, NIL, .F., .T.)
...
Static Function ReportPrint(oReport)
	...
	cQuery += "	FROM SE2010 "
	cQuery += "	WHERE  SE2010.D_E_L_E_T_ <> '*' AND E2_SALDO > 0 AND E2_VENCTO < GETDATE() AND E2_TIPO <> 'PA' "

	IF cGRUPO == "000002" .OR. cGRUPO == "000003" .OR. cGRUPO == "000005" .OR. cGRUPO == "000006"
		cQuery += " AND E2_XXIC NOT IN  (SELECT E2_XXIC FROM SE2010 WHERE E2_XXIC = 'ADMINISTRACAO') AND "
	...
	While !Eof()
		...
		IncProc("Imprimindo Contas a Pagar em Atraso"+alltrim(TRBFIN->TMP_VENCREAL))
