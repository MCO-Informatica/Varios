#Include 'Protheus.ch'
#INCLUDE 'TOPCONN.CH'
User Function relat045()

	Local oReport := nil
	Local cPerg:= Padr("RELAT045",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Contas a Pagar - Abertos",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	TRCell():New(oSection1,"TMP_CONTRATO",	"TRBFIN",	"Contrato"  	,"@!"				,22)
	TRCell():New(oSection1,"TMP_TITULO"		,"TRBFIN","Título"			,"@!"				,09)
	TRCell():New(oSection1,"TMP_TIPO"		,"TRBFIN","Tipo"			,"@!"				,06)
	TRCell():New(oSection1,"TMP_NATUREZA"	,"TRBFIN","Natureza"		,"@!"				,12)
	TRCell():New(oSection1,"TMP_CODFORN"	,"TRBFIN","Código"			,"@!"				,12)
	TRCell():New(oSection1,"TMP_FORNECE"	,"TRBFIN","Fornecedor"		,"@!"				,40)
	TRCell():New(oSection1,"TMP_VENCTO"		,"TRBFIN","Vencimento"		,""					,19)
	TRCell():New(oSection1,"TMP_VENCREAL"	,"TRBFIN","Vencto.Real"		,""					,19)
	TRCell():New(oSection1,"TMP_VALOR"		,"TRBFIN","Valor"			,"@E 999,999,999.99",22,,,,,"RIGHT")
	TRCell():New(oSection1,"TMP_SALDO"		,"TRBFIN","Saldo"			,"@E 999,999,999.99",22,,,,,"RIGHT")
	TRCell():New(oSection1,"TMP_BAIXA"		,"TRBFIN","Baixa"			,""					,19)
	TRCell():New(oSection1,"TMP_HIST"		,"TRBFIN","Histórico"		,""					,80)

	//oBreak1 := TRBreak():New(oSection2,{|| (TRBFIN->TMP_CONTRATO) },"Subtotal:",.F.)
	TRFunction():New(oSection1:Cell("TMP_VALOR")	,NIL,"SUM",,"@E 999,999,999.99",,,.F.,.T.)
	TRFunction():New(oSection1:Cell("TMP_SALDO")	,NIL,"SUM",,"@E 999,999,999.99",,,.F.,.T.)
...
Static Function ReportPrint(oReport)
	...
	cQuery += "			E2_HIST as 'TMP_HIST', E2_BAIXA AS 'TMP_BAIXA'"
	cQuery += "	FROM SE2010 "
	cQuery += "	WHERE  SE2010.D_E_L_E_T_ <> '*' AND E2_BAIXA = '' AND "
	...
	While !Eof()
		...
		IncProc("Imprimindo Contas a Pagar - Abertos"+alltrim(TRBFIN->TMP_VENCREAL))

		//imprimo a primeira seção
		oSection1:Cell("TMP_CONTRATO"):SetValue(TRBFIN->TMP_CONTRATO)
		oSection1:Cell("TMP_TIPO"):SetValue(TRBFIN->TMP_TIPO)
		oSection1:Cell("TMP_NATUREZA"):SetValue(TRBFIN->TMP_NATUREZA)
		oSection1:Cell("TMP_CODFORN"):SetValue(TRBFIN->TMP_CODFORN)
		oSection1:Cell("TMP_FORNECE"):SetValue(TRBFIN->TMP_FORNECE)
			IF TMP_VENCTO = ""
				oSection1:Cell("TMP_VENCTO"):SetValue("")
			ELSE
				oSection1:Cell("TMP_VENCTO"):SetValue(Substr(TRBFIN->TMP_VENCTO,7,2) + "/" + Substr(TRBFIN->TMP_VENCTO,5,2) + "/" + Substr(TRBFIN->TMP_VENCTO,1,4))
			ENDIF

			//oSection2:Cell("TMP_VENCTO"):SetValue(TRBFIN->TMP_VENCTO)

			IF TMP_VENCREAL = ""
				oSection1:Cell("TMP_VENCREAL"):SetValue("")
			ELSE
				oSection1:Cell("TMP_VENCREAL"):SetValue(Substr(TRBFIN->TMP_VENCREAL,7,2) + "/" + Substr(TRBFIN->TMP_VENCREAL,5,2) + "/" + Substr(TRBFIN->TMP_VENCREAL,1,4))
			ENDIF
		oSection1:Cell("TMP_VALOR"):SetValue(TRBFIN->TMP_VALOR)
		oSection1:Cell("TMP_VALOR"):SetAlign("RIGHT")

		oSection1:Cell("TMP_SALDO"):SetValue(TRBFIN->TMP_SALDO)
		oSection1:Cell("TMP_SALDO"):SetAlign("RIGHT")

		IF TMP_BAIXA = ""
			oSection1:Cell("TMP_BAIXA"):SetValue("")
		ELSE
			oSection1:Cell("TMP_BAIXA"):SetValue(Substr(TRBFIN->TMP_BAIXA,7,2) + "/" + Substr(TRBFIN->TMP_BAIXA,5,2) + "/" + Substr(TRBFIN->TMP_BAIXA,1,4))
		ENDIF
		oSection1:Cell("TMP_HIST"):SetValue(TRBFIN->TMP_HIST)
