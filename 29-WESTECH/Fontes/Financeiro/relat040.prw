...
User Function relat040()
	Local oReport := nil
	Local cPerg:= Padr("RELAT040",10)
...
Static Function RptDef(cNome)
	...
	oReport := TReport():New(cNome,"Contas a Receber - Diario",cNome,{|oReport| ReportPrint(oReport)},"Descrição do meu relatório")
	...
	oSection1:= TRSection():New(oReport, 	"NCM",	 	{"SE1"}, 			, .F.	, 	.F.)
	...
	oSection2:= TRSection():New(oReport, "Contas a Receber - Diario", {"SE1"}, NIL, .F., .T.)
	...
	TRCell():New(oSection2,"TMP_VENCTO"		,"TRBFIN","Vcto"		,""						,22)
	TRCell():New(oSection2,"TMP_VENCREAL"	,"TRBFIN","Vcto.Real"		,""					,22)
	...
	TRCell():New(oSection2,"TMP_BAIXA"		,"TRBFIN","Baixa"			,""					,25)
	TRCell():New(oSection2,"TMP_FLUXO"		,"TRBFIN","Fluxo"  			,"@!"				,15)
...
Static Function ReportPrint(oReport)
	...
	cQuery += "		E1_NOMCLI as 'TMP_CLIENTE',CAST(E1_EMISSAO as Date) as 'TMP_EMISSAO', E1_VENCTO as 'TMP_VENCTO', E1_VENCREA as 'TMP_VENCREAL', "
	cQuery += "		IIF(E1_TIPO = 'RA' AND E1_BAIXA <> '', -E1_VALOR, E1_VALOR)  as 'TMP_VALOR', "
	cQuery += "		E1_SALDO AS 'TMP_SALDO',  "
	cQuery += "		E1_HIST AS 'TMP_HIST', E1_BAIXA AS 'TMP_BAIXA', E1_FLUXO AS 'TMP_FLUXO'"
	cQuery += "	FROM SE1010 "
	cQuery += "	WHERE SE1010.D_E_L_E_T_ <> '*' AND "

	IF cGRUPO == "000002" .OR. cGRUPO == "000003" .OR. cGRUPO == "000005" .OR. cGRUPO == "000006"
		cQuery += " E1_XXIC NOT IN  (SELECT E1_XXIC FROM SE1010 WHERE E1_XXIC = 'ADMINISTRACAO') AND "
		cQuery += " E1_XXIC NOT IN  (SELECT E1_XXIC FROM SE1010 WHERE E1_XXIC = 'XXXXXX') AND "
		cQuery += " E1_XXIC NOT IN  (SELECT E1_XXIC FROM SE1010 WHERE E1_XXIC = ' ') AND "
	END IF
	...
	cQuery += " E1_CLIENTE    <= '" + MV_PAR06 + "' AND "
	...
	ELSEIF MV_PAR07 = 3
		cQuery += " E1_FLUXO    = '' "
	ELSEIF MV_PAR07 = 4
		cQuery += " E1_FLUXO    >= '' AND E1_FLUXO <= 'Z' "
	ENDIF
	...
	While !Eof()
		...
		While TRBFIN->TMP_VENCREAL == cNcm
			...
			ENDIF
			oSection2:Cell("TMP_CONTRATO"):SetValue(TRBFIN->TMP_CONTRATO)
			oSection2:Cell("TMP_FLUXO"):SetValue(TRBFIN->TMP_FLUXO)
