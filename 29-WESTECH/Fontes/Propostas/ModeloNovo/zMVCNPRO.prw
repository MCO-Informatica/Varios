...
Static Function MenuDef()
    ...
    //Adicionando opções
    ADD OPTION aRot TITLE 'Visualizar' ACTION 'VIEWDEF.xMNPROP' OPERATION MODEL_OPERATION_VIEW   ACCESS 0 //OPERATION 1
    ADD OPTION aRot TITLE 'Legenda'    ACTION 'u_zMVC11Leg'     OPERATION 6                      ACCESS 0 //OPERATION X
    ADD OPTION aRot TITLE 'Incluir'    ACTION 'VIEWDEF.xMNPROP' OPERATION MODEL_OPERATION_INSERT ACCESS 0 //OPERATION 3
    ADD OPTION aRot TITLE 'Alterar'    ACTION 'VIEWDEF.xMNPROP' OPERATION MODEL_OPERATION_UPDATE ACCESS 0 //OPERATION 4
...
Static Function ModelDef()
    ...
    oModel := MPFormModel():New('xMNPROPm',  , { |oMdl| COMP011POS( oMdl ) })

    oModel:AddFields('SZ9MASTER',/*cOwner*/,oStPai)

    oModel:AddGrid('ZZMDETAIL','SZ9MASTER',oStFilho6,/*bLinePre*/, /*bLinePost*/,/*bPre - Grid Inteiro*/,/*bPos - Grid Inteiro*/,/*bLoad - Carga do modelo manualmente*/)  //cOwner é para quem pertence
   ...
   //aAdd(aSZFRel, {'ZF_FILIAL', 'Z9_FILIAL'} )
    aAdd(aSZFRel, {'ZZM_NPROP',  'Z9_NPROP'})
...
oModel:SetRelation('ZZMDETAIL', { { 'ZZM_NPROP', 'Z9_NPROP' } }, SZF->(IndexKey(1)) )

 //IndexKey -> quero a ordenação e depois filtrado
    //oModel:GetModel('ZZMDETAIL'):SetUniqueLine({"ZF_IDVEND"})  //Não repetir informações ou combinações {"CAMPO1","CAMPO2","CAMPOX"}
...
Static Function ViewDef()
    Local oView     := Nil
    Local oModel        := FWLoadModel('xMNPROP')
	...
	oView:EnableTitleView('VIEW_ZZM','Detalhamento')
...
Static Function COMP021BUT( oModel )
	...
	Local oStPai        := FWFormStruct(2, 'SZ9')
	Local oModelZZM   	:= oModel:GetModel( 'ZZMDETAIL' )
	...
	Local nQuantZZM		:= oModel:GetValue( 'ZZMDETAIL', 'ZZM_QUANT' )
	Local nTotalZM 		:= oModel:GetValue( 'ZZMDETAIL', 'ZZM_TOTAL' )
