...
±±³Fun‡…o    ³ RELEST   ³ Autor ³                       ³ Data ³          ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ POSICAO DE ESTOQUE                                         ³±±
...
nValorR  := 0,;
cFilOK   := cFilAnt,;
cAl1	 := "SB1", cAl2 := "SB2", cQry, lExcl := .f.,aStru := {},dDataRef
...
#IFDEF TOP
	...
	cQuery := "SELECT * FROM " + RetSqlName("SB2") + " B2, " + RetSqlName("SB1") + " B1"
	...
	For ni := 1 to Len(aStru)
		If aStru[ni,2] != 'C'
			TCSetField(cAl2, aStru[ni,1], aStru[ni,2],aStru[ni,3],aStru[ni,4])
		Endif
	Next
#ELSE
  dbSetOrder(1)
...
While !Eof()
   IF lExcl
	...
	dDataRef := Iif(Empty(mv_par19),dDataBase,mv_par19)
...
#ENDIF
			If ((( (cAl1)->B1_TIPO  >= MV_PAR08) .And. ( (cAl1)->B1_TIPO  <= MV_PAR09)) .And. ;
				...
				If  ((mv_par14 == 1)                      .Or. ;
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					If (mv_par20==2) .And. (QtdComp(nValor)==QtdComp(0))
						dbSelectArea( cAl2 )
						dbSkip()
						Loop
					EndIf
...
cCab01 := OemToAnsi(STR0014)        //"REFERENCIA      TP GRUP DESCRICAO             UM FL ALM   SALDO       EMPENHO PARA     ESTOQUE      ___________V A L O R___________"
...
While !Eof()
	...
	If (aPrnDET == nil)
		...
		aPrnDET[ DET_REF  ] := SUBSTR(FIELD->REFEREN,1,15)
	...
	If lPrintDET .Or. lPrintTOT
		...
		Do Case
			...
			Case !(aPrnDET[ DET_REF ] == cLPrnCd)
				
				//cLPrnCd := aPrnDET[ DET_CODE ] ; lPrintCAB := .T.
				cLPrnCd := aPrnDET[ DET_REF ] ; lPrintCAB := .T.
		EndCase
		
		If lPrintCAB .Or. lPrintOUT
			
			//@ Li,000 PSay aPrnDET[ DET_CODE ]
			@ Li,000 PSay aPrnDET[ DET_REF  ]
