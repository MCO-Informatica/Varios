...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						If !"PEDIDO INTERNO "+SC5->C5_NUM $ cMensCli
							If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
								cMensCli += " "
							EndIf
							cMensCli += "PEDIDO INTERNO "+SC5->C5_NUM
						EndIf
						...
						If SC5->C5_FORMAPG == 'CH'
							If !" ( Cobranca em Cheques )" $ cMensCli
								cMensCli += " ( Cobranca em Cheques )"
							EndIf	
						ElseIf SC5->C5_FORMAPG == 'BO'
							If !" ( Cobranca em Boleto Bancario )" $ cMensCli
								cMensCli += " ( Cobranca em Boleto Bancario )"
							EndIf
						...
						Elseif SC5->C5_FORMAPG == 'DC'
							If !" ( Cobranca em Deposito Bancario )" $ cMensCli
								cMensCli += " ( Cobranca em Deposito Bancario )"
							EndIf
						...
						Elseif SC5->C5_FORMAPG == 'R$'
							If !" ( Cobranca em Dinheiro )" $ cMensCli
								cMensCli += " ( Cobranca em Dinheiro )"
							EndIf
						Elseif SC5->C5_FORMAPG == 'CC'
							If !" ( Cobranca em Cartao Credito )" $ cMensCli
								cMensCli += " ( Cobranca em Cartao Credito )"
							EndIf
						...
						//³ INICIO   - NUM LOTE E DATA VALIDADE - MCINFOTEC 05/01/2019					                    ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
                        // Alterado em 29/12/2020 - Luiz - Se Validade = 31/12/2049 será alterado para "Indeterminado"
						If !Empty((cAliasSD2)->D2_LOTECTL)

							If (cAliasSD2)->D2_DTVALID = "20491231"
								cDescProd := ALLTRIM(cDescProd)+" - LOTE: "+ALLTRIM((cAliasSD2)->D2_LOTECTL)+" - VAL.: INDETERMINADO"
							else
								cDescProd := ALLTRIM(cDescProd)+" - LOTE: "+ALLTRIM((cAliasSD2)->D2_LOTECTL)+" - VAL.: "+DTOC(STOD((cAliasSD2)->D2_DTVALID))
							Endif
						
						EndIf
						
						If !Empty(SC6->C6_X_LOTE)

							If DTOS(SC6->C6_X_DTLOT) = "20491231"
								cDescProd := ALLTRIM(cDescProd)+" - LOTE: "+ALLTRIM(SC6->C6_X_LOTE)+" - VAL.: INDETERMINADO"
							else
								cDescProd := ALLTRIM(cDescProd)+" - LOTE: "+ALLTRIM(SC6->C6_X_LOTE)+" - VAL.: "+DTOC((SC6->C6_X_DTLOT))
							Endif
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							0,;//(nDesconto+nDescIcm+nDescRed+nDescNfDup),;
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_VUNIT,D1_CLASFIS,D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_LOTECTL,D1_DTVALID, 
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				/*	 
				//LUCAS - UPDUO
				If !Empty((cAliasSD1)->D1_X_LOTE)

					If DTOS((cAliasSD1)->D1_X_DTLOT) = "20491231"
						cDescProd := ALLTRIM(cDescProd)+" - LOTE: "+ALLTRIM((cAliasSD1)->D1_X_LOTE)+" - VAL.: INDETERMINADO"
					else
						cDescProd := ALLTRIM(cDescProd)+" - LOTE: "+ALLTRIM((cAliasSD1)->D1_X_LOTE)+" - VAL.: "+DTOC((cAliasSD1)->D1_X_DTLOT)
					Endif
	...
	IF cVeramb >= "4.00"
	   //Ajute para alimentar o aDetPag
		...
		If cTipo == "1"
		  cChvPag := SF2->F2_COND
		Else
		  cChvPag := IIF(!Empty(SF1->F1_COND),SF1->F1_COND,"001")
		EndIf

		/* NOTA TECNICA 01/09/2021 TRECHO ANTIGO
		If	cTPNota $ '3-4'
		...
		*/
		...
		// NOTA TECNICA 01/09/2021 TRECHO NOVO
...
IF lPe01Nfe     
	...
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,cNatOper}
...
cString += '<qCom>'+ConvType(aProd[09],15,8)+'</qCom>'
...
If aProd[8] <> aProd[11]  // aProd[8] = B1_UM  e  aProd[11] = B5_UMDIPI - pega diferente para segunda unidade de medida 
	cString += '<qTrib>' + ConvType(aProd[09],15,8) + '</qTrib>'
Else
	cString += '<qTrib>' + ConvType(aProd[09],15,8) + '</qTrib>'
Endif
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],21,6))+'</vUnTrib>'	
...
//--->Customização feita por Lucas Baía (UPDUO)
	...
	IF ((aNota[5]) $ "D/B" .and. ((!Empty(aNfVinc))) .OR. !Empty(SFT->FT_NFORI))

		IF Alltrim(SFT->FT_CFOP) $ AllTrim(SUPERGETMV("MV_DEVCFOP"))
			cTPNota:= "1" //Finalidade Normal.
		ELSE
			cTPNota:= "4" //Finalidade Devolução de Mercadoria.
		ENDIF
		
	ENDIF

	IF ((aNota[5]) $ "N" .and. (Empty(aNfVinc)))
