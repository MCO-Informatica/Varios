...
cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],21,4))+'</vUnCom>'
...
If aProd[8] <> aProd[11]  // aProd[8] = B1_UM  e  aProd[11] = B5_UMDIPI - pega diferente para segunda unidade de medida 
	cString += '<qTrib>' + ConvType(aProd[09],15,4) + '</qTrib>'
Else
	cString += '<qTrib>' + ConvType(aProd[09],15,4) + '</qTrib>'
Endif
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],21,4))+'</vUnTrib>'	
