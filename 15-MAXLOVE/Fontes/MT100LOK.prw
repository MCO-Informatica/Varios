	...
	If !_lDelet
		...
		If Rastro(_cProduto)
			...
			If Posicione("SF4",1,xFilial("SF4")+_cTes,"F4_ESTOQUE")$"S"
				...
				If Empty(_cLoteCtl)
					_nOpcao := Aviso("LOTE","Atenção! Favor incluir o número do lote para reastreabilidade!", {"OK"} )
					If _nOpcao  == 2
						_lRet:=.F.
					EndIf
				EndIf
				...
				If Empty(_dDtValid) .or. _dDtValid <= dDataBase
					_nOpcao := Aviso("DATA VALIDADE","Atenção Favor, incluir a data de validade para rastreabilidade!", {"OK"} )
					If _nOpcao  == 2
						_lRet:=.F.
					EndIf
				EndIf
