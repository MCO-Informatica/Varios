...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	cSql := "UPDATE CD2010 SET CD2_PDDES = 100, CD2_VDDES = ROUND(CD2_BC * ((CD2_ALIQ - CD2_ADIF)/100),2) WHERE CD2_ADIF > 0 AND D_E_L_E_T_ = ' ' AND CD2_IMP = 'CMP' " //REJEIÇÃO 699
	TcSqlExec(cSql)


	cSql := "UPDATE SA1010 SET A1_CONTRIB = '2', A1_IENCONT = '1' WHERE A1_INSCR = ' ' AND D_E_L_E_T_=' ' AND A1_IENCONT <> '1' AND A1_CONTRIB <> '2' " //REJEIÇÃO 805
	TcSqlExec(cSql)

	//cSql := "UPDATE CD2010 SET CD2_VDDES = ROUND(((CD2_BC * (CD2_ALIQ/100)) - (CD2_BC * (CD2_ADIF/100))) * ((CD2_PDDES/100)*4),2) WHERE CD2_ADIF > 0 AND D_E_L_E_T_ = ' ' AND CD2_IMP = 'CMP' " //REJEIÇÃO 815
	//TcSqlExec(cSql)
					...
					*/
					...
					If lContinua
						...
						If !Empty(SC5->C5_PEDECOM)
							If !"PEDIDO WEB "+SC5->C5_PEDECOM $ cMensCli
								If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
									cMensCli += " "
								EndIf
								cMensCli += "PEDIDO WEB "+SC5->C5_PEDECOM
							EndIf
						EndIf 
						...
						If Ascan(aRecPed, { |x| x == SC5->(RECNO()) }) == 0
							...
							EndIf
						EndIf
						If !("Pedido Interno: " + Alltrim(SC5->C5_NUM) + Space(80-Len("Pedido Interno: "+Alltrim(SC5->C5_NUM))) $ cMensFis)
							cMensFis += "Pedido Interno: " + Alltrim(SC5->C5_NUM) + Space(80-Len("Pedido Interno: "+Alltrim(SC5->C5_NUM)))
						EndIf	
						If !Empty( cNumNfCup )
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "TST" 
									...
									nALIQTot += IIf(CD2->CD2_ALIQ > 0,CD2->CD2_ALIQ,0)
	...
	If Len(aIcmsST)>0	
		...
		*/
				...
				If (cIdDest == "2" .and. cIndFinal == "1" .and. cIndIEDest == "9") .and. (Len(aISSQN)== 0) .and. ;
					...
					If Len(aICMUFDest) > 0 .And. !(aNota[04] == "0" .and. aNota[5] == "N" .And. aICMUFDest[05] == 0) //.AND. Type("aICMUFDest[5]")<>"U"
						cString += '<imposto>'
