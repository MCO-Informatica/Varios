...
@description compensaÁ„o entre carteiras rotina Padr„o custumizada para a MAXLOVE
@type function
@version  1.0
...
User Function FINA450X(nPosArotina,aAutoCab,nOpcAuto)
	...
	If __lImpCC == NIL
		__lImpCC	:= SuperGetMV("MV_CC10925",.T.,1 ) == 2  // Gera PCC/IRF na compensaùùo entre carteiras quando PCC/IRF na baixa
...
User Function FA450CMP(cAlias,nReg,nOpcx, aCpos , aAutoCab )
	...
	{"TITULO"	,, STR0006,"@X"},;  //"Numero Tùtulo"
	...
	{"VALACES"	,, STR0101,"@E 9,999,999,999.99"},; //"Valores Acessùrios"
	...
	Local nRegSE1 	:= 0
	Local nRegSE2  	:= 0
	Local lPrimeiro	:= .T. //validaùùo do help de controle de docuemntos
	Local aAlt   	:= {}
	Local nTxMoeda  := 0
	Local oCli450	:= NIL
	Local oLjCli    := NIL
...
//informaÁıes do cliente - 25/09/16
	dCliIniVen	:= Ctod("  /  /  ")
	dCliFimVen	:= Ctod("  /  /  ")
	dCliIniEmi	:= Ctod("  /  /  ")
	dCliFimEmi	:= Ctod("  /  /  ")
	...
	// Verifica se data do movimentoù menor que data limite de movimentacao no financeiro
	...
	While .T.
		...
		If !lF450Auto
			...
			@ 28+nEspLin, 075+nEspLarg	SAY STR0022 SIZE 15, 10 OF oPanel PIXEL  //"AtÇ:"
			@ 28+nEspLin, 160+nEspLarg	SAY STR0021 SIZE 15, 10 OF oPanel PIXEL  //"De:"
			@ 28+nEspLin, 225+nEspLarg	SAY STR0022 SIZE 15, 10 OF oPanel PIXEL  //"AtÇ:"
			...
			@ 115+nEspLin, 075+nEspLarg	SAY STR0022 SIZE 15, 10 OF oPanel PIXEL  //"AtÇ:"
			@ 115+nEspLin, 160+nEspLarg	SAY STR0021 SIZE 15, 10 OF oPanel PIXEL  //"De:"
			@ 115+nEspLin, 225+nEspLarg	SAY STR0022 SIZE 15, 10 OF oPanel PIXEL  //"AtÇ:"
		...
		//Adiciona campos para Mùxico, Peru y Colombia
		...
		If __lMetric
			// Metrica do tempo para exibir a tela de titulos para marcaùùo
		...
		If !lF450Auto
			...
			DEFINE MSDIALOG oDlg1 TITLE STR0027 PIXEL FROM oSize:aWindSize[1],oSize:aWindSize[2] To oSize:aWindSize[3],oSize:aWindSize[4] OF oMainWnd //"Compensaùùo Entre Carteiras"
			oDlg1:lMaximized := .T.
		...
		//------------------------------------------------------
		//ùIntegracao com o SIGAPCO para lancamento via processo
		//------------------------------------------------------
		PcoIniLan("000018")
			...
			Begin Transaction
				...
				While ! Eof()
					...
					If TRB->MARCA == cMarca
						...
						//Salvo o cùdigo do cliente e a loja para posicionar antes da contabilizaùùo
						...
						If !Empty(TRB->RECEBER)
							...
							// ja estaù somado no nValRec e seria somado novamente na
							...
							If cPaisLoc == "BRA"
								...
								//Se existir taxa contratada nùo deve calcular a correùùo monetaria, pois no
								//momento da compensaùùo nùo ù possivel manipular a taxa da moeda.
								If SE1->E1_TXMOEDA == 0
							...
							If nValRec > 0
								...
								If lBaixou
									...
									If SE5->(MsSeek(xFilial("SE5") + SE5->E5_IDORIG + "BA"))  // Gravar o E5_IDENTEE para todos os registros, para nùo afetar o estorno.
										...
										// nova versùo para gravar SE5 e demais tabelas
										SE5->(dbGoTo(nRecSE5))
										...
										//Atualizo o nùmero da compensaùùo nos valores acessorios (DC|MT|JR|CM|VA)
										//Mantido na forma antiga pois essas informaùùes nùo sùo repassadas para as FK6
										//Este trecho serù retirado quando a SE5 nùo for mais gravada
										dbSelectArea("SE5")
										nRecAtu := SE5->(Recno())
										...
										//chamada da Funùùo que cria o Histùrico de Cobranùa
										FinaCONC(aAlt)
						...
						ElseIf !Empty(TRB->PAGAR)
							...
							//------------------------------------------------------
							// Se nùo for baixar total o titulo, desconsidera abat.
							// no calculo do valor recebido
							// Caso o valor do titulo seja maior que o restante a
							// compensar, baixa-se apenas a diferenùa.
							//-------------------------------------------------------
							If cPaisLoc == "MEX" .and. lMonedaC .and. FindFunction("Fn450VlMnd")
							...
							//Recomponho o valor do IRRF e PCC para que o valor pago vù corretamente para a baixa automùtica do titulo.
							If __lImpCC
							...
							If nValPgto > 0
								...
								//Grava o codigo da compensaùùo e o flag de contabilizaùùo
				...
				If __lMetric
					// Metrica do tempo para exibir a tela de titulos para marcaùùo
					FwCustomMetrics():setAverageMetric("TempoGravaùùo", "financeiro-protheus_tempo-conclusùo-processo_seconds", nFim)
				Endif
	...
	If MsgYesNo("Deseja imprimir a compensaÁ„o?")
...
Verifica se data final ù maior que data inicial 
...
Static Function FA450Chec1(lTitFuturo,aSelFil,aTmpFil)
	//PCREQ-3782 - Bloqueio por situaùùo de cobranùa
	Local cString := ""
	Local cTmpSE1Fil := ""
...
Static Function FA450Chec2(lTitFuturo,aSelFil,aTmpFil)
	...
	Else
		...
		// Se nùo considera tùtulos transferidos, filtra (exibe) apenas os tùtulos que estùo em carteira.
		If mv_par03 == 2
		...
		//AAF - Titulos originados no SIGAEFF nùo devem ser alterados
		...
		|Os titulos a pagar que estùo com solicitaùùo de transferencia em aberto 
		|Nùo devem entrar na rotina de Compensaùùo 
		-------------------------------------------------------------------------*/
		If lPrjCni
...
/*
	...
	*/
	...
	IndRegua("TRB",cArquivo,cChave,,,"Criando Õndice...")
...
Static Function Fa450Repl(TRB, cAliasSE1,cAliasSE2,aAutoCab, aImpos)
	...
	Local lMvLjIntFS 	:= SuperGetMV("MV_LJINTFS", , .F.) //Integraùùo com o Financial Services Habilitada?
	Local nPccTit		:= 0
	Local nIRFTit		:= 0
	Local lIntSJURI     := SuperGetMv("MV_JURXFIN",.T.,.F.) // Integraùùo com o mùdulo SIGAPFS
	Local lOk 			:= .T.
	Local aPcc			:= Array(7)
	...
	While !(cAliasSE2)->(Eof())
		...
		//------------------------------------------------------------------------------------------------
		// Caso tenha integracao SIGAPFS (MV_JURXFIN = .T.), valida as regras para manipulacao do tùtulo.
		//------------------------------------------------------------------------------------------------
		...
		Else
			...
			//Preparaùùo para a gravaùùo de retenùùo do PCC
			If Len(aPCC) > 4
				AAdd(aImpos, aPCC[5])
...
Static Function Fa450Inverte(cMarca,oTotalP,oTotalR,oSelecP,oSelecR,lTodos,aChaveLbn)
	...
	Local lValDocs		:= .T.
	Local lFinVDoc		:= IIF(GetNewPar("MV_FINVDOC","2")=="1",.T.,.F.)		//Controle de validacao de documentos obrigatorios
	Local lPrimeiro		:= .T. // na validaùùo de amarraùùo fin x documentos, deve passar apenas uma vez pelo help
	Local lSelecP		:= .F.
	Local lSelecR		:= .F.
	Local nVlPagar		:= 0
	Local nVlReceb		:= 0
	...
	While !lTodos .Or. !Eof()
		...
		lCompensa := .T.
		//--------------------------------------------------------------
		// So executa o P.E. quando o registro/titulo estù sendo marcado.
		// Nao chama na "desmarcaùùo".
		//--------------------------------------------------------------
		If ExistBlock("F450Conf") .and. MARCA != cMarca
			...
			If TRB->RECEBER != 0
				...
				dbSeek(Substr(TRB->CHAVE,1,nTamChavE1))
				//----------------------------------------------------------------------------------
				// Caso seja um Titulo ù Receber, o parametro passado para o PE terah conteudo "R",
			...
			Else
				...
				dbSeek(Substr(TRB->CHAVE,1,nTamChavE2))
				//--------------------------------------------------------------------------------
				// Caso seja um Titulo ù Pagar, o parametro passado para o PE terah conteudo "P",
			...
			If !(TRB->TIPO $ MVABATIM)
				...
				Else
					...
					If lValDocs
						...
						Else
							If !lTodos
								MsgAlert(STR0082, STR0083) // "Existe outro usuùrio utilizando o titulo. Nùo ù permitida a compensaùùo do titulo quando ele estù sendo utilizado por outro usuùrio" ## "Atenùùo"
							Endif
						Endif
					EndIf
				Endif
...
Static Function Fa450Edit(oSelecP,oSelecR,nTamChavE1,nTamChavE2)
	...
	@ 78, 09 SAY STR0101  SIZE 54, 7 OF oDlg PIXEL //"Valores Acessùrios"
...
Valida o valor digitado no ajuste de valores de compensaùùo
...
Funcao que verifica se Campos necessùrios estao preenchidos
...
User Function FA450CAN(cAlias, cCampo, nOpcx, aCampos, aAutoCab)
	...
	If !lF450Auto
		...
		If lAltFilial
			...
			@ 32+nEspLin, 14+nEspLarg SAY STR0062 SIZE 49, 07 OF oPanel4 PIXEL //"Nro. Compensaùùo"
		...
		Else
			...
			@ 11+nEspLin, 14+nEspLarg SAY STR0062 SIZE 49, 07 OF oPanel4 PIXEL //"Nro. Compensaùùo"
		Endif

		If lAltFilial
			cFilAnt := aFilCod[oCbx1:nAt] // Atualizar a CFilAnt nesse trecho, pois nùo ù atualizada acima. Necessùrio para o estorno quando tùtulo cp e cr estùo em filiais diferentes.
		EndIf
	...
	If nOpct == 1
		...
		If lRet .And. LockByName(cCompCan,.T.,.F.)
					...
					Else
						If SE1->(dbSeek(cChaveSe1))
							...
							If SE5->(DbSeek(xFilial("SE5")+cIdFk1))
								...
								//Gera os lanùamentos de Estorno/Cancelamento SE5
				...
				While (cAliasFK2)->(!Eof())
					...
					If SE2->(dbSeek(cChaveSe2))
						...
						If __lPccCC .and. lPccBxCP
							...
							//se tem impostos retidos, sù pode cancelar a ultima baixa.
							...
							//Restaura a variavel de contabilizaùùo que ù zerada pelo FINA080
							VALOR := VLRINSTR
			...
			End Transaction
		Elseif lRet
			MsgAlert(STR0084, STR0083) // "Existe outro usuùrio cancelando esta compensaùùo. Nùo ù permitido o cancelamento da mesma compensaùùo por dois usuùrios ao mesmo tempo." ## "Atenùùo"
		Endif
...
/*/{Protheus.doc} F450VerEst
	Funùùo responsavel por retornar se a operaùùo selecionada ù de estorno
	ou cancelamento, quando a rotina utiliza execauto
	@type  Function
	@author Vitor Duca
	@since 21/09/2021
	@version 1.0
	@return lEstorno, Logical, Indica se a operaùùo ù um estorno
/*/
...
Funùùo para montar as tabelas temporarias da FK1 e FK2

@param cCompCan - Identificador da compensaùùo 
...
lEstorno		Identifica se ù um processo de estorno (.T.) ou cancelamento (.F.)
...
Static Function Fa450GrvEst(lUsaFlag,lEstorno,aFlagCTB)
	...
	If AllTrim( SE5->E5_TABORI ) == cAliasFK
		...
		//Limpar o nùmero da compensaùùo nos valores acessorios (DC|MT|JR|CM|VA)
		SE5->(dbSetOrder(7))
...
3. Reservado                                                
4. Tipo de Transaùùo a ser efetuada:                       
...
Static Function MenuDef()
	...
	// Ponto de entrada para inclusao de uma nova opùùo no menu
...
Legenda da rotina, referentes a situaùùo dos titulos
...
Static Function Fa450Leg(nReg)
	...
	If Empty(nReg)
		uRetorno := {}
		IF lCtLiPag
			aAdd(uRetorno, {' Empty(E2_DATALIB) .AND. (SE2->E2_SALDO+SE2->E2_SDACRES-SE2->E2_SDDECRE) > GetMV("MV_VLMINPG") .AND. E2_SALDO > 0 ', aLegen[4][1]})// Titulo Aguardando Liberaùùo
		Endif
...
Verifica se o tùtulo sù tem saldo de impostos e nesse caso
verifica se o valor a compensar ù superior ao saldo a pagar
		...
		If nSelecP > nSelecR
			...
			While !Eof()
				If TRB->MARCA == cMarca .and. !Empty(TRB->PAGAR)
					...
					//no IF abaixo, o saldo do tùtulo ù o valor exato dos impostos. Exemplo: Tem um tùtulo de 10.000, mas informou 9535 no momento da baixa
					If SE2->E2_SALDO == SE2->(E2_PIS-E2_VRETPIS)+SE2->(E2_COFINS-E2_VRETCOF)+SE2->(E2_CSLL-E2_VRETCSL)
						...
						// para essa situaùùo o valor a ser compensado precisa ser maior ou igual ao saldo do tùtulo
					EndIf
				EndIf
				DbSkip()
			EndDo
			If nSelecR < nVlSldImp
				Aviso(STR0083,STR0095 ,{STR0094}) //"atenùùo" "Quando o saldo do tùtulo ù referente aos impostos e MV_CC10925 = 2, a compensaùùo precisa ser de valor igual ou maior que esse saldo."//"ok"
				lRet := .F.
			EndIf
			RestArea(aAreaSE2)
		EndIf
...
Obtùm o identificador do processo de CEC no cancelamento
...
Static Function F450VldCar()
	...
	If nSelecP == 0 .Or. nSelecR == 0
		...
		If nSelecP == 0
			...
			//STR0119 "Os tùtulos a pagar enviados via ExecAuto nùo foram encontrados." | STR0113 Nenhum tùtulo a pagar foi selecionado." | STR0120 "Selecione um tùtulo a pagar."
		Else
			...
			//STR0121 "Os tùtulos a receber enviados via ExecAuto nùo foram encontrados." | STR0114 Nenhum tùtulo a receber foi selecionado." | STR0122 "Selecione um tùtulo a receber."
		Endif
	EndIf

	IF __lVLDCRP //Ponto de Entrada para Validaùùes Complementares
...
@param nRecno    Recno do tùtulo da SE2 (Contas a pagar)

@Return lRet   .T. Se o tùtulo ù valido para ser manipulado.
...
Verifica a retenùùo dos impostos PCC e IRRF do Contas a Pagar
...
Static Function F450VerImp(nPccTit, nIrfTit, aPcc, aIrrf, nValPgto,lMarcaTit)
	...
	//Retorna se o IRRF ù retido na baixa e posiciona SA2/SED
	lIRRFBaixa := F450IsIrBx()

	If __lPccCC
		// essa chamada ù feita para alimentar a variùvel nPccRet e nIrfRet e para recalcular o PCC antes da baixa do tùtulo a pagar
...
Funùùo que busca os valores complementares da compensaùùo na FK6

@param cAlias ,A Tabela de Origem do Dado (Obrigatorio)
@param cIdFk  ,ID da Origem do Dado (Obrigatorio)
@param cTipo  ,Tipodoc especùfico (JR, MT, DC, VA)
...
Ajuste de base historica para gravaùùo do campo FK2_IDPROC
para ser utilizado na query da funùùo A450CalCn
...
Static Function A450RecFk2(cCompCan As Character, cAliasFK2 As Character, cAliasFK1 As Character)
	...
	While (cAliasSE5)->(!Eof())
		If FK2->(DbSeek(xFilial("FK2")+(cAliasSE5)->IDORIG))
			...
			Endif
		Else
			//Migra o registro de SE5 posicionado que ainda nùo foi migrado
...
Faz a verificaùùo se o IRRF ù retido na baixa - Contas a pagar
