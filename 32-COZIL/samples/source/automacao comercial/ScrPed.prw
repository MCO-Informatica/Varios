...
Local aProdGarantia :={}			// Array com os parโmetros do rkmake de impressใo do relat๓rio de Garantia
Local lMvGarFP	:= SuperGetMV("MV_LJGarFP", ,.F.)	    // Define o conc
Local lLibQtdGE	:= SuperGetMv("MV_LJLIBGE", , .F.) // Libera quantidade da garantia estendida  .. por default ้ Falso

If ValType(PARAMIXB) == "A" .AND. Len(PARAMIXB) > 0 .AND. ValType(PARAMIXB[1]) == "A" .AND. Len(PARAMIXB[1]) > 0
