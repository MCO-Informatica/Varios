...
for a:=1 to len(aItens)
	...
	oPrn:Box(nCol,0050, nCol + 3,3150)  					// Controle de Impressao das Pautas
		nCol := nCol + 40
	Endif
    ...
    For c:= 1 To Len (aItens[a,4])
		oPrn:Say(nCol + 40 - 10,0520, aItens[a, 4,c ]           ,oFont7,100) 	//Descricao
		nCol := nCol + 40 													
...
For c:= 1 To Len (aItens[a,4])
		oPrn:Say(nCol - 10,0520, aItens[a, 4,c ]           ,oFont7,100) 	//Descricao
		nCol := nCol + 40 													
		if nCol > 2140  //1800												// Controle de Impressao de Pautas
			lCont := .T.   ;  ImpRodaP()  ;  ImpCabecP()  ;  nCol := 890
		Endif		
...
oPrn:SayBitmap(1940,2550,cBitIso,300,300 )
...
oPrn:Box(0790,0188, 2200,0510)    					//Codigo
oPrn:Box(0790,0513, 2200,1963)    					//Descricao
oPrn:Box(0790,1966, 2200,2290)    					//Foto
oPrn:Box(0790,2293, 2200,2503)    					//Preco Unitario
oPrn:Box(0790,2506, 2200,2585)    					//Aliquota IPI
oPrn:Box(0790,2588, 2200,2847)    					//Valor do IPI
oPrn:Box(0790,2850, 2200,3150)	  					//Valor Total

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Terceira Caixa de Texto - Totais					   ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrn:Box(2290,0050, 2210,3150)
...
Endif
oPrn:Say(0300,2250, "Vendedor: "+cNomVen                       ,oFont3,100)
oPrn:Say(0350,2250, "Tel:      "+"("+cDddVen+") - "+cTelVen    ,oFont3,100)
oPrn:Say(0400,2250, "E-Mail   "+cEmaVen                        ,oFont3,100)

If SCJ->CJ_TPFRETE = "C"
	oPrn:Say(0450,2250, "Frete:  CIF"    ,oFont3,100)  
ElseIf SCJ->CJ_TPFRETE = "F"
	oPrn:Say(0450,2250, "Frete:  FOB"    ,oFont3,100)  

EndIf

If SCJ->CJ_FRETE <> 0 
	oPrn:Say(0500,2250, "Valor Frete: "+Transform(SCJ->CJ_FRETE,"@e 999,999,999.99")       ,oFont3,100)
ElseIf nFreteTotal <> 0
	oPrn:Say(0500,2250, "Valor Frete: "+Transform(nFreteTotal,"@e 999,999,999.99")       ,oFont3,100)
Else
	oPrn:Say(0500,2250, "Valor Frete: NÃO POSSUI VALOR"     ,oFont3,100)
Endif
oPrn:Say(0550,2250, "Condição de Pagamento: "+cCondPag ,oFont3,100)
...
oPrn:Say(0830,0520, "Descricao do Equipamento"     		,oFont2,100)
...
if lCont
	...
	oPrn:Say(2225,2650, cTotal                         ,oFont4,100)
   ...
   If SCJ->CJ_XTPPV == "A"	// Se for um orçamento de assistencia tecnica, a impressao é diferente
   		...
   		oPrn:Say(nCol + 160,0030, "1.Pagamento: SUJEITO A APROVAÇÃO PELA ÁREA FINANCEIRA",oFont5,100)
   		oPrn:Say(nCol + 220,0030, "2.Prazo de Entrega de peças e serviços: 05 dias após aprovação via e-mail.",oFont5,100)
   		oPrn:Say(nCol + 280,0030, "3.Validade da proposta: 10 Dias",oFont5,100)
   		oPrn:Say(nCol + 340,0030, "4.Frete: Por conta do cliente",oFont5,100)
   		oPrn:Say(nCol + 400,0030, "5.Considerações: Solicitamos que a confirmação do orçamento seja assinada e devolvida via fax ou e-mail nos contatos: ",oFont5,100)
   		oPrn:Say(nCol + 460,0050, "(11) 2832-8066 / assistenciatecnica@cozil.com.br",oFont5,100)
