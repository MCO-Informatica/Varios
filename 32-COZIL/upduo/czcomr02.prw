...
/*/
User Function CZCOMR02( cAlias, nReg, nOpcx )
...
While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == cFilSC7 .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
	...
	For nVias := 1 to mv_par09
		...
		While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == cFilSC7 .And. SC7->C7_NUM == cNumSC7
			...
			nValTotSC7 := xMoeda(SC7->C7_TOTAL,SC7->C7_MOEDA,Val(cMoeda),SC7->C7_DATPRF,MsDecimais(SC7->C7_MOEDA),nTxMoeda)
		...
		TransForm(xMoeda(nDescProd,SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF,MsDecimais(SC7->C7_MOEDA),nTxMoeda) , cPicC7_VLDESC ),;
		2022 , 050 )
		...
		If Empty(MV_PAR13) //"Local de Entrega  : "
			...
			oReport:PrintText("Local de Entrega: "+Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_END"))+" - "+Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_MUN"))+"/"+Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_EST"))+" CEP: "+Trans(Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_CEP")),cPicA2_CEP),, 050 )
		Else
		...
		oReport:PrintText("Local de Cobranca: "+Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_END"))+" - "+Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_MUN"))+"/"+Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_EST"))+" CEP: "+Trans(Alltrim(Posicione("SA2",1,xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_CEP")),cPicA2_CEP),, 050 )
		...
		nLinPC := oReport:Row()
		oReport:PrintText( STR0073 + Transform(xMoeda(nTotFrete,SC7->C7_MOEDA,Val(cMoeda),SC7->C7_DATPRF,MsDecimais(SC7->C7_MOEDA),nTxMoeda) , tm(nTotFrete,14,MsDecimais(Val(cMoeda)))) ,nLinPC,1320 ) //"Frete    :"
		oReport:PrintText( STR0074 + Transform(xMoeda(nTotDesp ,SC7->C7_MOEDA,Val(cMoeda),SC7->C7_DATPRF,MsDecimais(SC7->C7_MOEDA),nTxMoeda) , tm(nTotDesp ,14,MsDecimais(Val(cMoeda)))) ,nLinPC,1815 ) //"Despesas :"
		oReport:SkipLine()
	...
	//Incluida validação para os pedidos de compras por item do pedido  (IP/alçada)			
		...
		If !lNewAlc
			...
			If SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3
				...
				oReport:PrintText("NOTA: Não aceitaremos titulos descontados. Factoring, banco entre outros.",,050 )
			Else
...
If !lNewAlc
	...
	@ li,142 - nDifColCC PSAY "|"
	li++
	@ li,001 PSAY "NOTA: Não aceitaremos titulos descontados. Factoring, banco entre outros."
	@ li,142 - nDifColCC PSAY "|"
	li++
	@ li,001 PSAY "|"
	@ li,002 PSAY Replicate("-",limite)
