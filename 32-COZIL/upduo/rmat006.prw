...
±±ºPrograma  ³ RMAT006  ºAutor  ³William L. Gurzoni  º Data ³  22/10/2012 º±±
...
//MsgAlert("Este relatório ainda está em fase de teste. Não utilize-o como forma principal para obtenção de dados.", "Atenção - Relatório alterado (v1.4)")
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ GeraEstrutura    ³ William L. Gurzoni º Data ³ 22/10/2012  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Static Function GeraEstrutura(cProduto)
	...
	Local i, nPosiInterna
	Local nCont	  // Variavel utilizada para buscar a ultima ocorrencia de nivel na estrutura
	Local nNivelAtual 	:= 0
	Local nNivel		:= 1
	Local cCodAtual	:= ""
	Local nQtBase		:= aCabec[1][5]
	Local tmp := {}	

	Local cProcesso := ""
	i	:= 1

	cProcesso := iif(empty(SG1->G1_XPROC01),"",capital(alltrim(SG1->G1_XPROC01)) ) + ;
				iif(empty(SG1->G1_XPROC02),""," / " + capital(alltrim(SG1->G1_XPROC02)) ) + ;
				iif(empty(SG1->G1_XPROC03),""," / " + capital(alltrim(SG1->G1_XPROC03)) )

	if empty(cProcesso)
		cProcesso := "- Selecione o Processo -"
	endif
	
	// Da carga no primeiro item do array	
	Aadd(	aEstrutura, {	SG1->G1_TRT											,;	// [01] SEQUENCIA
						SG1->G1_COD											,;	// [02] CODIGO DO PRODUTO
						SG1->G1_COD											,;	// [03] COMPONENTE
						nNivel												,;	// [04] NIVEL
						Alltrim(Str(SG1->G1_XQTDUNI * aCabec[1][5]))				,;	// [05] Quantidade Total
						AllTrim(SG1->G1_UMUNIT )									,;	// [06] Unidade de Medida Unitaria
						Alltrim(Str(SG1->G1_XQTDUNI * nQtBase))						,;	// [07] Quantidade Unitaria
						...
						cProcesso												})	// [13] Processo
	tmp := aEstrutura
	// Cria estrutura de array para os demais itens da estrutura
	While Len(aEstrutura) >= i 
		SG1->(MsSeek(xFilial("SG1") + aEstrutura[i][3]))
		If (SG1->(!EOF()))                    
			cCodAtual		:= SG1->G1_COD                                                    
			
			//Busca a ultima ocorrencia de nivel no array. Pois caso haja itens repetidos em 
			//niveis diferentes devera sempre utilizar o ultimo adicionado no array      
			/*nCont := 1
			While nCont <= Len(aEstrutura)
				If (AScan(aEstrutura, {|x| x[3]==SG1->G1_COD .And. x[1] == SG1->G1_TRT}, nCont)) != 0
					nNivelAtual 	:= aEstrutura[AScan(aEstrutura, {|x| x[3]==SG1->G1_COD .And. x[1] == SG1->G1_TRT}, nCont)][4] + 1
				EndIf                                                
				nCont := nCont + 1
			EndDo*/
			nNivelAtual := aEstrutura[i][4] + 1
		EndIf           
		
		nPosiInterna := i
		
		While SG1->(!EOF()) .And. AllTrim(SG1->G1_COD) == AllTrim(aEstrutura[i][3])
		    	...
		    	If nNivelAtual > 2
		    	     // Replica o ultimo elemento
				Aadd(aEstrutura, aEstrutura[Len(aEstrutura)])   
				// Adiciona elemente nulo no array para acerto dos niveis				
		    		Ains(aEstrutura, (nPosiInterna := nPosiInterna + 1))
		    		// Adiciona elemento de nivel inferior no array
		    		aEstrutura[nPosiInterna]	:= { ;
					SG1->G1_TRT											,;	// [01] SEQUENCIA
					SG1->G1_COD											,;	// [02] CODIGO DO PRODUTO
					SG1->G1_COMP											,;	// [03] COMPONENTE
					nNivelAtual 											,;	// [04] NIVEL
					Alltrim(Str(SG1->G1_XQTDUNI * aCabec[1][5]))				,;	// [05] Quantidade Total
					AllTrim(SG1->G1_UMUNIT )									,;	// [06] Unidade de Medida Unitaria
...
Private	cVersao			:= "1.4"	//Versao do relatorio	
...
i := j := k := 2 // Inicia com dois devido o primeiro item do array ser o pai da arvore
...
While i <= Len(aEstrutura) 
	...
	cLinha += '	    <Cell ss:StyleID="s107"><Data ss:Type="String">' + aEstrutura[i][13] + '</Data><NamedCell ss:Name="Print_Area"/></Cell>	 ' + Chr(13)+Chr(10)	//cLinha += '	    <Cell ss:StyleID="s107"><Data ss:Type="String">- Selecione o Processo -</Data><NamedCell ss:Name="Print_Area"/></Cell>	 ' + Chr(13)+Chr(10)
	...
	cLinha += '	    <Cell ss:StyleID="s110"><Data ss:Type="String">' + AllTrim(Str(aEstrutura[i][4])) + '</Data><NamedCell ss:Name="Print_Area"/></Cell>	 ' + Chr(13)+Chr(10)
...
If lExec .Or. ( !lProc .And. !lExec )
	If !Empty(cPath)
		...
		Else
			...
			cScript += '	  </Names>	 ' + Chr(13)+Chr(10)
			cScript += '	  <Table ss:ExpandedColumnCount="15" ss:ExpandedRowCount="1000" x:FullColumns="1" x:FullRows="1" ss:DefaultRowHeight="15">	 ' + Chr(13)+Chr(10)
