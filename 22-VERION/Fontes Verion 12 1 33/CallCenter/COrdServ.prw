...
// montagem das bolinhas coloridas
	aCores:= {	{'ZA_STATUS == "B"'  , 'BR_BRANCO'        },;
		...
		{'ZA_STATUS == "R"'  , 'BR_CANCEL'     },;
		{'ZA_STATUS == "L"'  , 'BR_PINK'          },;
		{'ZA_STATUS == "S"'  , 'BR_AMARELO'       },;
		{'ZA_STATUS == "J"'  , 'BR_CINZA'         },;
		{'ZA_STATUS == "O"'  , 'BR_AZUL'          },;
		{'ZA_STATUS == "P"'  , 'BR_LARANJA'         },;
		{'ZA_STATUS == "M"'  , 'BR_MARRON'   },;
		{'ZA_STATUS == "C"'  , 'BR_MARROM'   },;
		{'ZA_STATUS == "W"'  , 'BR_AZUL_CLARO'    },;
		{'ZA_STATUS == "Y"'  , 'BR_PRETO'        },;
		{'ZA_STATUS == "X"'  , 'BR_VIOLETA'      },;
		{'ZA_STATUS == "Z"'  , 'BR_MARRON_OCEAN'       },;
		{'ZA_STATUS == "E"'  , 'DISABLE'          },;
		{'ZA_STATUS == "Q"'  , 'BR_PRETO_3'         },;
		{'ZA_STATUS == "T"'  , 'BR_VERDE_ESCURO'        }}
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	Local aPages	    := {"HEADER"}
	Local aTitles	    := {"Modificação","Serv.Executado","Teste","Relatorio","Solicitação"}  // FOLDER´S DO PROGRAMA
	Local aArrayF4	    := {}
	Local aArrayline    := {}
	Local aHeadCpos	    := {}
	Local _cCD1         := ""
	...
	Private aReg        := {}
	Private APOSOBJ     := {}
	Private aOper       := {"","1-Conserto","2-Estoque","3-Manutencao","4-Montagem Hidraulica","4-Montagem Eletronica","5-Transformacao","6-Usinagem","7-Fabrica","8-Eletronica","9-Engenharia"}
	Private aOpGr       := {"","S-Sim","N-Nao"}
	Private aAcao       := {"","1-Compras","2-Cortar Barra"}
	Private cAcao		:= ""
	Private aPecas      := {"","1-Sim","2-Nao"}
	Private cPecas		:= ""
...
// ALTERACAO
	IF  !RetCodUsr() $ cUsuMAST
		If _nmnu = 4
			IF SZA->ZA_STATUS $ "E/S/J/O/M/C/W/Y/Z/X/Q/T"
				_nvis := 1
				_nmnu := 1
				...
				ELSEIF SZA->ZA_STATUS $ "Q/T"
					MSGALERT("Ordem de Servico, no processo de ENGENHARIA. Operação de ALTERAÇÃO, invalida !!!", "Ordem de Servico - Status Pendente")
				ENDIF
			ENDIF
		ENDIF
	ENDIF
	...
	Else
		...
		_cOrdPr   := SZA->ZA_OP
		coper     := IIF(SUBSTR(SZA->ZA_TIPO,1,1) = "1","1-Conserto",IIF(SUBSTR(SZA->ZA_TIPO,1,1) ="2","2-Estoque",IIF(SUBSTR(SZA->ZA_TIPO,1,1) ="3","3-Manutencao",IIF(SUBSTR(SZA->ZA_TIPO,1,1) ="4","4-Montagem Hidraulica",IIF(SUBSTR(SZA->ZA_TIPO,1,1) ="4","4-Montagem Eletronica",IIF(SUBSTR(SZA->ZA_TIPO,1,1) ="5","5-Transformacao",IIF(SUBSTR(SZA->ZA_TIPO,1,1) ="6","6-Usinagem",IIF(SUBSTR(SZA->ZA_TIPO,1,1) ="7","7-Fabrica","9-Engenharia"))))))))
		CVEND     := SZA->ZA_VEND
		...
		cAcao     := IIF(Val(SZA->ZA_ACAO)>0,aAcao[Val(SZA->ZA_ACAO)+1],"")
		cPecas    := IIF(Val(SZA->ZA_PECAS)>0,aPecas[Val(SZA->ZA_PECAS)+1],"")
		//coper     := SZA->ZA_XDESCTP
	Endif
...
// Campos do Cabecalho Padrão
	...
	If _nmnu = 3  //inclusao
		...
		@ 040,425 MSGET _cPedve                           OF oDlg PIXEL SIZE 045,006 F3 "SC5" when .f.
	endif

	@ 040,475 SAY OemToAnsi("Ação")                 OF oDlg PIXEL SIZE 045,006
	If _nmnu = 3  //inclusao
		@ 040,500 MSCOMBOBOX ooper VAR cAcao ITEMS aAcao  OF oDlg PIXEL SIZE 055,010
	Else
		@ 040,500 MSCOMBOBOX ooper VAR cAcao ITEMS aAcao  OF oDlg PIXEL SIZE 055,010 //when .f.
	endif

	@ 025,520 SAY OemToAnsi("Pendente Peças")             OF oDlg PIXEL SIZE 045,006
	If _nmnu = 3  //inclusao
		@ 025,560 MSCOMBOBOX ooper VAR cPecas ITEMS aPecas  OF oDlg PIXEL SIZE 035,010
	Else
		@ 025,560 MSCOMBOBOX ooper VAR cPecas ITEMS aPecas  OF oDlg PIXEL SIZE 035,010 //when .f.
	endif
...
//endif
	...
	@ 055,405 SAY OemToAnsi("Operecao")               OF oDlg PIXEL SIZE 055,006
	If _nmnu = 3  //inclusao
		@ 055,440 MSCOMBOBOX ooper VAR coper ITEMS aoper  OF oDlg PIXEL SIZE 075,010
	Else
		@ 055,440 MSCOMBOBOX ooper VAR coper ITEMS aoper  OF oDlg PIXEL SIZE 075,010 //when .f.
	Endif


	@ 055,520 SAY OemToAnsi("Gera OP")               OF oDlg PIXEL SIZE 045,006
	If _nmnu = 3  //inclusao
		@ 055,550 MSCOMBOBOX ooger VAR _cGerOp ITEMS aOpGr  OF oDlg PIXEL SIZE 035,010
	Else
		@ 055,550 MSCOMBOBOX ooger VAR _cGerOp ITEMS aOpGr  OF oDlg PIXEL SIZE 035,010 when .f.
	Endif
...
// Campos do Grid Contatos
	...
	While !Eof() .And. SX3->X3_ARQUIVO == "SZF"
		...
		If X3Uso(SX3->X3_USADO) .And. cNivel >= SX3->X3_NIVEL
			Aadd(aHeadSZF   , { AllTrim(X3Titulo()),SX3->X3_CAMPO  ,SX3->X3_PICTURE,SX3->X3_TAMANHO,SX3->X3_DECIMAL,;
				SX3->X3_VALID      ,SX3->X3_USADO  ,SX3->X3_TIPO   ,SX3->X3_F3     ,SX3->X3_CONTEXT,;
				X3Cbox()           ,SX3->X3_RELACAO,".T."})
			nUsado++
		EndIf
	...
	If _nmnu = 3  //inclusao
		...
		aAuxSZF := {}
		For nX := 1 to Len(aheadSZF)
			IF ALLTRIM(aheadSZF[NX,8])=="C"
				Aadd(aAuxSZF,SPACE(aheadSZF[NX,4]))
			ELSEIF ALLTRIM(aheadSZF[NX,8])=="N"
				Aadd(aAuxSZF,0)
			ELSEIF ALLTRIM(aheadSZF[NX,8])=="D"
				Aadd(aAuxSZF,CTOD("  /  /  "))
			ENDIF
		Next nX

		Aadd(aAuxSZF,.F.)
		Aadd(aColSZF,aAuxSZF)
	else
		// SELECT dos CONTATOS DA EMPRESA CONTRIBUINTE
		/*
		cquer5 := ""
		cquer5 := "SELECT ZF_FILIAL,ZF_IDOS,ZF_NUMSC,ZF_PRODUTO,MAX(C7_DATPRF) C7_DATPRF, MAX(C7_NUM) C7_NUM "
		cquer5 += " FROM "+RetSqlName("SZF")+" SZF"
		cquer5 += " LEFT JOIN "+RetSqlName("SC7")+" SC7 ON C7_NUMSC = ZF_NUMSC AND ZF_PRODUTO = C7_PRODUTO AND SC7010.D_E_L_E_T_ =' ' "
		cquer5 += " WHERE SZF.D_E_L_E_T_ = ' ' AND ZF_IDOS = '"+_cNros+"' GROUP BY ZF_FILIAL, ZF_IDOS, ZF_NUMSC, ZF_PRODUTO ORDER BY ZF_IDOS, ZF_NUMSC ASC "
		*/

		cquer5 := ""
		cquer5 := "SELECT * "
		cquer5 += " FROM "+RetSqlName("SZF")+" SZF"
		cquer5 += " WHERE SZF.D_E_L_E_T_ = ' ' AND ZF_IDOS = '"+_cNros+"' ORDER BY ZF_IDOS, ZF_NUMSC ASC "
		...
		While !Eof()
			...
			//_cCT1  := quer5->ZF_NUMSC
			AADD(aAuxSZF ,quer5->ZF_PRODUTO)
			AADD(aAuxSZF ,quer5->ZF_NUMSC)
			AADD(aAuxSZF ,StoD(quer5->ZF_DTNECES))
			AADD(aAuxSZF ,quer5->ZF_IDOS)
			AADD(aAuxSZF ,.F.)
...
// Monta Grid de Contatos Vazio.
	oOcorSZF := MsNewGetDados():New(000,000,175,645,nOpc,"U_ASZFLinOk","U_ASZFTUDOK","",,,999,"AllwaysTrue","","AllwaysTrue",oFolder:aDialogs[5],aHeadSZF,acolSZF)
...
// Exclusao                 2
	If _nmnu = 5
		...
		DBSELECTAREA("SZF")
		DBSETORDER(1)
		IF DBSEEK(XFILIAL("SZF")+_cNros)
			WHILE !EOF() .AND. SZF->ZF_IDOS == _cNros
				DBSELECTAREA("SZF")
				RECLOCK("SZF",.f.)
				DbDelete()
				MsUnLock("SZF")
...
// PROCESSO DE INCLUSAO DOS DADOS !!!!
	...
	ElseIf SUBSTR(COPER,1,1) $ "9"  // os da eletronica
		IF _nmnu = 3  // inclusao de os
			ZA_STATUS    := "Q"
		endif
	Endif
	...
	IF LEN(oOcorSZB:ACOLS) > 0
		...
		For nCntFor := 1 To Len(oOcorSZB:ACOLS)
			If ( !oOcorSZB:aCols[nCntFor][7] )  // valida se a linha esta ativa ou nao....
				...
				//Adicionados campos que podem ser alterados (Caire)
				SZB->ZB_SERVICO := oOcorSZB:aCols[nCntFor][aScan(oOcorSZB:aHeader,{|x| Alltrim(x[2]) == "ZB_SERVICO"})]
				SZB->ZB_QTDADE 	:= oOcorSZB:aCols[nCntFor][aScan(oOcorSZB:aHeader,{|x| Alltrim(x[2]) == "ZB_QTDADE"})]
				SZB->ZB_PRODUTO := oOcorSZB:aCols[nCntFor][aScan(oOcorSZB:aHeader,{|x| Alltrim(x[2]) == "ZB_PRODUTO"})]
				SZB->ZB_OBS 	:= oOcorSZB:aCols[nCntFor][aScan(oOcorSZB:aHeader,{|x| Alltrim(x[2]) == "ZB_OBS"})]
	...
	IF LEN(oOcorSZC:ACOLS) > 0
		...
		For nCntFor := 1 To Len(oOcorSZC:ACOLS)
			If ( !oOcorSZC:aCols[nCntFor][20] )
				...
				//Adicionados campos que podem ser alterados (Caire)
				SZC->ZC_TESTE 	:= 	oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_TESTE"})]
				SZC->ZC_VAZAO 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_VAZAO"})]
				SZC->ZC_RPM 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_RPM"})]
				SZC->ZC_PRES001 :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_PRES001"})]
				SZC->ZC_PRES002 :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_PRES002"})]
				SZC->ZC_PRES003 :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_PRES003"})]
				SZC->ZC_PRES004 :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_PRES004"})]
				SZC->ZC_PRES005 :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_PRES005"})]
				SZC->ZC_DRENO 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_DRENO"})]
				SZC->ZC_VALALIV :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_VALALIV"})]
				SZC->ZC_PILOTO 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_PILOTO"})]
				SZC->ZC_PRESCAR :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_PRESCAR"})]
				SZC->ZC_TEMPERA :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_TEMPERA"})]
				SZC->ZC_DTINICI :=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_DTINICI"})]
				SZC->ZC_HRINI 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_HRINI"})]
				SZC->ZC_DTFIM 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_DTFIM"})]
				SZC->ZC_HRFIM 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_HRFIM"})]
				SZC->ZC_HRREAL 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_HRREAL"})]
				SZC->ZC_OUTROS 	:=  oOcorSZC:aCols[nCntFor][aScan(oOcorSZC:aHeader,{|x| Alltrim(x[2]) == "ZC_OUTROS"})]
	...
	IF LEN(oOcorSZD:ACOLS) > 0
		...
		For nCntFor := 1 To Len(oOcorSZD:ACOLS)
			If ( !oOcorSZD:aCols[nCntFor][12] )
				...
				//Adicionados campos que podem ser alterados (Caire)
				SZD->ZD_QTDSAI 	:= oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_QTDSAI"})]
				SZD->ZD_PRODUTO := oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_PRODUTO"})]
				SZD->ZD_SLDSAI 	:= oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_SLDSAI"})]
				SZD->ZD_QTDEENT := oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_QTDEENT"})]
				SZD->ZD_PRODENT := oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_PRODENT"})]
				SZD->ZD_DESCENT := oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_DESCENT"})]
				SZD->ZD_SLDENT 	:= oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_SLDENT"})]
				SZD->ZD_OP 		:= oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_OP"})]
				SZD->ZD_OBS 	:= oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_OBS"})]
				SZD->ZD_TPOPERA := oOcorSZD:aCols[nCntFor][aScan(oOcorSZD:aHeader,{|x| Alltrim(x[2]) == "ZD_TPOPERA"})]
...
// TABELA SZE --> RELATORIO
			...
			IF LEN(oOcorSZE:ACOLS) > 0
				...
				For nCntFor := 1 To Len(oOcorSZE:ACOLS)
					If ( !oOcorSZE:aCols[nCntFor][8])
						...
						//Adicionados campos que podem ser alterados (Caire)
						SZE->ZE_EMISSAO := oOcorSZE:aCols[nCntFor][aScan(oOcorSZE:aHeader,{|x| Alltrim(x[2]) == "ZE_EMISSAO"})]
						SZE->ZE_HORAINI := oOcorSZE:aCols[nCntFor][aScan(oOcorSZE:aHeader,{|x| Alltrim(x[2]) == "ZE_HORAINI"})]
						SZE->ZE_DTFIM   := oOcorSZE:aCols[nCntFor][aScan(oOcorSZE:aHeader,{|x| Alltrim(x[2]) == "ZE_DTFIM"})]
						SZE->ZE_HORAFIM := oOcorSZE:aCols[nCntFor][aScan(oOcorSZE:aHeader,{|x| Alltrim(x[2]) == "ZE_HORAFIM"})]
						SZE->ZE_FUNC    := oOcorSZE:aCols[nCntFor][aScan(oOcorSZE:aHeader,{|x| Alltrim(x[2]) == "ZE_FUNC"})]
						SZE->ZE_NOME    := oOcorSZE:aCols[nCntFor][aScan(oOcorSZE:aHeader,{|x| Alltrim(x[2]) == "ZE_NOME"})]
						SZE->ZE_OBS     := "ALTERAÇÃO"
	...
	IF LEN(oOcorSZF:ACOLS) > 0
		...
		If Len(oOcorSZF:ACOLS) > 0
			...
			// INCLUSAO DE DADOS NA TABELA SZD....
			For nCntFor := 1 To Len(oOcorSZF:ACOLS)
				//If ( !oOcorSZF:aCols[nCntFor][12] )

				//If !oOcorSZF:acolS[nCntFor,5]

				If !oOcorSZF:acolS[nCntFor][Len(oOcorSZF:acolS[nCntFor])]
					DBSELECTAREA("SZF")
					RECLOCK("SZF",.T.)

					SZF->ZF_FILIAL 	 := XFILIAL("SZD")
					SZF->ZF_IDOS   	 := _cNros
					SZF->ZF_PRODUTO  := oOcorSZF:acolS[nCntFor,1]
					SZF->ZF_NUMSC  	 := oOcorSZF:acolS[nCntFor,2]
					SZF->ZF_DTNECES  := oOcorSZF:acolS[nCntFor,3]
...
Static Function MenuDef()
	...
	aSZAENG:= { {"Apontar"      ,"u_OSAPONTA()"    ,0,5,0,NIL} ,;
		{"Pendente"     ,"u_OSPENDEN(3)"   ,0,5,0,NIL} ,;
		{"Estornar"     ,"u_OSESTORN()"   ,0,5,0,NIL} ,;
		{"Encerrar"     ,"u_OSENCERR(4)"   ,0,5,0,NIL}}
	...
	Private aRotina := {{ "Pesquisar"   ,"AxPesqui"         ,0,1,0,.F.},;
		...
		{ "Engenharia"  ,aSZAENG         	,0,5,0,NIL},;
		{ "Rel Analise Estr."  ,"U_MATX875()" ,0,5,0,NIL},;
...
USER FUNCTION ASZFLinOk()
	lSZF2 := .T.

	/*_nPosS01   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_IDOS"})
	_nQtdS01   := oOcorSZF:acolS[ii,_nPosS01] // SALDO SAIDA*/

	_nPosS02   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_PRODUTO"})
	_nQtdS02   := oOcorSZF:acolS[n,_nPosS02] // SALDO SAIDA

	_nPosS03   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_NUMSC"})
	_nQtdS03   := oOcorSZF:acolS[n,_nPosS03] // SALDO SAIDA

	_nPosS04   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_DTNECES"})
	_nQtdS04   := oOcorSZF:acolS[n,_nPosS04] // SALDO SAIDA
...
// -------------------------------------------------
// FUNCAO NA VALIDACAO DA TODO O FOLDER MODIFICACAO
USER FUNCTION ASZFTUDOk()
	lSZF2T := .T.
	Local ii := 1

// Verifica se máquina informada existe
	For ii:= 1 to len(oOcorSZF:aCols)

		//If !oOcorSZF:aCols[ii][12]

		/*_nPosS01   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_IDOS"})
		_nQtdS01   := oOcorSZF:acolS[ii,_nPosS01] // SALDO SAIDA*/

		_nPosS02   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_PRODUTO"})
		_nQtdS02   := oOcorSZF:acolS[ii,_nPosS02] // SALDO SAIDA

		_nPosS03   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_NUMSC"})
		_nQtdS03   := oOcorSZF:acolS[ii,_nPosS03] // SALDO SAIDA

		_nPosS04   := ascan(aHeadSZF,{|_vAux|alltrim(_vAux[2])=="ZF_DTNECES"})
		_nQtdS04   := oOcorSZF:acolS[ii,_nPosS04] // SALDO SAIDA
...
User Function LEGM80()
	BRWLEGENDA(cCADASTRO,"Legenda",{{"BR_BRANCO"      	,"Gerar OP"        },;
		{"ENABLE"         	,"Inclusao"        },;
		{"BR_CANCEL"      	,"Recusa Almox"    },;
		{"BR_PINK"        	,"Aprovado"        },;
		{"BR_AMARELO"     	,"Separação"       },;
		{"BR_CINZA"       	,"Pendente Almox"  },;
		{"BR_AZUL"        	,"Fabrica"         },;
		{"BR_LARANJA"     	,"Pendente Fabr"   },;
		{"BR_MARRON"      	,"Recusa Fabrica"  },;
		{"BR_MARROM"      	,"Encerrado Fab"   },;
		{"BR_AZUL_CLARO"  	,"Eletronica"      },;
		{"BR_PRETO"       	,"Pendente Eletr"  },;
		{"BR_VIOLETA"       ,"Recusa Eletr"    },;
		{"BR_PRETO_3"       ,"Pendente Eng"    },;
		{"BR_VERDE_ESCURO"  ,"Encerrado Eng"   },;
		{"DISABLE"          ,"Encerrada"       }})
/*
BRWLEGENDA(cCADASTRO,"Legenda",{{"BR_BRANCO"      ,"Gerar OP"        },;
								{"ENABLE"         ,"Inclusao"        },;
								...
								{"Pend_Eng"       ,"Pendente Eng"    },;     								
								{"Encer_Eng"      ,"Encerrado Eng"   },;     								
								{"DISABLE"        ,"Encerrada"       }})*/
...
IF EMPTY(SZA->ZA_STATUS) .OR. SZA->ZA_STATUS $ "L/S/O"
	...
	ELSEIF _NDEPTO = 3
		SZA->ZA_STATUS := "Q"
		SZA->ZA_RECUSA := __CUSR + " - " + ALLTRIM(UsrRetName(__CUSR)) + " - PENDENTE ENGENHARIA EM : " + DTOC(DDATABASE)+ " - " + TIME()
	ENDIF
	MSUNLOCK("SZA")
...
USER FUNCTION OSENCERR(_NDTP)
	...
	IF _NDTP = 2
		...
		ENDIF
		RETURN
	ELSEIF _NDTP = 4     // encerra a ordem de servico sem gerar movimentacao no estoque Pela ENGENHARIA
		IF !lMsErroAuto
			_DAPODT01 := CTOD("  /  /  ")  // data inicio
			_CAPOT001 := space(06) // hora inicio
			_DAPODT02 := CTOD("  /  /  ")  // data final
...
USER FUNCTION OSESTORN()
	...
	IF SZA->ZA_STATUS $ "S/J/M/Q"

		IF SZA->ZA_STATUS $ "S/M/Q"
			// ARQUIVO DA MODIFICACAO
			DBSELECTAREA("SZD")
			DBSETORDER(1)
			IF DBSEEK(XFILIAL("SZD")+__CZDOS)
