...
Local lFatura := IIf(SUA->UA_VKGFAT == 'S',.T.,.F.)
	...
	IF DBSEEK(XFILIAL("SUB")+_CORC)
		WHILE !EOF() .AND. _CORC == SUB->UB_NUM
			...
			IF DBSEEK(XFILIAL("SC6")+_CPEDIDO+_CIT+_CPRD)
				...
				SC6->C6_QTDLIB  := IIF(lFatura,SUB->UB_QTDLIB,0)
				SC6->C6_QTDEMP  := IIF(lFatura,SUB->UB_QTDLIB,0)
			...
			IF SUB->UB_QTDLIB > 0 .AND. lFatura
               RECLOCK("SC9",.T.)                         
