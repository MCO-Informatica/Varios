...
description Relatorio de Pedido de Venda
@author  author Caire Vieira
@since   date 18/10/2022
@version P12.1.33 V1
/*/
//-------------------------------------------------------------------

User Function Matr731a()
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	...
	Local wnrel   := "MATR73A" // Nome do Arquivo utilizado no Spool
	Local nomeprog:= "MATR73A"
	Local cPerg   := "MTR730    "

	Private Tamanho := "G" // P/M/G
//Private Limite  := 132 // 80/132/220   220
	Private Limite  := 220 // 80/132/220   220
...
Static Function ImpItem(nItem,nPesLiq,li,nTotQtd,nTotVal,nPesBru,cAliasSC6,cAliasSC5)
	...
	If MV_PAR03 <> 1
		@li,025 psay SUBS(IIF(Empty((cAliasSC6)->C6_DESCRI),SB1->B1_DESC,(cAliasSC6)->C6_DESCRI),1,80)//30
		@li,100 psay GetAdvFVal( "SB2", "B2_LOCALIZ", xFilial("SB2")+(cAliasSC6)->C6_PRODUTO+(cAliasSC6)->C6_LOCAL, 1, "" )
	Else
		@li,025 psay SUBS(IIF(Empty((cAliasSC6)->C6_DESCRI),SB1->B1_DESC,(cAliasSC6)->C6_DESCRI),1,80)//30
		@li,090 psay GetAdvFVal( "SB2", "B2_LOCALIZ", xFilial("SB2")+(cAliasSC6)->C6_PRODUTO+(cAliasSC6)->C6_LOCAL, 1, "" )
	EndIf

	If MV_PAR03 <> 1
		@li,124 psay (cAliasSC6)->C6_TES//050
	EndIf

	If MV_PAR03 <> 1
		@li,128 psay (cAliasSC6)->C6_UM//054
		@li,131 psay (cAliasSC6)->C6_QTDVEN	Picture PesqPict("SC6","C6_QTDVEN",10)//056
	Else
		@li,108 psay (cAliasSC6)->C6_UM//054
		@li,111 psay (cAliasSC6)->C6_QTDVEN	Picture PesqPict("SC6","C6_QTDVEN",10)//056
	EndIf

	If MV_PAR03 <> 1
		@li,141 psay (cAliasSC6)->C6_PRCVEN	Picture PesqPict("SC6","C6_PRCVEN",12)//067
	EndIf
/*
If cPaisLoc == "BRA"
...
*/
	...
	If MV_PAR03 <> 1
//97
		@li,155   psay (cAliasSC6)->C6_VALOR+nValImp Picture PesqPict("SC6","C6_VALOR",14,nDecs)//80
//112
		@li,171   psay (cAliasSC6)->C6_ENTREG//095
//122
		@li,182   psay (cAliasSC6)->C6_VRDESC    Picture "99.9"//105

	EndIf

	If MV_PAR03 <> 1
//128
		@li,189   psay (cAliasSC6)->C6_QTDEMP  Picture PesqPict("SC6","C6_QTDLIB",10)//112
		@li,199   psay (cAliasSC6)->C6_VRSLDPR Picture PesqPict("SC6","C6_QTDLIB",10)//120
	Else
		@li,124   psay (cAliasSC6)->C6_QTDEMP  Picture PesqPict("SC6","C6_QTDLIB",10)//112
		@li,134   psay (cAliasSC6)->C6_VRSLDPR Picture PesqPict("SC6","C6_QTDLIB",10)//120
	EndIf
...
//@li,150   psay nUltLib Picture PesqPict("SD2","D2_QUANT",10)
...
//@ li,000 psay Replicate("-",132)
	...
	@ li,131 psay nTotQtd    Picture PesqPict("SC6","C6_QTDVEN",10)//056
	If MV_PAR03 <> 1
		If ( cPaisLoc=="BRA" )
			@ li,152 psay nTotVal    Picture PesqPict("SC6","C6_VALOR",17)//094
		Else
...
Static Function ImpCabec(li,aPedCli,cAliasSC5)
	...
	If cPaisLoc == "BRA"
		...
		If MV_PAR03 <> 1
			cHeader := "It Codigo              Desc. do Material                                                             Endereço                TES UM      Quant       Unit.           Total  Entr       Desc         Emp      Pend"
		Else
			cHeader := "It Codigo               Desc. do Material                                                 Endereço          UM       Quant       Emp       Pend"
		EndIf
//          	99 xxxxxxxxxxxxxxx XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX XXXXXXXXXXXXXXXXXXXXXXX 999 XX  999999.9999,999,999.99  999,999,999.99 99/99/9999 99.9   999999.99 999999.99
...
//@ li,000 psay Replicate("-",limite-60)
	...
	If RetSqlName("SC5") == "SC5010"
		@ li,125 psay 'FORMA DE PAGAMENTO: ' + (cAliasSC5)->C5_XFRMPGT + ' - ' + IIf((cAliasSC5)->C5_XFRMPGT=='1','BOLETO ',Iif((cAliasSC5)->C5_XFRMPGT=='2','CARTÃO ','PIX ')) + ' - ' + Upper((cAliasSC5)->C5_XLINKPG)
	EndIf
