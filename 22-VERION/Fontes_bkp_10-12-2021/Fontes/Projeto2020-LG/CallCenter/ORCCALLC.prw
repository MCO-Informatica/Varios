...
User Function ORCCALLC(cNum,cvropc)


// --------------------------------------------------------------------------
// Programa para emissใo de Pedido de Venda da Verion utilizando Word/ AEM
//---------------------------------------------------------------------------
cabec1     :=""
cabec2     :=""
cabec3     :=""
wnrel      :="ORCCALLC"
titulo     :="Emissao Or็amento com base no Call Center"
cDesc1     :="Emite Or็amento com base no Call Center "
cDesc2     :=""
cDesc3     :=""
cString    :="SUA"
nElementos := 0
...
if cvropc == "2"
   RptStatus({|| RptDetail(cNUM,cvropc)}) 
else
   RptDetail(cNUM,cvropc)  
endif

Return         

// chamada do relatorio principal....... 
Static Function RptDetail(cNUM,cvropc)
...
dbSelectArea("SE4")
dbSetOrder(1)
DbSelectArea("SUA") //CABEวALHO DO PEDIDO 
dbSetOrder(1)
dbGoTop()
dbseek(xfilial("SUA")+par01)

// gravacao no servidor.....
if cvropc == "1"                                                                                         
	crec      :=  ALLTRIM(str(SUA->(RECNO())))
	cFilNam   := '-' + cNUM +  '-' + DtoS(dDataBase) + '-' + SubStr(Time(),1,2) + SubStr(Time(),4,2) + SubStr(Time(),7,2)+"-"+ cRec 
	cDirTmp   := GETMV("MV_XWORD") + "nova_logo\"
	// cDirSav   := "P:\Pub_Vendas\orc\"
	cDirSav   := "P:\Pub_Vendas\orc\"
	cFilExt   := '.doc'                                                               
endif

While !Eof() .and. SUA->UA_NUM <= par02 .and. SUA->UA_FILIAL = xfilial("SUA") 
	_cmensa:=1
	If _Totitens == 0
		_Ctotal    := 0
		_cNum      := SUA->UA_NUM
		...
		_cOBSVE    := if(Empty(SUA->UA_VEROBS),'','Observa็ใo : '+  alltrim(SUA->UA_VEROBS))
 	...
 	//			MaFisAdd(cProduto,cTes,nQuant,nPrunit,nValdesc,,,,0,0,0,0,(nTFat+nValDesc),0,0,0)
				...
				_nVSUB    := MaFisRet(nItem,"IT_VALSOL") //fValSub() =====>fudeu
			...
			DbSkip()
		END
	_nvez:= _nvez-1 //numero de itens no or็amento
	Endif    
    ...
    cAer := PraRBox()
  	cArquivo := GETMV("MV_XWORD") + "nova_logo\" + cAer	
	_Nvalor   := 20
	...
	//   oWord := OLE_CreateLink('TMsOleWord97')	
   	...
   	if !oWord <> '0'
        ...
        IF cvropc == "1"  // IMPRESSAO NO SERVIDOR
 		...
 		If SUA->UA_NUMSC5 > '      '
	   	...
	   	OLE_SetDocumentVar(oWord,"prt_data"       ,strzero(day(_cData),2) +" de " + mesextenso(_cData) +" de "+ strzero(year(_cData),4) )  //_cData)
	   	...
	   	OLE_SetDocumentVar(oWord,"PRT_TPFRE"        ,alltrim(_cTPFrete))
		...
		// IF cemp = '01' .and. upper(alltrim(cAer)) <> "COTAEMV.DOT"
		// 	IF upper(alltrim(cAer)) == 'COTCOMPV.DOT'
		// 		OLE_ExecuteMacro(oWord,"tabitensUH")
		// 	ELSEIF upper(alltrim(cAer)) == 'COTCONSV.DOT'
		// 		OLE_ExecuteMacro(oWord,"tabitensCONS")
		// 	ELSE
		// 		OLE_ExecuteMacro(oWord,"tabitens")
		// 	ENDIF
		// ELSEIF cemp = '01' .and. upper(alltrim(cAer)) == "COTAEMV.DOT"
		...
		IF cemp = '01' 
			IF upper(alltrim(cAer)) == 'COTPADRV.DOT'
				OLE_ExecuteMacro(oWord,"tabitens")
			ELSEIF upper(alltrim(cAer)) == 'COTCOMPV.DOT'
				OLE_ExecuteMacro(oWord,"tabitensUH")
			ELSEIF upper(alltrim(cAer)) == 'COTCONSV.DOT'
				OLE_ExecuteMacro(oWord,"tabitensCONS")
			ELSEIF upper(alltrim(cAer)) == 'COTAEMV.DOT'
				OLE_ExecuteMacro(oWord,"tabitensAEM")
			ENDIF   
	    Else
			OLE_ExecuteMacro(oWord,"tabitensA")
	    Endif
...
*/
        ...
        IF cvropc == "2"  // IMPRESSAO NO SERVIDOR
        ...
        ELSE
			...
			OLE_SaveAsFile(oWord, cDirSav + _cnreduz + cFilNam + cFilExt,,, .F., oleWdFormatDocument)   // cavalini 
   		ENDIF
	OLE_CloseFile( oWord )
	OLE_CloseLink( oWord )
	if .not. empty(cNUM)
...
Local aParB := {}
Local aRetP := {}
Local aOptR := {'Produto', 'Projetos Hidraulicos', 'AP Completo', 'Conserto', 'livre', 'livre' }
//  TESTE
//Local aOptF := {'TOTPADR','TOTCOMP','   ','   ', '  ', '     '}
// PRODUCAO
Local aOptF := {'COTPADR','COTCOMP','COTAEM','COTCONS', '  ', '     '}
Local nTipR := 1
 
aAdd(aParB,{3, 'Selecionar', nTipR, aOptR, 100, '', .T.})           

	If ! ParamBox(aParB, 'Lista de Leiate Modelo Word', @aRetP,,,,,,,, .F., .F.)
		Return 'Livre.dot'
	EndIf
	...
	IF cemp = '01'
		Return aOptF[nTipR]+ 'V.dot' 
	Else
		Return aOptF[nTipR]+ 'A.dot'
	Endif	
...
ฑฑบFuno    ณVALIDPERG บ Autor ณ RICARDO CAVALINI   บ Data ณ  02/04/2002 บฑฑ
