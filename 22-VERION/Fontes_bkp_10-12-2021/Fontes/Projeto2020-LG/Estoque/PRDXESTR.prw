...
User Function PRDXESTR() 

Local oProcess
Local cPerg		:= Padr( "PRDXESTRU", LEN( SX1->X1_GRUPO ) )
Local aSays		:={}
Local aButtons	:={}
Local nOpca 	:= 0
Local cCadastro := "Relatorio de Produtos x Estruturas"

Private nReg        := 0
Private aSocios     := {}

AADD(aSays,"Esta rotina tem como objetivo gerar informacoes de" )
AADD(aSays,"dos produtos bloqueado que estao nas estruturas. ")
...
cQuery := " SELECT DISTINCT G1_COD,G1_COMP,'PECAS' AS 'ESTRUTURA' "
cQuery += " FROM " + RetSqlName("SG1") + " G1 (NOLOCK) "
cQuery += " INNER JOIN " + RetSqlName("SB1") + " B1A (NOLOCK) ON B1_COD = G1_COD AND B1_MSBLQL <> '1' AND B1A.D_E_L_E_T_ = '' "
cQuery += " WHERE G1.D_E_L_E_T_ = '' AND G1_COMP IN (SELECT B1_COD "
cQuery += " 										  FROM " + RetSqlName("SB1") + " B1 (NOLOCK) "
cQuery += " 										  WHERE B1_MSBLQL = '1' AND B1_ESTRUT = '3' AND B1.D_E_L_E_T_ = '') "
cQuery += " UNION ALL "
cQuery += " SELECT DISTINCT G1_COD,G1_COMP,'SUB_KIT' AS 'ESTRUTURA' "
cQuery += " FROM " + RetSqlName("SG1") + " G1 (NOLOCK) "
cQuery += " INNER JOIN " + RetSqlName("SB1") + " B1A (NOLOCK) ON B1_COD = G1_COD AND B1_MSBLQL <> '1' AND B1A.D_E_L_E_T_ = '' "
cQuery += " WHERE G1.D_E_L_E_T_ = '' AND G1_COMP IN (SELECT B1_COD "
cQuery += " 										  FROM " + RetSqlName("SB1") + " B1 (NOLOCK) "
cQuery += " 										  WHERE B1_MSBLQL = '1' AND B1_ESTRUT = '2' AND B1.D_E_L_E_T_ = '') "
cQuery += " UNION ALL "
cQuery += " SELECT DISTINCT G1_COD,G1_COMP,'KIT' AS 'ESTRUTURA' "
cQuery += " FROM " + RetSqlName("SG1") + " G1 (NOLOCK) "
cQuery += " INNER JOIN " + RetSqlName("SB1") + " B1A (NOLOCK) ON B1_COD = G1_COD AND B1_MSBLQL <> '1' AND B1A.D_E_L_E_T_ = '' "
cQuery += " WHERE G1.D_E_L_E_T_ = '' AND G1_COMP IN (SELECT B1_COD "
cQuery += " 										  FROM " + RetSqlName("SB1") + " B1 (NOLOCK) "
cQuery += " 										  WHERE B1_MSBLQL = '1' AND B1_ESTRUT = '1' AND B1.D_E_L_E_T_ = '') "
cQuery += " ORDER BY 3,1"
...
While !eof()                    
     ...
     If lOk 
        aadd(aSocios,{CHR(160)+__CCOD,CHR(160)+__CCMP,__CEST})
     EndIf
...
Private cTitulo2 := "Estruturas"
Private cLine2   := "{ aSocios[oListBox2:nAt][1] "  
Private aHead_2  := {"Codigo_PA","Componente","Estrutura"}
