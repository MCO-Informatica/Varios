...
While !Eof()
	...
	_CLIGAC  := _ULTLIG(cQuery->A1_COD,cQuery->A1_LOJA)
	...
	AADD(_aDados,{_CODCLI, _NOM,_END,_MUN,_BAI,_CEP,_EST,_DDI,_DDD,_TEL,_FAX,_CONTATO,_TELCON,_CELCON,_FAXCON,_ORIG,_VEND1,_NOM1,_VEND2,_NOM2,_DPRICOM,_DDTORC, _CLIGAC})
...
//=============================================//
// ACHA ULTIMA LIGAวรO - Chamado 9150/9079	   //
//=============================================//
STATIC FUNCTION _ULTLIG(_CLIENTE,_LOJA)
_cULTLIG  := ""
CQUERY2 := ""

DbSelectArea("SUA")
cQuery2 := "SELECT UA_EMISSAO DATAUA, UA_INICIO HORAUA"
cQuery2 += " FROM "
cQuery2 += RetSqlName("SUA") + " SUA "
cQuery2 += " WHERE "
cQuery2 += " SUA.UA_EMISSAO = ("
cQuery2 += " 	SELECT MAX(UA_EMISSAO) FROM "+ RetSqlName("SUA") +" "
cQuery2 += " 	WHERE UA_CLIENTE = '"+_CLIENTE+"' "
cQuery2 += " 	AND UA_LOJA = '"+_LOJA+"' "
cQuery2 += " 	AND UA_FILIAL = '"+xFilial("SUA")+"' "
...
_cULTLIG := DTOC(STOD(CQUERY2->DATAUA)) + " - " + ALLTRIM(SUBSTR(CQUERY2->HORAUA, 1, 5) )
...
//**********************************
Local aCabec := {"CODIGO","NOME","ENDERECO","MUNICIPIO","BAIRRO","CEP","ESTADO","DDI","DDD","TELEFONE","FAX","CONTATO","TELEFONE","CELULAR","FAX","ORIGEM","VENDEDOR1","NOME","VENDEDOR2","NOME","DT.UL.P.VDA","DT.UL.ORCAM","ULT. LIGAวรO"}
Local aDados := aDados
