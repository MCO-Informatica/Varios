...
// IMPRIME BOLETO ITAU
	...
	private _cDigCep  := ""
	Private _cTime	  := ""
	Private _nMulta	   := 0 //(SuperGetMv("MV_XMULTAP" ,,0.00)) // % de Multa Padrão 2%.
	Private _nValMult  := 0
	Private _nVezes    := 0
	...
	If cBoleto $ 'S/I|E'  .AND. LEN(aTitulos) > 0 
		...
		IF ! cChamada  $ "JOBP" // BOLETO NORMAL
			cAssunto := "BOLETO DE PAGAMENTO  " +  aTitulos [1,2]
			cAnexo   := cPathInServer + '\' +   strtran(_cArq , ".rel", ".pdf")
		...
		ELSE // boletos não pagos 
			cAssunto := "BOLETO DE PAGAMENTO PENDENTE " +  aTitulos [1,2]
			cAnexo   := cPathInServer + '\' +   strtran(_cArq , ".rel", ".pdf")
			...
			cCorpo += 'Prezado(a), ' + aTitulos[1,1] +  '<br>'
			cCorpo += '<br>'
			cCorpo += ' Não identificamos em nosso sistema o pagamento do boleto referente '
			cCorpo += 'a&nbsp; sua compra NF: ' + aTitulos[1,2] + ', conforme abaixo:<b>'
			cCorpo += '<br>'
			cCorpo += '<table style="text-align: left; width: 522px; height: 69px;" border="1"'
			cCorpo += 'cellpadding="2" cellspacing="2">'
