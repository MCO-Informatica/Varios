...
AADD(_aStru,{"T_desc"    ,"C",0080,0})//Descricao do Produto
AADD(_aStru,{"T_ncm"     ,"C",0010,0})//NCM
AADD(_aStru,{"T_exncm"   ,"C",0003,0})//EX-NCM
...
_aCABEC := {"PRODUTO","DESCRICAO","NCM","EX_NCM","SALDO_ATUAL","RESERVA","EMPENHO","COMPRAS","MOEDA","IPI","PRECO","PENDENTE"}
_adados := {}
...
//		_NCOMPRA := SB2->B2_SALPEDI
    ...
    aAdd(_aDados,{ chr(160)+_WPROD,chr(160)+_WDESC,SB1->B1_POSIPI,SB1->B1_EX_NCM,_NSLDATU,_NRESERV,_NEMPENH,_NCOMPRA,chr(160)+_WMOEDA,_WIPI,_WAA,_NVERPEND})
...
While !Eof()
	...
	@ nlin,105 PSAY NFS->T_ncm
	@ nlin,116 PSAY NFS->T_exncm
	@ nlin,120/*078*/ PSAY NFS->T_PRECO   Picture "@E 999,999.99"
	@ nlin,131 /*089*/ PSAY NFS->T_SALDO   Picture "@E 999,999.99"
	@ nlin,142 /*100*/ PSAY NFS->T_RESERVA Picture "@E 999,999.99"
	@ nlin,153 /*111*/ PSAY NFS->T_EMPENHO Picture "@E 999,999.99"
	@ nlin,164 /*122*/ psay NFS->T_COMPRAS Picture "@E 999,999.99"
	nLin := nLin + 1
	DbSelectArea('NFS')
	DbSkip()
End
...
Else
	//ALERT("Arquivo nใo copiado com sucesso." + Alltrim(MV_PAR05)+_cNomArq  )
...
STATIC FUNCTION _cAltProd(_cProd)
	...
	if !Empty(_cProds)
		// Qual produto possui mais alternativas para gerar cabe็alho
		if _nSize > nMaxPAlt
			nMaxPAlt := _nSize
		endif
	endif
...
If File(cFile)
	If Aviso("Aviso", "Arquivo jแ existe. Sobrescreve? ", {"Sim", "Nao"},2) == 2 
		Return
	EndIf
EndIf
...
// Cabe็alho de produtos alternativos
if nMaxPAlt > 0
...
FWrite(nHandle,"PRODUTO;DESCRICAO;NCM;EXNCM;PRECO;SALDO;RESERVA;EMPENHO;COMPRAS;MOEDAS;IPI;PENDENTE"+cCabAlt+chr(13)+chr(10))
