...
Processa({||SB1COPI()},"Processando de Replicacao de Produtos !!!")
Return

// INICIO DA MONTAGEM DA TELA
Static Function SB1COPI()

Private cDelFunc  := ".F." // Validacao para a exclusao. Pode-se utilizar ExecBlock
PRIVATE aTelaEP   := {}
PRIVATE cCadastro := OemToAnsi("Cadastro de Produto")
...
@ 000,000 To 290,410 Dialog oDlg0 Title "Cadastro de Produto"
@ 010,005 SAY "Produto"
@ 010,050 Get __cCodPrd   SIZE 050,10 F3 "SB1" Valid c_Dprod(__cCodPrd) 
...
if cSvEmpAnt <> cOrEmpAnt .and. !(EqualFullName("SB1",cEmpAnt,cSvEmpAnt))
	
	
	IF !(EqualFullName("SB1",cEmpAnt,cSvEmpAnt))
		aArea  := SB1->(GetArea())
		nOrder := SB1->(IndexOrd())
		
		
		EmpChangeTable("SB1",cEmpAnt,cSvEmpAnt,nOrder )
		...
		dbSetOrder(1)
		MsSeek("SB1")
		While !Eof() .And. TRW->X2_CHAVE == "SB1"
		...
		dbSetOrder(1)
		MsSeek("SB1")
		While !Eof() .And. TRR->X3_ARQUIVO == "SB1"
	...
	dbSetOrder(1)
	MsSeek("SB1")
	While !Eof() .And. SX2->X2_CHAVE == "SB1"
...
ACARDEMP := U_C_CAR01(cEmpAnt,cFilAnt,CMODO,cStat,"SB1")
_APRPFIL := {}
_AOUTFIL := {}
			...
			If lOk
				For nCntFor := 1 To Len(aCols)
					If ( !aCols[nCntFor][nUsado+1] )
                        ...
                        If _lTdCrr
						   SB1->B1_FILIAL := alltrim(_APRPFIL[Gl,2])
							For nCntFor2 := 1 To nUsado
								If ( aHeader[nCntFor2][10] != "V" ) .or. ( ALLTRIM(UPPER(aHeader[nCntFor2][2])) != "B1_FILIAL" )
									SB1->(FieldPut(FieldPos(aHeader[nCntFor2][2]),aCols[nCntFor][nCntFor2]))
								EndIf
							Next nCntFor2
							MsUnLock()
						Endif	
...
For jp := 1 to len(_AOUTFIL)
	...
	If ( nScan==0 )
		aadd(_aunicafil,{ _AOUTFIL[jp,1],"SB1"})
	Endif
Next jp
...
For mc := 1 to Len(_aunicafil)
	_cCmum   := TableId("SB1",cOrEmpAnt,"SB1",_aunicafil[mc,1])
	...
	if !_cCmum  // Se nao for comum o SB1.
		...
		For me := 1 to Len(_AOUTFIL)
			If _cEmpMom == _AOUTFIL[me,1]
				...
				IF !(EqualFullName("SB1",cEmpNew,cEmpAnt))
					aArea  := SB1->(GetArea())
					nOrder := SB1->(IndexOrd())
					
					
					EmpChangeTable("SB1",cEmpNew,cEmpAnt,nOrder )
					...
					DbGotop()
					MsSeek("SB1")
					While !Eof() .and. alltrim(TRX->X2_CHAVE) == "SB1"
					...
					If lOk
						For nCntFor := 1 To Len(aCols)
							If ( !aCols[nCntFor][nUsado+1] )
								...
								if _lTdCrr 
									SB1->B1_FILIAL := alltrim(_AOUTFIL[Gl,2])
									For nCntFor2 := 1 To nUsado
										If ( aHeader[nCntFor2][10] != "V" ) .or. ( ALLTRIM(UPPER(aHeader[nCntFor2][2])) != "B1_FILIAL" )
											SB1->(FieldPut(FieldPos(aHeader[nCntFor2][2]),aCols[nCntFor][nCntFor2]))
										EndIf
...
if cSvEmpAnt <> cEmpnew
	...
	EmpChangeTable("SB1",cEmpAnt,cEmpnew,nOrder )
endif	
