...
±±ºPrograma  ³ES_POSEST ºAutor  ³LogosTechnology     º Data ³  31/07/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Geracao de planilha excel com posicao de estoque            º±±
±±º          ³dos ultimos 12 meses                                        º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³Projeto CiaTech                                             º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function ES_POSEST()
...
AADD(aSays,OemToAnsi( "em Excel com a movimentacao de estoque dos ultimos  " ) )
AADD(aSays,OemToAnsi( "12 meses. Colunas MIE, MIS, COMPRA, VENDA E KARDEX. " ) )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ES_PMSSR  ºAutor  ³Microsiga           º Data ³  07/31/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
If Eof() .and. Bof()
	ApMsgStop('Não foram encontrados projetos para os parametros digitados, por favor verifique.')
	lRet := .F.
Endif
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ES_POSEST ºAutor  ³Microsiga           º Data ³  11/29/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local clArq3  	:= ""
Local cDirTemp    := "C:\TEMP" // GetTempPath()
...
If FERROR() != 0
	Alert("Não foi possível abrir ou criar o arquivo: " + clArqExl )
	return
else
	...
	//Efetua a gravacao aqui com o Fwrite
	//	nlHandle := FOpen(Alltrim(cDirTemp) + clArqExl)
	//	FSEEK(nlHandle,0,2)
	//	FWRITE(nlHandle, clArq3)
	//	fClose(nlHandle)
	//	cTst := memoread( Alltrim(cDirTemp) + clArqExl )
	//	alert( cTst )
	...
	While !Eof()
		...
		For _y := nMeses to 1 step -1
			clArq3	:= '    <Cell ss:StyleID="s99"><Data ss:Type="String">' + Transform(cArqTmp->&("MIE_MES"+Strzero(_y,2)),'@E 999,999,999') + '</Data></Cell>'+CRLF
			FWRITE(nlHandle, clArq3)			
			clArq3	:= '    <Cell ss:StyleID="s99"><Data ss:Type="String">' + Transform(cArqTmp->&("MIS_MES"+Strzero(_y,2)),'@E 999,999,999') + '</Data></Cell>'+CRLF
			FWRITE(nlHandle, clArq3)			
			clArq3	:= '    <Cell ss:StyleID="s99"><Data ss:Type="String">' + Transform(cArqTmp->&("COM_MES"+Strzero(_y,2)),'@E 999,999,999') + '</Data></Cell>'+CRLF
			FWRITE(nlHandle, clArq3)			
			clArq3	:= '    <Cell ss:StyleID="s99"><Data ss:Type="String">' + Transform(cArqTmp->&("VEN_MES"+Strzero(_y,2)),'@E 999,999,999') + '</Data></Cell>'+CRLF
			FWRITE(nlHandle, clArq3)			
			clArq3	:= '    <Cell ss:StyleID="s99"><Data ss:Type="String">' + Transform(cArqTmp->&("KAR_MES"+Strzero(_y,2)),'@E 999,999,999') + '</Data></Cell>'+CRLF
			FWRITE(nlHandle, clArq3)			
			clArq3	:= '    <Cell ss:StyleID="s99"><Data ss:Type="String">' + Transform(cArqTmp->&("KAR_CUS"+Strzero(_y,2)),'@E 999,999,999.99') + '</Data></Cell>'+CRLF
			FWRITE(nlHandle, clArq3)		
		Next
	...
	If File(Alltrim(cDirTemp) + clArqExl)
		If ApOleClient("MsExcel")
			...
			oExcelApp:SetVisible(.T.)
		   	ShellExecute("open",Alltrim(cDirTemp) + clArqExl,"","",2)
			//oExcelApp:Destroy()
		Else
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³CriPrgBr  ºAutor  ³Gelson              º Data ³  03/01/09   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ES_POSEST ºAutor  ³Microsiga           º Data ³  10/26/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ES_POSEST ºAutor  ³Microsiga           º Data ³  10/27/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
2 - Operação Automática (Apropriação Indireta)
3 - Operação Manual (Apropriação Indireta)
4 - Transferência (Custo Médio no Estoque por Local)
5 - Requisição para OP na NF (Usa o custo da NF)
6 - Requisição Valorizada
7 - Transferência Multipla (Desmontagem de Produtos)
8 - Integração com NF de Produtos Importados (SIGAEIC)
...
Do Case
	...
	Case cMov == 'KA2'	//valor kardex
		...
		While !Eof() .and. alltrim(cProduto) == alltrim(SB2->B2_COD)
			...
			cRet += CalcEst(cProduto,SB2->B2_LOCAL, dData  )[2]
