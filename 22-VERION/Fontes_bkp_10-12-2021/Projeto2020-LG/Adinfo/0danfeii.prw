...
Else
	...
	oDANFE := Nil
EndIf
// claudia cabral rotina de impressão/envio e bordero de boletos itaú
If ExistBlock("XRBOL001")
	For xx := 1 to Len(aBoletos)
		u_XRBOL001(aBoletos[xx,1],aBoletos[xx,2], aBoletos[xx,3], aBoletos[xx,4],,,"DANFE" )
	Next
ENDIF
//-------------------- fim claudia	
RestArea(aArea)
	...
	While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
		...
		If  Empty((cAliasSF3)->F3_DTCANC) .And. &cWhere //.And. AModNot((cAliasSF3)->F3_ESPECIE)=="55"
			...
			If (SubStr((cAliasSF3)->F3_CFO,1,1)>="5" .Or. (cAliasSF3)->F3_FORMUL=="S") .And. aScan(aNotas,{|x| x[4]+x[5]+x[6]+x[7]==(cAliasSF3)->F3_SERIE+(cAliasSF3)->F3_NFISCAL+(cAliasSF3)->F3_CLIEFOR+(cAliasSF3)->F3_LOJA})==0
				...
				AADD(aBoletos,{(cAliasSF3)->F3_CLIEFOR, (cAliasSF3)->F3_LOJA, (cAliasSF3)->F3_SERIE, (cAliasSF3)->F3_NFISCAL})
