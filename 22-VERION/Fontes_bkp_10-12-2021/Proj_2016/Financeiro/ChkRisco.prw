...
±±ºPrograma  ³CHKRISCO  ºAutor  ³Fernando Macieira   º Data ³ 19/Mar/07   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Rotina que analisa e determina o risco do cliente, como E,  º±±
±±º          ³quando o titulo possuir vencimento superior a 28 dias, este º±±
±±º          ³definido no parametro MV_XVRISCO.                           º±±
...
aAdd(aSays,"título possuir vencimento superior aos dias definidos no parâmetro ")
...
If nOpc == 1                                                         
	Pergunte("CHKRISCO", .T.)
	If msgBox("Confirma a execução da análise?","Confirma","YesNo")
		Processa({||RunRisco()},"Processando análise de duplicatas vencidas...")
	EndIf
EndIf
...
Else 
	msgStop("Nenhum cliente alterado","Atenção")
EndIf
Return
...
cDir      := cGetFile("Arquivos Texto|*.TXT|Todos os Arquivos|*.*",;
             OEMtoAnsi("Selecione o diretório de gravaçãodo arquivo "+AllTrim(cNomArq)+".txt"),0,"SERVIDOR\",.T.,GETF_LOCALFLOPPY+GETF_LOCALHARD+GETF_NETWORKDRIVE+GETF_RETDIRECTORY)
cDir      := AllTrim(cDir)+cNomArq+".TXT"

If At(".TXT",cDir) > 0
	nHdlText := fCreate(cDir)
Else
	Aviso("Clientes Risco E","Não foi selecionado nenhum arquivo para gravar",{"&Ok"},,"Sem Arquivo")
EndIf
