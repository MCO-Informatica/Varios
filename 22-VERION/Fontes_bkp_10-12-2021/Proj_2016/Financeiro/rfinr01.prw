...
±±º Programa    ³ RFINR01  ³ Impressao de Recibo Financeiro                               º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º Solicitante ³ 03.01.07 ³ Vanderleia                                                   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º Autor       ³ 04.01.07 ³ Top Consulting                                               º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
If !MsSeek(xFilial("SE1")+mv_par05+mv_par06+mv_par01+mv_par02+mv_par03+mv_par04)
	Aviso(	cCadastro,;
			"Título: "+mv_par01+"/"+mv_par02+"/"+mv_par03+"/"+mv_par04+Chr(13)+Chr(10)+;
			"Cliente/Loja: "+mv_par05+"/"+mv_par06+Chr(13)+Chr(10)+;
			"O título para o cliente informado não foi localizado no contas a receber."+Chr(13)+Chr(10)+;
			"Não é possível efetuar a impressão, verifique os parâmetros.",;
			{"&Continua"},,;
...
If lProcessa
	...
	//³ Procura por cheques associados ao título ³
	...
	If oPrint:IsPrinterActive()
		...
		//³ Definicao do Quadro de Cabeçalho ³
...
//		cTexto	:= "(" + AllTrim(Extenso(nValor))
		...
		IncLinha(@nLin,40)
		cTexto	:= "Referente a Nota Fiscal número "
		cTexto	+= SE1->E1_NUM + " Série " + AllTrim(SE1->E1_PREFIXO)
		...
		//³ Definicao do Quadro de Composição Valores ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		oPrint:Say(nLin,nCol+1220, "Composição dos Valores",						oFont10n)
		IncLinha(@nLin,40)
...
//		oPrint:Say(nLin,nCol+1720, Transform(nValor, "@E 999,999,999.99"),		oFont10)
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Definicao do Quadro de Cheques s/ Título ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If Len(aCheques) > 0
			oPrint:Say(nLin,nCol+0020, "Banco/Agencia/Conta/Cheque                 Valor       Emissão    Data Boa    Emitente                    CPF/CNPJ",	oFont10n)
			IncLinha(@nLin,40)
...
//				cTexto	:= Transform(aCheques[nLoop,05], "@E 999,999,999.99")
			...
			Next nLoop
			IncLinha(@nLin,40)
			oPrint:Say(nLin,nCol+0020, "Atenção: A quitação só será válida após a compensação dos Cheques.",oFont10n)
			IncLinha(@nLin,80)
		EndIf
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Definição do Quandro de Aprovações ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		nLin	:= 2740
...
While !Eof()
	...
	aAdd( aRet, {	RFINR01B->E5_BANCO,;										// 1-Banco
					...
					iif (RFINR01B->E5_MOTBX = 'CCD'," C. DEBITO"," DEBITO")))),;	// 4-Número
					RFINR01B->E5_VALOR,;										// 5-Valor Baixado
					RFINR01B->E5_DATA,;											// 6-Data Emissão
					stod(""),;																					// 7-Data Boa RFINR01B->E5_DATAO
					"      ",; 																					// 8-Emitente  Substr(RFINR01A->EF_EMITENT,1,18)
					RFINR01B->E5_HISTOR,;																		// 9-Histórico
