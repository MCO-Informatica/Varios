...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ"±±
±±ºPrograma  ³PE01NFESEFAZ ºAutor  ³ Alex Rodrigues - Adinfo  º Data ³  04/09/18    º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Ponto de entrada para tratar informações da NFe enviadas no          º±±
±±º          ³ arquivo XML para a Sefaz                                             º±±
...
If aNota[4] == "1"  //Saída
	...
	If DbSeek(xFilial("SC5")+aInfoItem[1][1]) 
		...
		If !Alltrim(SC5->C5_NUM) $ cMensCli // Incluído por Flávio Sardão em 13/09/2010.
			cMensCli += "Ped.Num.: " + Alltrim(SC5->C5_NUM)
		EndIf
	
		If !Alltrim(SC5->C5_VEND1) $ cMensCli // Incluído por Flávio Sardão em 13/09/2010.
		...
		If !Alltrim(SA1->A1_ENDENT) $ cMensCli .And. !Empty(SA1->A1_ENDENT)   // Alterado por Flávio Sardão em 13/09/2010. - Verion - CUSTOMIZADO
			IF  SD2->D2_TIPO $ "DB"
				cMensCli += ", Endereço de Entrega: "
		...
		If !Alltrim(SC5->C5_VRCOLET) $ cMensCli .And. !Empty(SC5->C5_VRCOLET)  // Incluído por Flávio Sardão em 13/09/2010. - Verion - CUSTOMIZADO
			cMensCli += ", " + AllTrim(SC5->C5_VRCOLET)
		EndIf					
		If !Alltrim("-PAGAMENTOS DEVEM SER FEITOS VIA BOLETO OU DEPOSITO") $ cMensCli// Incluído por Fabio em 13/04/2014. - Verion - CUSTOMIZADO
		 	cMensCli += "   -PAGAMENTOS DEVEM SER FEITOS VIA BOLETO OU DEPOSITO APENAS EM NOME DE NOSSA EMPRESA - DÚVIDAS CONTATAR O FINANCEIRO (11)2100-7400"
		Endif      
	Endif
Else // NF entrada       
	...
	//IMp.Importação :         340,58-Pis:        55,85-Cofins : 257,96
	//Base ICMS (    3.227,52/0,82)*18% = ICMS ( 708,48)
	//DI 09/169232-3 de 01/12/2009  Num Adição: 001
	//CAI001014/09                       
