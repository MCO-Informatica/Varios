...
*/
User Function GVRFAT01()
_nSldAtual := 0
_nSldPen   := 0
_nPosLDb   := ascan(aHeader,{|_vAux|alltrim(_vAux[2])=="C6_QTDLIB"})
_nPosPrd   := ascan(aHeader,{|_vAux|alltrim(_vAux[2])=="C6_PRODUTO"})
_nPosLcl   := ascan(aHeader,{|_vAux|alltrim(_vAux[2])=="C6_LOCAL"})
_nPosQtd   := ascan(aHeader,{|_vAux|alltrim(_vAux[2])=="C6_QTDVEN"})
_nPosSld   := ascan(aHeader,{|_vAux|alltrim(_vAux[2])=="C6_VRSLDPR"})
...
If DbSeek(Xfilial("SB2")+_cPrdPv+_clclpv)
   _nSldAtual := SB2->B2_QATU // (SB2->B2_QATU-(SB2->B2_RESERVA+SB2->B2_QEMP))
Endif
