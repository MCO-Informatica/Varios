...
// Luiz - 10/12/2019
// Possibilidar emissão de boleto pelos parâmetros enviados pelo relatório RFATR01 (Pedidos)
...
User Function BOLETO(aBoletos)
    ...
    Default aBoletos   := {}
    // 23/01/2020 - Luiz - array para passar informações para emissão de boleto 
...
//           Alert("Emp: "+aBoletos[nCont][1]+" - Nota: "+aBoletos[nCont][2]+aBoletos[nCont][3]+" - Ped: "+aBoletos[nCont][4])
	...
	// Processa( {||ReEmiBol(lDanfe)},"Imprimindo os Boletos...")   -  execução dentro do IF acima
     ...
     nDias := SE1->E1_VENCREA - SE1->E1_EMISSAO 
        ...
        Loop  
            ...
            If lDanfe
                ...
                If SE1->E1_EMIBOL == "S"
                    Alert("O pedido "+SE1->E1_PEDIDO+" NF "+SE1->E1_PREFIXO+"-"+SE1->E1_NUM+" já teve o seu boleto emitido - Nova emissão ignorada")				
					DbSkip()
					Loop
                EndIf
...
/*   nDias := SE1->E1_VENCREA - SE1->E1_EMISSAO 
      ...
      Loop  
          ...
          /*  if SE1->E1_FORMAPG <> "BO"   // Luiz - 23/01/2020 - Se forma de pagamento não é BO ignora emissão de boleto
          DBSkib()
          Loop
            endif */
...
/*
            ...
            If Empty(SE1->E1_AGEDEP) .Or. Empty(SE1->E1_CONTA) .Or. Empty(SE1->E1_PORTADO)
                ...
                if lDanfe      // 23/01/2020 - Luiz - Utilizar Bco/Age/Conta parametrizado se impressão automática
              cFilAnt := _cEmpFat
...
/*
    ...
    Else      // se MV_PAR01 (prefixo)é vazio - se for emissão pelo relatório de Pedidos não executa o ELSE
