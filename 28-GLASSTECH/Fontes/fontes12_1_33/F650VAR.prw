...
If MV_PAR07 = 1 .AND. AllTrim(Upper(MV_PAR02))$ "BRDFAC.RET|"
	...
	Else
		...
		EndIf
		*/
		If ( Empty(cAux) .or. se1->e1_valor != 0 .AND. se1->e1_valor != (_aValores[1,8]-_aValores[1,9]) ) .AND. Substring(cNumTit, 1, 1) $ "2/1"
			cE1_NUM 	:=	StrZero(Val(Substring(cNumTit, 2, LEN(AllTrim(cNumTit))-2)),TamSx3("E1_NUM")[1])
			cE1_PARC	:=	Right(alltrim(cNumTit), 1)
...
If funname() != "FINR650" .AND. lFDVldVl
