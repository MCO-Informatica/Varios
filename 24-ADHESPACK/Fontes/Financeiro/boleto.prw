...
/*/{Protheus.doc} BOLETO
Função para Reeimprimir Boleto Bancancario
@type function
@version 12.1.33
@author Anderson Martins
@since 10/14/2022
@param [1] lDanfe - Define se a impressão é a partir da DANFE
@param [2] aParam - Parametros do titulo que sera impresso
@param aParam[1] - Prefixo do titulo
@param aParam[2] - Do Titulo Financeiro
@param aParam[3] - Ate o titulo financeiro
@param aParam[4] - Da Parcela Inicial
@param aParam[5] - Ate a Parcela Final
/*/
...
Static Function ReEmiBol(lDanfe)
    ...
    LOCAL aDadosBanco  := {}
    LOCAL aDatSacado
    LOCAL aBolText     := {"APOS VENCIMENTO COBRAR MULTA DE 2% SOBRE O VALOR DO BOLETO" ,;
                           "APOS O VENCIMENTO COBRAR MORA DIARIA DE R$ .............. " ,;
                           "SUJEITO A PROTESTO 05 DIAS APOS O VENCIMENTO"}
...
/*
     nDias := SE1->E1_VENCREA - SE1->E1_EMISSAO 
            If nDias < 7
        MsgStop("O Titulo : "+SE1->E1_PREFIXO+SE1->E1_NUM+" nao irá gerar boleto devido ao prazo." )
        DbSkip()
        Loop  
            EndIf */
...
/*
			...
			aBco := {'','',''} //SelBcoDia()
                ...
                If Empty(aBco[1])
				MsgStop("O título "+SE1->E1_PREFIXO+SE1->E1_NUM+SE1->E1_PARCELA+" não se encontra em borderô. Favor revisar")
				Return
                EndIf
			cBanco   := aBco[1]
...
/*
		...
		//Para emissao de boleto
			...
			Alltrim(SEE->EE_AGBCOCO)   ,;	// [3]Agência
	...
	//cBcoCon := Alltrim(SEE->EE_CONVENI)
		...
		aDadosTit   := {If(Len(Alltrim(SE1->E1_NUM)) > 6,Substr(Alltrim(SE1->E1_NUM),4),Alltrim(SE1->E1_NUM))+cParcela						,;  // [1] Número do título
...
/*
	...
	//Posiciona na Arq de Parametros CNAB
		...
		DbSetOrder(1)
            If !DbSeek(xFilial("SEE")+cBanco+cAgencia+cNumCon)
			MsgStop("sBanco nao existe nos parametro Bancarios, nao sera gerado Boleto" )
			DbSelectArea("SE1")
			DbSkip()
			Loop
            EndIf
...
Impressao de Boleto Grafico do Banco
@type function
@version 12.1.33
@author Anderson Martins
@since 10/14/2022
...
/*/{Protheus.doc} DACNN
Faz a verificacao e geracao do digito Verificador no Modulo 11 para NN
@type function
@version 12.1.33
@author Anderson Martins
@since 10/14/2022
...
/*/{Protheus.doc} NumItau
Gera o numero sequencial para o Banco Itau
@type function
@version 12.1.33
@author Anderson Martins
@since 10/14/2022
...
/*/{Protheus.doc} Agencia
Verifica a existencia das perguntas criando-as caso seja necessario (caso nao existem)
@type function
@version 12.1.33
@author Anderson Martins
@since 10/14/2022
...
/*/{Protheus.doc} Ret_Bar001
Codigo de Barras do Banco do Brasil 
@type function
@version 12.1.33
@author Anderson Martins
@since 10/14/2022
...
Tela de digitação do email e corpo do email.
@type function
@version 12.1.33
@author Anderson Martins
@since 10/14/2022
