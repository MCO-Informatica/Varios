			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If lAglutina
					If Len(aProd) > 0		
						...
						Else
							nX := aScan(aProd,{|x| x[24] == (cAliasSD2)->D2_CODISS .And. x[23] == IIF(SB1->(FieldPos("B1_TRIBMUN"))<>0,Iif(Empty(SB1->B1_CNAE),Iif(cEmpAnt=='03','331471000',SM0->M0_CNAE),RetFldProd(SB1->B1_COD,"B1_TRIBMUN")),"")})
						Endif					
						EndIf	
						If nX > 0						
							aProd[nX][9] := 1							
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								Iif(cEmpAnt=='03','331471000',SM0->M0_CNAE),;
								SF3->F3_RECISS,;
								SF3->F3_ISSSUB,;
								SF3->F3_ISSMAT,;   
								Iif(cEmpAnt=='03',IIF(SB1->(FieldPos("B1_TRIBMUN"))<>0,RetFldProd(SB1->B1_COD,"B1_TRIBMUN"),""),IIF(SB1->(FieldPos("B1_TRIBMUN"))<>0,Iif(Empty(SB1->B1_CNAE),Iif(cEmpAnt=='03','331471000',SM0->M0_CNAE),RetFldProd(SB1->B1_COD,"B1_TRIBMUN")),"")),;
								SF3->F3_CODISS,;
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf,aRetSF3)	
	...
	nAliqISs := 0	// Envio de Aliquota Zerada Agora Pra Sjc - 3L Systems 02/07/2022
	...
	for nX := 1 to len( aProd )
		...
		Else
			...
			cString	+= "<aliquota>" + allTrim( iif( nAliqISs > 0, convType( nAliqISs, 7, 4 ), convType(nAliqISs, 7, 4) ) ) + "</aliquota>"
		EndIF	
