			...
			Else
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						if vldIntermed(cIndPres, cAmbiente)
...
//-----------------------------------------------------------------------
/*/{Protheus.doc} vldIntermed
Retorna de acordo com o indicador de presença indica se pode ser informado
o indIntermed
@param  cVerIndP, String, indicado de presença
@param  cAmbiente, String, Numero do ambiente (1-producao/2-homologacao)
...
/*/
//-----------------------------------------------------------------------
static function vldIntermed(cIndPres, cAmbiente)
local lRet		:= .F.
local cSufix	:= ""
local cPrefix	:= ""
local cField	:= ""
local cError	:= ""
local cMVNT006	:= SuperGetMv('MV_NT20006',.F.,'{"prod_1":"01/09/2021","homolog_1":"05/04/2021","prod_n":"05/04/2021","homolog_n":"01/02/2021"}')
local oParam	:= nil

if cIndPres $ "1,2,3,4,9"
	oParam	:= JsonObject():New()
	if !empty(cMVNT006) .and. empty((cError := oParam:FromJSON(cMVNT006)))

		cPrefix	:= iif(cAmbiente == "1","prod","homolog")
		cSufix	:= iif(cIndPres == "1",cIndPres,"n")
		cField 	:= cPrefix + "_" + cSufix

		if !empty(oParam[cField])
			lRet := date() >= cToD(oParam[cField])
		endIf
	else
		conout("Problema ao realizar o parse do parametro 'MV_NT20006': " + cError)
	endIf
endIf
