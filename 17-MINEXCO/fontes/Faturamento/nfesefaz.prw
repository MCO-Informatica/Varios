...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						//----> INICIO CUSTOMIZACAO
						If !Empty(SC5->C5_OBSMIN) .And. !AllTrim(SC5->C5_OBSMIN) $ cMensFis
							If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
								cMensFis += " "
							EndIf
							cMensFis += AllTrim(SC5->C5_OBSMIN)
						EndIf
						...
						/*
						If !Empty(SC5->C5_PLACA) .Or. !Empty(SC5->C5_UFPLACA)
							aadd(aVeiculo,SC5->C5_PLACA)
							aadd(aVeiculo,SC5->C5_UFPLACA)
							aadd(aVeiculo,"")
							_cMotorista := SC5->C5_MOTORIS
						EndIf
						
							
						//----> VEICULO DO MANIFESTO
						If !Empty(SC5->C5_VEICULO)
							dbSelectArea("DA3")
							dbSetOrder(1)
							dbSeek(xFilial("DA3")+SC5->C5_VEICULO,.f.)
						...
						*/
						IF !Alltrim(SF2->F2_TIPO) $ "D/B"
							...
							If !Empty(SA1->A1_ENDENT) .And. (SA1->A1_ENDENT) <> (SA1->A1_END)
								If !"Local Entrega: " + Alltrim(SA1->A1_ENDENT) + Space(80-Len("Local Entrega: "+Alltrim(SA1->A1_ENDENT))) $ cMensFis
									cMensFis += "Local Entrega: " + Alltrim(SA1->A1_ENDENT) + Space(80-Len("Local Entrega: "+Alltrim(SA1->A1_ENDENT)))
								EndIf
								
								If !"               " + Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE) + Space(80-Len("               "+Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE))) $ cMensFis
									cMensFis += "               " + Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE) + Space(80-Len("               "+Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE)))
								EndIf
							ElseIf !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .and. SF2->F2_CLIENTE+SF2->F2_LOJA <> SF2->F2_CLIENT+SF2->F2_LOJENT
								If !"Local Entrega: " + Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_END")) + Space(80-Len("Local Entrega: "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_END")))) $ cMensFis
									cMensFis += "Local Entrega: " + Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_END") + Space(80-Len("Local Entrega: "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_END")))))
								EndIf
								
								If !"               " + Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_BAIRRO"))+" - "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_MUN"))+" - "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_EST"))+" - "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_CEP")) + Space(80-Len("               "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_BAIRRO"))+" - "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_MUN"))+" - "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_EST"))+" - "+Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_CEP")))) $ cMensFis
									cMensFis += "               " + Alltrim(SA1->A1_BAIRRO)+" - "+Alltrim(SA1->A1_MUN)+" - "+Alltrim(SA1->A1_EST)+" - "+Alltrim(SA1->A1_CEP) + Space(80-Len("               "+Alltrim(SA1->A1_BAIRRO)+" - "+Alltrim(SA1->A1_MUN)+" - "+Alltrim(SA1->A1_EST)+" - "+Alltrim(SA1->A1_CEP)))
								EndIf
							EndIf
							
							If !Empty(SA1->A1_ENDCOB) .And. (SA1->A1_ENDCOB) <> (SA1->A1_END)
								If !"Local Cobranca: " + Alltrim(SA1->A1_ENDCOB) + Space(80-Len("Local Cobranca: "+Alltrim(SA1->A1_ENDCOB))) $ cMensFis
									cMensFis += "Local Cobranca: " + Alltrim(SA1->A1_ENDCOB) + Space(80-Len("Local Cobranca: "+Alltrim(SA1->A1_ENDCOB)))
								EndIf
								
								If !"               " + Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC) + Space(80-Len("               "+Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC))) $ cMensFis
									cMensFis += "               " + Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC) + Space(80-Len("               "+Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC)))
								EndIf
...
If  !lIssQn    
	...
	If Len(aICMUFDest) > 0 .and. (cIdDest == "2" .and. cIndFinal == "1" .and. cIndIEDest == "9") .and. (Len(aISSQN)== 0) .and. (cAmbiente == "2" .or. (cAmbiente == "1" .and. FsDateConv(aNota[03],"YYYY") >= "2016" .and. lDateRefNf )) .and. (iif(Len(aComb) > 0 .And. !Empty(aComb[01]),aComb[01] $ NfeCodANP(),.T.)) 
		...
		Endif
		cString += '<pFCPUF>'+ConvType(aICMUFDest[02],8,4)+'</pFCPUF>' //pFCPUFDest
		cString += '<Aliquota>'+ConvType(aICMUFDest[03],8,4)+'</Aliquota>' //pICMSUFDest
		...
		*/
		If (valType(aPPDifal)== "A" .and. Len(aPPDifal)>0 .and. Year(aNota[03]) >= aPPDifal[1][1])
			...
			If Alltrim(Str(aProd[33])) == "4" .Or. Alltrim(Str(aProd[33])) == "7" .Or. Alltrim(Str(aProd[33])) == "12"
				...
				cString += '<pFCPUF>'+ConvType(0,8,4)+'</pFCPUF>' //pFCPUFDest
				cString += '<Aliquota>'+ConvType(0,8,4)+'</Aliquota>' //pICMSUFDest
