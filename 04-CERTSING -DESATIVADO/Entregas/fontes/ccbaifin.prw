...
//BLOCO ABAIXO - ROGERIO BISPO (UPDUO) - 30/3/2021
//Identifica existencia de NFs para compensação com parcela sem preenchimento devido a troca de forma de pagamento
//Altera vencimentos e natureza dos titulos
If cTipReg == "B" .and. !empty(cParSql)
	...
	cSql := "SELECT E1_PARCELA, E1_TIPO, R_E_C_N_O_ RECE1 "
	cSql += " FROM "+RetSqlName("SE1")
	cSql += " WHERE E1_FILIAL = '"+xFilial("SE1")+"' AND "
	cSql += " E1_PEDIDO = '"+cPedido+"' AND E1_TIPO ='NF ' AND E1_PARCELA = ' ' AND "
	cSql += " E1_SALDO > 0 AND "
	cSql += " D_E_L_E_T_ = ' ' "
	cTrbSql1:=GetNextAlias()
	PLSQuery( cSql, cTrbSql1 )
	...
	If !(cTrbSql1)->(Eof())
		...
		cSql += " E1_PEDIDO = '"+cPedido+"' AND E1_TIPO='PR' AND"
		cSql += " D_E_L_E_T_ = ' ' "
		cTrbSql2 := GetNextAlias()
		PLSQuery( cSql, cTrbSql2 )

		if !(cTrbSql2)->(Eof())
			dUltimoVenc := StoD((cTrbSql2)->MAXVENCREA)
			cUltNaturez := (cTrbSql2)->MAXNATUREZ
		endif
		(cTrbSql2)->(DbCloseArea())

		While !(cTrbSql1)->(Eof())

			AAdd( aSe1NF,{cPedido, (cTrbSql1)->RECE1} )

			if !empty(cUltNaturez) .and. cUltNaturez != sc5->c5_xnature

				SE1->( dbGoTo( (cTrbSql1)->RECE1 ) )

				SE1->( RecLock( "SE1", .F. ) )
				SE1->E1_VENCTO  := dUltimoVenc
				SE1->E1_VENCREA := dUltimoVenc
				SE1->E1_NATUREZ := cUltNaturez
				SE1->(MsUnLock())
			endif
	...
	(cTrbSql1)->(DbCloseArea())

	If len(aSe1NF)>0
		//User funtion do M460fim
		...
		//aparan[3] Parametro lVldSldRes como conteudo falso, pra poder compensar valores maiores que 0.2 - ROGERIO BISPO (UPDUO) - 30/03/2021
		U_VldRecPg( aSe1Nf,.T. , .f.) 
	Endif
