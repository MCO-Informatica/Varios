...
//+------------+---------------+----------------------------------------------------------------------------------+--------+----------+
//| 09/10/2020 | Bruno Nunes   |Ausência de informações - Relatório analítico                                     | 1.00   | PROT-255 |
//+------------+---------------+----------------------------------------------------------------------------------+--------+----------+
...
Static Function procRel()
	...
	//Carrega parametros digitados pelo usuário
	aAdd( aPar,{ 1  ,"Treinado em? " ,cToD(Space(8)),"","","","",50,.F.})
	aAdd( aPar,{ 1  ,"Treinado até?" ,cToD(Space(8)),"","","","",50,.F.})
	aAdd( aPar,{ 1  ,"Criado em?"	 ,cToD(Space(8)),"","","","",50,.F.})
	aAdd( aPar,{ 1  ,"Criado até?"	 ,cToD(Space(8)),"","","","",50,.F.})
	if !ParamBox( aPar, 'Parâmetros' , @aRet, bValid, , , , , ,"FLUIG02" , .T., .F. )
		msgInfo("Geração de relatório cancelado", "Processamento Cancelado")
		return
	endif
	...
	while ( cAlias )->( !EOF() )
		...
		aAdd( aAux, cValToChar( ( cAlias )->documentId ) 	 )
		aAdd( aAux, cValToChar( ( cAlias )->version ) 		 )
		aAdd( aAux, ( cAlias )->DS_PRINCIPAL_DOCUMENTO		 )
		aAdd( aAux, ( cAlias )->NOME					 	 )
		aAdd( aAux, cCC					 	     			 )
		aAdd( aAux, cCCDesc					 	 			 )
		aAdd( aAux, dToC( sToD( ( cAlias )->DT_CRIACAO   ) ) )
		aAdd( aAux, dToC( sToD( ( cAlias )->DT_APROVACAO ) ) )
		aAdd( aAux, dToC( sToD( ( cAlias )->dateExecuted ) ) )
...
@return null, Nulo
/*/
static function valParam( aRet )
	local lValidou := .T.

	default aRet := {}

	if !empty( aRet )
		//Valido os parametros treinade de e ate
		if !empty( aRet[nPARAM_TREINADO_DE] )
			if aRet[nPARAM_TREINADO_DE] > aRet[nPARAM_TREINADO_ATE]
				lValidou := .F.
				msgInfo( 'Data "Treinamento em?" é maior que data "Treinamento até?" ', "Validação" )
			endif
		endif

		//Valido os parametros criado de e ate
		if !empty( aRet[nPARAM_CRIADO_DE] )
			if aRet[nPARAM_CRIADO_DE] > aRet[nPARAM_CRIADO_ATE]
				lValidou := .F.
				msgInfo( 'Data "Criado em?" é maior que data "Criado até?" ', "Validação" )
			endif
		endif
	endif
...
@return null, Nulo
/*/
static function loadFluig( cAlias, aRet )
	local cQuery := ""
	local lCarregou := .F.
	...
	if !empty(aRet) .and. !empty(cAlias)
		cQuery := " SELECT DISTINCT documentId "
		cQuery += "       ,version "
		...
		If !Empty(aRet[nPARAM_TREINADO_DE])
			cQuery += " AND dateExecuted Between TO_DATE('" + DtoC(aRet[nPARAM_TREINADO_DE]) + "','DD/MM/YYYY') And TO_DATE('" + DtoC(aRet[nPARAM_TREINADO_ATE]) + "','DD/MM/YYYY') "
		EndIf

		//Filtro data da criação
		If !Empty(aRet[nPARAM_CRIADO_DE])
			cQuery += " AND DT_CRIACAO Between TO_DATE('" + DtoC(aRet[nPARAM_CRIADO_DE]) + "','DD/MM/YYYY') And TO_DATE('" + DtoC(aRet[nPARAM_CRIADO_ATE]) + "','DD/MM/YYYY') "
		EndIf
...
static function loadCabec( aLinha, aCabec )
	...
	aAdd(aCabec, {"Id Docto" 		,"C", 015, 0})
	aAdd(aCabec, {"Versão" 			,"C", 012, 0})
	aAdd(aCabec, {"Desc.Docto" 		,"C", 050, 0})
	aAdd(aCabec, {"Nome" 			,"C", 040, 0})
	aAdd(aCabec, {"Centro de Custo" ,"C", 040, 0})
	aAdd(aCabec, {"Descrição C.C." 	,"C", 040, 0})
	aAdd(aCabec, {"Dt.Criação"		,"C", 030, 0})
	aAdd(aCabec, {"Dt.Aprovação"	,"C", 030, 0})
	aAdd(aCabec, {"Dt.Treinamento"	,"C", 030, 0})
	...
	aAdd(aAux, "Id Documento" )
	aAdd(aAux, "Versão" )
	aAdd(aAux, "Desc.Documento" )
	aAdd(aAux, "Nome" )
	aAdd(aAux, "Centro de Custo")
	aAdd(aAux, "Descrição C.C.")
	aAdd(aAux, "Dt.Criação" )
	aAdd(aAux, "Dt.Aprovação" )
	aAdd(aAux, "Dt.Treinamento" )
