...
Static Function GerarAnali(aLista, nTotal, aTotal, cTipoRelat, lIsProc)
	...
	if !lPortalPon
		lProcessa := !empty(aLista) .and. MV_PAR09 == 1 .and. !lIsProc//Composição acumulado
	else
		lProcessa := !empty(aLista) .and. !lIsProc//Composição acumulado
	endif
...
Static Function CabCompAcu( cTipoRelat, lIsProc )
	...
	if !lPortalPon
		lProcessa :=  MV_PAR09 == 1 .and. !lIsProc //Composição acumulado
	else
		lProcessa := .T.
	endif
