...
User Function UCNTA180J(aParam)
...
cQuery	:= "  SELECT CN9_FILIAL, CN9_NUMERO, CN9_REVISA, CN9_USUAVA, CN9_PROXAV, CN9_ULTAVA FROM " + RetSqlName('CN9') + " CN9"
cQuery	+= "  WHERE CN9.CN9_FILIAL  = '" + xFilial('CN9') + "'"
cQuery	+= "    AND CN9.CN9_USUAVA <> ''"
cQuery	+= "    AND CN9.CN9_PROXAV <> ''"
cQuery	+= "    AND CN9.CN9_PROXAV < '" + DTOS(dData) + "'"
cQuery	+= "    AND CN9.CN9_SITUAC = '05'"  //Apenas contratos vigentes
cQuery	+= "    AND CN9.D_E_L_E_T_ = ' '"
cQuery	+= "  ORDER BY CN9_USUAVA, CN9_NUMERO, CN9_REVISA"
...
While CN9TMP->(!Eof())
	nPos := Ascan(aNotifCtr,{|x| x[01] == CN9TMP->CN9_USUAVA})
	If nPos == 0
		aadd(aNotifCtr,{CN9TMP->CN9_USUAVA,{{CN9TMP->CN9_FILIAL,CN9TMP->CN9_NUMERO,CN9TMP->CN9_REVISA,"","",CN9TMP->CN9_PROXAV,CN9TMP->CN9_ULTAVA}}})
	Else
...
cQuery	:= "  SELECT CNB_FILIAL, CNB_CONTRA, CNB_REVISA, CNB_PROXAV, CNB_ULTAVA, CNB_PRODUT, CNB_DESCRI, CN9_FILIAL, CN9_NUMERO, CN9_REVISA, CN9_USUAVA"
...
cQuery	+= "  ORDER BY CN9_USUAVA, CNB_CONTRA, CNB_REVISA"
cQuery := ChangeQuery(cQuery)
...
While CNBTMP->(!Eof())
	nPos := Ascan(aNotifCtr,{|x| x[01] == CNBTMP->CN9_USUAVA})
	If nPos == 0
		aadd(aNotifCtr,{CNBTMP->CN9_USUAVA,{{CNBTMP->CNB_FILIAL,CNBTMP->CNB_CONTRA,CNBTMP->CNB_REVISA,CNBTMP->CNB_PRODUT,CNBTMP->CNB_DESCRI,CNBTMP->CNB_PROXAV,CNBTMP->CNB_ULTAVA}}})
	Else
...
For nX:=1 to Len(aNotifCtr)
	If PswSeek( aNotifCtr[nX,01], .T. )
		...
		If !Empty(cServer) .And. !Empty(cConta) .And. !Empty(cPaswd) .And. !Empty(cEmDest) .And. !Empty(cMsg)
				...
				U_CSWF001( "contratos@certisign.com.br" , cEmDest , cAssun , cMsg , "" )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³ SchedDefº Autor ³ TOTVS              º Data ³  23/12/10   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Funcao auxiliadora na execucao de agendamento do Schedule  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³                                                            º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
