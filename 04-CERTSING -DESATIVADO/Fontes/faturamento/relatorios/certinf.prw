...
User Function CERTINF()        // incluido pelo assistente de conversao do AP5 IDE em 19/11/99
...
SetPrvt("XNUM_NF,XSERIE,xSerieSE1,XEMISSAO,XTOT_FAT,XLOJA,XFRETE")
...
If mv_par04 == 2
   ...
   While !eof() .and. SF2->F2_DOC <= mv_par02 .and. lContinua
      ...
      xNUM_DUPLIC :=SF2->F2_DUPL            // Numero da Duplicata
		// xNUM_DUPLIC :=If(!Empty(SF2->F2_DUPL), SF2->F2_DUPL, 0)    // Numero da Duplicata
      ...
      xVAL_IRRF   :=SF2->F2_VALIRRF         // Valor IRRF
      xVAL_COFI   :=SF2->F2_VALCOFI         // Valor COFINS
      xVAL_CSLL   :=SF2->F2_VALCSLL         // Valor CSLL
      xVAL_PIS    :=SF2->F2_VALPIS          // Valor PIS 
	   xSerieSE1	:=If(!Empty(SF2->F2_DUPL), GetMv("MV_1DUPREF"), '') 
      ...
      xTIPO_PRO:={}                         // Tipo mercadoria ou serviço
      ...
      xVALOR_DUP :={}                       // Valor
		...
		If !Empty(SF2->F2_DUPL) 
			 DbSeek(xFilial("SE1")+&xSerieSE1+xNum_Duplic, .T.)       // by 
		Endif 
	   xDUPLICATAS:= If(!Empty(SF2->F2_DUPL) ,.T.,.F.) 
		// xDUPLICATAS:= If(DbSeek(xFilial("SE1")+&xSerieSE1+xNum_Duplic ,.T.) ,.T.,.F.) 
...
For J:=1 to len(xTIPO_ITEM)              
    ...
    if subs(xTIPO_ITEM[J],1,1) == "S" .and. nLin < 32
      ...
      elseif nLin == 37 
         ...
         w:= .T.      //Imprimiu o Valor dos Serviços
      endif
      */
      nLin := nLin+1
Next        
...
@ 01, 040 PSAY xEND_EMP
@ 01, 120 PSAY xNUM_NF               // Numero da Nota Fiscal
@ 02, 040 PSAY xCOMP_EMP 
@ 03, 040 PSAY xTEL_EMP 
@ 04, 018 PSAY "WWW.CERTISIGN.COM.BR"
@ 04, 040 PSAY "SAC: 03007892378"
...
If mv_par04 == 1
  ...
  Else                                // Alteração para emitir duas CFOPs na Nota Fiscal de Saida
    
   For I := 1 to Y                 // Somente impressão de duas CFOPs se houver tempo por remover esse For e testar
	    If I = 1
	       If !EMPTY(xCFPR)
...
@ 06, 86 PSAY xINSC_EMP Picture"@R 999.999.999.999"
...
@ 11, 078 PSAY Left(xBAIRRO,22)                                   // Bairro
@ 11, 103 PSAY xCEP_CLI Picture"@R 99999-999"           // CEP
...
If xTPFRETE=='C' .AND. !Empty(xNOME_TRANSP)                                  // Frete por conta do
   @ 45, 076 PSAY "1"                              // Emitente (1)
ElseIf !Empty(xNOME_TRANSP)                                              //     ou
