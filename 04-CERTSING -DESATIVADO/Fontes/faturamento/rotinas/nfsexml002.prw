...
If cTipo == "1"
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If !Empty(SC5->C5_XMENSUG) .And. !AllTrim(FORMULA(SC5->C5_XMENSUG)) $ cMensFis
				cMensFis += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(FORMULA(SC5->C5_XMENSUG))),AllTrim(FORMULA(SC5->C5_XMENSUG)))
			EndIf
			...
			While !Eof() .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
				...
				Do Case
					...
					Case AllTrim(CD2->CD2_IMP) == "ISS"
						...
						Else         
							//------------------------------------------------------
							// Caso nao tenha ESTADO ou CODIGO MUNICIPIO posicionar
							// atraves da tabela PA7 para buscar Estado ou Cod Mun. 
							// Alteracao: Douglas Parreja
							//------------------------------------------------------
							if Empty(aDest[09]) .Or. Empty(aDest[07]) 
							
								dbSelectArea("PA7")					
								//------------------------------------------------------
								// Posicionar atraves do CEP
								// aDest[10] -> (A1_CEP)
								//------------------------------------------------------
								if !Empty( aDest[10] )
									PA7->( DbSetOrder(1) )	// PA7_FILIAL + PA7_CODCEP
									if PA7->( MsSeek( xFilial("PA7") + alltrim(aDest[10] ) ) ) 
										cEstPA7 := alltrim( PA7->PA7_ESTADO )                       
										cMunPA7 := alltrim( PA7->PA7_CODMUN )
										if !Empty( cEstPA7 )
											cMunISS := ConvType2(aUF[aScan(aUF,{|x| x[1] == cEstPA7 })][02]+Iif(Empty(aDest[07]), cMunPA7, aDest[07]) )
										endif
									endif						
								//------------------------------------------------------
								// Caso nao tenha CEP, buscar pelo Codigo do Municipio
								// aDest[07] -> (A1_COD_MUN)
								//------------------------------------------------------
								elseif !Empty( aDest[07] ) 
									PA7->( DbSetOrder(4) )	// PA7_FILIAL + PA7_CODMUN
									if PA7->( MsSeek( xFilial("PA7") + alltrim(aDest[07] ) ) )
										cEstPA7 := alltrim( PA7->PA7_ESTADO )                       
										cMunPA7 := alltrim( PA7->PA7_CODMUN )
										if !Empty( cEstPA7 )
											cMunISS := ConvType2(aUF[aScan(aUF,{|x| x[1] == cEstPA7 })][02]+aDest[07] )
										endif
									endif
								endif								
							else
...
//Renato Ruy - 18/11/2013 - Tratamento do Tipo de Logradouro.
cXtpLogr:= ""
cXLograd:= ""
If "AV "$UPPER(aDest[03])
	cXtpLogr:= "Av"
	cXLograd:= SubStr(AllTrim(aDest[03]),4,len(AllTrim(aDest[03])))
ElseIf "AVENIDA"$UPPER(aDest[03])
	cXtpLogr:= "Av"
	cXLograd:= SubStr(AllTrim(aDest[03]),8,len(AllTrim(aDest[03])))
ElseIf "AV."$UPPER(aDest[03])
	cXtpLogr:= "Av"
	cXLograd:= SubStr(AllTrim(aDest[03]),5,len(AllTrim(aDest[03])))
ElseIf "ROD "$UPPER(aDest[03])
   	cXtpLogr:= "Rod"
   	cXLograd:= SubStr(AllTrim(aDest[03]),5,len(AllTrim(aDest[03])))
ElseIf "RODOVIA"$UPPER(aDest[03])
	cXtpLogr:= "Rod"
	cXLograd:= SubStr(AllTrim(aDest[03]),5,len(AllTrim(aDest[03])))
ElseIf "QUADRA"$UPPER(aDest[03])
	cXtpLogr:= "Q"
	cXLograd:= SubStr(AllTrim(aDest[03]),8,len(AllTrim(aDest[03])))
ElseIf "RUA"$UPPER(aDest[03])
	cXtpLogr:= "Rua"
	cXLograd:= SubStr(AllTrim(aDest[03]),5,len(AllTrim(aDest[03])))
ElseIf "R."$UPPER(aDest[03]) 
	cXtpLogr:= "Rua"
	cXLograd:= SubStr(AllTrim(aDest[03]),4,len(AllTrim(aDest[03])))
ElseIf "R "$UPPER(aDest[03])
	cXtpLogr:= "Rua"
	cXLograd:= SubStr(AllTrim(aDest[03]),3,len(AllTrim(aDest[03])))
ElseIf "TRAVESSA"$UPPER(aDest[03])
	cXtpLogr:= "Tv"
	cXLograd:= SubStr(AllTrim(aDest[03]),10,len(AllTrim(aDest[03])))
ElseIf "TR "$UPPER(aDest[03])
	cXtpLogr:= "Tv"
	cXLograd:= SubStr(AllTrim(aDest[03]),4,len(AllTrim(aDest[03])))
ElseIf "ALAMEDA"$UPPER(aDest[03])
	cXtpLogr:= "Al"
	cXLograd:= SubStr(AllTrim(aDest[03]),4,len(AllTrim(aDest[03])))
ElseIf "PRACA"$UPPER(aDest[03])
	cXtpLogr:= "Pc"
	cXLograd:= SubStr(AllTrim(aDest[03]),7,len(AllTrim(aDest[03])))
ElseIf "SITIO"$UPPER(aDest[03])
	cXtpLogr:= "Sit"
	cXLograd:= SubStr(AllTrim(aDest[03]),7,len(AllTrim(aDest[03])))
ElseIf "ST "$UPPER(aDest[03])
	cXtpLogr:= "Sit"
	cXLograd:= SubStr(AllTrim(aDest[03]),4,len(AllTrim(aDest[03])))
ElseIf "ESTRADA"$UPPER(aDest[03])
	cXtpLogr:= "Est"
	cXLograd:= SubStr(AllTrim(aDest[03]),9,len(AllTrim(aDest[03])))
ElseIf "EST."$UPPER(aDest[03])
	cXtpLogr:= "Est"
	cXLograd:= SubStr(AllTrim(aDest[03]),5,len(AllTrim(aDest[03])))
ElseIf "BECO"$UPPER(aDest[03]) 
	cXtpLogr:= "Bc"
	cXLograd:= SubStr(AllTrim(aDest[03]),6,len(AllTrim(aDest[03]))) 
ElseIf "CORREGO"$UPPER(aDest[03]) 
	cXtpLogr:= "Crg"
	cXLograd:= SubStr(AllTrim(aDest[03]),9,len(AllTrim(aDest[03])))       
ElseIf "COND "$UPPER(aDest[03])
	cXtpLogr:= "Cod"
	cXLograd:= SubStr(AllTrim(aDest[03]),6,len(AllTrim(aDest[03])))
Else
	cXtpLogr:= "Rua"
	cXLograd:= AllTrim(aDest[03])
EndIf

cString += '<RazaoSocialTomador>'+NoAcento2(StrTran(AllTrim(aDest[02]),"&","E"))+'</RazaoSocialTomador>'
cString += '<TipoLogradouroTomador>' + cXtpLogr + '</TipoLogradouroTomador>'
cString += '<LogradouroTomador>'+SubStr(NoAcento2(StrTran(cXLograd,"&","E")),1,50)+'</LogradouroTomador>'
cString += '<NumeroEnderecoTomador>'+SubStr(NoAcento2(StrTran(AllTrim(aDest[04]),"&","E")),1,50)+'</NumeroEnderecoTomador>'

If !Empty(aDest[05])
	cString += '<ComplementoEnderecoTomador>'+NoAcento2(StrTran(SubStr(AllTrim(aDest[05]),1,30),"&","E"))+'</ComplementoEnderecoTomador>'
Else
...
cString += '<BairroTomador>'+NoAcento2(StrTran(AllTrim(aDest[06]),"&","E"))+'</BairroTomador>'
...
cString += '<CidadeTomadorDescricao>'+NoAcento2(StrTran(AllTrim(aDest[08]),"&","E"))+'</CidadeTomadorDescricao>'
cString += '<UFTomador>'+AllTrim(aDest[09])+'</UFTomador>'
cString += '<CEPTomador>'+AllTrim(aDest[10])+'</CEPTomador>' 
cString += '<EmailTomador>'+StrTran(AllTrim(aDest[16]),"_","")+'</EmailTomador>'
If cCodMun $ '4303905-4307906' //Campo Bom-RS  - Farroupilha-RS
	cString += '<NomepaisTomador>'+NoAcento2(AllTrim(aDest[12]))+'</NomepaisTomador>'
Else
	cString += '<DescrPaisTomador>'+NoAcento2(AllTrim(aDest[12]))+'</DescrPaisTomador>'                                           
EndIf
Return(cString)
