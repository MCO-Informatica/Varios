...
@return null, Nulo
/*/
user function ESPA020()
	local nHeightBtn   := 15

	private aBrwCabec := { 'Pedido Site', 'Pedido Venda','Produto','Desc. Prod.', 'NF','Série NF', 'Data do Faturamento', 'Link', 'Recno SC5', 'Pedido GAR' }
	private aBrowse    := {}
	private oBrowse    := nil
	...
	if pergunte("ESPA010", .T.)
		oDlgP := tDialog():New(180,180,750,1366,'Reprocessamento de NF Produto - Certisign',,,,,CLR_BLACK,CLR_WHITE,,,.T.)
		...
		oButton1 := tButton():New( 002, 002, "Reprocessar NF de produtos", oPanel2,{|| Processa( {|| procFat() }, "Aguarde...", "Reprocessando NF de produtos.",.F.) }, 050,nHeightBtn,,,.F.,.T.,.F.,,.F.,,,.F. )
...
static function loadList()
	...
	oBrowse:Resetlen()
	if len(aBrowse) == 0
		aAdd(aBrowse, aFill(array(9), "") )
	endif

	oBrowse:SetArray( aBrowse )
	oBrowse:bLine := {||{ aBrowse[oBrowse:nAt,01],aBrowse[oBrowse:nAt,02],aBrowse[oBrowse:nAt,03],aBrowse[oBrowse:nAt,04],aBrowse[oBrowse:nAt,05],aBrowse[oBrowse:nAt,06],aBrowse[oBrowse:nAt,07],aBrowse[oBrowse:nAt,08],aBrowse[oBrowse:nAt,09] } }
...
Static Function getSC6()
	...
	if oQuery:Consultar( cQuery )
		...
		while (cAlias)->(!EoF())
			...
			aAdd( aItem , dtoc( stod( (cAlias)->C6_DATFAT ) ) ) //07
			aAdd( aItem , (cAlias)->C6_XNFSFW  ) //08
			aAdd( aItem , (cAlias)->REC  )       //09
			aAdd( aItem , (cAlias)->C6_PEDGAR  ) //10
...
static function procFat()
	...
	oLog := CSLog():New()
	oLog:SetAssunto( "[ ESPA020 ] Processamento de faturamento - U_VNDA190P" )
