	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				aadd(aDest,SubStr(SA1->A1_BAIRRO,1,28))
				...
				aadd(aDest,FormatAll(VldIE2(SA1->A1_INSCR,IIF(SA1->(FIELDPOS("A1_CONTRIB"))>0,SA1->A1_CONTRIB<>"2",.T.))))
				aadd(aDest,SA1->A1_SUFRAMA)
				aadd(aDest,SA1->A1_EMAIL)          
				aadd(aDest,FormatAll(IIF('ISENTO' $ Upper(SA1->A1_INSCRM), '',SA1->A1_INSCRM))) 
			...
			Else
				...
				aadd(aDest,SubStr(SA2->A2_BAIRRO,1,28))
				...
				aadd(aDest,FormatAll(VldIE2(SA2->A2_INSCR)))
				aadd(aDest,"")//SA2->A2_SUFRAMA
				aadd(aDest,SA2->A2_EMAIL)
				aadd(aDest,FormatAll(IIF('ISENTO' $ Upper(SA2->A2_INSCRM), '',SA2->A2_INSCRM))) 
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				cString	+= pagtos( aDupl,cCondPag,cCodMun )
...
Static Function tomador( aDest )
	...
	If !lSemTomador 		
		...
		ElseIf "ALAMEDA"$UPPER(aDest[03])
			cXtpLogr:= "2" //Nao tem codigo para alameda, deixei o padrao rua
			cXLograd:= SubStr(AllTrim(aDest[03]),4,len(AllTrim(aDest[03])))
		ElseIf "PRACA"$UPPER(aDest[03])
			cXtpLogr:= "2" //Nao tem codigo para praca, deixei o padrao rua
		...
		ElseIf "COND "$UPPER(aDest[03])
			cXtpLogr:= "2" //Nao tem codigo para condominio, adicionei o padrao
			cXLograd:= SubStr(AllTrim(aDest[03]),6,len(AllTrim(aDest[03])))
		Else
			cXtpLogr:= "2"
		...
		//Renato Ruy - 08/11/2017
		//Alterei para buscar os dados tratados
		cString	+= "<tipologr>"+cXtpLogr+"</tipologr>"
		cString	+= "<logradouro>" + SubStr(NoAcento2(StrTran(cXLograd,"&","E")),1,50) + "</logradouro>"
		...
		cString	+= "<numend>" + SubStr(NoAcento2(StrTran(AllTrim(aDest[04]),"&","E")),1,50) + "</numend>"
		if !empty( allTrim( aDest[5] ) )
			cString	+= "<complend>" + NoAcento2(StrTran(SubStr(AllTrim(aDest[05]),1,30),"&","E")) + "</complend>"
		endif
		cString	+= "<tipobairro>1</tipobairro>"
		cString	+= "<bairro>" + NoAcento2(StrTran(AllTrim(aDest[06]),"&","E")) + "</bairro>"
		...
		cString	+= "<cidade>" + NoAcento2(StrTran(AllTrim(aDest[08]),"&","E")) + "</cidade>"
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf)	
	...
	for nX := 1 to len( aProd )
		...
		//Busca a mensagem correta do ponto de entrada
		cString	+= "<discr>" +StrTran(StrTran(NoAcento2(StrTran(Execblock("MTDESCRNFE",.F.,.F.,{SF2->F2_DOC,SF2->F2_SERIE,SF2->F2_CLIENTE,SF2->F2_LOJA}),"|","kk")),"kk","|"),"&amp"," ")+ StrTran(StrTran(NoAcento2(StrTran(AllTrim(cCargaTrb),"|","kk")),"kk","|"),"&amp"," ")+ "</discr>"
...
/*/
//-----------------------------------------------------------------------
static function pagtos( aDupl,cCondPag,cCodMun )
...
Static Function FormatAll(cInsc,lContr)
