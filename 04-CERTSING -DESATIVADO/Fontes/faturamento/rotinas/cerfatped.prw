...
User Function CerFatPE()
	...
	//Alterado - André Sant'ana - Compila
	...
	cSql += "            AND ( ( C5_XORIGPV <> '8' AND C5_EMISSAO >= '" + DtoS( Date() - nDias ) + "') OR ( C5_XORIGPV = '8' AND C5_XLIBFAT='S'  AND C5_EMISSAO >= '" + DtoS( Date() - nDCurso ) + "') ) "
	cSql += "            ) BD1 "
...
User Function CerFatCa( aParam ) 
	...
	//Alterado André Sant'an - Compila
	cSql += "					( ( C5_XORIGPV <> '8' AND SC5.C5_EMISSAO BETWEEN '" + DtoS( Date() - nDias )+ "' AND  TO_CHAR((SYSDATE -1),'YYYYMMDD' ))  OR ( C5_XORIGPV = '8' AND C5_XLIBFAT='S'  AND C5_EMISSAO >= '" + DtoS( Date() - nDCurso ) + "') ) "
...
User Function FatPltGr(cXC5_NUM)
	// Faturamento da Política de Garantia
	...
	fatConout("PREPARACAO EXECUCAO FATPLTGR")
	...
	dEmisDe := StoD( GetMV( "MV_XCORFPL",, DtoS(dEmisDe) ) )  // CORTE DA PLT - EXECUTAR SOMENTE A PARTIR DE 2021 - REUNICO YURI, ALINE

	oLog := CSLog():New( cSX6_LOG )
	oLog:SetAssunto( "[ FatPltGr ] - JOB Politica de Garantia" )

	fatConout("INICIO EXECUCAO FATPLTGR - EMISSAO PEDIDO A PARTIR DE "+DtoC(dEmisDe))
	...
	nDiasPltV := GetMV( "MV_XDIAPGV",, 31 )   //dias que se apropria da receita do pedido a partir da validagco
	...
	BeginSql Alias "PLTGRT"
		...
		WHERE 
			...
			AND 
			(
				/* RECEBIMENTOS DE PEDIDOS GAR QUE ESTÃO VALIDADOS A MAIS DE 3O DIAS E NÃO FORAM EMITIDOS*/       
				( DATVAL>' 'and DATVAL<=%Exp:DtoS(dValidAte)%  And PEDGANT=' ' AND ( SALDO_PR>0 OR SALDO_NCC>0) )
				
			OR
				(
					( /*AUTORIZAÇÕES DE PAGAMENTO DE CARTÃO DE PEDIDOS GAR QUE NÃO FORAM VALIDADOS OU NÃO FORAM EMITIDOS A MAIS DE 180 DIAS.*/
						(EMIS_PR >=%Exp:DtoS(dEmisDe)%  AND EMIS_PR<=%Exp:DtoS(dEmisAte)% AND ( SALDO_PR>0 OR SALDO_NCC>0))
						OR 
						/* RECEBIMENTOS DE PEDIDOS GAR (CARTÃO OU BOLETO) QUE NÃO FORAM VALIDADOS OU NÃO FORAM EMITIDOS A MAIS DE 180 DIAS.*/
						(EMIS_NCC >= %Exp:DtoS(dEmisDe)% AND EMIS_NCC<= %Exp:DtoS(dEmisAte)% AND SALDO_NCC>0 )
					)
					
					/* E QUE A DATA DE NÃO FORAM VALIDADOS NOS ULTIMOS 30 DIAS  */	
					AND (DATVAL<=%Exp:DtoS(dValidAte)%)       
					/* E QUE NÃO SEJA SALDO RESIDUAL DE COMPENSAÇÃO DE TÍULOS - SERÃO TRATADOS NA ROTINA FCOMPSUB().*/
					AND VALOR_NCC>0.02
				)
			)
	...
	While PLTGRT->( !Eof() )
		...
		IF PLTGRT->C6_XOPER $ '51,61' //.AND. EMPTY(PLTGRT->NOTA)
			//Prepara faturamento somente do Serviço e grava na tabela de legado para controle e referencia
			lFat        := .T.
			lServ       := .T.
			...
			If lRet 
				...
				If SC5->(MsSeek(xFilial("SC5")+cPedVenda))
					...
					StartJob("U_VNDA190P",GetEnvServer(),.F.,'01','02',nRecPed,lFat,lProd, lServ, lEnt,lRecPgto,lGerTitRecb,cTipTitRecb,PLTGRT->RECSC6)   // ACRESCENTEI PARAMETRO RECNO SC6 PARA POSICIONAR SC6 NA FUNGCO VNDA190 PEDGAR - mgomes.upduo - 24/03/2021
		...
		ElseIf PLTGRT->C6_XOPER $ '52,62' //.AND. EMPTY(PLTGRT->NOTA)
			...
			//Não fatura produtos em garantia. Apenas registra na tabela de Legado para contabilização
		...
		WHILE !Eof().And. SC6->C6_NUM==cPedVenda
				...
				IF SC6->C6_XOPER $ '51,61' .AND. EMPTY(SC6->C6_NOTA)
					//Prepara faturamento somente do ServiC'o e grava na tabela de legado para controle e referencia
					lFat        := .T.
					lServ       := .T.
					...
					//Atualiza o produto da GTLEGADO quando o csdigo Pedido GAR i diferente da SC6
				...
				ElseIf SC6->C6_XOPER $ '52,62' .AND. EMPTY(SC6->C6_NOTA)
					...
					//NC#o fatura produtos em garantia. Apenas registra na tabela de Legado para contabilizaC'C#o
		...
		If !EMPTY(cPedVenda) .AND. lFat
			...
			//Faz distribuiC'C#o e monitora a quantidade de thread em execuC'C#o
			nThread := 0
			aUsers 	:= Getuserinfoarray()
			aEval(aUsers,{|x| IIF( ALLTRIM(UPPER(x[5])) == "U_VNDA190P".OR.ALLTRIM(UPPER(x[5])) == "U_CerFatAL" .OR.ALLTRIM(UPPER(x[5])) == "PRC",nThread++,nil )  })
	...
	fatConout("FIM EXECUCAO FATPLTGR")
Return                  
...
Static Function fGrvLeg(cPedVenda, cType, cProduto,	nVlrPrd, nVlrPis, nVlrCofins, dDataProc, dDtRef, cTpRef, cPedGar, cXnpSite  )
    ...
    Else
        lRet := .F. //Retorna Falso para não processar faturamento
    Endif
...
User Function fGrvLeg()
	...
	ELSE
		...
		aadd(aNewStru,{"GT_REVCTBPRD","D",8,0})    // DATA REVERSAO DO LANC PRD NA CONTABILDIADE
...
User Function FCompSub(aParam,cPedErp)
	...
	fatConout( "PREPARACAO EXECUCAO FCOMPSUB" )
	...
	While cEmisDe <= DtoS(dDatabase + nDias)
		...
		fatConout( "INICIO EXECUCAO FCOMPSUB - EMISSAO PEDIDO A PARTIR DE "+DtoC(StoD(cEmisDe)) )
		...
		cEndQry += "                  OR ( ( SALDO_PR > 0 OR SALDO_NCC > 0 ) AND ARQVTEX = 'CHARGEBACK' )" + CRLF
		cEndQry += "				)" + CRLF
		cEndQry += "		)" + CRLF
	...
	fatConout("FIM EXECUCAO FCOMPSUB INICIADO EM "+DtoC(StoD(cEmisIn)))
	...
	While (cTRB)->( !Eof() )
		...
		If ( EMPTY((cTRB)->DOCUMENTO) .AND.;
			...
			( !EMPTY((cTRB)->DOCUMENTO) .AND.;
				...
				( ( (cTRB)->SALDO_PR > 0 .OR. (cTRB)->SALDO_NCC > 0 ) .AND. ALLTRIM((cTRB)->ARQVTEX) == 'CHARGEBACK' ) ;	//( ( (cTRB)->VALOR_PR == (cTRB)->VALOR_NCC ) .AND. (cTRB)->VALOR_NF == 0 .AND. ( (cTRB)->SALDO_PR > 0 .OR. (cTRB)->SALDO_NCC > 0 ) .AND. ALLTRIM((cTRB)->ARQVTEX) == 'CHARGEBACK' ) ;
				);
			)
			...
			If Empty( (cTRB)->ARQVTEX )
				lVldData := .T.                                                                                               
				/*
				//Se o VALOR Total da NF é Igual ao VALOR Total da NCC OU diferença entre OS VALORES é menor igual a 0.02 OU a diferença entre OS SALDOS é menor igual a 0.02, 
				//Então força a compensação dos SALDOS em aberto independente do valor do saldo das parcelas
				//Senão força a compensação somente de saldos resíduais < 0.02 de parcelas
				...
				*/
				...
				lVldSldNF := iif(	(cTRB)->SALDO_PR>0 .and.((cTRB)->VALOR_NF-(cTRB)->VALOR_PR>=-0.02 .AND. (cTRB)->VALOR_NF-(cTRB)->VALOR_PR<=0.02),.F.,.T.)//Se saldo do PR maior que Zero, Ignora Saldo da NF. Considera o Valor o que irá forçar a substituição do PR pela NF
				...
				If Len(aSe1Nf) > 0
					...
					//aparan[3] //Valida apenas saldo residual 0,01, default(.t.)., Se .f. vai compesar todos os valores em aberto para NCC e NF. 
					...
					//aparan[4] //Ignora parcela e saldo do PR. default(.t.)., Se .f. força a subsituição do PR pela NF.
								
					U_VldRecPg( aSe1Nf,lVldData,lVldSldRes,lVldSldNF )		
				Endif

			ElseIf ( (cTRB)->SALDO_PR > 0 .OR. (cTRB)->SALDO_NCC > 0 ) .AND. Alltrim( (cTRB)->ARQVTEX ) == 'CHARGEBACK'
				fCompCHA( cPedido, cPedSite, cBandeira,(cTRB)->VALOR_NCC )
			EndIF
...
User Function FatPltVl()
    ...
    Else
        MsgAlert('Processo cancelado pelo usuário','[FatPltVl] - Ajuste GtLegado')
    EndIF
Return
...
Static Function A010Txt( aRET )
    ...
    IF !File(cFile)
        MsgAlert("Arquivo texto: "+cFile+" não localizado")
        Return
    Endif
...
User Function FcompPLT()
	...
	fatConout("PREPARACAO EXECUCAO FCOMPPLT")
	If lJob
		RpcSetType( 3 )
		RpcSetEnv( '01', '02' )
	EndIf

	//Usado para debugar um pedido específico
	If !Empty(cPedErp)
	   cWhere := "%AND C5_NUM = '"+cPedErp+"'%"
	EndIf

	dEmisDe := StoD(GetMV( "MV_XCORPLT",, DtoS(dEmisDe) ) ) // CORTE DA SUB
	fatConout( "INICIO EXECUCAO FCOMPPLT - EMISSAO PEDIDO A PARTIR DE "+DtoC(dEmisDe) )
	...
	BeginSql Alias "FCOMPPLT"
		...
		WHERE
			...
			%Exp:cWhere%    // Específico para debug
		) BASE 
		...
		WHERE 
			...
			(BASE.DOCUMENTO=' ' AND
			(
				VALOR_PR>0 //--NAO PODERIA EXISTIR PR PARA BOLETO OU VENDA SEM DOCUMENTO DE AUTORIZACAO 
				OR
				(
				(VALOR_NF=VALOR_NCC) AND ( SALDO_NF>0 OR SALDO_NCC>0 )// --FALTA COMPENSACAO
				)
				OR 
				(
				( ((TOTPED-VALOR_NF)-(SALDO_NCC))<>0 )  AND          (SALDO_NF>0   AND   SALDO_NCC>0) //-- FATURADO E NAO RECEBIDO
				)
			)
			)
			OR 
	...
	While FCOMPPLT->( !Eof() )   
		...
		lVldData := .F. //.T.                                                                                                  
		/*
		//Se o VALOR Total da NF é Igual ao VALOR Total da NCC OU diferença entre OS VALORES é menor igual a 0.02 OU a ddiferença entre OS SALDOS é menor igual a 0.02, 
		//Então força a compensação dos SALDOS em aberto independente do valor do saldo das parcelas
		//Senão força a compensação somente de saldos residuais < 0.02 de parcelas
		...
		*/
		lVldSldRes := .F. //se está no select é porque existe um erro ou falta compesação. Então Força a compensação independente de saldo ou parcela
		lVldSldNF := iif(	FCOMPPLT->SALDO_PR>0 .and.(FCOMPPLT->VALOR_NF-FCOMPPLT->VALOR_PR>=-0.02 .AND. FCOMPPLT->VALOR_NF-FCOMPPLT->VALOR_PR<=0.02),.F.,.T.)//Se saldo do PR maior que Zero, Ignora Saldo da NF. Considera o Valor o que irá forçar a substituição do PR pela NF
	...
	fatConout("FIM EXECUCAO FCOMPPLT")
...
@since 13/12/2018
/*/
Static Function fCompCHA( cPedido, cPedSite, cBandeira, nTotNcc)
	Local cBco			:= ""
	Local cAge			:= ""
	Local cCta			:= ""
	Local cSql			:= ""
	...
	cSql := "SELECT R_E_C_N_O_ RECE1, E1_VALOR, E1_SALDO "
	cSql += " FROM "+RetSqlName("SE1")
	cSql += " WHERE E1_FILIAL = '"+xFilial("SE1")+"' AND "
	cSql += " E1_PEDIDO = '"+cPedido+"' AND E1_TIPO='PR' AND"
	//cSql += " E1_SALDO > 0 AND "
	cSql += " D_E_L_E_T_ = ' ' ORDER BY E1_PARCELA "
	
	cTrbSql := GetNextAlias()
	PLSQuery( cSql, cTrbSql )
	nTotPr := 0
	While !(cTrbSql)->(Eof())
		nTotPr += (cTrbSql)->e1_valor
		if nTotPr <= (nTotNcc+0.02) .and. (cTrbSql)->e1_saldo > 0
	...
	//If Len(aSe1PR) == 0
		...
		// Identifica as NF para compensar com as NCC antes de baixar por CHA.
		cSql := ""
		cSql += "SELECT E1_TIPO, R_E_C_N_O_ RECE1 "
		cSql += " FROM "+RetSqlName("SE1")
		cSql += " WHERE E1_FILIAL = '"+xFilial("SE1")+"' AND "
...
@author Bruno Nunes
@since 17/09/2020
@alteragco: 24/03/2021 - Tambim ira alterar o Valor no campo GT_VLRPRD - por mgomes.upduo
/*/ 
