...
User Function CSFT120LEG(aParam)
	...
	If !lJob
		Conout("[CSFT120LEG] Rotina não está rodando em Job. Processo abortado.")
		Return
	Else
		RpcSetType( 3 )
	...
	Else
		Conout("[CSFT120LEG] Parâmetro de filial incorreto.")
		Return
	EndIf
	...
	Else
		...
		Conout("[CSFT120LEG] Não há registros para processamento. Encerrando rotina.")
	EndIf
...
Static Function CSFT120Run(aPedidos)
    ...
    For Ni := 1 To Len(aPedidos)
        ...
        If SC5->(dbSeek(xFilial("SC5") + SC6->C6_NUM))
            ...
            Conout("[CSFT120LEG] Carregando impressão Pedido: " + cValToChar(aPedidos[Ni]) + " C6_NUM: " + SC6->C6_NUM)

            If CSFT120Print()
                
                Conout("[CSFT120LEG] Carregando Notificação do Checkout: " + cValToChar(aPedidos[Ni]) + " C6_NUM: " + SC6->C6_NUM)
                NotificaCheckout()
    ...
    If SF2->(dbSeek(xFilial("SF2") + SC6->C6_NOTA + SC6->C6_SERIE))
        ...
        If validaNFE()
            If AllTrim(SC6->C6_SERIE) == "2"  .Or. AllTrim(SC6->C6_SERIE) == "3"
                ...
                Conout("[CSFT120LEG] Carregando impressão PRODUTO: " + cValToChar(SC6->(Recno())) + " Pedido/Item: " + SC6->C6_NUM + SC6->C6_ITEM)
                aRet := U_GARR010({.T.})
                lRet := aRet[1]
                Conout("[CSFT120LEG] Impressão PRODUTO: (" + cValToChar(lRet) + ") " + cValToChar(SC6->(Recno())) + " Pedido/Item: " + SC6->C6_NUM + SC6->C6_ITEM)

                If !lRet
                    Conout("[CSFT120LEG] Impressão PRODUTO: (" + cValToChar(lRet) + ") " + cValToChar(SC6->(Recno())) + " Pedido/Item: " + SC6->C6_NUM + SC6->C6_ITEM + aRet[2] + "/" + aRet[3] + "/" + aRet[4])
                EndIf

            ElseIf AllTrim(SC6->C6_SERIE) == "RP2"
                
                Conout("[CSFT120LEG] Carregando impressão SOFTWARE: " + cValToChar(SC6->(Recno())) + " Pedido/Item: " + SC6->C6_NUM + SC6->C6_ITEM)
                lRet := U_GARR020({.T.},.T.)[1]
                Conout("[CSFT120LEG] Impressão SOFTWARE: (" + cValToChar(lRet) + ") " + cValToChar(SC6->(Recno())) + " Pedido/Item: " + SC6->C6_NUM + SC6->C6_ITEM)
...
Static Function NotificaCheckout()
    ...
    IF lOk
        ...
        EndIF
    Else
        Conout("[CSFT120LEG] Mensagem JSON não enviada: " + cJson)
    EndIF        
...
static function validaNFE()
    ...
    Conout("[CSFT120LEG] A Nota Fiscal não foi aprovada: " + SF2->F2_DOC + "/" + SF2->F2_SERIE + ": " + AllTrim(SF2->F2_ARET))
	...
	ElseIf AllTrim(SC6->C6_SERIE) == "RP2" .and. empty( SF2->F2_CODNFE )
	    ...
	    lValido := .F.
	EndIf
	Conout("[CSFT120LEG] Flag atualizado com código de Erro. " + SC6->C6_NUM + "/" + SC6->C6_ITEM)
