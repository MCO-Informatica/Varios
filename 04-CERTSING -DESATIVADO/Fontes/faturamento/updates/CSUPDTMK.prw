...
#Define DETALHE 4
Static cCSUpdCtl := "CSTKUPDCTL"

//+-------------+---------+-------+-----------------------+------+-------------+
//|Programa:    |CSUPDTMK |Autor: |David Alves dos Santos |Data: |01/06/2017   |
//|-------------+---------+-------+-----------------------+------+-------------|
//|Descricao:   |Gerenciador de updates CallCenter - SIGATMK.                  |
//|-------------+--------------------------------------------------------------|
//|Nomenclatura |CS  = Certisign.                                              |
//|do codigo    |UPD = Update.                                                 |
//|fonte.       |TMK = Modulo CallCenter SIGATMK.                              |
//|-------------+--------------------------------------------------------------|
//|Uso:         |Certisign - Certificadora Digital.                            |
//+-------------+--------------------------------------------------------------+
User Function CSUPDTMK()
	...
	If ( lOpen := MyOpenSm0Ex() )
		...
		SM0->(DbGoTop())
   		...
   		TITLE 'Gerenciador de Updates SIGATMK - Protheus | Especifico Certisign' FROM 00,00 TO 410,760 PIXEL
...
Static Function GetListUpd(lLoadAtu,lEmpty)
	...
	For nAux := 1 To 9999
   		...
   		If FindFunction( 'u_CTKU' + cUpdate)
 	     	lExecUpd := .F.
      		cUsuario := ""
      		cMaquina := ""
      		cDataUpd := ""
      		cHoraUpd := ""
      		If (cCSUpdCtl)->(lExecUpd:=MsSeek('CTKU'+cUpdate,.F.))
         		conout("Update u_CTKU"+cUpdate+ " ja executado.")
         		If !lLoadAtu
            		Loop
         		EndIf
      		...
      		EndIf
      		conout("Carregando informacoes update u_CTKU"+cUpdate)
      		aRet := &('StaticCall(CTKU' + cUpdate + ',UpdGetInfo)' )
...
//|Uso:         |Certisign - Certificadora Digital.                              |
//+-------------+----------------------------------------------------------------+
Static Function PrxOrdX3(cAliasX3)
