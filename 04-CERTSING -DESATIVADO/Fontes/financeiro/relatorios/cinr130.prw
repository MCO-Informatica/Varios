...
TRCell():New(oSection1,"CLIENTE",,'Código',,nTamCli,.F.,,,,,,,.F.)  //"Codigo-Lj-Nome do Cliente"
			...
			If ! SE1->E1_TIPO $ MVABATIM
				If ! (SE1->E1_TIPO $ MVRECANT+"/"+MV_CRNEG) .And. ;
					...
					If mv_par33 != 1  //somente deve considerar abatimento no saldo se nao listar
						...
						Endif
					Else       
					    // Subtrai o Abatimento, pois caso contrário, sempre irá listar o titulo como pendente a
					    // receber  
...
#IFDEF TOP
	...
	If ExistProc( cProcedure ) .and. ( TcSrvType() <> "AS/400" ) //Retirado verificação de versão da Procedure
		aResult := {}
...
While E5_FILIAL + E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO == ;
   ...
   SE5->(DbGoTo(__BAIXA->(Recno())))
	...
	Endif
 	...
 	If !lRelat // Colocado este tratamento para conseguir compor o saldo no relatório de titulos a receber. Antes do Tratamento, fazia skip e ignorava o registro nos relatórios
