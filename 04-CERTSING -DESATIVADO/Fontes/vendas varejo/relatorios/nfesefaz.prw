...
If !Empty(aNota)
	...
	//--Rbeghini 21.01.2020
	//--Processo cartórios
	cMsgEntrega := PDEntrega()
	
	cMensCli := cMensCli + CRLF + cMsgEntrega
...
//--------------------------------------------------------------------------
// Rotina | PDEntrega | Autor | Rafael Beghini       | Data | 24.01.2020
//--------------------------------------------------------------------------
// Descr. | Informa o endereço do entrega do fornecedor
//        | Utilizado em "Informações complementares" da DANFE
//--------------------------------------------------------------------------
Static Function PDEntrega()
	Local aAreaPD 	:= GetArea()
	Local cRet		:= ''
	IF SF2->F2_TIPO == 'B' .AND. .NOT. Empty( SF2->F2_CARGA )
		SA2->( dbSetOrder(1) )
		IF SA2->( dbSeek( xFilial('SA2') + SF2->F2_CLIENT + SF2->F2_LOJENT ) ) .And. .NOT. Empty(SA2->A2_CEPENT)
			cRet += ' Local de entrega: ' + rTrim(SA2->A2_ENDENT) + ' ' + rTrim(SA2->A2_COMPLEE) +;
			' Bairro: ' + rTrim(SA2->A2_BAIRROE) + ' UF: ' + rTrim(SA2->A2_ESTE) + ' Municipio: ' +;
			rTrim(SA2->A2_MUNE) + ' CEP: ' + rTrim(SA2->A2_CEPENT) + ' ' + CRLF
		EndIF
	EndIF
	RestArea( aAreaPD )
Return( cRet )
