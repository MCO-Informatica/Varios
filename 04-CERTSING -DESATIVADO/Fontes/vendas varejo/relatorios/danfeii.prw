...
User Function PrtNfeSef(cIdEnt, cVal1		, cVal2		, oDanfe,;
						oSetup, cFilePrint	, lIsLoja	, nTipo, cRootPath, lDeneg	)
...
Private lDenega := IIF( lDeneg == NIL, .F., IIF( lDeneg, .T., .F. ) )
...
If nTipo <> 1
	...
	oDanfe:lServer := IsBlind() //oSetup:GetProperty(PD_DESTINATION)==AMB_SERVER
	// ----------------------------------------------
	// Define saida de impressão
	// ----------------------------------------------
	IF !IsBlind() .AND. FunName() <> "CSGERNF"
...
//		oDanfe:SetDevice( nDevice )
		...
		U_DANFEProc(@oDanfe, .F., cIDEnt, Nil, Nil, @lExistNFe, lIsLoja)
		Return( lExistNFe )
	EndIF
...
IF oDanfe:lInJob .OR. IsInCallStack('U_VNDA560') 
	Pergunte("NFSIGW",.F.)	
	MV_PAR01 := SF2->F2_DOC
	MV_PAR02 := SF2->F2_DOC
	MV_PAR03 := SF2->F2_SERIE
...
If ntipo <> 1
	...
	PRIVATE oFont45N   := TFontEx():New(oDanfe,"VERDANA"        ,45,45,.T.,.T.,.F.)// 12 
	PRIVATE oFont55N   := TFontEx():New(oDanfe,"VERDANA"        ,55,55,.T.,.T.,.F.)// 12
...
//INFORMA QUE A NOTA ESTA DENEGADA - RAFAEL TOTVS 27.07.2015
IF lDenega
	IF cCodNfe $ '232/302'
		aAdd( aMensagem, 'Situação NF: Denegação de Uso - Situação do destinatário')
	Else
		aAdd( aMensagem, 'NF DENEGADA')
	EndIF
ElseIF cCodNfe $ U_RetCodDene()
	IF cCodNfe $ '232/302'
		aAdd( aMensagem, 'Situação NF: Denegação de Uso - Situação do destinatário')
	Else
		aAdd( aMensagem, 'NF DENEGADA')
...
//RAFAEL TOTVS 27.07.2015
If Empty(cCodAutDPEC) .And. (((Val(SubStr(oNF:_INFNFE:_IDE:_SERIE:TEXT,1,3)) >= 900).And.(oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"23") .Or. (oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"1|6|7") .And. !lDenega .And. !cCodNfe $ U_RetCodDene()
	oDanfe:Say(148,352,"PROTOCOLO DE AUTORIZAÇÃO DE USO",oFont08N:oFont)
Else
	IF Empty(cCodAutDPEC) .And. (((Val(SubStr(oNF:_INFNFE:_IDE:_SERIE:TEXT,1,3)) >= 900).And.(oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"23") .Or. (oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"1|6|7") .And. ( lDenega .OR. cCodNfe $ U_RetCodDene() )
		oDanfe:Say(148,352,"PROTOCOLO DE DENEGAÇÃO DE USO",oFont08N:oFont)
	EndIF
Endif
...
//RAFAEL BEGHINI //Imprime a informação na transversal
IF lDenega .OR. cCodNfe $ U_RetCodDene()
	IF oDanfe:lInJob
		oDanfe:Say(600,015, 'N.F. DENEGADA PELA SEFAZ', oFont55N:oFont, nil, CLR_GRAY, 330 ) 
	Else
		oDanfe:Say(600,010, 'N.F. DENEGADA PELA SEFAZ', oFont45N:oFont, nil, CLR_GRAY, 330 ) 
	EndIF
EndIF
...
User Function RetCodDene()
	Local cCodDene    := ''
	Local cMV_CODDENE := 'MV_DANFEDN'

	If .NOT. SX6->( ExisteSX6( cMV_CODDENE ) )
		CriarSX6( cMV_CODDENE, 'C', 'Codigos de uso denegado da NFe para impressao da DanfeII.prw', "'110','301','205','232','302','303','304','305','306'" )
	Endif

	cCodDene := GetMv( cMV_CODDENE )
