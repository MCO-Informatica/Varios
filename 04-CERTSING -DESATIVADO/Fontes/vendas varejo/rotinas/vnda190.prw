...
IF .NOT. lMV_FAT
	Conout('[VNDA190] - MV_190FAT esta FALSO, bloqueia o faturamento')
EndIF
IF .NOT. lMV_SERV
	Conout('[VNDA190] - MV_190SERV esta FALSO, bloqueia o faturamento de servigo')
EndIF
IF .NOT. lMV_PROD
	Conout('[VNDA190] - MV_190PROD esta FALSO, bloqueia o faturamento de produto')
EndIF
IF .NOT. lMV_ENT
	Conout('[VNDA190] - MV_190ENT esta FALSO, bloqueia o faturamento de entrega')
EndIF
IF .NOT. lMV_REC
	Conout('[VNDA190] - MV_190REC esta FALSO, bloqueia a geragco de recibo')
EndIF
...
If LockByName("VNDA190"+cPedido+Alltrim(cPedGarIt))
	...
	//Se a origem for Cursos, impede o faturamento ati que a equipe fiscal realize a analise do pedido e mude status do campo c5_XlibFAt para "S"    
	...
	If lFat .And. lMV_FAT
		...
		//3Faturamento de Hardware - NOTA FISCAL DE PRODUTO - SEFAZ                   3
		...
		If QRYHRD->(!Eof()) .and. lProd .And. lMV_PROD
			...
			While QRYHRD->(!EOF())
				...
				//Verifica se o pedido tem itens liberados e nco faturados
			...
			If SC5->C5_TPCARGA <> "1"  //Vendas normais
				...
				/*
				Parbmetro: MV_GARSHRD
				Descrigco: Define a Sirie da Nota Fiscal de Venda e Entrega de Hardwares
				...
				*/
				...
				If !aRet[1]
					...
					If lLog
						SZQ->(Reclock("SZQ"))
						SZQ->ZQ_STATUS := "5"
						SZQ->ZQ_OCORREN := "Inconsistjncia Faturamento"
						SZQ->ZQ_DATA := ddatabase
						SZQ->ZQ_HORA:=time()
						SZQ->(MsUnlock())
					Endif
				...
				If lLog .AND. !Empty(Alltrim(SC6->C6_SERIE+SC6->C6_NOTA))
					...
					SZQ->ZQ_OCORREN := "Faturado e NF nco transmitida."
					SZQ->ZQ_DATA := ddatabase
					SZQ->ZQ_HORA:=time()
					SZQ->(MsUnlock())
				Endif
				...
				If DbSeek(xFilial("SF2") + SC6->C6_NOTA + SC6->C6_SERIE)
					...
					If nRecSF2Hrd > 0
						...
						//ZDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD?
						//3Posiciono no novo titulo gerado pelo faturamento, e gavo nele o nosso numero e o numero do pedido gerado pelo site3
						//@DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDY
						DbSelectArea("SE1")
						...
						If aRetTrans[1]
							...
							While .T.
								...
								If aRetTrans[2] == "000169"
									...
									//IMPRIME A DANFE COM A INFORMAGCO DE NOTA DENEGADA
							...
							Else
								lRet := .f.
			    				cMensagem += "Inconsistjncia ao Gerar Danfe de Produto "
			    				If lLog
									SZQ->(Reclock("SZQ"))
									SZQ->ZQ_STATUS := "5"
			    ...
			    Else
			    	lRet := .f.
			    	cMensagem += "Inconsistjncia ao Gerar Nota de Produto do Pedido "+cPedido+". Necessario Realizar Processo manualmente. "
			...
			Else  //Delivery
				...
				If lLog
					SZQ->(Reclock("SZQ"))
					SZQ->ZQ_STATUS  := '5'
					SZQ->ZQ_OCORREN := "Nco faturado, depende de montagem de carga."
					SZQ->ZQ_DATA := ddatabase
					SZQ->ZQ_HORA:=time()
					SZQ->(MsUnlock())
				Endif
				...
				If DbSeek(xFilial('SA1') + SC5->(C5_CLIENTE+C5_LOJACLI))
					...
					cCorpo	+= "Esta i uma mensagem automatica, referente ` confirmagco da identificagco do pagamento da compra realizada no Portal de Vendas Certisign" + CRLF + CRLF
					cCorpo	+= "Os dados do Pedido sco: " + CRLF
					cCorpo	+= "Csdigo - " + SC5->C5_XNPSITE + CRLF
					cCorpo	+= "Data da Compra -  " + DtoC(SC5->C5_EMISSAO) + CRLF + CRLF + CRLF + CRLF
					cCorpo	+= "Com isso, seu Pedido foi encaminhado para area de logmstica, para entrega no enderego informado." + CRLF + CRLF
					cCorpo	+= "Agradecemos sua atengco," + CRLF
					cCorpo	+= "Equipe Certisign" + CRLF
					// Envio e-mail para o contato com a alteragco dos dados cadastrais
					U_VNDA290(cCorpo, SA1->A1_EMAIL, "Confirmagco Pagamento - Portal de Vendas Certisign")
	        	EndIf
	        	...
	        	If lGeraVDI .And. SC5->C5_TIPMOV == "1" .and. !SE1->(DbSeek(xFilial("SE1")+SC5->C5_CLIENTE+SC5->C5_LOJACLI+PadR(cPrefix,TamSX3("E1_PREFIXO")[1])+PadR("",TamSX3("E1_PARCELA")[1])+cTipPr))
					...
					cHistSE5		:= "Identificagco Pagamento Pedido "+cPedido
		...
		Elseif lProd .And. lMV_PROD //Forga transmissgao e geragco do PDF da DANFE

			cMsgH := "Nco foram encontrados itens de Venda de Hardware ou itens faturados anteriormente para o Pedido "+cPedido+". Tentativa e transmissao da nota."
			...
			ElseIf nRecSF2Hrd > 0
				...
				Endif
			Else
				U_GTPutOUT(cID,"N",cPedLog,{"VNDA190",{.F.,"E00012",cPedLog,"InconsistC*ncia no faturamento do pedido "+cPedido}},cXnpSite)
			EndIf
		EndIf
		
		//retorna para empresa e filial antes da aletagco
		If lFatFil
			FatFil(nil,cFilOld)
		Endif
		...
		//O espelho i gravado pela rotina GARR010.
		...
		//ZDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD?
		//3Faturamento de Software - NOTA FISCAL DE SERVICO - PREFEITURA DE SAO PAULO 3
		//@DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDY
		...
		If QRYSFW->(!Eof()) .and. lServ .And. lMV_SERV
			U_GTPutIN(cID,"S",cPedLog,.F.,{"U_VNDA190",cPedLog,"Faturamento dos Servigos do Pedido "+cPedido},cNpSite)
			...
			If !aRet[1]
				cMensagem := aRet[2]
				U_GTPutOUT(cID,"S",cPedLog,{"VNDA190",{.F.,"E00012",cPedLog,cMensagem},"Faturamento da Venda de Servigo do Pedido "+cPedido},cXnpSite)
			...
			If DbSeek(xFilial("SF2") + SC6->C6_NOTA + SC6->C6_SERIE)
				...
				If nRecSF2Sfw > 0
					...
					// Se chegou uma nota de serviC'o executa a transmissao para a PMSP
					...
					// A Totvs nao disponibilizou a transmigco da RPS para a prefeitura via WEBSERVICES.
					// Esta funcao sempre vai retornar TRUE ateh que a funcao da tranmissco seja criada
 ...
 //							SZQ->ZQ_NF2 := Alltrim(cNotaSfw)
							...
							SZQ->(MsUnlock())
						Endif
						U_GTPutOUT(cID,"S",cPedLog,{"NF_SERVICO_VENDA",aRetEspPMSP,"Faturamento da Venda de Servigo do Pedido "+cPedido},cNpSite)
					Else
						lRet := .f.
						cMensagem += "Inconsistjncia ao Gerar RPS de Servigo "
						If lLog
							SZQ->(Reclock("SZQ"))
							SZQ->ZQ_STATUS := "5"
			...
			Else
				lRet := .F.
				cMensagem += "Inconsistjncia ao Gerar Nota de Servigo do Pedido "+cPedido+". Necessario Realizar Processo manualmente. "
		...
		ElseIf lServ .And. lMV_SERV
			...
			If nRecSF2Sfw > 0 .and. Empty(SC5->C5_XNFSFW)
				...
				// Se chegou uma nota de servigo executa a transmissao para a PMSP
	...
	If (lRecPgto .or. lGerTitRecb) .And. lMV_REC
		...
		If !Empty(SC5->C5_CHVBPAG) .and. lRecPgto
			U_VNDA481({SC5->(Recno())},nil,"Liberagco de Pagamento Automatica ao Gerar Recibo de Pagamento")
		EndIf
	Endif
	...
	If lEnt .And. lMV_ENT
		...
		//ZDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD?
		//3Faturamento de Hardware - NOTA FISCAL DE PRODUTO - SEFAZ - ENTREGA         3
		//@DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDY
		cQueryHrd:=	" SELECT  SC6.R_E_C_N_O_ RECHRD "
		cQueryHrd+=	" FROM    " + RetSQLName("SC6") + " SC6, " + RetSQLName("SB1") + " SB1 "
		cQueryHrd+=	" WHERE   SC6.C6_FILIAL = '" + xFilial("SC6") + "' AND "
		...
		If QRYHRD->(!Eof())
		    ...
		    Else
		    	lRet := .f.
		    	cMensagem += "Inconsistjncia ao Gerar Nota de Entrega do Pedido "+cPedido+". Necessario Realizar Processo manualmente. "
		    	U_GTPutOUT(cID,"E",cPedLog,{"NF_PRODUTO_ENTREGA",{.F.,"E00011",cPedLog,cMensagem}},cNpSite)

		    EndIf
		Else     //Forga transmissao e geragco do PDF da DANFE

			cMsgH := "Nco Foram Encontrados Itens de Entrega de Hardware para o Pedido "+cPedido
	...
	UnLockByName("VNDA190"+cPedido+Alltrim(cPedGarIt))
Else
	U_GTPutOUT(cID,iif(lFat,IIF(lProd,"N","S"),"E"),cPedLog,{"U_VNDA190",{.F.,"E00002",cPedLog,"Nco foi possivel realizar LockByName"},"Faturamento do Pedido "+cPedido},cNpSite)
EndIf
...
If !Empty(SC5->C5_XNUMVOU)
	...
	if empty(oVoucher)  //conout("[VNDA190-VOUCHER] O pedido Erp "+sc5->c5_num+" nco foi faturado.")
	   U_GTPutOUT(cID,"V",cPedLog,{"VNDA190",{.F.,"E00012",cPedLog,"O pedido Erp "+sc5->c5_num+" nco foi faturado."},"Faturamento da Venda de Produtos do Pedido "+cPedLog},cPedSite)
       return
    else
...
Else
	...
	If SC5->C5_TPCARGA <> "1"
		cOperVen	:= cOperVenH
		If ! SC6->C6_XOPER $ cOperNPF //Verifica se C) nova operaC'C#o
			cOperVen :='52'
			cOperEntH:='53'
			cOperVenS:='51'
	  	Endif
...
If !empty(cErrorMsg)
	lRet := .f.
	cMensagem := "Inconsistjncia no Faturamento: "+CRLF+cErrorMsg
EndIf
...
Fungco personalizada para geragco de Recibo de Pagamento mediante pedido de vendas
...
cBcoCnab := cBanco
	...
	//Se a origem for Cursos, Muda status do campo para Pendente. Assim a equipe fiscal pode analisar situagco do pedido quanto a tributagco
	IF SC5->C5_XORIGPV=='8' .AND. !SC5->C5_XLIBFAT =='S'//Pedidos de Cursos ainda nco liberados pelo Fiscal
		RecLock("SC5", .F.)
		SC5->C5_XLIBFAT:= "P" //Muda status para pendente de analise fiscal
		SC5->(MsUnlock())
    End
	...
	/*
	// 06/03/17 (David) - retirada a atualizagco do pedido na tabela SZQ, pois nco i necessario
	alterar o status para 7-em processamento caso seja impressco de recibo. Alinhado com ZILA
	...
	*/
	...
	// Verifica se existe tmtulo gerado para este pedido.
	...
	If Len(aSe1TipMov)=0
		...
		If lGerTitRecb
		   ...
		   /**Verifica necessidade de debloquear vendedor para realizar a inclusao do titulo**/
		   lVenBloq := .f.
		   aAreaSA1 := sa1->(GetArea())
		   sa1->(DbSetOrder(1))
		   if sa1->(DbSeek(xFilial()+sc5->c5_cliente+sc5->c5_lojacli)) .and. !empty(sa1->a1_vend)
		      lVenBloq := BqlDblqVen("1",sa1->a1_vend)
		   endif
		   /*************/
		   ...
		   /**Caso o vendedor tenha sido desbloqueado, bloquear novamente pois o titulo ja foi incluido**/
		   if lVenBloq
		      lVenBloq := BqlDblqVen("2",sa1->a1_vend)
		   endif
		   RestArea(aAreaSA1)
		   /**********/
		...
		If lRet  .And. lRecPgto
			...
			If aRet[1]
				...
				If !Empty(cRecPgto)
					...
					U_GTPutOUT(cID,"R",cPedLog,{"RECIBO_PAGAMENTO",{.T.,"M00001",cPedLog,"Geragco do Recibo com sucesso para tipo "+cTipMov+ " Pedido "+cPedido+" Pagamento "+Alltrim(SC5->C5_TIPMOV)}},cNpSite)
				Else
					If lLog
						SZQ->(Reclock("SZQ"))
						SZQ->ZQ_STATUS := "5"
						...
						SZQ->(MsUnlock())
					Endif
					U_GTPutOUT(cID,"R",cPedLog,{"RECIBO_PAGAMENTO",{.F.,"E00001",cPedLog,"Inconsistjncia na geragco do recibo para tipo " +cTipMov+ " Pedido "+cPedido+" Pagamento "+Alltrim(SC5->C5_TIPMOV)}},cNpSite)
				EndIf
			Else
				U_GTPutOUT(cID,"R",cPedLog,{"RECIBO_PAGAMENTO",{.F.,"E00001",cPedLog,"Inconsistjncia na geragco do recibo para tipo "+cTipMov+ " Pedido "+cPedido+" Pagamento "+Alltrim(SC5->C5_TIPMOV)}},cNpSite)
			EndIf
		Else
			U_GTPutOUT(cID,"R",cPedLog,{"RECIBO_PAGAMENTO",{.F.,"E00001",cPedLog,"Inconsistjncia na geragco do tmtulo referente recibo para tipo "+cTipMov+ " Pedido "+cPedido+" Pagamento "+Alltrim(SC5->C5_TIPMOV)}},cNpSite)
		EndIf
	Else
	
		U_GTPutOUT(cID,"R",cPedLog,{"RECIBO_PAGAMENTO",{.T.,"M00001",cPedLog,"Ja existe Recibo "+cTipMov+" para o Pedido "+cPedido+" Pagamento "+Alltrim(SC5->C5_TIPMOV)}},cNpSite)
    
    EndiF    
	
	//Sempre verifica se exitem pendencias de compensagco ou substuigco
	
	// Localiza as NFs do pedido para Substituigco dos PR ou Compensagco das NCCs
	cSql := "SELECT R_E_C_N_O_ RECE1 "
	cSql += " FROM "+RetSqlName("SE1")
	cSql += " WHERE E1_FILIAL = '"+xFilial("SE1")+"' AND "
	cSql += " E1_TIPO ='NF' AND"
	...
	// Localiza as NCCs do pedido para Substituigco dos PR
	cSql := "SELECT R_E_C_N_O_ RECE1 , E1_PARCELA PARCELA, E1_EMISSAO EMISSAO, E1_SALDO "
	cSql += " FROM "+RetSqlName("SE1")
	cSql += " WHERE E1_FILIAL = '"+xFilial("SE1")+"' AND "
	...
	While !(cTrbSql)->(Eof())
		AAdd( aSe1NCC, { (cTrbSql)->RECE1 , (cTrbSql)->PARCELA,(cTrbSql)->EMISSAO,(cTrbSql)->E1_SALDO } )
		(cTrbSql)->(DbSkip())
	End
	...
	If Len(aSe1NCC)>0 
		For nx:= 1 to Len(aSe1NCC)	
			nContr := 0
			// Localiza os PR para substituigco
			cSql := "SELECT R_E_C_N_O_ RECE1 , E1_PARCELA PARCELA, E1_SALDO "
			cSql += " FROM "+RetSqlName("SE1")
			cSql += " WHERE E1_FILIAL = '"+xFilial("SE1")+"' AND "
			cSql += " E1_TIPO ='PR' AND"
			cSql += " E1_PREFIXO ='RCP' AND"
			if !empty(aSe1NCC[NX][2])
...
Static Function FatFil(cPedido,cFilFat)
	...
	//////////////////////////////////////////
	If Empty(cEstFilFat)
		conout("[FATFIL] Nco ha definigco para tratamento de faturamento por filiais.")
		Return(.F.)
	Endif
	//////////////////////////////////////////
	...
	If !Empty(cPedido)
		If 	!Empty(cEstFilFat) .and.;
			...
			Else
				conout("[FATFIL] Filial do cliente nco encontrada "+cEstFilFat+" "+cPedido+" "+Alltrim(SA1->A1_EST))
			EndIf
		EndIf
	EndIf
	...
	If !Empty(cFilFat) .and. SM0->( dbSeek( cEmpAnt + cFilFat, .T. ) )
		...
		lRet 	:= .T.
	Else
		conout("[FATFIL] Filfat em branco ou sm0 nco encontrado "+cEmpAnt + cFilFat+" "+cFilAnt)
	EndIf
Return(lRet)
...
//-----------------------------------------------------------------------
// Descr. | Cria os parbmetro de controle do faturamento
//-----------------------------------------------------------------------
...
User Function VN190_MV()
	...
	If .NOT. SX6->( ExisteSX6( 'MV_190PROD' ) )
		...
		cMSG += 'MV_190PROD - contezdo: "31/10/18"' + CRLF
	Endif
	...
	If .NOT. SX6->( ExisteSX6( 'MV_190ENT' ) )
		...
		cMSG += 'MV_190ENT  - contezdo: "31/10/18"' + CRLF
	Endif
	...
	If .NOT. SX6->( ExisteSX6( 'MV_190REC' ) )
		...
		cMSG += 'MV_190REC  - contezdo: " "' + CRLF
	Endif
	MsgInfo('Parbmetros de faturamento criados com sucesso conforme: ' + CRLF + cMSG,'VN190_MV')
Return
...
static function getVoucher(  cPedSite, cPedLog  )
	...
	if !empty( cPedSite ) 
		...
		if !empty( cID )
			...
			if empty(oPedido)  //conout("[VNDA190-VOUCHER] Arquivo xml ref pedido Site "+alltrim(cPedSite)+") nco encontrado.")
...
static function montarXML( cID, cPedLog, cPedSite, cError, cWarning, nPed )
	...
	else  //conout("[VNDA190-VOUCHER] Arquivo xml nco foi encontrado. Error: "+cError)
...
/* 
Data criagco 11/06/2021
fungco para bloquear e desbloquear vendedor
