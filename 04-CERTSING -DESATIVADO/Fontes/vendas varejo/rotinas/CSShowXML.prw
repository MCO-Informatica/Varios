...
//+-------------------------------------------------------------------+
//| Rotina | CSShowXM | Autor | Rafael Beghini | Data | 31/07/2019    | 
//+-------------------------------------------------------------------+
//| Descr. | Rotina para apresentar o XML com os dados de venda       |
//+-------------------------------------------------------------------+
//| Uso    | CertiSign Certificadora Digital                          |
//+-------------------------------------------------------------------+
User Function CSShowXM()
    ...
    Private cTitulo := '[CSShowXM] - XML de Vendas'
    Private cIDxml  := ''
    ...
    IF nOpc == 1
        ...
        IF ParamBox( aPAR, cTitulo, @aRET )
        	bAction := { || cIDxml := A010Find( aRET[2] ), iif( Empty( cIDxml ), MsgAlert("Pedido não localizado, por favor verifique", "Atenção"), A010Show(cIDxml) ) } 
            FwMsgRun( nil, bAction , cTitulo, 'Aguarde, localizando XML para apresentar os dados...')
        Else
    ...
    IF .NOT. Empty( cXML )
        DEFINE MSDIALOG oDlg TITLE '' OF oMainWnd FROM aC[1],aC[2] TO aC[3], aC[4] PIXEL STYLE nOr(WS_VISIBLE,WS_POPUP)
            ...
            @ 9,258 	BUTTON oBtnL ;
					...
					SIZE 50,14 ;
					DIALOG oTPanelPar ;
					PIXEL ACTION  FwMsgRun(, {|| A010Visual( cPEDIDO ) }, cTitulo, 'Aguarde..' )
            ...
            @ 9,378 	BUTTON oBtnL ;
					...
					SIZE 50,14 ;
					DIALOG oTPanelPar ;
					PIXEL ACTION  A010Downlo( cRootPath, cNameFile )
...
//+-------------------------------------------------------------------+
//| Rotina | A010Visual | Autor | Rafael Beghini | Data | 31/07/2019  | 
//+-------------------------------------------------------------------+
//| Descr. | Rotina para visualizar o Pedido                          |
//+-------------------------------------------------------------------+
Static Function A010Visual(cC5_XNPSITE)
...
//+-------------------------------------------------------------------+
//| Rotina | A010Downlo | Autor | Rafael Beghini | Data | 31/07/2019  | 
//+-------------------------------------------------------------------+
//| Descr. | Realiza o download do XML                                |
//+-------------------------------------------------------------------+
Static Function A010Downlo( cDir, cNameFile )
