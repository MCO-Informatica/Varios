...
User Function MntGtLeg()
	...
	Local cTitulo := "Acerto da tabela Polmtica de Garantia (GTLEGADO) "
	...
	aAdd(aPAR,{9,"Informe parbmetros para corregco da Polmtica de Garantia",200,7,.T.})
	aAdd(aPAR,{1,"Emissco de :",dInicio,"","","",".T.",0,.T.})
	aAdd(aPAR,{1,"Emissco ati:",dFim,"","" ,"",".T.",0,.T.})
	...
	While lCont
		...
		if ParamBox(aPAR,"Ajuste GTLegado...",@aRET)
			...
			if aRET[3] - aRET[2] > 15
			    MsgInfo("Este programa somente executara um limite de 15 dias por permodo definido !")
				Loop
			elseif aRET[3] - aRET[2] < 0
			    MsgInfo("Verifique os parbmetros de data !")
				Loop
			endif
			...
			IF .NOT. (cTRB)->( EOF() )
				...
				//@ 37,080 SAY oEmis VAR "EMISSCO:"+dtoc(dEmis) SIZE 120,11 COLOR CLR_BLACK PIXEL OF oDlg PICTURE "@D"
				...
				//---------------------------------------------------------
				//chamar a fungco que cria a estrutura do aHeader
				//---------------------------------------------------------
				...
				Aadd(aCabec, {"Emissco",	"C5_EMISSAO",	"@D",		08,         0,	        "",			"",	        "D",        "",     "R",        "",     "",         ""})
				Aadd(aCabec, {"Item",		"C6_ITEM",		"@!",		02,         0,	        "",			"",	        "C",        "",     "R",        "",     "",         ""})
				Aadd(aCabec, {"Operagco",	"C6_XOPER",		"@!",		02,         0,	        "",			"",	        "C",        "",     "R",        "",     "",         ""})
				...
				aCabecalho := aCabec
				//---------------------------------------------------------	
				//Monta o browser com inclusco, remogco e atualizagco
				//---------------------------------------------------------
				...
				//---------------------------------------------------------
				//Carregar os itens que irco compor o conteudo do grid
				//---------------------------------------------------------
				While .NOT. (cTRB)->( EOF() )
				...
				//---------------------------------------------------------
				//Crio o menu que ira aparece no botco Agues relacionadas
				//---------------------------------------------------------
				aadd(aBotoes,{"NG_ICO_LEGENDA", {||Legenda()},"Legenda","Legenda"})
				EnchoiceBar(oDlg, {|| Iif( csTelaProcess(dInicio,dFim), oDlg:End(), .F.) }, {|| oDlg:End() },,aBotoes)
					
				ACTIVATE MSDIALOG oDlg CENTERED
			else

				MsgInfo("Sem Politica de Garantia neste permodo !")
		...
		Else
			MsgInfo('Processo cancelado pelo usuario','[AtuGtLeg] - Ajuste GTLegado')
			lCont := .f.
		EndIF
	end
Return
...
@since 23/03/2021
/*/
Static function csTelaProcess(dInicio,dFim)
...
Local nI   := 0
	...
	While .NOT. (cTRB)->( EOF() )
		If (cTRB)->C6_XOPER $ '51,61' .and. (cTRB)->recgt > 0 		//GT_DTESTBAIXA
			aret := atuLegProd( (cTRB)->recgt, (cTRB)->C6_PEDGAR, (cTRB)->C6_PRODUTO, (cTRB)->C5_XNPSITE, (cTRB)->C5_NUM, (cTRB)->C5_EMISSAO, (cTRB)->C6_VALOR, (cTRB)->C6_NOTA, (cTRB)->totlin, (cTRB)->C6_DATFAT )
		Endif
		if len(aret) > 0
	...
	(cTRB)->( dbGotop() )
	While .NOT. (cTRB)->( EOF() )
		/* Trata Software apenas quando há faturamento */
		...
		Else
			/* O Hardware será tratado APENAS se não houver faturamento */
			If (cTRB)->C6_XOPER $ '52,62'
...
/*/ 
static function atuLegProd( cRecLeg, cPedGar, cProduto, cPedSite, cPedVenda, cEmissao, nVlrServS, cDocFis, ntotlin, cDatFat )
	local aRet :={}
	local oLog := nil
    ...
    if dbSeek( cPedGar + cValToChar( cRecLeg ) ) 
		while !GTLEGADO->( EOF() ) .and. GTLEGADO->GT_PEDGAR  == cPedGar 
		
		    if GTLEGADO->GT_PEDVENDA == cPedVenda

			if ntotlin == 1 .and. GTLEGADO->GT_PRODUTO == cProduto
...
/*******************Impressco dos acertos gerados no programa ************************/
...
oReport := TReport():New('GTLEGADO',"Acertos GTLEGADO no permodo de "+DtoC(dInicio)+" ati "+DtoC(dFim),/*cPerg*/,{|oReport|xImprArray( oReport, aCols )}, "Acertos GTLEGADO" )

oSection := TRSection():New(oReport,'GTLEGADO')

TRCell():New(oSection, "cDescri"  , "" ,"Descrigco","!@"                        ,30                     ,/*lPixel*/,{|| cDescri	})
TRCell():New(oSection, "cPedido"  , "" ,"Pedido"   ,PesqPict("SC5","C5_NUM")    ,TamSx3("C5_NUM")[1]    ,/*lPixel*/,{|| cPedido	})
TRCell():New(oSection, "cEmissao" , "" ,"Emissco"  ,PesqPict("SC5","C5_EMISSAO"),TamSx3("C5_EMISSAO")[1],/*lPixel*/,{|| cEmissao})
