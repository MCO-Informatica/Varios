...
/*
---------------------------------------------------------------------------
| Rotina    | UPDSDK02    | Autor | Gustavo Prudente | Data | 01.11.2013  |
|-------------------------------------------------------------------------|
...
---------------------------------------------------------------------------
*/
User Function UPDSDK02( cEmpAmb, cFilAmb )
...
// Grupos de atendimento na tabela SUW
aAdd( aSX3, { ;
	"SUW"																	, ; //X3_ARQUIVO
	"11"																	, ; //X3_ORDEM
	"UW_XGRPATE"															, ; //X3_CAMPO
	...
	100																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	"Grp Aut Resp"															, ; //X3_TITULO
	"Grp Aut Resp"															, ; //X3_TITSPA
	"Grp Aut Resp"															, ; //X3_TITENG
	"Grp Aut Resp"															, ; //X3_DESCRIC
	"Grp Aut Resp"															, ; //X3_DESCSPA
	"Grp Aut Resp"															, ; //X3_DESCENG
	"@!"																	, ; //X3_PICTURE
	""																		, ; //X3_VALID
...
// Grupos de atendimento que permitem transferencia no cadastro de grupos
aAdd( aSX3, { ;
	"SU0"																	, ; //X3_ARQUIVO
	"61"																	, ; //X3_ORDEM
	"U0_XGRPATE"															, ; //X3_CAMPO
	...
	100																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	"Grp Transf."															, ; //X3_TITULO
	"Grp Transf."															, ; //X3_TITSPA
	"Grp Transf."															, ; //X3_TITENG
	"Grp Transf."															, ; //X3_DESCRIC
	"Grp Transf."															, ; //X3_DESCSPA
	"Grp Transf."															, ; //X3_DESCENG
	"@!"																	, ; //X3_PICTURE
	""																		, ; //X3_VALID
	DEF_X3USADO																, ; //X3_USADO
	""																		, ; //X3_RELACAO
...
// Descricao do Script dinamico ADE
aAdd( aSX3, { ;
	"ADE"																	, ; //X3_ARQUIVO
	"D8"																	, ; //X3_ORDEM
	"ADE_TITSCR"															, ; //X3_CAMPO
	...
	""																		, ; //X3_VALID
	DEF_X3USADO																, ; //X3_USADO
	'IIF(!INCLUI,Posicione("SUZ",1,xFilial("SUZ")+ADE->ADE_CODSCRI,"UZ_DESC"),"")'	, ; //X3_RELACAO
	""																		, ; //X3_F3
	1																		, ; //X3_NIVEL
	Chr(254) + Chr(192)														, ; //X3_RESERV
	""																		, ; //X3_CHECK
...
// Codigo do grupo na tabela de perguntas respondidas do script atendimento
aAdd( aSX3, { ;
	"ACI"																	, ; //X3_ARQUIVO
	"12"																	, ; //X3_ORDEM
	"ACI_XCODGP"															, ; //X3_CAMPO
	...
	2																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	"Grupo Atend"															, ; //X3_TITULO
	"Grupo Atend"															, ; //X3_TITSPA
	"Grupo Atend"															, ; //X3_TITENG
	"Grupo Atend"															, ; //X3_DESCRIC
	"Grupo Atend"															, ; //X3_DESCSPA
	"Grupo Atend"															, ; //X3_DESCENG
	""																		, ; //X3_PICTURE
	""																		, ; //X3_VALID
	DEF_X3USADO																, ; //X3_USADO
	"" 																		, ; //X3_RELACAO
...
// Indicar se a ocorrencia pode ser selecionada na opcao "Editar" da pesquisa de historico
aAdd( aSX3, { ;
	"SU9"																	, ; //X3_ARQUIVO
	"17"																	, ; //X3_ORDEM
	"U9_XEDTPSQ"															, ; //X3_CAMPO
	...
	"Pesq. Hist."															, ; //X3_TITSPA
	"Pesq. Hist."															, ; //X3_TITENG
	"Ocorrencia para pesquisa de historico"									, ; //X3_DESCRIC
	"Ocorrencia para pesquisa de historico"									, ; //X3_DESCSPA
	"Ocorrencia para pesquisa de historico"									, ; //X3_DESCENG
	""																		, ; //X3_PICTURE
...
aAdd( aSX3, { ;
	...
	"Exec. Camp.?"															, ; //X3_TITSPA
	"Exec. Camp.?"															, ; //X3_TITENG
	"Exec. Camp.?"															, ; //X3_DESCRIC
	"Exec. Camp.?"															, ; //X3_DESCSPA
	"Exec. Camp.?"															, ; //X3_DESCENG
	"@!"																		, ; //X3_PICTURE
...
AAdd( aSIX, { ;
	'SUK'						, ; //INDICE
	'2'							, ; //ORDEM
	'UK_FILIAL+UK_CODIGO+UK_CODPERG+UK_CODRESP' 		, ; //CHAVE
	'Codigo+Pergunta+Resposta'   					, ; //DESCRICAO
	'Codigo+Pergunta+Resposta'   					, ; //DESCSPA
	'Codigo+Pergunta+Resposta'    				, ; //DESCENG
	'U'							, ; //PROPRI
	''							, ; //F3
	'SUK_2'				, ; //NICKNAME
	'S'							} ) //SHOWPESQ
...
AAdd( aSIX, { ;
	'SUK'						, ; //INDICE
	'3'							, ; //ORDEM
	'UK_FILIAL+UK_CODATEN+UK_CODPERG+UK_CODRESP' 		, ; //CHAVE
	'Atendimento+Pergunta+Resposta'   					, ; //DESCRICAO
	'Atendimento+Pergunta+Resposta'   					, ; //DESCSPA
	'Atendimento+Pergunta+Resposta'    				, ; //DESCENG
	'U'							, ; //PROPRI
	''							, ; //F3
	'SUK_3'				, ; //NICKNAME
	'S'							} ) //SHOWPESQ
...
AAdd( aSIX, { ;
	'ACI'						, ; //INDICE
	'5'							, ; //ORDEM
	'ACI_FILIAL+ACI_ROTINA+ACI_ATEND+ACI_CODCAM+ACI_CODSCR' 		, ; //CHAVE
	'Rotina + Atendimento + Campanha + Script'   					, ; //DESCRICAO
	'Rotina + Atendimento + Campanha + Script'   					, ; //DESCSPA
	'Rotina + Atendimento + Campanha + Script'    				, ; //DESCENG
	'U'							, ; //PROPRI
	''							, ; //F3
	'ACI_5'				, ; //NICKNAME
	'S'							} ) //SHOWPESQ
...
AAdd( aSXB, {"XGRPAT","1","01","RE","Grp Atendimento"        ,"Grp Atendimento"        ,"Grp Atendimento"        ,"SU0"           ,""})
AAdd( aSXB, {"XGRPAT","2","01","01",""             ,""             ,"",".T.",""})
AAdd( aSXB, {"XGRPAT","5","01","","","","","U_F3GRPATE()",""})
