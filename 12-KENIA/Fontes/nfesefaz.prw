	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				aadd(aDest,alltrim(SA1->A1_EMAIL)+';fernando@kenia.com.br;alexsandro@kenia.com.br')
			...
			Else
				...
				Else
					...
					aadd(aDest,alltrim(SA2->A2_EMAIL)+';fernando@kenia.com.br;alexsandro@kenia.com.br')					
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					//----> CUSTOMIZACAO: DESPREZAR NOTAS DE RETORNO DE BENEFICIAMENTO
					If Alltrim((cAliasSD2)->D2_CF) $ "5902/6902/5925/6925" .And. !(cAliasSD2)->D2_TES $ "600" 
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						cTpOrig  := IIF(nCountIT > 0 .And. Len(aNfVinc[nCountIT]) > 9, aNfVinc[nCountIT][10], "") //Pegar tipo da nota de origem
		            	 ...
		            	 //----> CUSTOMIZACAO: ALTERA CARACTER NO CODIGO DO PRODUTO DE ACORDO COM CONTEUDO DO CAMPO C5_PAPELET
			            If SC5->C5_PAPELET$"O"
				...
				//----> CUSTOMIZACAO: Dados adicionais da nota.                                                                                          
	   			If !" - DOCUMENTO EMITIDO POR ME OU EPP OPTANTE PELO  SIMPLES NACIONAL - PERMITE O APROVEITAMENTO DO CREDITO DE ICMS NO VALOR DE R$" $ Alltrim(cMensCli)
					cMensCli += Iif(cSerie$"4  "," - DOCUMENTO EMITIDO POR ME OU EPP OPTANTE PELO  SIMPLES NACIONAL - PERMITE O APROVEITAMENTO DO CREDITO DE ICMS NO VALOR DE R$ "+Transform((SF2->F2_VALFAT*(GETMV("MV_X_ICMS")/100)),"@E 999,999,999.99")+" CORRESPONDENTE A ALIQUOTA DE "+Transform(GETMV("MV_X_ICMS"),"@E 999.99")+"%","")
				EndIf
	            	
	            If !" - SO ACEITAREMOS DEVOLUCAO DAS PECAS INTACTAS E COM A ETIQUETA DE CODIGO DE BARRAS" $ Alltrim(cMensCli)
					cMensCli += " - SO ACEITAREMOS DEVOLUCAO DAS PECAS INTACTAS E COM A ETIQUETA DE CODIGO DE BARRAS"
				EndIf
	            //----> FIM CUSTOMIZACAO
			...
			If FieldPos("F1_TRANSP") > 0 .And. !Empty(SF1->F1_TRANSP)
				...
				aadd(aTransp,SA4->A4_EMAIL	)
					If !Empty(SF1->F1_PLACA)
						...
						aadd(aVeiculo,"")//RNTC
					EndIf		
			//----> CUSTOMIZACAO: DADOS TRANSPORTADORA
			ElseIf FieldPos("F1_X_TRANS") > 0 .And. !Empty(SF1->F1_X_TRANS)
				dbSelectArea("SA4")
				dbSetOrder(1)
				MsSeek(xFilial("SA4")+SF1->F1_X_TRANS)
				...
				aadd(aTransp,SA4->A4_EMAIL	)
					If !Empty(SF1->F1_PLACA)
						...
						aadd(aVeiculo,"")//RNTC
					EndIf		
			//----> FIM CUSTOMIZACAO: DADOS TRANSPORTADORA
			EndIf
