...
If _lGeraMSG
	If !Empty(_cMP1)
		Msgalert("O(s) componente(s) " + _cMP1 +" está(ão) bloqueado(s) para uso e não pode(m) atender a OP " + M->C2_NUM)
		_lRet 		:= 	.f.
		_cMP1 		:= 	""
		_cMP2 		:= 	""
		_lGeraMSG	:=	.f.
	ElseIf !Empty(_cMP2)
		Msgalert("Não existe saldo suficiente do(s) componente(s) " + _cMP2 +" para atender a OP " + M->C2_NUM)
		_lRet 		:= 	.f.
		_cMP1 		:= 	""
		_cMP2 		:= 	""
		_lGeraMSG	:=	.f.
