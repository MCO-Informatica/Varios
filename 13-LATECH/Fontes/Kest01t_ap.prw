...
User Function Kest01t()        // incluido pelo assistente de conversao do AP6 IDE em 12/02/05
...
Do While  !Eof()
	...
	If TRB->TIPO$"KC TC"
		...
		TRB->TINGINDO := 0
		TRB->REPROCES := 0
		TRB->SALDO    := TRB->ESTOQUE+TRB->CRU+TRB->TINGINDO-TRB->REPROCES-TRB->PEDIDOS
	Else
...
cQuery := ''
cQuery := "SELECT C5_NUM, C5_TIPO, C6_NUM, C6_ITEM, C6_DESCRI, C6_PRODUTO, C6_QTDVEN, C6_QTDENT, C6_BLQ   "
cQuery := cQuery + "FROM "+RetSQLName("SC5")+" T1, "+RetSQLName("SC6")+" T2 "
cQuery := cQuery + "WHERE T1.C5_FILIAL = T2.C6_FILIAL AND "
cQuery := cQuery + "T1.C5_NUM = T2.C6_NUM AND T1.C5_TIPO = 'N' AND "
cQuery := cQuery + "T2.C6_PRODUTO >= '"+ _pi+"' AND T2.C6_PRODUTO <= '"+_pf+"' AND "
