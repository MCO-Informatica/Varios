...
User Function Sf2460i()        // incluido pelo assistente de conversao do AP6 IDE em 12/02/05
...
SetPrvt("_CALIAS,_CREGIST,_CORDEM,_CEMP,_CASC5,_CRSC5")
SetPrvt("_COSC5,_CPAPEL,_CDESCP,_CASB2,_CRSB2,_COSB2")
SetPrvt("_CREGSE1,_CORSE1,_CREGSE3,_CORDSE3,_CPREFIXO,_CTITULO")
SetPrvt("_CCHAVE,_CCHAVE1,_PATH,_CRSD2,_COSD2,_CRSE1")
SetPrvt("_COSE1,")
...
If DbSeek(xFilial("SC5")+SD2->D2_PEDIDO)
   _cPAPEL      := SC5->C5_PAPELET
   _cDESCP      := SC5->C5_DESCPON
Else
   _cPAPEL      := "N"
   _cDESCP      := "N"
EndIf
...
//IF _cDESCP == "S"      // desconto pontualidade 2 %
//	 GravaPont()
//EndIf


IF _cEmp == "01" .and. _cPAPEL == "O"
    ...
    */
    ...
    DbSetOrder(1)
    DbSeek( xFilial("SE1") + _cPREFIXO + _cTITULO )
...
//		   NSE1->E1_DESCFIN := SE1->E1_DESCFIN
    ...
    EndDo
     ...
     DbSetOrder(1)
     DbSeek( xFilial("SE3") + _cPREFIXO + _cTITULO )
...
// Grava Desconto de Pontualidade
//FUNCTION GravaPont

//	   DbSelectArea("SE1")
//	   _cRSE1  := RECNO()	 // salva area SE1
//	   _cOSE1  := IndexOrd()
//
//	   DbSeek( xFilial("SE1") + SF2->F2_SERIE + SF2->F2_DOC )
//		 While SE1->E1_FILIAL == xFilial("SE1") .and. ;
//		   SE1->E1_NUM == SF2->F2_DOC .and. !Eof()
//			   Reclock("SE1", .f.)
//				  SE1->E1_DESCFIN := 2
//			   MsUnlock()
//		   DbSkip()
