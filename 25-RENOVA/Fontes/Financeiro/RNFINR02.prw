...
±±º Programa    ³ RNFINR02  ³ Impressão individual do titulo a receber                    º±±
±±º             ³          ³                                                              º±±
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Query para seleção dos titulos a pagar
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
DbSelectArea( "SA6" )
...
If 	oPrint:IsPrinterActive()
	...
	While (cAliasQry)->(!EOF())
		...
		RptStatus( { |lEnd| RunRepo2(@lEnd) }, "Aguarde...", "Imprimindo Título..."+'/'+(cAliasQry)->(E1_PREFIXO)+'-'+(cAliasQry)->(E1_NUM), .T. )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±º Programa    ³ RunRepo2³ Efetua a impressão do titulo a receber                       º±±
±±º             ³          ³                                                              º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
If MV_PAR15 = 1
	...
	oPrint:Say(nLin,nCol+0050, "Recebemos a importância de " + Transform((cAliasQry)->(E1_VLCRUZ), "@E 999,999,999.99") , oFont10 )
	nLin := IncLinha2(nLin,50)
	...
	oPrint:Say(nLin,nCol+0200, "São Paulo, " + aDiaSemana[ dow(STOD((cAliasQry)->(E1_EMISSAO))) ] + ", " + Substr( (cAliasQry)->(E1_EMISSAO),7,2 ) + " de " + aMes[ Val(Substr( (cAliasQry)->(E1_EMISSAO),5,2 ))] + " de " + Substr( (cAliasQry)->(E1_EMISSAO),1,4 ) , oFont10 )    
