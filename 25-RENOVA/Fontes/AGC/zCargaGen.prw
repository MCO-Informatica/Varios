...
@version 1.0
	...
	15 - MATA120 - SC7 - Pedido de compras
...
User Function zCargaGen()
	...
	cAviso += " 15 - MATA120 - Sc7 - Pedido de compras"+STR_PULA 
	//cAviso += " 16 - FINA171 - SEH - Aplicaçao Emprestimo"+STR_PULA
	cAviso += "--"+STR_PULA
...
Static Function fConfirm(nTipo)
	...
	//Pedido de compras
		ElseIf cCmbTip == "15"
		cRotina	:= "MSExecAuto({|x, y| MATA120(x, y)}, aDados, 3) "
		cTabela	:= "SC7"
		cCampoChv	:= "C7_NUM"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 2
		...
		ElseIf cCmbTip == "16"
		cRotina	:= "MSExecAuto({|x, y| FINA171(x, y)}, aDados, 3) "
		cTabela	:= "SEH"
		cCampoChv	:= "SEH_NUMERO"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo		:= 6
		SetFunName("FINA171")
		*/
