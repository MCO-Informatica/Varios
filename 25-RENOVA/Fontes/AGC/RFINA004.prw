...
// A partir da útima versão do RFINA003 - Com De-Para de Fornecedor e Controle de Representante x Representado
// ==> FORMA DE PADRONIZAR O FONTE PARA NÃO MEXER MAIS A NÃO SER QUE MUDEM AS REGRAS
// ==> VARIÁVEIS LOCAIS NAO UTILIZADAS COMENTADAS - EVITAR WARNINGS
...
Private lFaseB := cUnidadeNeg <> "03" .and. cFilAnt <> "1330001"    
...
cQuery += "AND E2_NUM LIKE '______004' "
...
Processa( {|| RFA4_PROC(nContador) }, "Aguarde...", "Processando liquidações...",.F.)
...
Static Function RFA4_PROC(nTotReg)
...
//Local nEnrola      := 0
Local nProcessado  := 0
Local cChaveForn   := ""         // Variável criada para o Controle de Representante x Representado
...
while !EOF()
    ...
    if !Empty(AllTrim(cFornChoice)) .and. cFornChoice <> TRB004->A2_XFORDES
        dbSelectArea("TRB004")
        DBSkip()
        Loop
    endif
    ...
    if !Empty(AllTrim(TRB004->E2_XFORREP))    
       cChaveForn := TRB004->E2_XFORREP + TRB004->E2_XLOJREP
    else
       cChaveForn := TRB004->A2_XFORDES + TRB004->A2_XLOJDES
    endif
    
    DBSelectArea("SA2")    // A verificação da validade do fornecedor pode ser mantida para evitar eventuais erros no cadastro 
    DbSetOrder(1)          // Campos A2_XFORDES/A2_XLOJDES
...
// Adicionalmente a esse Filtro incluirá o retorno do PE FA565FIL
    aGetAut1 := {}
//  aAdd(aGetAut1, {'cFornDe'   , TRB004->E2_FORNECE}) 
    aAdd(aGetAut1, {'cFornDe'   , '      '}) 
    aAdd(aGetAut1, {'cLojaDe'   , '  '    }) 
//    aAdd(aGetAut1, {'cFornAte'  , TRB004->E2_FORNECE})     
    aAdd(aGetAut1, {'cFornAte'  , 'ZZZZZZ'})    
    aAdd(aGetAut1, {'cLojaAte'  , 'ZZ'              })     
//  aAdd(aGetAut1, {'cForn565'  , TRB004->E2_FORNECE}) 
//  aAdd(aGetAut1, {'cLoja'     , cLojaForn   })
    aAdd(aGetAut1, {'cForn565'  , TRB004->A2_XFORDES}) 
    aAdd(aGetAut1, {'cLoja'     , TRB004->A2_XLOJDES}) 
    ...
    cNatAuto := TRB004->E2_NATUREZ

// Cabeçalho da Tela dos títulos de destino
    aGetAut2 := {}
//  aAdd(aGetAut2, {'cFornece' , TRB004->E2_FORNECE})       // Mesmo Fornecedor destino do Filtro
    aAdd(aGetAut2, {'cFornece' , TRB004->A2_XFORDES})
//  aAdd(aGetAut2, {'cLoja'    , cLojaForn })               // Mesma Loja destino do Filtro
    aAdd(aGetAut2, {'cLoja'    , TRB004->A2_XLOJDES})
...
// Número da Liquidação atual na Filial posicionada
    ...
    cNumTitPagar := cLiquid+"005"
    cNumTitSaldo := cLiquid+"006"
...
//                   {'nValLiq' , 6005} } )
    ...
    if nProcessado = nTotReg - 1
       ApMsgInfo("Irá processar o penúltimo fornecedor: "+TRB004->A2_XFORDES, "Aviso")
    else
       if nProcessado = nTotReg
          ApMsgInfo("Irá processar o último fornecedor: "+TRB004->A2_XFORDES, "Aviso")
       endif
    endif
    ...
    If lMsErroAuto    
        MostraErro()
        cMsg := "Executado com erro - Fornecedor: "+TRB004->A2_XFORDES+chr(13)+chr(10)
        if !Empty(AllTrim(TRB003->E2_XFORREP))
           cMsg += "Representante do Fornecedor "+TRB004->E2_XFORREP+chr(13)+chr(10)+chr(13)+chr(10)
        else 
           cMsg += chr(13)+chr(10)
        endif        
        ...
        MsUnlock()
        if lMostra 
           ApMsgInfo("Processado Fornecedor "+TRB004->A2_XFORDES+"/"+TRB004->A2_XLOJDES+" com sucesso.", "Aviso") 
...
DEFINE MSDIALOG oDlg TITLE "Rotina de Reliquidação Automática" FROM 000, 000  TO 400, 300 COLORS 0, 16777200 PIXEL
   @ 004, 011 Say oSay Prompt "     RELIQUIDAÇÃO FASE 4 = Geração títulos 005/006      "  SIZE 200, 010 OF oDlg PIXEL Color CLR_RED
   @ 015, 011 Say oSay Prompt "Essa rotina irá gerar reliquidação automática de títulos" Size 200,010 Of oDlg Pixel Color CLR_BLACK  
   @ 025, 011 Say oSay Prompt "       de acordo com as opções preenchidas abaixo     " Size 200,010 Of oDlg Pixel Color CLR_BLACK
...
//   @ 140, 060 MsGet oLojaChoice Var cLojaChoice Size 010,005 Of oDlg Pixel Color CLR_BLACK
   ...
   oButton1:= TButton():New(180,020, "OK",oDlg,{||iif(u_RFA4_VALID(),oDlg:End(),lCancelado:=.F.)}, 40,10,,,.F.,.T.,.F.,,.F.,,,.F. )
...
User Function RFA4_VALID()   // ====> VERIFICAR - UM FORNECEDOR ESCOLHIDO / CADASTRO DE FORNECEDORES - XFORDES (?)
...
if !empty(cFornChoice)
   cLojaChoice := RFA4_Fornece(cFornChoice)
   dbSelectArea("SA2")
   dbSetOrder(1)
...
// Função para retornar a loja destino do fornecedor - criado para manter padrão
// Alterado em 27/04/2021 para passar validar o campo A2_XLOJDES - Loja destino do De-Para 
Static Function RFA4_Fornece(cCodFor)
...
// Função para somar todos os títulos do fornecedor atual selecionado em TRB004
Static Function RFA4_Valor(cOpc)
...
// Natureza do titulo igual natureza do registro do TRB004 em execução
cQuery += "AND E2_NATUREZ = '"+TRB004->E2_NATUREZ+"' "

// Fornecedor destino título igual fornecedor do registro do TRB004 em execução - implementado em 27/04/2021
...
// Filtrar o Fornecedor Representado selecionado no registro do TRB004 em execução - implementado em 15/06/2021
cQuery += "AND E2_XFORREP = '"+TRB004->E2_XFORREP+"' AND E2_XLOJREP = '"+TRB004->E2_XLOJREP+"' "
