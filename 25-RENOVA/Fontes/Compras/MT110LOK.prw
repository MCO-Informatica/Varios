...
// Volta Trava
	...
	Local _cTpcomp   := Ascan(aHeader,{|x| Alltrim(x[2]) == "C1_XCOMEME"})//Inserido por andre 26/07
    Local _nToSc     :=""
...
//Fim Volta Trava
	...
	If _lExecuta
		...
		_nPosEmer := aScan(aHeader,{|x| ALlTrim(x[2]) == "C1_XCOMEME"} ) // Campo para Cp emergencial "Sim/Não"
		...
		If ((aCols[n,_nPosFore]) = '1' .OR. (aCols[n,_nPosEmer])='1')  .AND. Empty(aCols[n,_nPosObse])// adicionado este trecho: .OR. (aCols[n,_nPosEmer])='1' 
			MsgInfo("Atenção: O Campo 'Forn. Exclusivo.' ou 'Compra Emergencial' foi preenchido com Sim."+Chr(13)+Chr(10)+Chr(13)+Chr(10)+Chr(13)+Chr(10)+;
			 "Favor preencher o campo 'Justificativa' detalhando o motivo da compra.!!! ")
			_lRet := .F.
		EndIf

		// Validar compra emergencial por item _nPosEmer
		// André Couto 26/07/2021
		/*If !( GdDeleted( n, aHeader, aCols ) ) 
	       cRet := If( n = 1, .T., AllTrim( aCols[n,_cTpcomp] ) == AllTrim( aCols[1,_cTpcomp] ) )
		Endif
		If !cRet
	   	Aviso( "Atencao", "Use o Tipo de compras "+ AllTrim(aCols[1,_cTpcomp] )+" definido no 1o. item.", { "Ok" }, 1, "Escolha um só tipo: 1-Emergencial ou 2-Normal!" )
   		_lRet := .F. 
		EndIf	
		
		//fim da validação compras emergenciais */
