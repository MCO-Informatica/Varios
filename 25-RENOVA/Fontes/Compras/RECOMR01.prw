...
/*/
//User Function RECOMR01( cAlias, nReg, nOpcx )
User Function RECOMR01()
...
//³ExpC5 : Descricao                                                       ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
//oReport:= TReport():New("RECOMR01",cTitle,"RECOMR01", {|oReport| ReportPrint(oReport,nReg,nOpcx)},STR0001+" "+STR0002)
oReport:= TReport():New("RECOMR01",cTitle,"RECOMR01", {|oReport| ReportPrint(oReport)},STR0001+" "+STR0002)
oReport:SetPortrait()
...
Private nPageWidth  := IIF(nTpImp==1.Or.nTpImp==6,2314,2290) // oReport:PageWidth()
Public nPagina     := 0
...
oReport:onPageBreak( { || nPagina++ , nPrinted := 0 , IIF(!_cImpCond, CabecPCxAE(oReport,oSection1,nVias,nPagina), /*NIL*/) })
...
While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
	...
	For nVias := 1 to mv_par09
		...
		While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM == cNumSC7
			...
			If oReport:Row() > oReport:LineHeight() * 100
				...
				oReport:SkipLine()
				oReport:PrintText("Pagina: "+STRZERO(nPagina,2),, 050 ) // Continua na Proxima pagina ....
				oReport:EndPage()
			EndIf
		...
		oReport:SkipLine()
		oReport:PrintText("Pagina: "+STRZERO(nPagina,2),, 1200 )
	...
	IF MV_PAR15 = 1
		ImpCond(oReport) //Imprime condicoes gerais
	ENDIF
	dbSelectArea("SC7")
	dbSkip()
...
wnrel:=SetPrint(cString,wnrel,If(lAuto,Nil,"RECOMR01"),@Titulo,cDesc1,cDesc2,cDesc3,.F.,,lComp,Tamanho,,!lAuto)
...
If nLastKey <> 27
	...
	If lAuto
		mv_par08 := Eval({|| cCont:=ChkPergUs(cUserId,"RECOMR01","08"),If(cCont == Nil,SC7->C7_TIPO,cCont) })
	EndIf
...
PutSx1("RECOMR01","15","Imrpime Condicoes Gerais?","Imrpime Condicoes Gerais?","Imprime Condicoes Gerais?","mv_chf" ,"N",01 ,0 ,1 ,"C" ,"" ,"" ,"" ,"" ,"mv_par15" ,"Sim ","" ,"" ,"" ,"Não","","","","","","","","","","","","","","","")
...
Local cBitmap := "LOGORENOVA.BMP" // Empresa+Filial
...
Static Function ImpCond(oReport)

	_cImpCond := .T.

	oReport:EndPage()

	oReport:PrintText("",, 700 )
	oReport:SkipLine()
	oReport:PrintText("",, 700 )
	oReport:SkipLine()
	oReport:PrintText("",, 700 )
	...
	oReport:PrintText("CONDIÇÕES GERAIS DE PEDIDO DE COMPRA DE MATERIAIS E/OU EQUIPAMENTOS",, 700 )
	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 45 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Primeira - Objeto",, 950 )

	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('1.1 As presentes condições gerais aplicar-se-ão a todos os pedidos que tiverem por objeto o fornecimento de material e/ou equipamento ("Materiais") à RENOVA ENERGIA S.A.,',, 010 )
	oReport:SkipLine()
	oReport:PrintText('sociedade por ações, com sede na Cidade de São Paulo, Estado de São Paulo, na Avenida Roque Petroni Júnior, 999, 4º andar, Vila Gertrudes, CEP: 04707-910, inscrita no ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('CNPJ/MF sob o n° 08.534.605/0001-74 e/ou a qualquer uma de suas empresas afiliadas, conforme qualificadas no pedido de compra ("Contratante"), a serem fabricados ou não',, 010 )
	oReport:SkipLine()
	oReport:PrintText('pela pessoa jurídica ou física contratada ("Contratada"), são parte integrante dos respectivos pedidos de compra.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()
	
	oReport:PrintText('1.2 Considerar-se-ão aceitas, pela Contratada, as presentes condições gerais, a partir da emissão da respectiva nota fiscal dos Materiais.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('1.3 Os Materiais deverão ser entregues nos locais, formas e condições descritos e indicados no pedido de compra.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()	

	oReport:PrintText('1.4 O pedido de compra não será celebrado em caráter de exclusividade ficando a Contratante totalmente livre e desimpedida para contratar operações semelhantes às aqui',, 010 )
	oReport:SkipLine()
	oReport:PrintText('previstas com terceiros, independentemente de qualquer comunicação ou notificação nesse sentido.',, 010 )
	oReport:SkipLine()
	...
	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 25 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Segunda – Prazo de Entrega dos Materiais",, 800 )

	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('2.1 O não cumprimento do prazo estabelecido no pedido poderá implicar, a critério exclusivo da Contratante, na prorrogação do vencimento dos pagamentos devidos (sem a ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('incidência de qualquer encargo, penalidade ou correção monetária), pelo período que perdurar o atraso, para o que, desde já, a Contratada dá a sua anuência.',, 010 )
	oReport:SkipLine()
	...
	oReport:PrintText('2.2 Na hipótese de caso fortuito, força maior ou qualquer outro impedimento estranho à vontade das partes, se não for possível o fornecimento dos Materiais no prazo',, 010 )
	oReport:SkipLine()
	oReport:PrintText('estabelecido, a Contratada deverá previamente notificar por escrito a Contratante sobre a situação e a causa do referido impedimento. Nesse caso, a Contratante poderá',, 010 )
	oReport:SkipLine()
	oReport:PrintText('poderá, a seu exclusivo critério, rescindir o pedido, sem qualquer ônus, penalidades ou indenização de qualquer natureza, sendo responsável tão-somente pelo pagamento dos ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('valores em aberto correspondentes aos Materiais até então entregues.',, 010 )
	oReport:SkipLine()
	...
	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 25 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Quarta – Condições de Faturamento e Pagamento",, 800 )

	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('4.1 Os pagamentos serão realizados pela Contratante à Contratada em até 30 (trinta) dias, contados a partir do recebimento dos Materiais (ex-works), salvo se expressamente',, 010 )
	oReport:SkipLine()
	oReport:PrintText('definido outro prazo no pedido de compra. Caso a data de pagamento não recaia em dia útil na Cidade de Salvador, Estado da Bahia, o pagamento será realizado no primeiro',, 010 )
	oReport:SkipLine()
	oReport:PrintText('dia útil subsequente.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('4.2 No documento de cobrança obrigatoriamente deverá constar os seguintes dados: (i) o valor a ser pago pela Contratante; (ii) o número do pedido de compra; e (iii) a data',, 010 )
	oReport:SkipLine()
	oReport:PrintText('do respectivo vencimento. Adicionalmente, o documento de cobrança não poderá ter data de emissão entre os dias 26 e 31 de cada mês.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('4.3 Na hipótese de ocorrência de erro, rasuras ou de haver divergência no documento de cobrança, a Contratante devolverá o mesmo para correção e contará um novo prazo de ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('vencimento a partir da sua reapresentação.',, 010 )
	oReport:SkipLine()
	...
	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 25 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Quinta – Obrigações da Contratada",, 800 )

	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('5.1 Além das demais obrigações assumidas neste instrumento, no pedido de compra e em seus respectivos anexos, constituem obrigações e ônus da Contratada:',, 010 )
	oReport:SkipLine()

	oReport:SkipLine()

	oReport:PrintText('i. fornecer os Materiais de acordo com o disposto no pedido de compra, respeitando o prazo de entrega e, observando os requisitos de qualidade, especificações técnicas',, 010 )
	oReport:SkipLine()
	oReport:PrintText('e demais condições estabelecidas pela Contratante;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('ii. responsabilizar-se perante a Contratante por quaisquer reclamações, demandas e reivindicações de qualquer tipo, causados por sua ação ou de seus subcontratados ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('relacionadas ao fornecimento dos Materiais;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()
	...
	oReport:PrintText('',, 010 )
	oReport:SkipLine()
	oReport:PrintText('',, 010 )
	oReport:SkipLine()

	oReport:PrintText('iii. informar sobre a evolução do fornecimento, objeto do pedido, reportando qualquer ocorrência que possa impedir a sua continuidade, sob pena de responder pelos danos',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('causados pela paralisação sem aviso prévio;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('iv. permitir o acompanhamento integral do carregamento dos Materiais por representante indicado pela Contratante;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('v. fornecer o packing list detalhado dos Materiais, se aplicável, incluindo os pequenos itens que compõem os Materiais, para que a Contratante possa efetuar a conferência',, 010 )
	oReport:SkipLine()
	oReport:PrintText('antes do carregamento dos mesmos;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('vi. fornecer para a Contratante, em até 30 (trinta) dias após a data de emissão do pedido de compra, lista com, no mínimo, 3 (três) empresas homologadas pela Contratada,',, 010 )
	oReport:SkipLine()
	oReport:PrintText('que possam reparar os Materiais no caso de eventual avaria e, dessa forma não afetem a garantia dos Materiais, conforme Cláusula Sexta abaixo;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('vii. informar à Contratante sobre a data de disponibilidade dos Materiais com até 20 (vinte) dias de antecedência da mesma, mediante envio de notificação contendo: ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('(i) a quantidade de Materiais; (ii) o modelo dos Materiais; e (iii) os dados do representante da Contratada que acompanhará a entrega dos Materiais;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('viii. cumprir as regras e as normas relativas à saúde e segurança do trabalho;',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('ix. não utilizar trabalho infantil, trabalho forçado ou análogo a condição de escravidão; e',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('x. adotar práticas que não violem a Lei nº 12.846/13 – Lei Anticorrupção.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()
	
	oReport:PrintText('5.1.1 A Contratada declara e garante que os Materiais são certificados e atendem às exigências das leis e regulações aplicáveis de acordo com a lei brasileira, incluindo',, 010 )
	oReport:SkipLine()
	oReport:PrintText('as exigências do Inmetro (“Instituto Nacional de Metrologia, Qualidade e Tecnologia"), quando aplicável.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()	
	
	oReport:PrintText('5.2 Constituem obrigações da Contratante:',, 010 )
	oReport:SkipLine()

	oReport:PrintText('(i) Efetuar, pontualmente, todos os pagamentos devidos em razão do pedido de compra, na forma e condições pactuadas; e',, 010 )
	oReport:SkipLine()
	oReport:PrintText('(ii) Colaborar com a Contratada, naquilo em que lhe for cabível, na adoção das providências para viabilizar a execução do fornecimento.',, 010 )
	oReport:SkipLine()	
	...
	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 25 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Sexta – Garantia",, 950 )

	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('6.1. Salvo prazo superior garantido pela Contratada, durante um período de 12 (doze) meses, contados a partir da entrega dos Materiais, a Contratada garantirá totalmente',, 010 )
	oReport:SkipLine()
	oReport:PrintText('a qualidade da entrega dos Materiais, a Contratada garantirá totalmente a qualidade dos respectivos Materiais e assume a obrigação de corrigir ou substituir, por conta ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('própria (incluindo custos de entrega), todas os Materiais com defeito de fabricação ou abaixo dos padrões de qualidade.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('6.2. Em qualquer hipótese, a Contratada deverá realizar o conserto ou substituição no prazo máximo de até 30 (trinta) dias, contados da notificação enviada pela ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('Contratante (trinta) dias, contados da notificação enviada pela Contratante neste sentido, salvo prazo diverso expressamente definido no pedido de compra.',, 010 )
	oReport:SkipLine()
	...
	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 25 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Sétima – Rescisão e Multas",, 900 )

	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('7.1. No caso da Contratada não cumprir com o prazo de entrega estabelecido no pedido de compra, a Contratada incorrerá, independentemente de qualquer formalidade, em multa',, 010 )
	oReport:SkipLine()
	oReport:PrintText('não compensatória equivalente a 0,33% (trinta e três décimos por cento) por dia de atraso, calculada com base no preço unitário de cada material em atraso, limitada a 20% ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('(vinte por cento) do total dos Preços, sem prejuízo de eventuais perdas e danos causados.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()
	
	oReport:PrintText('7.2. No caso de inadimplemento, pela Contratada, de qualquer obrigação estabelecida nas presentes condições gerais, no pedido de compra ou em qualquer dos seus anexos, se ',, 010 )
	oReport:SkipLine()	
	oReport:PrintText('houver, exceto na hipótese do item 7.1. acima, ensejará na aplicação de multa não compensatória de 2% (dois por cento) sobre o valor total dos Preços, para cada infração',, 010 )
	oReport:SkipLine()
	oReport:PrintText('cometida, desde que o inadimplemento não seja sanado no prazo de 05 (cinco) dias, contados do recebimento da notificação enviada pela Contratante neste sentido.',, 010 )
	oReport:SkipLine()	
	oReport:SkipLine()

	oReport:PrintText('7.3. As multas estabelecidas acima serão consideradas como uma dívida líquida e a Contratante fica autorizada a reter os pagamentos devidos à Contratada ou promover a ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('cobrança extrajudicial ou judicial.',, 010 )
	oReport:SkipLine()	
	oReport:SkipLine()

	oReport:PrintText('7.4. A Contratada concorda que qualquer inadimplemento deste instrumento e/ou do pedido de compra, inclusive de quaisquer anexos que os constituam, que não sejam sanados',, 010 )
	oReport:SkipLine()
	oReport:PrintText('no prazo de 10 (dez) dias, a contar da data da sua comunicação expressa, será motivo suficiente para a Contratante, a seu próprio critério e agindo de boa-fé, rescindir',, 010 )
	oReport:SkipLine()
	oReport:PrintText('o mencionado pedido no todo ou em parte ressalvado os direitos da Contratada em relação a pagamentos remanescentes à época da rescisão relativos às entregas efetivadas, ',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()
	...
	oReport:PrintText('dos quais poderão ser deduzidos todos os valores relativos às multas e/ou indenizações devidos pela Contratada à Contratante, conforme cláusulas específicas estabelecidas',, 010 )
	oReport:SkipLine()
	oReport:PrintText('no pedido de compra e/ou neste instrumento.',, 010 )
	oReport:SkipLine()
	...
	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 25 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Oitava – Disposições Finais",, 850 )
	oReport:SkipLine()
	...
	oReport:PrintText('8.1 Qualquer disposição deste instrumento e/ou do pedido de compra que seja declarada proibida, inválida ou inexequível, em nenhuma hipótese invalidará ou afetará o ',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('presente instrumento e/ou o pedido de compra como um todo ou as demais disposições nele estabelecidas. No caso de qualquer uma das cláusulas do presente instrumento',, 010 )
	oReport:SkipLine()	
	oReport:PrintText('e/ou do pedido ser declarada proibida, inválida ou inexequível, as partes comprometem-se a negociar, em boa-fé, a substituição desta por outra que seja válida e eficaz.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()	

	oReport:PrintText('8.2 Para dirimir eventuais dúvidas, conflitos ou omissão gerados por discrepâncias entre os documentos, será considerada a seguinte ordem de prevalência:',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText(chr(61623)+' Pedido de compra emitido pela Contratante;',, 010 )
	oReport:SkipLine()
	oReport:PrintText(chr(61623)+' O presente instrumento; e',, 010 )
	oReport:SkipLine()	
	oReport:PrintText(chr(61623)+' Demais anexos do pedido de compra.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('8.3 Nenhuma das partes poderá revelar, motivar ou permitir a revelação de quaisquer informações relacionadas ao pedido, sem consultar a outra parte, nem tampouco usar ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('(se não com o propósito de finalizar as transações do pedido de compra), ou fazer declarações e/ou revelar qualquer informação recebida em conexão ao pedido de compra.',, 010 )
	oReport:SkipLine()	
	oReport:SkipLine()

	oReport:PrintText('8.4 A Contratante terá o direito de rejeitar, parcial ou integralmente, o fornecimento dos Materiais, sempre que este for considerado em desacordo com o especificado no',, 010 )
	oReport:SkipLine()
	oReport:PrintText('pedido de compra e/ou em seus anexos, bem como neste instrumento, sendo observado, inclusive, os posicionamentos pertinentes ao controle de qualidade.',, 010 )
	oReport:SkipLine()	
	oReport:SkipLine()

	oReport:PrintText('8.5 O pedido de compra, assim que emitido, substituirá e revogará todos os entendimentos verbais ou escritos havidos anteriormente entre a Contratante e a Contratada ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('relativos ao fornecimento dos Materiais pactuado.',, 010 )
	oReport:SkipLine()	
	oReport:SkipLine()

	oReport:PrintText('8.6 As alterações ao pedido de compra somente poderão ser consideradas como válidas e eficazes se forem formalizadas por escrito.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('8.7 Toda e qualquer comunicação entre a Contratada e a Contratante relativa ao pedido de compra deverá ser feita por meio de carta com Aviso de Recebimento (AR), ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('fac-símile com comprovação de envio, ou qualquer outro meio eletrônico, exclusivamente para o endereço e respectiva área responsável pela compra, indicada no pedido.',, 010 )
	oReport:SkipLine()	
	...
	oReport:Box( oReport:Row(),010,oReport:Row() + oReport:LineHeight() * 3, nPageWidth + 25 )
	oReport:SkipLine()
	oReport:PrintText("Cláusula Nona – Do Foro",, 950 )

	oReport:SkipLine()
	oReport:SkipLine()
	oReport:SkipLine()

	oReport:PrintText('9.1 Fica eleito o Foro da Cidade de São Paulo, com renúncia expressa de qualquer outro, por mais privilegiado que seja, para dirimir eventuais questões relativas ao ',, 010 )
	oReport:SkipLine()
	oReport:PrintText('pedido de compra, a este instrumento e seus respectivos anexos.',, 010 )
	oReport:SkipLine()
	oReport:SkipLine()
