...
*/
User Function CTBR400U(cContaIni, cContaFim, dDataIni, dDataFim, cMoeda, cSaldos,;
				 cBook, lCusto, cCustoIni, cCustoFim, lItem, cItemIni, cItemFim,;
				 lClVl, cClvlIni, cClvlFim,lSaltLin,cMoedaDesc,aSelFil,cEC05Ini, cEC05Fim )
...
PRIVATE cTipoAnt	:= ""
PRIVATE cPerg	 	:= "CTR400Z"
PRIVATE nomeProg  	:= "CTBR400U"  
...
SM0->(DbGoTop())
While !Eof() 
		AADD(_aEmpFil,{Alltrim(SM0->M0_CODFIL),SM0->M0_FILIAL})
	   	DbSkip()
End
SM0->(DbGoTo(_nReg))
...
If lOk
	If lImpRazR4 
		CTBR400UR4(aCtbMoeda,lCusto,lItem,lCLVL,@cArqTmp,aSelFil,lTodasFil )
	Else
		CTBR400UR4(aCtbMoeda,lCusto,lItem,lCLVL,@cArqTmp,aSelFil,lTodasFil )
...
±±ºPrograma  ³ CTBR400UR4 º Autor ³                    º Data ³  15/09/09  º±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function CTBR400UR4(aCtbMoeda,lCusto,lItem,lCLVL,cArqTmp,aSelFil,lTodasFil )
...
Local nTamHist	:= 100 //If(cPaisLoc$"CHI|ARG",29,40)
...
TRCell():New(oSection2,"DOCUMENTO"	,""       ,STR0034,/*Picture*/,If(nTamLote < 20, 20,nTamLote)+20,/*lPixel*/,{|| cArqTmp->LOTE+cArqTmp->SUBLOTE+cArqTmp->DOC+cArqTmp->LINHA },/*"LEFT"*/,,/*"LEFT"*/,,,.F.)// "LOTE/SUB/DOC/LINHA"
TRCell():New(oSection2,"HISTORICO"	,""		  ,STR0035,/*Picture*/,nTamHist+12	,/*lPixel*/,{|| cArqTmp->HISTORICO},/*"LEFT"*/,.T.,/*"LEFT"*/,,,.F.)// "HISTORICO"	
TRCell():New(oSection2,"XPARTIDA"	,"cArqTmp",STR0036,/*Picture*/,35,/*lPixel*/,/*{|| }*/,/*"LEFT"*/,,/*"LEFT"*/,,,.F.)// "XPARTIDA"
TRCell():New(oSection2,"Filial"		,""		  ,"Fil"/*STR0052*/,/*Picture*/,4,/*lPixel*/,/*{|| }*/)// "FILIAL"
TRCell():New(oSection2,"NOMEFIL"	,""		  ,"Nome Filial"/*STR0052*/,/*Picture*/,20,/*lPixel*/,/*{|| }*/)// "FILIAL"
TRCell():New(oSection2,"CCUSTO"		,"cArqTmp",Upper(cSayCusto),/*Picture*/,aTamCusto[1]+10,/*lPixel*/,{|| IIF(lCusto == .T.,cArqTmp->CCUSTO,Nil) },/*"LEFT"*/,,/*"LEFT"*/,,,.F.)// Centro de Custo
TRCell():New(oSection2,"DESCUS"	    ,"cArqTmp",'Descr Centro Custo',/*Picture*/,TamSx3('CTT_DESC01')[1],/*lPixel*/,/*{|| }*/)		//"DESCRICAO"
TRCell():New(oSection2,"ITEM"		,"cArqTmp",Upper(cSayItem) ,/*Picture*/,nTamItem+10,/*lPixel*/,{|| IIF(lItem == .T.,cArqTmp->ITEM,Nil) },/*"LEFT"*/,,/*"LEFT"*/,,,.F.)// Item Contabil
TRCell():New(oSection2,"DESITEM"    ,"cArqTmp",'Descr Projeto',/*Picture*/,TamSx3('CTD_DESC01')[1],/*lPixel*/,/*{|| }*/)		//"DESCRICAO"
...
TRCell():New(oSection2,"DESCLVL"    ,"cArqTmp",'Descr Camada',/*Picture*/,TamSx3('CTH_DESC01')[1],/*lPixel*/,/*{|| }*/)		//"DESCRICAO"
TRCell():New(oSection2,"EC05"		,"cArqTmp",'Classe Orc' ,/*Picture*/,nTamCLVL,/*lPixel*/,{|| IIF(lCLVL == .T.,cArqTmp->EC05,Nil) },/*"LEFT"*/,,/*"LEFT"*/,,,.F.)// Entidade 05 - Conta Orçamentária
TRCell():New(oSection2,"DESEC05"    ,"cArqTmp",'Descr Classe Orc',/*Picture*/, TamSx3('CV0_DESC')[1],/*lPixel*/,/*{|| }*/)		//"DESCRICAO"
TRCell():New(oSection2,"DESGRUPO"   ,"cArqTmp",'Grupo Custos ',/*Picture*/, TamSx3('CV0_DESC')[1],/*lPixel*/,/*{|| }*/)		//"DESCRICAO"
TRCell():New(oSection2,"DESTIPO"    ,"cArqTmp",'Tipo Custos  ',/*Picture*/, TamSx3('CV0_DESC')[1],/*lPixel*/,/*{|| }*/)		//"DESCRICAO"
...
oSection2:Cell("NOMEFIL"):lHeaderSize	:= .F.
oSection2:Cell("CCUSTO"):lHeaderSize	:= .F.
oSection2:Cell("DESCUS"):lHeaderSize	:= .F.
oSection2:Cell("ITEM"):lHeaderSize		:= .F.
oSection2:Cell("DESITEM"):lHeaderSize	:= .F.
oSection2:Cell("CLVL"):lHeaderSize		:= .F.
oSection2:Cell("DESCLVL"):lHeaderSize	:= .F.
oSection2:Cell("EC05"):lHeaderSize		:= .F.
oSection2:Cell("DESEC05"):lHeaderSize	:= .F.
oSection2:Cell("DESGRUPO"):lHeaderSize	:= .F.
oSection2:Cell("DESTIPO"):lHeaderSize	:= .F.
...
If nTipoRel == 3 // Sintetico
    ...
    oSection2:Cell("NOMEFIL"):Disable() 	  	
...
Local cEC05Ini		:= mv_par37 // Imprime Classe Orçamentária?
Local cEC05Fim		:= mv_par38 // Ate a Classe Orçamentária ?
Local cContaAnt		:= ""
Local cDescConta	:= ""
Local cCodRes		:= ""
Local cResCC		:= ""
...
If oReport:GetOrientation() == 1 .or. nTipoRel > 1 // Resumido ou Sintetico     
   	...
   	oSection2:Cell("DESCUS"):Disable()	
  	oSection2:Cell("ITEM"):Disable()
   	oSection2:Cell("DESITEM"):Disable()	
  	oSection2:Cell("CLVL"):Disable()  
   	oSection2:Cell("DESCLVL"):Disable()	
  	oSection2:Cell("EC05"):Disable()  
   	oSection2:Cell("DESEC05"):Disable()	
  	oSection2:Cell("DESGRUPO"):Disable()  
   	oSection2:Cell("DESTIPO"):Disable()	
...
If lImpLivro
	...
	If IsBlind()
		CTBGerRaz(	,,,,@cArqTmp,cContaIni,cContaFim,cCustoIni,cCustoFim,;
								...
								aSetOfBook,lNoMov,cSaldo,lJunta,"1",lAnalitico,,,cFiltro,lSldAnt,aSelFil, .T. /*lExterno*/,cEC05Ini, cEC05Fim)
	Else
	MsgMeter({|	oMeter, oText, oDlg, lEnd | CTBGerRaz(	oMeter,oText,oDlg,lEnd,@cArqTmp,cContaIni,cContaFim,cCustoIni,cCustoFim,;
							cItemIni,cItemFim,cCLVLIni,cCLVLFim,cMoeda,dDataIni,dDataFim,;
							aSetOfBook,lNoMov,cSaldo,lJunta,"1",lAnalitico,,,cFiltro,lSldAnt,aSelFil, .F. /*lExterno*/,cEC05Ini, cEC05Fim) },;
...
//		oReport:Init() 
		While lImpLivro .And. cArqTmp->(!Eof()) .And. !oReport:Cancel()
			...
			Else 					
				...
				aSaldoAnt	:= SaldoCT7Fil(cArqTmp->CONTA,dDataIni,cMoeda,cSaldo,"CTBR400U",,,aSelFil,,lTodasFil)	
			EndIf
			...
			cNormal := CT1->CT1_NORMAL
 			...
 			oSection2:Cell("DESCCC"):SetBlock( { || cDescConta} )  
			oSection2:Cell("DESCCC"):SetSize(LEN(cDescConta)+3)
			...
			Do While cArqTmp->(!Eof() .And. CONTA == cContaAnt ) .And. !oReport:Cancel()   
				...
				If mv_par08 < 3 //Se for relatorio analitico ou resumido  				
					...
					If AScan( _aEmpFil, { |x| x[1] == cArqTmp->FILORI } ) > 0
						oSection2:Cell("NOMEFIL"):SetBlock({|| _aEmpFil[AScan( _aEmpFil, { |x| x[1] == Alltrim(cArqTmp->FILORI) }),2]  } )
					Else
						oSection2:Cell("NOMEFIL"):SetBlock({|| '          ' } )					
					End	 
					...
					If lCusto
						...
						Else 
							...
							dbSelectArea("cArqTmp")
						Endif  
			 			oSection2:Cell("DESCUS"):SetBlock( { ||  CTT->CTT_DESC01 } )  
					Endif
					If lItem 						//Se imprime item 
						...
						Else
							...
							dbSelectArea("cArqTmp")					
						Endif
			 			oSection2:Cell("DESITEM"):SetBlock( { || CTD->CTD_DESC01 } )  
					Endif
					If lCLVL //Se imprime classe de valor
						...
						Else
							...
							dbSelectArea("cArqTmp")					
						Endif			
			 			oSection2:Cell("DESCLVL"):SetBlock( { || CTH->CTH_DESC01 } )  
					Endif
                   	
					If !Empty(Alltrim(cArqTmp->EC05))
						dbSelectArea("CV0")
						dbSetOrder(2)
						IF dbSeek(xFilial("CV0")+cArqTmp->EC05)				
			 				oSection2:Cell("DESEC05"):SetBlock( { ||  CV0->CV0_DESC } )  
			 				_aSintOrc:=SintOrcam(cArqTmp->EC05)
			 				If Len(_aSintOrc) == 2
			 				    oSection2:Cell("DESGRUPO"):SetBlock( { ||  _aSintOrc[2] } )  
				 				oSection2:Cell("DESTIPO"):SetBlock( { ||  _aSintOrc[1] } )
			 				ElseIf Len(_aSintOrc) == 1
				 				oSection2:Cell("DESTIPO"):SetBlock( { ||  _aSintOrc[1] } )
				 			End	  
							dbSelectArea("cArqTmp")
		 				Else	
		 					oSection2:Cell("DESEC05"):SetBlock( { || "            "  } )  
		 					oSection2:Cell("DESGRUPO"):SetBlock( { || "            "  } )  
		 					oSection2:Cell("DESTIPO"):SetBlock( { || "            "  } )  
                    	End                        
                    Else
	 					oSection2:Cell("DESEC05"):SetBlock( { || "            "  } )  
	 					oSection2:Cell("DESGRUPO"):SetBlock( { || "            "  } )  
...
Static Function ImpCompl(oSection2)
	...
	oSection2:Cell("DESCUS"		):Hide()
	oSection2:Cell("ITEM"		):Hide()
	oSection2:Cell("DESITEM"	):Hide()
	oSection2:Cell("CLVL"		):Hide()
	oSection2:Cell("DESCLVL"	):Hide()
	oSection2:Cell("EC05"		):Hide()
	oSection2:Cell("DESEC05"	):Hide()
	oSection2:Cell("DESGRUPO"		):Hide()
	oSection2:Cell("DESTIPO"	):Hide()
	oSection2:Cell("CLANCDEB"	):Hide()
	oSection2:Cell("CLANCCRD"	):Hide()
	oSection2:Cell("CTPSLDATU"	):Hide()
	oSection2:Cell("FILIAL"		):Hide()
	oSection2:Cell("NOMEFIL"	):Hide()
	...
	oSection2:Cell("DESCUS"		):Show()
	oSection2:Cell("ITEM"		):Show()
	oSection2:Cell("DESITEM"	):Show()
	oSection2:Cell("CLVL"		):Show()
	oSection2:Cell("DESCLVL"	):Show()
	oSection2:Cell("EC05"		):Show()
	oSection2:Cell("DESEC05"	):Show()
	oSection2:Cell("DESGRUPO"	):Show()
	oSection2:Cell("DESTIPO"	):Show()
	oSection2:Cell("CLANCDEB"	):Show()
	oSection2:Cell("CLANCCRD"	):Show()
	oSection2:Cell("CTPSLDATU"	):Show()   
	oSection2:Cell("FILIAL"		):Show()
	oSection2:Cell("NOMEFIL"	):Show()
...
Local cEC05Ini		:= mv_par37
Local cEC05Fim		:= mv_par38
Local cContaAnt	:= ""
Local cDescConta	:= ""
Local cCodRes		:= ""
Local cResCC		:= ""
...
Static Function CtbGerRaz(oMeter,oText,oDlg,lEnd,cArqTmp,cContaIni,cContaFim,cCustoIni,cCustoFim,;
						...
						nTipo,cUFilter,lSldAnt,aSelFil,lExterno, cEC05Ini, cEC05Fim)
...
Local aCampos
Local cChave
Local nTamHist	:=  100 //Len(CriaVar("CT2_HIST"))
Local nTamItem	:= Len(CriaVar("CTD_ITEM"))
Local nTamCLVL	:= Len(CriaVar("CTH_CLVL"))
Local nDecimais	:= 0    
...
aCampos :={	{ "CONTA"		, "C", aTamConta[1], 0 },;  		// Codigo da Conta
			...
			{ "EC05"		, "C", 11		, 0 },;			// Classe Orçamentaria
...
//#IFDEF TOP
	//If TcSrvType() != "AS/400" .And. cTipo == "1" .And. FunName() == 'CTBR400' .And. TCGetDb() $ "MSSQL7/MSSQL"		
	If TcSrvType() != "AS/400" .And. cTipo == "1" .And. TCGetDb() $ "MSSQL7/MSSQL"
		CtbQryRaz(oMeter,oText,oDlg,lEnd,cContaIni,cContaFim,cCustoIni,cCustoFim,;
				cItemIni,cItemFim,cCLVLIni,cCLVLFim,cMoeda,dDataIni,dDataFim,;
				aSetOfBook,lNoMov,cSaldo,lJunta,cTipo,c2Moeda,cUFilter,lSldAnt,aSelFil,lExterno,cEC05Ini, cEC05Fim)	
	Else
//#ENDIF
	// Monta Arquivo para gerar o Razao
	CtbRazao(oMeter,oText,oDlg,lEnd,cContaIni,cContaFim,cCustoIni,cCustoFim,;
			cItemIni,cItemFim,cCLVLIni,cCLVLFim,cMoeda,dDataIni,dDataFim,;
			aSetOfBook,lNoMov,cSaldo,lJunta,cTipo,c2Moeda,nTipo,cUFilter,lSldAnt,aSelFil,lExterno,cEC05Ini, cEC05Fim)
//#IFDEF TOP
	EndIf
//#ENDIF	
	...
	If TcSrvType() != "AS/400"
		...
		If cTipo == "1"
			...
			cVldEnt := 	"CT2_CCD>='" + cCustoIni + "' AND " +;
						...
						"CT2_CLVLDB<='" + cClVlFim + "' AND " +;						
						"CT2_EC05DB>='" + cEC05Ini + "' AND " +;
						"CT2_EC05DB<='" + cEC05Fim + "'"						
		...
		cQuery	+= " RIGHT JOIN " +RetSqlName("CT1")+ " CT1 ON " 
		cQuery	+= " CT1.CT1_FILIAL = '" + xFilial("CT1") + "' 
		cQuery	+= " AND CT2.CT2_DEBITO = CT1.CT1_CONTA" 
//		cQuery  += " AND CT1.CT1_DTEXSF < '"+DTOS(dDataIni) + "' AND CT1.CT1_DTEXIS >= '"+DTOS(dDataIni)+"'" //Denis debito
		...
		cQuery := ChangeQuery(cQuery) //Denis
...
//		cQuery  += " AND CT1.CT1_DTEXSF < '"+DTOS(dDataIni) + "' AND CT1.CT1_DTEXIS >= '"+DTOS(dDataIni)+"'" //Denis debito
		...
		//cQuery  += " AND CT1.CT1_DTEXSF < '"+DTOS(dDataIni) + "' AND CT1.CT1_DTEXIS >= '"+DTOS(dDataIni)+"'" //Denis credito
...
If cTipo == "1" //Denis - grava o conteúdo do cArqTmp
...
dbSelectArea("cArqTmp")
Replace HISTORICO	With AllTrim(HISTORICO) + ' ' + _cHist
...
cQuery	+= " ISNULL(CT2_DC,'') DC, ISNULL(CT2_LOTE,'') LOTE, ISNULL(CT2_SBLOTE,'') SUBLOTE, ISNULL(CT2_DOC,'') DOC, ISNULL(CT2_LINHA,'') LINHA, ISNULL(CT2_CREDIT,'') XPARTIDA, ISNULL(CT2_HIST,'') HIST, ISNULL(CT2_SEQHIS,'') SEQHIS, ISNULL(CT2_SEQLAN,'') SEQLAN, '1' TIPOLAN, ISNULL(CT2_EC05CR,'') EC05  "	
...
cQuery	+= " AND CT1.D_E_L_E_T_ = '' "
cQuery  += " AND CT1.CT1_DTEXSF >= '"+DTOS(dDataIni) + "' AND CT1.CT1_DTEXIS <= '"+DTOS(dDataIni)+"'"
...
cQuery	+= " ISNULL(CT2_DC,'') DC, ISNULL(CT2_LOTE,'') LOTE, ISNULL(CT2_SBLOTE,'')SUBLOTE, ISNULL(CT2_DOC,'') DOC, ISNULL(CT2_LINHA,'') LINHA, ISNULL(CT2_DEBITO,'') XPARTIDA, ISNULL(CT2_HIST,'') HIST, ISNULL(CT2_SEQHIS,'') SEQHIS, ISNULL(CT2_SEQLAN,'') SEQLAN, '2' TIPOLAN,  ISNULL(CT2_EC05DB,'') EC05  "	
...
While !Eof()                                              
	...
	If &("cArqCT2->("+cUFilter+")")						
		...
		Else
			...
			Replace EC05        With cArqCT2->EC05
...
±±³Fun‡„o    ³SaldoCT7Fil³ Autor ³ Renato F Campos      ³ Data ³ 11.03.2009			³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡„o ³ Saldo do Plano de Contas por filial                         			³±±
...
If !lDefTop .Or. Empty(xFilial("CT7")) .Or. Len(aSelFil) <= 1  
	cFilAux	:= IIF( Len(aSelFil) == 1, aSelFil[1] , Nil)
	cFilEsp	:= xFilial("CT7",cFilAux)	
	RETURN SaldoCT7(cConta,dData,cMoeda,cTpSald,cRotina,lImpAntLP,dDataLP,cFilEsp,cArqCt7)
EndIf
...
cQuery := "SELECT DISTINCT CT7.CT7_FILIAL"
cQuery += " FROM " + RetSqlName( "CT7" ) + " CT7"
cQuery += " WHERE CT7_CONTA = '" + cConta  + "'"
cQuery += " AND CT7_MOEDA   = '" + cMoeda  + "'"
cQuery += " AND CT7_TPSALD  = '" + cTpSald + "'"
cQuery += " AND D_E_L_E_T_  = ' '"
If !lTodasFil
	cQryFil := "CT7_FILIAL " + GetRngFil( aSelFil ,"CT7", .T., @cCT7TmpFil)
	cQuery += " AND " + cQryFil
Endif 

cQuery := ChangeQuery(cQuery)

dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),"FILCT7",.T.,.F.)

// efetuo a recuperação do saldo por filial
While FILCT7->( !Eof() )

	aSaldo := SaldoCT7(cConta,dData,cMoeda,cTpSald,cRotina,lImpAntLP,dDataLP,FILCT7->CT7_FILIAL,cArqCt7)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function CtbVlDtFim(cCadAlias,dDataIni)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ SINTORCAM ºAutor ³ Walter Silva (INOVA)º Data ³ 11/06/2015º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Obtem Contas Sintéticas de Contas Orçamentárias            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function SintOrcam(_cConta)
...
While !Empty(_cConta)                  	
	  If dbSeek(xFilial("CV0")+_cConta)				
		 //Obtendo a Conta Superior
		 _cConta:=CV0->CV0_ENTSUP
		 If !Empty(_cConta)     
		    _nCont++     
		    If _nCont < 3
			    _aSintOrc[_nCont]:=  Posicione('CV0',2,xFilial('CV0')+_cConta,'CV0_DESC')    
			End    
         End
      End   
