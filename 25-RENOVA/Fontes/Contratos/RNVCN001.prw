...
Local oModel
Local cContrato := aCab[ aScan(aCab,{|x| AllTrim(x[1])=="CND_CONTRA"}), 2]
//Local cNumMed   := ""
//Local aMsgDeErro:= {}
Local lRet      := .f. 
Local nI := 0
...
cn9->(DbSetOrder(1))
if cn9->(DbSeek(xFilial()+cContrato))
	 ...
	 if oModel:CanActivate()
		...
		for nI := 1 to len(aCab)
			oModel:SetValue('CNDMASTER' , acab[ni,1],acab[ni,2])
		next

		//cPlanilha := aCab[ aScan(aCab,{|x| AllTrim(x[1])=="CND_NUMERO"}), 2]
		//dDtvenc   := aCab[ aScan(aCab,{|x| AllTrim(x[1])=="CND_XDTVEN"}), 2]
		//cParcel   := aCab[ aScan(aCab,{|x| AllTrim(x[1])=="CND_PARCEL"}), 2]

		aCompets := CtrCompets()
		cCompet   := aCab[ aScan(aCab,{|x| AllTrim(x[1])=="CND_COMPET"}), 2]
		cRccomp := alltrim(str(aScan(aCompets, {|x| AllTrim(x) == cCompet })))

		//oModel:SetValue('CNDMASTER' , "CND_COMPET",cCompet)		//Selecionar competência
		//oModel:SetValue("CNDMASTER" , "CND_RCCOMP", "25")
		oModel:GetModel("CNDMASTER"):SetValue("CND_RCCOMP", cRccomp)

        //oModel:SetValue('CNDMASTER' , "CND_CONTRA",cn9->cn9_numero)
		//oModel:SetValue("CNDMASTER" , "CND_DTVENC",dDtvenc)

		for nI := 1 To oModel:GetModel("CXNDETAIL"):Length()
			oModel:GetModel("CXNDETAIL"):GoLine(nI)
			oModel:SetValue("CXNDETAIL" , "CXN_CHECK" , .t.)
			//oModel:SetValue("CXNDETAIL" , "CXN_PARCEL", cParcel )
			//oModel:SetValue("CXNDETAIL" , "CXN_DTVENC", dDtvenc )
		next

		for nI :=  1 to len(aItens)
			if len(aItens[nI]) > 0
				aItem := aItens[nI]
				nQuant := aItem[ aScan(aItem,{|x| AllTrim(x[1])=="CNE_QUANT"}), 2]
				cProdIt := aItem[ aScan(aItem,{|x| AllTrim(x[1])=="CNE_PRODUT"}), 2]
				for nX := 1 To oModel:GetModel("CNEDETAIL"):Length()
					if nX != aScan(aI,nX)
						oModel:GetModel("CNEDETAIL"):GoLine(nX)
						cProdCNE := oModel:GetValue("CNEDETAIL", "CNE_PRODUT")
						if cProdCNE == cProdIt
							for nZ := 1 to len(aItem)
								oModel:SetValue("CNEDETAIL" , aItem[nZ,1] ,aItem[nZ,2] )
							next
							//oModel:SetValue("CNEDETAIL" , "CNE_QUANT" ,nQuant )
							nTotmed += oModel:GetValue("CNEDETAIL", "CNE_VLTOT")
							aadd(aI, nX)
						endif
					endif
				next
		...
		for nX := 1 To oModel:GetModel("CNEDETAIL"):Length()
			if aScan(aI,nX) == 0
				oModel:GetModel("CNEDETAIL"):GoLine(nX)
				oModel:GetModel("CNEDETAIL"):DeleteLine()
			endif
		next
...
If lMsErroAuto
	...
	if cn9->(eof())
		cRet += "O contrato "+cContrato+" não foi encontrado"+CRLF
	elseIf lAutoErrNoFile
		aAutoErro := oModel:GetErrorMessage()	//GetAutoGrLog()
		cRet += aAutoErro[6]+" "+CRLF
	Else
		MostraErro()
	Endif
Else
...
If !lMsErroAuto
	...
	//cVenci := "20220825"
	//cxn->(RecLock("CXN", .F.))
	//cxn->cxn_dtvenc := stod(cVenci)
	//cxn->(MSUnlock())

	//cna->(dbSetOrder(1))
	//cna->(dbseek(xfilial()+cnd->cnd_contra+cnd->cnd_revisa+cnd->cnd_numero))

	//cnf->(dbSetOrder(2))
	//cnf->(dbseek(xfilial()+cnd->cnd_contra+cnd->cnd_revisa+cxn->cxn_cronog+cnd->cnd_compet))
	...
	Else
		...
		cSql := "select * from "+RetSqlName("SC7")+" c7 where c7_filial = '"+sc7->(xfilial())+"' "
		cSql += "and c7_contra = '"+cnd->cnd_contra+"' and c7_contrev = '"+cnd->cnd_revisa+"' "
		cSql += "and c7_planilh = '"+cnd->cnd_numero+"' and c7_medicao = '"+cnd->cnd_nummed+"' and c7.d_e_l_e_t_ = ' '"
		TcQuery cSql New Alias "tped"
		if !tped->(eof())
			cNumPed := tped->c7_num
		endif
		tped->(DbCloseArea())
		...
		if !empty(cNumPed)
			...
			cne->(dbSetOrder(1))
			cne->(dbseek(xfilial()+cnd->cnd_contra+cnd->cnd_revisa+cnd->cnd_numero+cnd->cnd_nummed))
			while !cne->(eof()) .and. cne->cne_contra == cnd->cnd_contra .and. ;
			...
			cxj->(RecLock("CXJ", .F.))
			cxj->cxj_numped := cNumPed
			cxj->(MSUnlock())

		endif
		//	cnf->(RecLock("CND", .f.))
		//	cnf->cnf_vlreal += nTotmed
...
aResult := {lMsErroAuto, FwCutOff(cDebug + cRet,.t.) }
