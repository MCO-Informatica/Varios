...
Static Function ProcArq(cArq)
	...
	aCompCab:= {"NRO_CHAVE","NRO_FATURA1","EMITENTE","CNPJ_EMITENTE","EMPRESA_PAGADORA","CNPJ_EMP_PAGADORA","VALOR_A_PAGAR1"}
...
If lProc
		...
		Next nY
					...
					For nX := 1 to LEN(aCabec)
							...
							If (aCabec[nX] == "NRO_CHAVE" )
								Aadd(aHeadSE2,{"NRO_CHAVE"     	, 	 ALLTRIM(aDados[nX]) , 																					Nil 	})
								Loop
							EndIf
							If aCabec[nX] == "NRO_FATURA1" 
								Aadd(aHeadSE2,{"NRO_FATURA1"  , 	ALLTRIM(aDados[nX]) , 																		Nil 	})
		
								Loop
							EndIf
							     				    					 					
							If aCabec[nX] == "EMITENTE" 
								Aadd(aHeadSE2,{"EMITENTE"      , 	ALLTRIM(aDados[nX]) , 																		Nil 	})
								Loop
							EndIf
							 
							If aCabec[nX] == "CNPJ_EMITENTE" 
								Aadd(aHeadSE2,{"CNPJ_EMITENTE"      , 	ALLTRIM(aDados[nX]) ,	 																	Nil 	})
								Loop
							EndIf	
							
							If aCabec[nX] == "EMPRESA_PAGADORA"
								Aadd(aHeadSE2,{"EMPRESA_PAGADORA" , 	ALLTRIM(aDados[nX]) , 																			Nil 	})
								Loop
							EndIf	
		
							If aCabec[nX] == "CNPJ_EMP_PAGADORA"
								Aadd(aHeadSE2,{"CNPJ_EMP_PAGADORA" , 	ALLTRIM(aDados[nX]) , 																			Nil 	})
								Loop
							EndIf			

							If aCabec[nX] == "VALOR_A_PAGAR1"
								Aadd(aHeadSE2,{"VALOR_A_PAGAR1" , 	ALLTRIM(aDados[nX]) , 																			Nil 	})
								Loop
							EndIf	
				...
				If !empty(aHeadSE2) //.AND. nErro == .F.//
					If lProc2	== .T.
						nColsCh	:= aScan( aHeadSE2, { |x| AllTrim(x[1]) == "NRO_CHAVE"			})
						nColsNft	:= aScan( aHeadSE2, { |x| AllTrim(x[1]) == "NRO_FATURA1"  		})
						nColsEmi	:= aScan( aHeadSE2, { |x| AllTrim(x[1]) == "EMITENTE" 			})
						nColsCem	:= aScan( aHeadSE2, { |x| AllTrim(x[1]) == "CNPJ_EMITENTE"     })
						nColsNpg	:= aScan( aHeadSE2, { |x| AllTrim(x[1]) == "EMPRESA_PAGADORA"  })//FORNECEDOR
						nColsCpg	:= aScan( aHeadSE2, { |x| AllTrim(x[1]) == "CNPJ_EMP_PAGADORA" })//EMPRESA FILIAL PAGADORA
						nColsVal	:= aScan( aHeadSE2, { |x| AllTrim(x[1]) == "VALOR_A_PAGAR1" 	})
						...
						aImport:=   {            {"NRO_CHAVE"       ,ALLTRIM(U_Limp(aHeadSE2[nColsCh][2]))				,Nil},;
							{"NRO_FATURA1" ,aHeadSE2[nColsNft][2]														,Nil},;
							{"EMITENTE"      ,aHeadSE2[nColsEmi][2]													,Nil},;
							{"CNPJ_EMITENTE"   ,aHeadSE2[nColsCem][2]													,Nil},;
							{"EMPRESA_PAGADORA"    ,aHeadSE2[nColsNpg][2]												,Nil},;
							{"CNPJ_EMP_PAGADORA"     ,aHeadSE2[nColsCpg][2]											,Nil},;
							{"VALOR_A_PAGAR1"   ,aHeadSE2[nColsVal][2]												,Nil}}
					  ...
					  //POSICIONA A GRAVAÇÃO CASO EXISTA O TITULO. 
							...
							//Gera Log.Não Foi possivel EncontraFr o Titulo
								If 	nErroC == .F.
									cMotivo := STR0034+aImport[2][2]+'/'+cCodFor
									GravaErro(.T.,.T.,.T.,cMotivo)
									cMotivo :=""
									nErroL := .T.
									nErro := .T.
