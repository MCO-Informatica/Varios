...
While (!FT_FEof())
	...
	If SubStr(aLinha[2], 1, 7) == "Empresa"
		...
		DbSelectArea("PA1")
		DbSetOrder(2)
		If PA1->(DbSeek(xFilial("PA1")+cCodAVD))
			cCnpj := PA1->PA1_CNPJ
			If cCNPJ <> SM0->M0_CGC
					...
					While QRYPLA->(!Eof())
						...
						If nValPar1 > 0
							...
							If QRYCRO->(!Eof())
								...
								aAdd(aCab, {"CND_OBS"   , "IMPORTACAO ROTINA AUTOMATICA", NIL})
						...
						If nValPar2 > 0
							...
							cQryCry := " SELECT *"+CRLF
							cQryCry += " FROM "+RetSqlName("CNF")+" CNF"+CRLF
							cQryCry += " WHERE CNF.D_E_L_E_T_ = ' '"+CRLF
							cQryCry += " AND CNF.CNF_FILIAL = '" + xFilial("CNF") + "' "+CRLF
							cQryCry += " AND CNF_CONTRA = '"+cContrato+"'"+CRLF
							cQryCry += " AND CNF_REVISA = '"+_cRevisa+"'"+CRLF
							cQryCry += " AND CNF_COMPET = '"+cComp2+"'"+CRLF
							cQryCry += " AND CNF_SALDO > 0  "+CRLF
							...
							TcQuery cQryCry New Alias "QRYCRY"
							...
							If QRYCRY->(!Eof())
								...
								aAdd(aCab, {"CND_NUMERO", cPlanilha                     , NIL})
								aAdd(aCab, {"CND_TIPPLA", cTpPlanil                     , NIL})
								aAdd(aCab, {"CND_PARCEL", QRYCRY->CNF_PARCEL            , NIL})
								aAdd(aCab, {"CND_XPA", "2"                              , NIL})
								aAdd(aCab, {"CND_XPMS", "2"                             , NIL})
								aAdd(aCab, {"CND_XCTRAN", cCodOns                       , NIL})
								aAdd(aCab, {"CND_XDTRAN", cSigOns                       , NIL})
						...
						If nValPar3 > 0
							...
							cQryCrz := " SELECT *"+CRLF
							cQryCrz += " FROM "+RetSqlName("CNF")+" CNF"+CRLF
							cQryCrz += " WHERE CNF.D_E_L_E_T_ = ' '"+CRLF
							cQryCrz += " AND CNF.CNF_FILIAL = '" + xFilial("CNF") + "' "+CRLF
							cQryCrz += " AND CNF_CONTRA = '"+cContrato+"'"+CRLF
							cQryCrz += " AND CNF_COMPET = '"+cComp3+"'"+CRLF
							cQryCrz += " AND CNF_SALDO > 0  "+CRLF
							...
							TcQuery cQryCrz New Alias "QRYCRZ"
							...
							If QRYCRZ->(!Eof())
								...
								aAdd(aCab, {"CND_NUMERO", cPlanilha                     , NIL})
								aAdd(aCab, {"CND_TIPPLA", cTpPlanil                     , NIL})
								aAdd(aCab, {"CND_PARCEL", QRYCRZ->CNF_PARCEL            , NIL})
								aAdd(aCab, {"CND_XPA", "2"                              , NIL})
								aAdd(aCab, {"CND_XPMS", "2"                             , NIL})
								aAdd(aCab, {"CND_PEDIDO", cNumPed                       , NIL})
...
_nT := 0
For _nT := 2 to 1024
	_cTab:= "SCGN"+STRZERO(_nT,6)
	_nAlias := Select(_cTab)   
	if _nAlias > 0
		(_cTab)->( DbClosearea() )
	Endif                                       
Next _nT
...
SA2->(dbSeek(cSeek := xFilial("SA2") + cCnpjFor))
While ! SA2->(Eof()) .And. A2_FILIAL+A2_CGC == cSeek
	If SA2->A2_MSBLQL <> "1"
		nA2Recno := SA2->(Recno())
		Exit
	Endif
