...
±±ºPrograma  ³FMAILJOB  ºAutor  ³Thiago Miguel   º Data ³  22/03/2019   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ FUNCAO PARA ENVIO DE EMAIL COM PEDIDO FUTUROS              º±±
±±º          ³                                                            º±±
...
//			Conout('Não foi possivel abrir a empresa: ' + aEmpres[nCount,1] + " filial " + aEmpres[nCount,2]) 
 ...
 /*FIM DO BLOCO*/
	...
	user function EnvioCMP(c_fil)
	    ...
	    Private clDtEnt := ''     //data de inclusao do orçamento
	    Private clDtEmiss := ''    //data de inclusão de follow-up   
		...
		cQuery += " WHERE SC7.C7_DATPRF > GETDATE ( ) - 265 "
		...
		_cHTML+= CRLF + '</TR>' 
		_cHTML+= CRLF + '<TR align="center">'
	    _cHTML+= CRLF + '	<TD>Entregas futuras: Compras a ser entregue após 7 dias:</TD>'  
	    _cHTML+= CRLF + '</TR>'  
	    _cHTML+= CRLF + '<TR align="center">'
	    _cHTML+= CRLF + '	<TD>'
		...
		_cHTML+= CRLF + '    	<TD><b>Descrição</b></TD>'
