...
*/
	//substituido o bloco acima pela linha abaixo [Mauro Nagata, Actual Trend, 19/04/2011]	    			
		            cNatOper := SF4->F4_TEXTO              // Natureza da Operacao
		            //fim bloco [Mauro Nagata, Actual Trend, 19/04/2011]
			    			EndIf
			    		EndIf
			    	endif
  ...
  //incluido bloco abaixo [Mauro Nagata, Actual Trend, 15/04/2013]
			...
			//fim bloco [Mauro Nagata, Actual Trend, 15/04/2013]            		
				...
				//	    cDescProd,;     
				//substiutido bloco acima pelo abaixo [Mauro Nagata, Actual Trend, 15/04/2013]
				aadd(aProd,	{Len(aProd)+1,;
					cCodProd,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					c_descprd,;
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			//substitutida a linha acima pela linha abaixo [Mauro Nagata, Actual Trend, 05/05/2011]
            aadd(aNota,SF1->F1_HORNFE)			
			If SF1->F1_TIPO $ "DB" 
			    dbSelectArea("SA1")
				dbSetOrder(1)
	...
	//Adicionado por Bruno 27/04/2011 - Inserir os dados Obs da Nota.
				...
				//comentado bloco abaixo LH ACTUAL 28/04/2017
			    //	If !ALLTRIM(SF1->F1_XOBSNF) $ cMensFis
		     	//		cMensFis +=" "+ALLTRIM(SF1->F1_XOBSNF)
			    //	EndIf  		   
			
			//Adicionado novo campo referente a Observação Adicional da NF de retorno/entrada -  LH ACTUAL 28/04/2017 
