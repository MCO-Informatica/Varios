...
If cTipo == "1"
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				While !Eof() .And. xFilial("SF3") == SF3->F3_FILIAL .And.;
					...
					If SX5->(dbSeek(xFilial("SX5")+"60"+SF3->F3_CODISS))
						...
						//Substituida linha acima pela abaixo [Bruno Parreira, Actual Trend, 10/10/2011]
						cServ := SC5->C5_XMENSNT
							EndIf
							If Empty(cServ)
								cServ := SX5->X5_DESCRI
							EndIf
	...
	//Substituida linha acima pela abaixo [Bruno Parreira, Actual Trend, 10/10/2011]
				If !AllTrim(SC5->C5_XMENSNT) $ cMensCli
	...
	//Substituida linha acima pela abaixo [Bruno Parreira, Actual Trend, 10/10/2011]
				  //	cMensCli += AllTrim(SC5->C5_XMENSNT)
				  	//cMensCli += IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, AllTrim(SF2->F2_MENNOTA),AllTrim(SC5->C5_XMENSNT))    
				  	cMensCli += IIF( SF2->(FieldPos("F2_MENNOTA")) > 0,AllTrim(SC5->C5_XMENSNT),AllTrim(SC5->C5_XMENSNT))
							EndIf
  ...
  //incluido bloco abaixo [Mauro Nagata, Actual Trend, 15/04/2013]
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³Adicionado por Alexandre em 24/10/2011 - Verifica se o campo de descricao da playpiso foi preenchido e utiliza      ³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			c_descprd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)
			c_descprd := Iif(cFilAnt = '02', IIF(Empty(SC6->C6_DESCRI),SB1->B1_XDESPLA,SC6->C6_DESCRI), c_descprd)
			//fim bloco [Mauro Nagata, Actual Trend, 15/04/2013]            		
...
Else
		...
		Else				
			...
			Else
				...
				Else
					...
					If !Upper(SA2->A2_EST) == "EX"				
					//aadd(aDest,VldIE(SA2->A2_INSCR))
					//[Mauro Nagata, Actual Trend, 20160105]
	...
	//Adicionado por Bruno 27/04/2011 - Inserir os dados Obs da Nota.
				//If !ALLTRIM(SF1->F1_XOBSNF) $ cMensCli
				//	cMensCli +=" "+ALLTRIM(SF1->F1_XOBSNF)
				//EndIf
				/*  lh actual abaixo
				If !ALLTRIM(SF1->F1_XOBSNF) $ cMensFis
					cMensFis +=" "+ALLTRIM(SF1->F1_XOBSNF)
				EndIf  
				*/  //LH ACTUAL     abaixo
				If !ALLTRIM(SF1->F1_XOBSNF2) $ cMensFis
					cMensFis +=" "+ALLTRIM(SF1->F1_XOBSNF2)
				EndIf 		
...
//substituida linha acima pela abaixo [Mauro Nagata, Actual Trend, 20170913]
cString += '<qTrib>' + ConvType(aProd[12], 15, Min(IIf(cTipo == "0", 4,4), 4)) + '</qTrib>' 
//cString += '<vUnTrib>'+ConvType(aProd[10]/aProd[12],21,8)+'</vUnTrib>'  
//[Mauro Nagata, Actual Trend, 20150616]
//cString += '<vUnTrib>'+ConvType(aProd[10]/aProd[12],21,4)+'</vUnTrib>'         
//[Mauro Nagata, Actual Trend, 201506105
//cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],21,8))+'</vUnTrib>'	
//substiuitda linha acima pela abaixo [Mauro Nagata, Actual Trend, 20170911]                  
//cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],21,8))+'</vUnTrib>' 
//subsituida linha acima pela abaixo [Mauro Nagata, Actual Trend, 20170914]                                                             
For nI := 0 To 100                             
 	nUnTrib := Round(aProd[10]/aProd[12],4)
 	If Round(aProd[12]*(nUnTrib+(nI*(If((aProd[12]*nUnTrib)<Round(aProd[10],2),1,-1)/10000)) ) ,2)= Round(aProd[10],2)
       Exit
    EndIf   	
Next                                                                                     
If Round(aProd[12]*(nUnTrib+(nI/10000)),2) != Round(aProd[10],2)
	For nI := 0 To 100                             
	 	nUnTrib := Round(aProd[10]/aProd[12],4)
	 	If Round(nUnTrib*(aProd[12]+(nI*(If((aProd[12]*nUnTrib)<Round(aProd[10],2),1,-1)/10000)) ),2)= Round(aProd[10],2)
	       Exit
	    EndIf   	
	Next                                                                                                   
	nPosIqTrib := At("<qTrib>",cString)   
	nPosFqTrib := At("</qTrib>",cString) 
	cString := Substr(cString,1,nPosIqTrib-1)+'<qTrib>'+ConvType(aProd[12]+(nI*(If((aProd[12]*nUnTrib)<Round(aProd[10],2),1,-1)/10000)), 15, Min(IIf(cTipo == "0", 4,4), 4)) + '</qTrib>'+Substr(cString,nPosFqTrib+8,Len(cString)-8) 
	cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(Round(aProd[10]/aProd[12],4),21,8))+'</vUnTrib>'
Else
	cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(Round((nUnTrib+(nI*(If((aProd[12]*nUnTrib)<Round(aProd[10],2),1,-1)/10000)) ),4),21,8))+'</vUnTrib>'	
EndIf	
