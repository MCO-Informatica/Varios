...
//cObs	:= Iif(!Empty(cProjet),"Projeto: "+cProjet+ " Site: "+cSite,"")
	...
	cProjet := Posicione("AJ7",2,xFilial("AJ7")+cNum+C7_ITEM,"AJ7_PROJET")	
        If cProjet <> cNewProj
		Aadd(aProjet,{Posicione("AJ7",2,xFilial("AJ7")+cNum+C7_ITEM,"AJ7_PROJET"), Posicione("AF8",1,xFilial("AF8")+Posicione("AJ7",2,xFilial("AJ7")+cNum+C7_ITEM,"AJ7_PROJET"),"AF8_DESCRI")})
		cNewProj := cProjet
        Endif
...
// Informe o endereço eletrônico do destinatário.
//oProcess:cTo := "joao@jestec.com.br;ricardo.souza@upduo.com.br"
oProcess:cTo := "fabio.favaretto@upduo.com.br"

//oProcess:ohtml:ValByName("usuario","Administrador")

oProcess:ohtml:ValByName("proc_link","http://playpisopisos114283.protheus.cloudtotvs.com.br:6801/workflow/messenger/emp01/compras/" + cMailID + ".htm")
//oProcess:ohtml:ValByName("proc_link","http://187.75.236.7:6801/workflow/messenger/emp01/compras/" + cMailID + ".htm")
...
ConOut("##>>>>>>>>>>>>>>> ENTROU NO APROVA NAO")
	...
	DBSetorder(1)
        If DBSeek(xFilial("SCR")+"PC"+cPedido)
            ...
            While !eof() .and. SCR->CR_NUM == cPedido
