...
±±ºPrograma  ³FINA001   ºAutor  ³Alexandre Sousa     º Data ³  02/11/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Replicacao de rateio para todos as parcelas do mesmo titulo º±±
...
RegToMemory("SE2")
    ...
    //como para debugar levaria muito tempo foi acordado com o sr.Fabio, diretor, que seria feito esta customização alternativa para que pudessem ratear 
	DbSelectArea("SEV")
    ...
    If DbSeek(xFilial("SEV")+SE2->E2_PREFIXO+SE2->E2_NUM+SE2->E2_PARCELA+SE2->E2_TIPO+SE2->E2_FORNECE+SE2->E2_LOJA+"1")
       If MsgBox("Existe(m) rateio(s) para ese título. Para continuar o(s) rateio(s) será(ão) excluído(s) o(s) rateio(s) deste título e parcelas pertencentes a este título. Deseja continuar ?","Rateio","YESNO")
          DbSelectArea("SE2")
          RecLock("SE2",.F.)
          SE2->E2_MULTNAT := " "
          SE2->(MsUnLock())  
...
If MsgYesNo("Deseja Gravar o rateio para todas as parcelas desse título?", "ATENÇÃO")
	a_area := SE2->(GetArea())
