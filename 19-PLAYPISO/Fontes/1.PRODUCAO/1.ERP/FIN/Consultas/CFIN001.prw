...
//	EndIf
	...
	oProcessx := MsNewProcess():New({|lEnd| BusView(oProcess)},"Demonstrativo de Resultados - PLAYPISO","Extraindo dados ... ",.F.)
	oProcessx:Activate()
...
//	fCabecxml()
	...
	oProcess := MsNewProcess():New({|lEnd| CriaAcols(oProcess)},"Demonstrativo de Resultados - PLAYPISO","Preparando Roteiro ... ",.F.)
...
//	oObj:IncRegua1("Atualizando query... ")
	...
	c_Query := " select * from FLUXOCAIXA_0308X WHERE EMP = 'PlayPiso' "
...
cQuery += " WHERE D_E_L_E_T_ <> '*' AND A6_FILIAL <> '01' "
...
#IFNDEF TOP
	If  lAllFil
		cCondWhile := "!Eof() .And. E5_DTDISPO <= d_dtate .And. E5_FILIAL <> '01' "
	Else
		cCondWhile := "!Eof() .And. E5_FILIAL == xFilial('SE5') .And. E5_DTDISPO <= d_dtate .And. E5_FILIAL <> '01' "
	EndIf
#ELSE
	If TcSrvType() != "AS/400"
		DbSelectArea("SE5")
...
Static Function fCabecxml()
	...
	Aadd(a_xml, '    <Cell ss:MergeAcross="12" ss:StyleID="s67"><Data ss:Type="String">Demonstrativo de Fluxo de Caixa - PLAYPISO</Data></Cell>')
	Aadd(a_xml, '   </Row>')
