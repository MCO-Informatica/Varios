...
_cQry := "SELECT * "
_cQry += _cEnter + " FROM sigamat_copia (NOLOCK)"
_cQry += _cEnter + " WHERE ativo = '1' "
IF !EMPTY(MV_PAR01)
	_cQry += _cEnter + " AND M0_CODFIL 		BETWEEN '" + _cFilialDe + "' AND '" + _cFilialAte + "'"
ENDIF
_cQry += _cEnter + " AND M0_CODFIL 		!= ''"
//_cQry += _cEnter + " AND B9_FILIAL 		BETWEEN '" + _cFilialDe + "' AND '" + _cFilialAte + "'"

IF SELECT('SM0FILIAIS') > 0
	dbSelectArea('SM0FILIAIS')
	dbCloseArea()
ENDIF

MsAguarde({|| DbUseArea(.T., "TOPCONN", TCGenQry(,,_cQry), 'SM0FILIAIS', .F., .T.)},'Verificando filiais do sistema...')

If ! MsgBox(_cTexto,iif(mv_par06 == 1,'Confirma fechamento de estoque ?','Confirma ajuste dos saldo atual do estoque?'),'YESNO')
	dbSelectArea('SM0FILIAIS')
	dbCloseArea()
	Return .T.
EndIf
...
// LOOP POR FILIAL
Do While SM0FILIAIS->M0_CODFIL >= MV_PAR02 .AND. SM0FILIAIS->M0_CODFIL <= MV_PAR03 .And. ! SM0FILIAIS->(EOF())
	...
	If mv_par06 <> 2
		...
		_cQuery += _cEnter + " AND B9_FILIAL 		= '" + SM0FILIAIS->M0_CODFIL + "' "
		If !empty(mv_par05)
			_cQuery += _cEnter + " AND B9_COD 		= '" + mv_par05 + "'"
		EndIf             
		MsAguarde({|| DbUseArea(.T., "TOPCONN", TCGenQry(,,_cQuery), '_SB9', .F., .T.)},'Verificando fechamentos de estoque...')
	...
	If mv_par06 <> 2
		...
		Processa({|| DELETASB9()},'Apagando fechamento anterior filial '+SM0FILIAIS->M0_CODFIL+'.')
...
aAdd(_aRegs,{_cPerg, "06", "Operação                   ","","","mv_ch6", "N",01,  0,  0, "C", "" , "mv_par06", "Fechamento estoque", "", "", "", "", "Saldo atual", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",""})
...
/*
cQuery 	+= "SELECT TOP 1 COALESCE(B9_DATA,A.DATA) AS FECHAMENTO "
cQuery 	+= "FROM ( "
cQuery 	+= "	SELECT '20121231' AS DATA "
cQuery 	+= "	) A "
cQuery 	+= "	LEFT JOIN  "
cQuery 	+= "		( "
cQuery 	+= "			SELECT TOP 1 B9_DATA  "
cQuery 	+= "			FROM "+RetSqlName("SB9")+" B WITH(NOLOCK)  "
cQuery 	+= "			WHERE B9_FILIAL = '"+SM0FILIAIS->M0_CODFIL+"'  "
If !empty(MV_PAR05)
	cQuery 	+= "		AND B9_COD 		= '"+MV_PAR05+"'  "
ENDIF
cQuery 	+= "			AND B9_DATA 	< '"+DTOS(DDATABASE)+"' "
...
MsAguarde({|| DbUseArea(.T., "TOPCONN", TCGenQry(,,cQuery), 'B9DATA', .F., .T.)},'Verificando data ultimo fechamento...')

B9DATA->(DbGotop())

If (! B9DATA->(Eof())) .And. ! Empty(B9DATA->B9_DATA)
	dDataFec		:= B9DATA->B9_DATA
EndIf
...
If mv_par06 <> 2		// para fechamento de estoque ou ambos
	...
	cRet += _cEnter + " AND D2_EMISSAO 		>  '"+dDataFec+"'"
	...
	cRet += _cEnter + " AND D2_FILIAL 		= '"+SM0FILIAIS->M0_CODFIL+"'"
...
If mv_par06 <> 2
	...
	cRet += _cEnter + " AND D1_DTDIGIT 		> '"+dDataFec+"'"
	...
	cRet += _cEnter + " AND D1_FILIAL 		= '" +SM0FILIAIS->M0_CODFIL+ "'"
...
If mv_par06 <> 2
	...
	cRet += _cEnter + " AND D3_EMISSAO 		> '"+dDataFec+"'"
	...
	cRet += _cEnter + " AND D3_FILIAL 		= '" +SM0FILIAIS->M0_CODFIL+ "'"
...
//Else
cRet += _cEnter + " AND B9_DATA 		= '"+dDataFec+"'"
//EndIf

If !empty(mv_par05)
	cRet += _cEnter + " AND B9_COD 		= '" + mv_par05 + "'"
EndIf
//cRet += _cEnter + " AND B9_FILIAL 	BETWEEN '" + _cFilialDe + "' AND '" + _cFilialAte + "'"SM0FILIAIS->M0_CODFIL
cRet += _cEnter + " AND B9_FILIAL 		= '" +SM0FILIAIS->M0_CODFIL+ "'"
cRet += _cEnter + " ) "
cRet += _cEnter + " ) X
...
cAssunto  := "Divergencias - Saldo Atual " + dtoc(dDataBase)

cRecebe   := "richard.cabral@laselva.com.br"				// GetMv('LS_SPEMTXT') 
...
For nX := 1 to Len(aDiverg)
	_cCorpo   += aDiverg[nX,1]+'     '+Padr(aDiverg[nX,2],15)+' '+Posicione("SB1",1,xFilial("SB1")+aDiverg[nX,2],"B1_DESC")+' '+Str(aDiverg[nX,3],8)+'   '+Str(aDiverg[nX,4],8)  + _cEnter
Next
