...
#DEFINE MAXITEMC   052 //lsv                                          // Máxima de caracteres por linha de produtos/serviços
...
cPerg := "NFSIGW" + cFilAnt + "  "		// lsv
Validperg()								// lsv
If Pergunte(cPerg,.T.)					// lsv
...
PRIVATE oFont08    := TFontEx():New(oDanfe,"Courier New",07,07,.F.,.T.,.F.)// 4   // LSV
...
If mv_par04 == 2	// lsv
	...
	cTes := SD2->D2_TES // LSV UTILIZADO PARA PEGAR A FORMULA DA TES                
	dbSelectArea("SC5")
	dbSetOrder(1)
...
Else
	...
	cTes := SD1->D1_TES // LSV UTILIZADO PARA PEGAR A FORMULA DA TES                
	dbSelectArea("SF4")
	dbSetOrder(1)
...
Else
	cModFrete := "1"
EndIf
If Type("oTransp:_ModFrete") <> "U"					// lsv
	cModFrete := oTransp:_ModFrete:TEXT				// lsv
ElseIf oNfe:_NFE:_INFNFE:_IDE:_SERIE:TEXT == '8'	// lsv
	If SC5->C5_TPFRETE=="C"							// LSV
		cModFrete := "0"							// LSV
	ElseIf SC5->C5_TPFRETE=="F"						// LSV
		cModFrete := "1"							// LSV
	ElseIf SC5->C5_TPFRETE=="T"						// LSV
		cModFrete := "2"							// LSV
	ElseIf SC5->C5_TPFRETE=="S"						// LSV
		cModFrete := "9"							// LSV
	Else											// LSV
		cModFrete := "1"						    // lsv
	EndIf											// LSV
...
cFilDes	 := Posicione("PA6",1, xFilial("PA6")+SC5->C5_NUM+SC5->C5_FILIAL,"PA6_FILDES")  // LSV 
cFormula := Posicione('SM4',1, xFilial("SM4") + Posicione('SF4',1,xFilial("SF4")+cTes,'F4_FORMULA'),'M4_FORMULA')  // LSV - Verifica Formula na Tes
...
oDanfe:Box(000,000,015,501)											//LSV
oDanfe:Say(006, 002, "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO", oFont07:oFont) //LSV
oDanfe:Say(012, 002, AllTrim(SM0->M0_FILIAL) + " - " + Transform(AllTrim(SM0->M0_CGC),"@r 99.999.999/9999-99") , oFont07:oFont)//LSV
oDanfe:Box(015,000,037,101) 										//LSV
oDanfe:Say(020, 002, "DATA DE RECEBIMENTO", oFont07N:oFont)			//LSV
oDanfe:Box(015,100,037,500)											//LSV
oDanfe:Say(020, 102, "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", oFont07N:oFont)	//LSV
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	For nX := 1 To Len(aValores[1])
		...
		For nY := 1 To Len(aValores[1][nX])
			...
			If (oPrinter:GetTextWidth(aValores[1][nX][nY], oFont) * nConsTex) > nAux
				...
				nAux := oFontSize:getTextWidth( Alltrim(aValores[1][nX][nY]), oFontCabec:Name, oFontCabec:nWidth, oFontCabec:Bold, oFontCabec:Italic )  // lsv
			EndIf
	...
	// Ajuste na coluna DESCRI - LSV		
	
	aTamCol[1] 	+= 2   //LSV - codigo
   	aTamCol[2] 	+= 15  //LSV - descricao
   	aTamCol[3] 	-= 3   //LSV
	aTamCol[7] 	-= 2   //LSV  	
   	aTamCol[8] 	-= 0   //LSV
    aTamCol[13] -= 2   //LSV
...
aAdd( aPerg , { cPerg, "03", "Da Serie                      ?","","", "mv_ch3", "C",  3 , 0, 0, "G", "", "mv_par03", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",""})
aAdd( aPerg , { cPerg, "04", "Tipo de Operacao              ?","","", "mv_ch4", "N",  1 , 0, 0, "C", "", "mv_par04", "NF. Entrada", "", "", "", "", "NF. Saida", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",""})
aAdd( aPerg , { cPerg, "05", "Imprime no verso              ?","","", "mv_ch5", "N",  1 , 0, 0, "C", "", "mv_par05", "Sim", "", "", "", "", "Não", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",""})
