...
If cTipo == "1"
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					aadd(aProd,	{Len(aProd)+1,;
						...
						"",;
						"",;
						0})  // Da posiÁ„o 31 a 34 tratamento realizado apenas para documento de saÌda por este motivo campos est„o zerados e vazios.
		    ...
		    If lQuery
		    	...
		    	dbSelectArea("SC5")
//RETIRADO SANDRO REIS 06/03/14 POIS AFETAVA O PE PE01NFESEFAZ		    	dbCloseArea()
		    EndIf
...
If !empty(aProd[30]) .And. cMvMsgTrib $ "2-3" .And. cTpCliente == "F"
	cCrgTrib := 'Valor Aproximado dos Tributos: R$ '+ ConvType(aProd[30],15,2)+'.'
EndIf
