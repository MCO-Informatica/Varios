...
#DEFINE MAXITEMC  012 // 012 LSV                                     // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN 110 // 200 LSV                                     // Máximo de caracteres por linha de dados adicionais
...
PRIVATE oFont07    := TFontEx():New(oDanfe,"Courier New",07,07,.F.,.T.,.F.)// 4   // LSV
PRIVATE oFont08    := TFontEx():New(oDanfe,"Times New Roman",07,07,.F.,.T.,.F.)// 4   // LSV
PRIVATE oFont08N   := TFontEx():New(oDanfe,"Times New Roman",06,06,.f.,.T.,.F.)  // lsv
...
If lMv_ItDesc
	For nX := 1 To nLenDet
		Aadd(aIndAux, {nX, SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),1,12)})	// lsv
	Next
...
For nY := 1 To nLenItens
	...
	If aAux[1][1][nY] == "-"
		oDanfe:Say(nLinha, aColProd[1][1] + 2, Replicate("- ", 94), oFont07:oFont)	// lsv
	Else
...
oDanfe:Say(MAXBOXV-10, INIBOXH+32, AllTrim(SM0->M0_FILIAL) + " - " + Transform(AllTrim(SM0->M0_CGC),"@r 99.999.999/9999-99") , oFont07N:oFont, , , 270 )//LSV
