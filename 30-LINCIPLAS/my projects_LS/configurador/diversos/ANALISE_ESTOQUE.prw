...
Private dUltFech     := dDataI := dDataF := CTOD(Space(08))
Private nSB9         := 0
Private nSBJ         := 0
Private nSBK         := 0
Private nKAR         := 0
...
AADD(aDetSB8   ,{CTOD(Space(08)),Space(10),Space(06),CTOD(Space(08)),0,0,Space(08),0,0,0})
AADD(aDetSD5   ,{CTOD(Space(08)),Space(03),Space(10),Space(06),CTOD(Space(08)),0,0,Space(06),Space(08),Space(06),Space(03),Space(06),Space(02),Space(01)})
AADD(aDetSBF   ,{Space(15),Space(10),0,Space(08),0,Space(01),Space(01),Space(01),0,0,Space(01)})
AADD(aDetSDB   ,{Space(03),CTOD(Space(08)),Space(03),Space(15),Space(10),0,0,Space(06),Space(08),Space(06),Space(03),Space(06),Space(02),Space(01)})
AADD(aDet01B8BF,{Space(10),0,0,0,0,0,0,Space(08)})
AADD(aDet02B8BF,{Space(10),0,0,0,0,Space(08)})
AADD(aDetSBJ   ,{Space(10),0,0,Space(08)})
AADD(aAnaSBJ   ,{Space(10),Space(06),CTOD(Space(08)),0,Space(08)})
AADD(aDetSBK   ,{Space(10),0,0,Space(08)})
AADD(aAnaSBK   ,{Space(15),Space(10),0,Space(08)})
AADD(aDetDASDA ,{CTOD(Space(08)),Space(10),0,0,0,Space(06),Space(03),Space(06),Space(03),Space(06),Space(02),Space(08)})
AADD(aDetDASB8 ,{CTOD(Space(08)),Space(10),Space(06),0,0,Space(03),Space(06),Space(03),Space(06),Space(02),Space(08),Space(01)})
AADD(aDocto    ,{Space(06),Space(03),Space(06),Space(02),0,0,0,0,0,0,0,0,Space(03),Space(08),Space(03),Space(01),Space(01)})
AADD(aNumSeq   ,{Space(06),0,0,0,0,0,0,0,0,Space(03),Space(08),CTOD(Space(08)),Space(06),Space(03),Space(06),Space(02),Space(03),Space(01),Space(01)})
...
@ 220,002 SAY "Sugestoes e melhorias p/ e-mail: "  SIZE 200,10 PIXEL OF oFolder:aDialogs[1] FONT oBold  COLOR CLR_HBLUE
@ 220,435 SAY "Versão 5.6"                                               SIZE 200,10 PIXEL OF oFolder:aDialogs[1] FONT oBold  COLOR CLR_HBLUE
...
Do Case
	Case SB1->B1_LOCALIZ == "S" .AND. SB1->B1_RASTRO == "L"
		IF nKAR != nSB8 .OR. nKAR != nSD5 .OR. nKAR != nSDB .OR. nKAR != nSBF .OR. nKAR != nSB2 .OR.; 
		   nSB8 != nSD5 .OR. nSBF != nSDB .OR. nSB8 != nSBF .OR. nSB2 != nSB8 .OR. nSB2 != nSBF
		...
		IF nSBJ == 0    .AND. nSBK == 0    .AND. nKAR == nSB8 .AND. nKAR == nSD5 .AND. nKAR == nSDB .AND. ;
		   nKAR == nSBF .AND. nKAR == nSB2 .AND. nSB8 == nSD5 .AND. nSBF == nSDB .AND. nSB8 == nSBF .AND. ;
		   nSB2 == nSB8 .AND. nSB2 == nSBF
			cMensagem := "OK"
		EndIf
...
While !Eof()
	...
	IF nPos == 0
		...
		Else
			AADD(aNumSeq,{DB_NUMSEQ,0,0,0,0,0,0,0,DB_QUANT,"SDB","OK",DB_DATA,DB_DOC,DB_SERIE,DB_CLIFOR,DB_LOJA,Space(01),Space(01),Space(01)})			//                              D1,D2,D3_E,D3_S,D5_E,D5_S,DB_E,DB_S
		EndIf
	Else
		IF DB_TM <= "500" .OR. Substr(DB_TM,1,2) $ 'DE/PR/MA'
...
IF cFolder = "02"
	...
	dDtValid := IF(Len(aDetSD5)>0,aDetSD5[oDetSD5:nAT][05],CTOD(Space(08)))
ElseIF cFolder = "13"
...
IF     cFolder = "03"
		...
		dDtValid := IF(Len(aDetSDB)>0,aDetSDB[oDetSDB:nAT][04],CTOD(Space(08)))
ElseIF cFolder = "13"
		cNumSeq  := aNumSeq[oNumSeq:nAT][01]
		dbSelectArea("SDB")
		dbSetOrder(1)
			...
			Do Case
				Case aCols[I,1] = "SD1"
					...
					IF aCols[I,15] // Deletar
						...
						MsUnlock()
					Else
						IF  aCols[I,02] != SD1->D1_DTDIGIT .OR.	aCols[I,03] != SD1->D1_DOC     .OR.;
							aCols[I,04] != SD1->D1_SERIE   .OR.	aCols[I,05] != SD1->D1_FORNECE .OR.;
							aCols[I,06] != SD1->D1_LOJA    .OR.	aCols[I,09] != SD1->D1_LOTECTL .OR.;
							aCols[I,10] != SD1->D1_NUMLOTE .OR.	aCols[I,12] != SD1->D1_QUANT   .OR.;
				...
				Case aCols[I,1] = "SD2"
					...
					IF aCols[I,15] // Deletar
						...
						MsUnlock()
					Else
						IF  aCols[I,02] != D2_EMISSAO .OR. aCols[I,03] != D2_DOC     .OR.;
							aCols[I,04] != D2_SERIE   .OR. aCols[I,05] != D2_CLIENTE .OR.;
							aCols[I,06] != D2_LOJA    .OR. aCols[I,09] != D2_LOTECTL .OR.;
							aCols[I,10] != D2_NUMLOTE .OR. aCols[I,11] != D2_LOCALIZ .OR.;
							aCols[I,12] != D2_QUANT   .OR. aCols[I,13] != D2_QTSEGUM
							RecLock("SD2",.F.)
				...
				Case aCols[I,1] = "SD3"
					...
					IF aCols[I,15] // Deletar
						...
						MsUnlock()
					Else
						IF  aCols[I,02] != D3_EMISSAO .OR. aCols[I,03] != D3_DOC     .OR.;
							aCols[I,09] != D3_LOTECTL .OR. aCols[I,10] != D3_NUMLOTE .OR.;
							aCols[I,11] != D3_LOCALIZ .OR. aCols[I,12] != D3_QUANT   .OR.;
				...
				Case aCols[I,1] = "SD5"
					...
					IF aCols[I,15] // Deletar
						...
						MsUnlock()
					Else
						IF  aCols[I,02] != D5_DATA    .OR. aCols[I,03] != D5_DOC     .OR.;
							aCols[I,04] != D5_SERIE   .OR. aCols[I,05] != D5_CLIFOR  .OR.;
							aCols[I,06] != D5_LOJA    .OR. aCols[I,09] != D5_LOTECTL .OR.;
							aCols[I,10] != D5_NUMLOTE .OR. aCols[I,12] != D5_QUANT   .OR.;
				...
				Case aCols[I,1] = "SDB"
					...
					IF aCols[I,15] // Deletar
						...
						MsUnlock()
					Else
						IF  aCols[I,02] != DB_DATA    .OR. aCols[I,03] != DB_DOC     .OR.;
							aCols[I,04] != DB_SERIE   .OR. aCols[I,05] != DB_CLIFOR  .OR.;
							aCols[I,06] != DB_LOJA    .OR. aCols[I,09] != DB_LOTECTL .OR.;
							aCols[I,10] != DB_NUMLOTE .OR. aCols[I,11] != DB_LOCALIZ .OR.;
							aCols[I,12] != DB_QUANT   .OR. aCols[I,13] !=  DB_QTSEGUM
							RecLock("SDB",.F.)
...
IF lSB8
	...
	SB8->B8_LOJA    := cLoja
	SB8->B8_PRODUTO := cProduto
	SB8->B8_QTDORI  := nQuant
	SB8->B8_QTDORI2 := nQuant2UM
	SB8->B8_LOTECTL := cLoteCtl
	SB8->B8_POTENCI := nPotencia
	MsUnlock()
...
EndIf
SBF->BF_QUANT   += (nQuant * nMultiplic)
SBF->BF_QTSEGUM += (nQuant2UM * nMultiplic)
...
AADD(aCols, {})
AADD(aCols[Len(aCols)], Space(03) )
AADD(aCols[Len(aCols)], CTOD(Space(08)))
AADD(aCols[Len(aCols)], Space(06) )
AADD(aCols[Len(aCols)], Space(03) )
AADD(aCols[Len(aCols)], Space(06) )
AADD(aCols[Len(aCols)], Space(02) )
