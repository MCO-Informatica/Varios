...
Private aUF     := {}
//variavel para mensagem de retorno de remessa
Private cMsDev  := ""
Private cMsgI	:= ""
Private cMsgII	:= ""
Private cMsgICM	:= ""
Private cCfop	:= ""
//variavel para mensagem na nota fiscal de entrada
Private cForm   := ""
Private cMsgEntr:= ""
...
cTipB   := CTIPO[1]
cSerie  := CTIPO[3] //PARAMIXB[3]
cNota   := CTIPO[4] //PARAMIXB[4]
cClieFor:= CTIPO[5] //PARAMIXB[5]
cLoja   := CTIPO[6] //PARAMIXB[6]
		...
		Else
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				IF SM0->M0_CODIGO = "01" .AND. SM0->M0_CODFIL = "02"
					cMsgI	:= LEFT("I - DOCUMENTO EMITIDO POR ME OU EPP OPTANTE PELO SIMPLES NACIONAL"+SPACE(125),125)
					cMsgII 	:= LEFT("II - NAO GERA DIREITO A CREDITO FISCAL DE ISS E DE IPI."+SPACE(125),125)
					If cCFOP == "5101 " .OR. cCFOP == "6101 "
						_aArea := GetArea()
						cMes := substr(DTOC(SF2->F2_EMISSAO),4,2)
						cAno := substr(DTOC(SF2->F2_EMISSAO),7,4)
						nValbrut := SF2->F2_VALBRUT
						DbSelectArea("SZA")
						DbSetOrder(1)
						DbSeek(xFilial("SZA")+cMes+"20"+cAno)
						nAliq	:= SZA->ZA_ALIQMES
						nVal	:= nValbrut*nAliq/100
						
						cMsgICM := "PERMITE O APROVEITAMENTO DO CREDITO DE ICMS NO VALOR DE R$ "+Transform(Val(STR(nVal)),"@ze 999,999.99")+", CORRESPONDENTE A ALIQUOTA DE " +Transform(Val(STR(nAliq)),"@ze 999.99")+"% NOS TERMOS DO ART 23 DA LEI COMPLEMENTAR 123/06"
						If !AllTrim(cMsgICM) $ cMensCli
							cMensCli += LEFT(cMsgICM+SPACE(250),250)
						EndIf
						RestArea(_aArea)
					EndIf
					
					If !AllTrim(cMsgI) $ cMensCli
						cMensCli += cMsgI
					EndIf
					If !AllTrim(cMsgII) $ cMensCli
						cMensCli += cMsgII
					EndIf
				EndIf
				
				If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
					cMensCli += LEFT(SC5->C5_MENNOTA+SPACE(240),240)
				EndIf
				// mensagem de retorno de remessa
				cMsDev := ""
				cMsDev := IIF(EMPTY(SC6->C6_NFORI),"",LEFT((("RETORNO DA SUA NF/SERIE/ITEM:"+SC6->C6_NFORI+"/ "+SC6->C6_SERIORI+"/"+SC6->C6_ITEMORI))+"DE "+DTOC(GETADVFVAL("SF1","F1_EMISSAO",XFILIAL("SF1")+SC6->C6_NFORI+SC6->C6_SERIORI,1,CTOD("")))+SPACE(100),100))//SPACE(250),250))
				If !Empty(cMsDev) .And. !AllTrim(cMsDev) $ cMensCli
					cMensCli += ALLTRIM(cMsDev)
				EndIf
				...
				IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)+IIF(EMPTY(SB1->B1_LCODCLI),""," "+(SB1->B1_LCODCLI))+IIF(EMPTY(SC6->C6_PEDCLI),""," "+"PC:"+(SC6->C6_PEDCLI)),;
				SB1->B1_POSIPI,;
				SB1->B1_EX_NCM,;
				(cAliasSD2)->D2_CF,;
				SB1->B1_UM,;
		...
		Else
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1
					...
					D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_DESCPRO
					FROM %Table:SD1% SD1
					WHERE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !Empty((cAliasSD1)->D1_TES) .And. !(cAliasSD1)->D1_TES $ cMsgEntr
				...
				(cAliasSD1)->D1_COD,;
				IIf(Val(SB1->B1_CODBAR)==0,"",Str(Val(SB1->B1_CODBAR),Len(SB1->B1_CODBAR),0)),;
				IIF(alltrim((cAliasSD1)->D1_CF)=="1604",(cAliasSD1)->D1_DESCPRO+" ("+EXTENSO((cAliasSD1)->D1_VALICM)+")",(cAliasSD1)->D1_DESCPRO),;
				SB1->B1_POSIPI,;
				SB1->B1_EX_NCM,;
				(cAliasSD1)->D1_CF,;
				SB1->B1_UM,;
...
If Len(aExp)>0 .And. !Empty(aExp[01])
	cString += '<exporta>'
	cString += '<UFEmbarq>'+ConvType(aExp[01])+ '</UFEmbarq>'
	cString += '<locembarq>'+ConvType(aExp[02])+ '</locembarq>'
	cString += '</exporta>'
EndIf
