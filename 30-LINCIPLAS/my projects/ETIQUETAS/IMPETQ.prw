...
For _nI := 1  To  Len(AllTrim(MV_PAR01))
	If _lDoc
		If Substr(AllTrim(MV_PAR01),_nI,1) <> "-"
			_cDoc := Substr(MV_PAR01,1,9) //+Substr(AllTrim(MV_PAR01),_nI-2,2)
			If !(Substr(_cDoc,2,8)+Substr(AllTrim(MV_PAR01),_nI,1) == "0")
				_cDoc01 := Substr(_cDoc01,1,9)+Substr(AllTrim(MV_PAR01),_nI-2,2)
			EndIf
		Else
			_lDoc := .F.
		EndIf
	Else
		_cSerie += Substr(AllTrim(MV_PAR01),_nI-2,2)
		If !(Substr(AllTrim(MV_PAR01),_nI,1) == "0")
			_cSer01 += Substr(AllTrim(MV_PAR01),_nI-2,2)
		EndIf
	EndIf
Next
...
SF2->(dbGoTop())
SF2->(dbSeek(xFilial("SF2")+_cDoc+_cSerie,.T.))

While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE == _cDoc+_cSerie
