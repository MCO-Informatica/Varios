...
Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE == _cDoc+_cSerie
	...
	Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA == SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA
        ...
        If !Empty(MV_PAR06)
		    If !(AllTrim(MV_PAR06) == AllTrim(SD1->D1_ITEM))
		        dbSkip()
		        Loop
		    EndIf
		EndIf
...
//				MSCBSAY(018,004,"H.SILVA INJECAO TERMOPLAS - LINCIPLAS" ,"N","B","019")
			...
			MSCBSAYBAR(015,014,"00"+SA1->A1_XCODSA2,"MB07","C",6.76,.F.,.F.,.F.,,2.5,1)
			...
			MSCBSAYBAR(015,029,"N"+AllTrim(_cDoc01)+"-"+AllTrim(_cSer01),"MB07","C",6.36,.F.,.F.,.F.,,3,2)
			...
			MSCBSAYBAR(015,052,"P"+AllTrim(_cLCODCLI),"MB07","C",6.36,.F.,.F.,.F.,,3,2)
...
//			MSCBSAY(038,072,STRZERO(_nQtdVen,5)		        	,"N","B","023")
//			MSCBSAYBAR(015,076,STRZERO(_nQtdVen,5),"MB07","C",7.36,.F.,.F.,.F.,,2,1)
			MSCBSAY(038,072,STR(_nQtdVen)		             	,"N","B","023")
			MSCBSAYBAR(015,076,"Q"+ALLTRIM(STR(_nQtdVen)),"MB07","C",7.36,.F.,.F.,.F.,,2,1)
			...
			MSCBSAYBAR(015,091,"S"+AllTrim(SA1->A1_XCODSA2)+_cA1NumSer,"MB07","C",6.36,.F.,.F.,.F.,,3,2)
			//
			MSCBWRITE("^FO544,64")
