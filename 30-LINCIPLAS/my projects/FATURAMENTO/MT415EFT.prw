...
If lFil02
	...
	While SCK->(!eof()) .and. cNumOrc == SCK->CK_NUM
		...
		SCK->(DbSkip())
	END
	alert("Faturamento do orçamento "+cNumOrc+" será pela filial 02!")
Else
	DbSelectArea("SCK")
	dbGotop()
	DbSeek(xFilial("SCK")+cNumOrc)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³MTA416BX  ºAutor  ³Rodrigo Okamoto     º Data ³  29/03/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Efetuar validação na efetivação do orçamento (Automático)  º±±
±±º          ³ MTA416BX - Automático                                      º±±
±±º          ³ MT415EFT - Manual                                          º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP                                                        º±±
...
While SCK->(!eof()) .and. cNumOrc == SCK->CK_NUM
	...
	SCK->(DbSkip())
END
//Caso tiver 1 item que seja SPIDER, todos os itens serão faturados pela filial 02
If lFil02
	DbSelectArea("SCK")
	dbGotop()
	DbSeek(xFilial("SCK")+cNumOrc)
