		...
		Else
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_CLASFIS,D2_PRCVEN,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,D2_VALCF3 %Exp:cField%
						FROM %Table:SD2% SD2
						WHERE
...
//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)+IIF(EMPTY(SB1->B1_LCODCLI),""," "+(SB1->B1_LCODCLI))+IIF(EMPTY(SC6->C6_PEDCLI),""," "+"PC:"+(SC6->C6_PEDCLI)),;  //IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI),;
					...
					(cAliasSD2)->D2_NUMLOTE,;//Numero do Lote
				   	(cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALPS3 + (cAliasSD2)->D2_VALCF3,;//Outras despesas + PISST + COFINSST  (Inclus„o do valor de PIS ST e COFINS ST na tag vOutros - NT 2011/004)
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
					SELECT D1_FILIAL,D1_DOC,D1_SERIE,D1_FORNECE,D1_LOJA,D1_COD,D1_ITEM,D1_TES,D1_TIPO,D1_NFORI,D1_SERIORI,D1_ITEMORI,D1_CF,D1_QUANT,D1_TOTAL,D1_VALDESC,D1_VALFRE,D1_SEGURO,D1_DESPESA,D1_CODISS,D1_VALISS,D1_VALIPI,D1_ICMSRET,D1_VUNIT,D1_CLASFIS,D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_LOTECTL,D1_NUMLOTE,D1_CUSTO,D1_ORIGLAN,D1_DESCICM,D1_II,D1_FORMUL,D1_VALPS3,D1_VALCF3,D1_DESCPRO%Exp:cField%
					FROM %Table:SD1% SD1
					WHERE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				// ADICIONADO DESCRI«√O DO REFERENTE AO CR…DITO DE CIAP - THIAGO 17/05/2011
				...
				aadd(aProd,	{Len(aProd)+1,;  
					(cAliasSD1)->D1_COD,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					IIF(!SUBSTR((cAliasSD1)->D1_CF,2,3)=="604",(cAliasSD1)->D1_DESCPRO,ALLTRIM((cAliasSD1)->D1_DESCPRO)+" ("+EXTENSO((cAliasSD1)->D1_TOTAL)+")"),; //SB1->B1_DESC,;
					SB1->B1_POSIPI,;
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
...
If !Empty(aNota)
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI)
	cString += "</infNFe>"
EndIf
...
If cVerAmb== "2.00"
	cString += '<vUnCom>'+ConvType(aProd[16],21,5)+'</vUnCom>'	
Else                                       
...
If cVerAmb== "2.00"
	cString += '<vUnTrib>'+ConvType(aProd[10]/aProd[12],21,5)+'</vUnTrib>'		
Else
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI)
...
EndIf
For nX=1  to Len(aNfVinc)
	If !aNfVinc[nX][7] $ cChvNFe  
		cChvNFe+=aNfVinc[nX][7]+"|" 
