#INCLUDE "PROTHEUS.CH"   					// Data Totvs 20/06/2018
#INCLUDE "COLORS.CH"						// Data Linciplas 30/07/2018
#INCLUDE "TBICONN.CH" 
...
//	LINCIPLAS   		cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI) 
						cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)+IIF(EMPTY(SB1->B1_LCODCLI),""," "+(SB1->B1_LCODCLI))+IIF(EMPTY(SC6->C6_PEDCLI),""," "+"PC:"+(SC6->C6_PEDCLI))
...
If !Empty(aNota)
	...
	IF cVeramb >= "4.00"
       ...
       //If aScan( aDetPag,{ |x|x[1] == "14"} ) > 0			
		...
		EndIf
//  LINCIPLAS			cString += NfeCob(aDupl,aFat,(Alltrim(cSerie)+ Alltrim(cNota)))
		//EndIf
