...
//variavel para mensagem na nota fiscal de entrada - LINCIPLAS
Private cForm   := ""
Private cMsgEntr:= ""
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)+IIF(EMPTY(SB1->B1_LCODCLI),""," "+(SB1->B1_LCODCLI))+IIF(EMPTY(SC6->C6_PEDCLI),""," "+"PC:"+(SC6->C6_PEDCLI))  //IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI),;
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							IIF(!(cAliasSD2)->D2_TIPO$"IP",Round( ( (cAliasSD2)->(D2_PRCVEN*D2_QUANT)+nDesconto+(cAliasSD2)->D2_DESCZFR )/(cAliasSD2)->D2_QUANT,8),0 ),; //// //Alterado - Sidney 0,;// O valor unitario sera obtido pela divisao do valor do produto pela quantidade comercial de acordo com o  Manual do Contribuinte 6.00 realizado na tag <vUnCom>(ConvType(aProd[10]/aProd[09],21,8)) 
							...
							IIF((SF4->F4_AGREG='N' .And. !AllTrim(SF4->F4_CF) $ cMVCfopTran) .Or. (SF4->F4_ISS='S' .And. SF4->F4_ICM='N'),"0","1"),;// Tipo de agregação de valor ao total do documento aProd[24]
							cInfAdic,;//Informacoes adicionais do produto(B5_DESCNFE) - aProd[25]
							nDescZF,;              //aProd[26]
							(cAliasSD2)->D2_TES,; //aProd[27]
							IIF(SB5->(FieldPos("B5_PROTCON"))<>0,SB5->B5_PROTCON,""),; //Campo criado para informar protocolo ou convenio ICMS  //aProd[28]
							IIf(SubStr(SM0->M0_CODMUN,1,2) == "35" .And. cTpPessoa == "EP" .And. nDescIcm > 0, nDescIcm,0),; //aProd[29]   
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
			   ...
			   EndIF
				...
				// MENSAGEM NA NOTA FISCAL DE ENTRADA PARA CREDITO ATIVO IMOBILIZADO - RODRIGO 05/01/2010				 
				// ADICIONADO DESCRIÇÃO DO REFERENTE AO CRÉDITO DE CIAP - THIAGO 17/05/2011
				cForm    := Posicione("SF4",1,xFilial("SF4")+(cAliasSD1)->D1_TES,"F4_FORMULA")
				cMsgEntr := Alltrim(Posicione("SM4",1,xFilial("SM4")+cForm,"M4_FORMULA")) 
				If !Empty(cMsgEntr) .And. !cMsgEntr $ cMensFis                               
					cMensFis += cMsgEntr
				EndIf
				
				// POR ALGUM MOTIVO NAO ESTA ENCONTRANDO O CAMPO (cAliasSD1)->D1_DESCPRO NO DICIONARIO DE DADOS, TROQUEI PELA FUNCAO POSICIONE() - THIAGO 09/08/2012
				cDescProdSD1	:= Posicione("SD1",1,xFilial("SD1")+(cAliasSD1)->D1_DOC,"D1_DESCPRO")
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					cCodProd,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					IIF(!SUBSTR((cAliasSD1)->D1_CF,2,3)=="604",cDescProdSD1,ALLTRIM(cDescProdSD1)+" ("+EXTENSO((cAliasSD1)->D1_TOTAL)+")"),; //SB1->B1_DESC,;
					SB1->B1_POSIPI,;
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde)
	...
	Local cIndicador	:= ""
	Local nX	        := 0
	Local cMailConf		:= GETMV("ST_MAILCFM") //Especifico LINCIPLAS	
	cVerAmb     := PARAMIXB[2] 
	...
	EndIf
	if !lBrinde
		cString += NfeTag('<EMAIL>',AllTrim(aDest[16])+cMailTrans+cMailConf)   //Especifico LINCIPLAS
	endif
...
//SIDNEY - SUPERTECH -> Arredondamento de casas decimais cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],21,8))+'</vUnCom>'
//cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(round(aProd[10]/aProd[09],4),21,8))+'</vUnCom>'  
//cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],21,8))+'</vUnCom>'
cString += '<vUnCom>'+IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[16],16,5))+'</vUnCom>'
// THIAGO - SUPERTECH -> Aumento das casas decimais de 2 para 5 casas
cString += '<vProd>' +ConvType(aProd[10],15,5)+'</vProd>'
