...
#DEFINE MAXITEMC  031                                                // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN 115                                                // Máximo de caracteres por linha de dados adicionais 125
...
Local lExistNfe := .F.
Local cDir := "C:\" 
Local cFilePrint:= cCliente := cNumNota := cSerieNF := cTransp := ""
Local QtdNotas:= 0
Local oDanfe      
local oDanfe1
...
if lExistNfe
	...
	QtdNotas:= (val(MV_PAR02)-VAL(MV_PAR01))+1
	cNumNota:= MV_PAR01
	cSerieNF:=MV_PAR03 
	
   /*	lFormaTMSPrinter := .T.
	nUsaPDF := 6
	oDanfe1 := FWMSPrinter():New(cNumNota,nUsaPDF,lFormaTMSPrinter,cDir, .T.)   
	oDanfe1	:= oDanfe
	oDanfe1:Print() */
	...
	oDanfe1	:= oDanfe
	oDanfe1 := FWMSPrinter():New("Danfe.rel", 6, lAdjustToLegacy, cDir, lDisableSetup,,oDanfe1,,.T.)
	//FWMsPrinter(): New ( < cFilePrintert >, [ nDevice], [ lAdjustToLegacy], [ cPathInServer], [ lDisabeSetup ], [ lTReport], [ @oPrintSetup], [ cPrinter], [ lServer] )
	
	
	// Ordem obrigátoria de configuração do relatório
	oDanfe1:SetResolution(72)
	oDanfe1:SetPortrait()
	oDanfe1:SetPaperSize(DMPAPER_A4) 
   //	oDanfe1:SetMargin(60,60,60,60) // nEsquerda, nSuperior, nDireita, nInferior 
	//oDanfe1:cPathPDF := "c:\" // Caso seja utilizada impressão em IMP_PDF
	...
	For X:=1 To QtdNotas
		DbSelectArea("SF2")
		DbSetOrder(1)
		IF (Dbseek(xFilial("SF2")+cNumNota+cSerieNF))
			cCliente:= SF2->F2_CLIENTE
			cTransp	:= SF2->F2_TRANSP	
			cFilePrint := cNumNota 
			U_MAILNFE(cDir,cFilePrint,cCliente,cNumNota,cSerieNF,cTransp)
		EndIF
		cNumNota := STR(val(cNumNota)+1	)
	Next X
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,lExistNfe)
...
Local aTotais       := {}
Local aAux          := {}
Local cPontilhado   := ""     // Usada para escrever o pontilhado entre produtos/serviços
Local nHPage        := 0 
Local nVPage        := 0 
Local nPosV         := 0
Local nPosVOld      := 0
...
nLenDet := Len(oDet)
/*For nX := 1 To 300
	AAdd(aItens, {"000000000000015", "descrição de teste", "", "", "", "", "", "", "", "", "", "", "", ""})
Next nX*/
For nX := 1 To nLenDet
	nPrivate := nX
...
/*For nX := 1 to 200
	cAux := "esta mensagem é de testes para sabermos o quando ´ impresso nas mensagens da nota"
	While !Empty(cAux)
		aadd(aMensagem,SubStr(cAux,1,MAXMENLIN))
		cAux := SubStr(cAux,MAXMENLIN+1)
	EndDo
...
aTamanho := ImpBox(nPosV,nHPage-Char2Pix(oDanfe,Repl("X",20),oFont08),0,nHPage,;
	{	{{"DATA DE EMISSÃO",ConvDate(oIdent:_DEmi:TEXT)}},;
		{{"DATA ENTRADA/SAÍDA",ConvDate(aDest[4])}},;
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
/*aTamanho := ImpBox(nPosV,0,0,nHPage,;
	{	{{"RAZÃO SOCIAL",aTransp[01]},{"FRETE POR CONTA","0-emitente/1-Destinatário [" + aTransp[02] + "]"},{"CÓDIGO ANTT",aTransp[03]},{"PLACA DO VEÍCULO",aTransp[04]},{"UF",aTransp[05]},{"CNPJ/CPF",aTransp[06]}},;
...
aTamanho := ImpBox(nPosV,0,0,nHPage,;
	{	{{"RAZÃO SOCIAL",aTransp[01]},{"FRETE POR CONTA","0-EMITENTE/1-DESTINATARIO "},{"CÓDIGO ANTT",aTransp[03]},{"PLACA DO VEÍCULO",aTransp[04]},{"UF",aTransp[05]},{"CNPJ/CPF",aTransp[06]}},;
		{{"ENDEREÇO",aTransp[07]},{"MUNICIPIO",aTransp[08]},{"UF",aTransp[09]},{"INSCRIÇÃO ESTADUAL",aTransp[10]}},;
		{{"QUANTIDADE",aTransp[11]},{"ESPECIE",aTransp[12]},{"MARCA",aTransp[13]},{"NUMERAÇÃO",aTransp[14]},{"PESO BRUTO",aTransp[15]},{"PESO LIQUIDO",aTransp[16]}}},;
	oDanfe,1,"TRANSPORTADOR/VOLUMES TRANSPORTADOS")

	oDanfe:Box(nPosV+40,nAuxH+850 ,nPosV+80,nAuxH+890)
	oDanfe:Say(nPosV+45,nAuxH+860,aTransp[02],oFont08:oFont)
...
// Monta o pontilhado
/*If SubStr(GetBuild(), 1, 12) > "7.00.081215P"    // Evita retrabalho ao sair build novo com correção no GetTextWidth()
	cPontilhado := Replicate(".", Int((nHPage - HMARGEM) / oDanfe:GetTextWidth(".", oFont08:oFont)))
EndIf
If Len(aAux) > 0
	If Len(aAux[1]) > 1
		If Len(aAux[1][2]) > 3
			...
			For nX := 3 To Len(aAux[1][2])
				...
				If !Empty(aAux[1][1][nX]) .And. Empty(aAux[1][1][nX - 1])
					...
					//If SubStr(GetBuild(), 1, 12) > "7.00.081215P"    // Evita retrabalho ao sair build novo com correção no GetTextWidth()
					  //	oDanfe:Say(nAuxV, HMARGEM, cPontilhado, oFont08:oFont)
					//Else    // Build 081215P ou anterior usará outro algoritmo para escrever o pontilhado
						For nY := 0 To nY < (nHPage - HMARGEM)
...
nPosH    := 1128
	...
	IF SM0->M0_CODIGO = "01" .AND. SM0->M0_CODFIL = "01"
	oDanfe:SayBitmap(nPosVOld+5,nPosHOld+5,cLogo,178,250)//300,090)	
	ELSE
...
If lItens
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	// Monta o pontilhado
	If SubStr(GetBuild(), 1, 12) > "7.00.081215P"    //StrCmp(SubStr(cBuild, 1, 12), "7.00.090818P") >= 0    // Evita retrabalho ao sair build novo com correção no GetTextWidth()
		cPontilhado := Replicate(".", Int((nHPage - HMARGEM) / oDanfe:GetTextWidth(".", oFont08:oFont)))
	EndIf
	If Len(aAux) > 0
		If Len(aAux[1]) > 1
...
If oWS:RETORNANOTASNX()
	If Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5) > 0
		For nX := 1 To Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5)
			...
			//Tratamento para gravar a hora da transmissao da NFe - alterado pelo Rafael 07.12.2010
			/*If !Empty(cProtocolo)
			 	oDHRecbto		:= XmlParser(cDHRecbto,"","","")
			    cDtHrRec		:= "  "//oDHRecbto:_ProtNFE:_INFPROT:_oDHRECBTO:TEXT
	...
	For nY := 1 To nMaxnY
		...
		If Valtype(aImp[nX][nY]) == "A"
			...
			For nZ := 1 To nMaxnZ									
				nAuxV += Char2PixV(oDanfe,"X",aFont[nTpFont][IIf(nZ==1,1,2)])+IIF(nZ>1,SAYVSPACE*nTpFont,-1*SAYVSPACE)
				If aAlign[nY] == "R"
					oDanfe:Say(nAuxV,nAuxH-oDanfe:GetTextWidth(aImp[nX][nY][nZ], aFont[nTpFont][IIf(nZ==1,1,2)]:oFont)/*Char2Pix(oDanfe,aImp[nX][nY][nZ],aFont[nTpFont][2])*/,aImp[nX][nY][nZ],aFont[nTpFont][IIf(nZ==1,1,2)]:oFont)
				Else
