...
While !Eof() .and. SE1->E1_NUM >= cTitIni .and. SE1->E1_NUM <= cTitFim .and. ;
	...
	// VERIFICA SE O TITULO/NF FOI FATURADO DEPOIS DO DIA 15/02/10, CASO NÃO TENHA SIDO UTILIZA O PROGRAMA COM A CONTA ANTIGA
	...
	DbSelectArea("SE1")
	DbSkip()
	//ENDIF // -> ENDIF DO IF QUE BLOQUEIA A IMPRESSÃO DO BOLETO CASO NÃO ESTEJA COM E1_PORTADO == "341" (ITAU)
EndDO
//EndIf
//Endif
...
oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox+25,_nTxtBox+0130,"Banco Itaú SA",ofont14B,100)
...
// Box Logotipo
//oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox,_nPosVer+1700,"Banco Itaú SA",ofont30B,100)
...
//oprn:say(_nPosHor+((_nLinha-1)*_nEspLin),_nPosVer+0010,"-Pague este título nas Agências ITAU (ou através do Sistema Integrado de Compensação)",ofont09B,100)
oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox,_nPosVer+0010,"Evite constrangimentos! Efetue o pagamento deste boleto até o vencimento",ofont09B,100)
...
oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox,_nPosVer+0010,"Caso o pagamento não seja realizado, será enviado ao SERASA para cobrança",ofont09B,100)
...
oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox,_nPosVer+0010,"Evite despesas adicionais com cobranças judiciais.",ofont09B,100)
...
oPrn:Box(_nPosHor+((_nLinha-1)*_nEspLin),_nPosVer+1650,_nPosHor+(_nLinha*_nEspLin),_nPosVer+2230)
oPrn:Say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox,_nPosVer+1660,"5(+) Outros Acréscimos",ofont08,100)
//oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox+10,_nPosVer+0010,"APOS O VENCIMENTO, SERÁ COBRADO UMA MULTA DE 2,00 % SOBRE O VALOR DO BOLETO ",ofont09B,100)
oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox    ,_nPosVer+0010,"APOS O VENCIMENTO, SERÁ COBRADO UMA MULTA DE 2,00 % SOBRE O VALOR DO BOLETO ",ofont09B,150)
oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox+33 ,_nPosVer+0010,"MAIS O VALOR DIÁRIO DE MORA.",ofont09B,150)

// Box Valor cobrado
_nLinha  += 1
oPrn:Box(_nPosHor+((_nLinha-1)*_nEspLin),_nPosVer+1650,_nPosHor+(_nLinha*_nEspLin),_nPosVer+2230)
oPrn:Say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox,_nPosVer+1660,"6(=) Valor cobrado",ofont08,100)
//oprn:say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox+10,_nPosVer+0010,"MAIS O VALOR DIÁRIO DE MORA.",ofont09B,100)
// Box Sacado
_nLinha  += 1
oPrn:Box(_nPosHor+((_nLinha-1)*_nEspLin),_nPosVer,_nPosHor+(_nLinha*_nEspLin)+_nEspLin,_nPosVer+2230)
oPrn:Say(_nPosHor+((_nLinha-1)*_nEspLin)+_nTxtBox,_nPosVer+0010,"Sacado:",ofont08,100)
...
±±ºFuncao    ³ CISFT073Bº Autor ³ Thiago Queiroz     º Data ³  11/05/03   º±±
...
Next _nI
//-------------------------------------------------------------
//--> a agencia já está sem o digito --------------------------
//-------------------------------------------------------------
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³ CISFT073Cº Autor ³ Thiago Queiroz     º Data ³  11/05/03   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³ CISFT073Dº Autor ³ Thiago Queiroz     º Data ³  11/05/03   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³ CISFT073Eº Autor ³ Thiago Queiroz     º Data ³  11/05/03   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
