...
IF lPerg
	...
	Processa( {|| U_LPPVVEN(MV_PAR01 ,MV_PAR02) }, "Aguarde...", "Processando Pedidos de Venda . . . "				,.F. )
	Processa( {|| U_LPPVFAT(MV_PAR01 ,MV_PAR02) }, "Aguarde...", "Processando Notas Fiscais . . . "					,.F. )
	Processa( {|| U_LPOPVFAT(MV_PAR01,MV_PAR02) }, "Aguarde...", "Processando Pedidos de Venda - Tipo B . . . "		,.F. )
	Processa( {|| U_LPONFFAT(MV_PAR01,MV_PAR02) }, "Aguarde...", "Processando Notas Fiscais - Tipo B. . . "			,.F. )
	...
	For x:=1 to len(aLinhas)
		For y:=1 to len(aColunas)
			IF X < 13
				IF Y == 1 		// 1 BRINDE PV
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPBrinde	(y,val(aLinhas[x][2])) 		, aColunas[y][1] })
					nDBrinde	+= U_LPBrinde(y,val(aLinhas[x][2]))
				ELSEIF Y == 2 	// 2 CAPACETES VENDIDOS PV
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPPagoPV	(y,val(aLinhas[x][2])) 		, aColunas[y][1] })
					nDPagoPV	+= U_LPPagoPV(y,val(aLinhas[x][2]))
					NTVLRPV		:= NTVLRPV + U_LPPagoPV(y,val(aLinhas[x][2]))
				ELSEIF Y == 3 	// 3 CAPACETES QTD PV
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPQtdPV	(y,val(aLinhas[x][2])) 		, aColunas[y][1] })
					nDQtdPV 	+= U_LPQtdPV(y,val(aLinhas[x][2]))
					nTQtdPV 	:= nTQtdPV + U_LPQtdPV(y,val(aLinhas[x][2]))
				ELSEIF Y == 4 	// 4 ACESSORIOS VENDIDOS PV
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPPgAssPV(y,val(aLinhas[x][2]))		, aColunas[y][1] })
					nDPgAssPV 	+= U_LPPgAssPV(y,val(aLinhas[x][2]))
					nTVlrAPV 	:= nTVlrAPV + U_LPPgAssPV(y,val(aLinhas[x][2]))
				ELSEIF Y == 5 	// 5 ACESSORIOS QTD PV
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPQTDAPV(y,val(aLinhas[x][2])) 		, aColunas[y][1] })
					nDQtdAssPV	+= U_LPQTDAPV(y,val(aLinhas[x][2]))
					nTQtdAPV 	:= nTQtdAPV + U_LPQTDAPV(y,val(aLinhas[x][2]))
					// PEDIDOS DE VENDA - OUTROS		//
				ELSEIF Y == 06 	// 11 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOPGPV(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nOPGPV 		+= U_LPOPGPV(y,val(aLinhas[x][2]))
					NTVLRPV		:= NTVLRPV + U_LPOPGPV(y,val(aLinhas[x][2]))
				ELSEIF Y == 07 	// 12 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOQtdPV(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nOQTDPV 	+= U_LPOQtdPV(y,val(aLinhas[x][2]))
					nTQtdPV 	:= nTQtdPV + U_LPOQtdPV(y,val(aLinhas[x][2]))
				ELSEIF Y == 08 	// 13 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOAPGPV(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nOAPGPV 	+= U_LPOAPGPV(y,val(aLinhas[x][2]))
					nTVlrAPV 	:= nTVlrAPV + U_LPOAPGPV(y,val(aLinhas[x][2]))
				ELSEIF Y == 09 	// 14 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOAQtdPV(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nTQtdAPV 	+= U_LPOAQtdPV(y,val(aLinhas[x][2]))
					nTQtdAPV 	:= nTQtdAPV + U_LPOAQtdPV(y,val(aLinhas[x][2]))
				...
				ELSEIF Y == 16 	// 6 BRINDE NF
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPNFBrinde(y,val(aLinhas[x][2])) 		, aColunas[y][1] })
					nNFDBrinde 	+= U_LPNFBrinde(y,val(aLinhas[x][2]))
				ELSEIF Y == 17 	// 7 CAPACETES NOTA FISCAL
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPPagoNF(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nDPagoNF	+= U_LPPagoNF(y,val(aLinhas[x][2]))
				ELSEIF Y == 18 	// 8 CAPACETES QTD NF
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPQtdNF(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nDQtdNF 	+= U_LPQtdNF(y,val(aLinhas[x][2]))
				ELSEIF Y == 19 	// 9 ACESSORIOS NOTA FISCAL
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPpgAssNF(y,val(aLinhas[x][2]))		, aColunas[y][1] })
					nDPgAssNF 	+= U_LPPgAssNF(y,val(aLinhas[x][2]))
				ELSEIF Y == 20 	// 10 ACESSORIOS QTD NF
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPQTDANF(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nDQtdAssNF 	+= U_LPQTDANF(y,val(aLinhas[x][2]))
					//////////////////
					// FATURADOS - NF				//
				ELSEIF Y == 21 	// 15 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOPGNF(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nOPGNF 		+= U_LPOPGNF(y,val(aLinhas[x][2]))
				ELSEIF Y == 22 	// 16 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOQtdNF(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nOQTDNF 	+= U_LPOQtdNF(y,val(aLinhas[x][2]))
				ELSEIF Y == 23 	// 17 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOAPGNF(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nOAPGNF 	+= U_LPOAPGNF(y,val(aLinhas[x][2]))
				ELSEIF Y == 24 	// 18 OUTROS
					aadd(aIndic, {alltrim(aLinhas[x][1])	, U_LPOAQtdNF(y,val(aLinhas[x][2]))			, aColunas[y][1] })
					nOAQTDNF 	+= U_LPOAQtdNF(y,val(aLinhas[x][2]))
	...
	U_LPMDW(aLinhas,aColunas,aIndic,"Gerencial","Gerencial Anual Spider - Sintético",0)
	...
	For m:=1 To 12
		For x:=1 to len(aLinhas)
			For y:=1 to len(aColunas)
				IF X < 32
					IF Y == 1 		// BRINDE PV
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDBrinde		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDBrinde 	+= U_LPDBrinde(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 2	// VALOR CAPACETES PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDPagoPV		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDPagoPV 	+= U_LPDPagoPV(y,val(aLinhas[x][1]),m)
						NTVLRPV 	:= NTVLRPV + U_LPDPagoPV(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 3 	// QUANTIDADE CAPACETES PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDQtdPV		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDQtdPV 	+= U_LPDQtdPV(y,val(aLinhas[x][1]),m)
						NTQTDPV 	:= NTQTDPV + U_LPDQtdPV(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 4 	// VALOR ACESSORIOS PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDPGAPV		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDPgAssPV 	+= U_LPDPGAPV(y,val(aLinhas[x][1]),m)
						NTVLRAPV 	:= NTVLRAPV + U_LPDPGAPV(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 5 	// QUANTIDADE ACESSORIOS PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDQtdAPV	(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDQtdAssPV 	+= U_LPDQtdAPV(y,val(aLinhas[x][1]),m)
						NTQTDAPV 	:= NTQTDAPV + U_LPDQtdAPV(y,val(aLinhas[x][1]),m)
						// OUTROS
					ELSEIF Y == 6 	// VALOR CAPACETES PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODPGPV		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOPGPV 	+= U_LPODPGPV(y,val(aLinhas[x][1]),m)
						NTVLRPV 	:= NTVLRPV + U_LPODPGPV(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 7 	// QUANTIDADE CAPACETES PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODqtdPV		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOqtdPV 	+= U_LPODQtdPV(y,val(aLinhas[x][1]),m)
						NTQTDPV 	:= NTQTDPV + U_LPODQtdPV(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 8 	// VALOR ACESSORIOS PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODAPGPV		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOAPGPV 	+= U_LPODAPGPV(y,val(aLinhas[x][1]),m)
						NTVLRAPV 	:= NTVLRAPV + U_LPODAPGPV(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 9 	// QUANTIDADE ACESSORIOS PEDIDO DE VENDA
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODAQPV		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOAqtdPV 	+= U_LPODAQPV(y,val(aLinhas[x][1]),m)
						NTQTDAPV 	:= NTQTDAPV + U_LPODAQPV(y,val(aLinhas[x][1]),m)
					...
					ELSEIF Y == 16 	// BRINDE NF
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPNFDBrinde	(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nNFDBrinde 	+= U_LPNFDBrinde(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 17 	// VALOR CAPACETES NOTA FISCAL
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDPagoNF		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDPagoNF 	+= U_LPDPagoNF(y,val(aLinhas[x][1]),m)
						nTVlrNF 	:= nTVlrNF + U_LPDPagoNF(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 18 	// QUANTIDADE CAPACETES NOTA FISCAL
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDQtdNF		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDQtdNF 	+= U_LPDQtdNF(y,val(aLinhas[x][1]),m)
						nTQtdNF 	:= nTQtdNF + U_LPDQtdNF(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 19 	// VALOR ACESSORIOS NOTA FISCAL
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDPGANF		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDPgAssNF 	+= U_LPDPGANF(y,val(aLinhas[x][1]),m)
						nTVlrANF 	:= nTVlrANF + U_LPDPGANF(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 20 	// QUANTIDADE ACESSORIOS NOTA FISCAL
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPDQtdANF	(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDQtdAssNF 	+= U_LPDQtdANF(y,val(aLinhas[x][1]),m)
						nTQtdANF 	:= nTQtdANF + U_LPDQtdANF(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 21 	// VALOR CAPACETES NOTA FISCAL OUTROS
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODPGNF		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOPGNF 	+= U_LPODPGNF(y,val(aLinhas[x][1]),m)
						nDPagoNF 	:= nDPagoNF + U_LPODPGNF(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 22 	// QUANTIDADE ACESSORIOS NOTA FISCAL OUTROS
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODqtdNF		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOqtdNF 	+= U_LPODqtdNF(y,val(aLinhas[x][1]),m)
						nTQtdNF 	:= nTQtdNF + U_LPODqtdNF(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 23 	// VALOR ACESSORIOS NOTA FISCAL OUTROS
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODAPGNF		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOAPGNF 	+= U_LPODAPGNF(y,val(aLinhas[x][1]),m)
						nTVlrANF 	:= nTVlrANF + U_LPODAPGNF(y,val(aLinhas[x][1]),m)
					ELSEIF Y == 24 	// QUANTIDADE CAPACETES NOTA FISCAL OUTROS
						aadd(aIndic, {alltrim(aLinhas[x][1])+"_"+alltrim(mesextenso(str(m))) , U_LPODAQNF		(y,val(aLinhas[x][1]),m)	, aColunas[y][1] })
						nDOAqtdNF	+= U_LPODAQNF(y,val(aLinhas[x][1]),m)
						nTQtdANF	:= nTQtdANF + U_LPODAQNF(y,val(aLinhas[x][1]),m)
		...
		// Gera Dimensão (Point Of View) Html
		U_LPMDW(aLinhas,aColunas,aIndic,mesextenso(m),"Relatorio gerencial - "+mesextenso(m),0)
...
//*--------------------------------------------------*
User Function LPMDW(aLin,aCol,aInd,cArq,cTit,nLink)
// *--------------------------------------------------*
...
IF LEN(aLin) > 0
	...
	cTexto 	+= '	</tr> ' + CRLF
	// LINHA 4
	nVal1  	:= U_LPRetAtr( .T.,"C",MV_PAR01,MV_PAR02)	// BRINDES
	nVal2  	:= U_LPRetAtr( .F.,"A",MV_PAR01,MV_PAR02)	// CAPACETES 	VALOR
	nVal3  	:= U_LPRetAtr( .T.,"A",MV_PAR01,MV_PAR02)	// CAPACETES 	QTD
	nVal4  	:= U_LPRetAtr( .F.,"B",MV_PAR01,MV_PAR02)	// ACESSORIOS 	VALOR
	nVal5  	:= U_LPRetAtr( .T.,"B",MV_PAR01,MV_PAR02)	// ACESSORIOS 	QTD
	nVal6  	:= U_LPORetAtr(.F.,"A",MV_PAR01,MV_PAR02)	// CAPACETES 	VALOR 	OUTROS
	nVal7  	:= U_LPORetAtr(.T.,"A",MV_PAR01,MV_PAR02)	// CAPACETES 	QTD		OUTROS
	nVal8  	:= U_LPORetAtr(.F.,"B",MV_PAR01,MV_PAR02)	// ACESSORIOS 	VALOR 	OUTROS
	nVal9  	:= U_LPORetAtr(.T.,"B",MV_PAR01,MV_PAR02)	// ACESSORIOS 	QTD		OUTROS
	...
	cTEXTO 	+= 	'  <td style="background-color: #FF0000; border-style: solid; border-width: 1" width="200" align="center"><font face="Arial" size="2" color="#000000"><a href="' + ''+'">'+IIF(0>0,TRANSFORM(U_LPRetAtr(.T.,"C",MV_PAR01,MV_PAR02),"@E 99,999,999.99"),"") +'</a></td>' + CRLF
	cTEXTO 	+= 	'  <td style="background-color: #FF0000; border-style: solid; border-width: 1" width="200" align="center"><font face="Arial" size="2" color="#000000"><a href="' + ''+'">'+IIF(0>0,TRANSFORM(U_LPRetAtr(.T.,"C",MV_PAR01,MV_PAR02),"@E 99,999,999.99"),"") +'</a></td>' + CRLF
	cTEXTO 	+= 	'  <td style="background-color: #FF0000; border-style: solid; border-width: 1" width="200" align="center"><font face="Arial" size="2" color="#000000"><a href="' + ''+'">'+IIF(0>0,TRANSFORM(U_LPRetAtr(.T.,"C",MV_PAR01,MV_PAR02),"@E 99,999,999.99"),"") +'</a></td>' + CRLF
	cTEXTO 	+= 	'  <td style="background-color: #FF0000; border-style: solid; border-width: 1" width="200" align="center"><font face="Arial" size="2" color="#000000"><a href="' + ''+'">'+IIF(0>0,TRANSFORM(U_LPRetAtr(.T.,"C",MV_PAR01,MV_PAR02),"@E 99,999,999.99"),"") +'</a></td>' + CRLF
	cTEXTO 	+= 	'  <td style="background-color: #FF0000; border-style: solid; border-width: 1" width="200" align="center"><font face="Arial" size="2" color="#000000"><a href="' + ''+'">'+IIF(0>0,TRANSFORM(U_LPRetAtr(.T.,"C",MV_PAR01,MV_PAR02),"@E 99,999,999.99"),"") +'</a></td>' + CRLF
...
Return(dRet)
// *--------------------------------------------------*
Static Function LPPictMTDW(cNat)
// *--------------------------------------------------*
