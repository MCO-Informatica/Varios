	...
	if Alltrim(_cEmlFor) <> ""
		...
		*/
		...
		while !eof() .and. SC8->C8_FILIAL = xFilial("SC8") ;
			...
			aAdd( (oHtml:ValByName( "it.descri"  )), SA5->A5_NOMPROD     )
			dbSelectArea("SB1")
			dbSetOrder(1)
		...
		// Encerra o processo anterior e inicia o de envio do email com o link do HTML
		oProcess:Free()
		//oProcess:= Nil
		...
		oProcess := TWFProcess():New("000003", "Cota็ใo de Pre็os")
		oProcess:NewTask( "Email", "\WORKFLOW\HTML\wflinkCota.htm" )
		oProcess:cSubject := "Solicita็ใo de Cota็ใo de Pre็os No " + _cC8_NUM + " LINCIPLAS"
		//oProcess:cTo:= "sidney@stch.com.br" //EH cEmlFor ษ AQUI QUE ELE MANDA O EMAIL
		oProcess:cTo:= _cEmlFor
		oProcess:oHtml:ValByName("proc_link","http://linciplas.dyndns.org:7914/messenger/emp01/cotacao/" + _cMailID + ".htm") 
		//oProcess:oHtml:ValByName("proc_link","http://192.168.0.210:7914/messenger/emp01/cotacao/" + _cMailID + ".htm")
...
if oProcess:oHtml:RetByName("Aprovacao") = "S"
	...
	//cEmailComp:='sidney@stch.com.br' //Aleterar para parametro
	cEmailComp:= SuperGetMV("ST_MAILCOM")
