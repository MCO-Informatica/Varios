...
±±ºDesc.     ³ PEDIDO DE COMPRAS ORDEM PRODUCAO (Emissao em formato Grafico)             º±±
...
Private	lEnd		:= .f.,;
		...
		If !Upper(ProcName(7)) $ 'MATA650*U_OPBN'
           	If !Pergunte(cPerg,.t.)
           		Return
           	Endif
			...
			Private	cObserv1	:= mv_par03			// 1a Linha de Observacoes
  		...
  		Else                   
  			If Empty(SC2->C2_XFORNEC)
  				MsgStop("Atenção Esta Impressão Serve Apenas Para OP´s de Beneficiamento com Fornecedor Preenchido !")
  				Return .f.
  			Endif

			Private	cNumPed  	:= SC2->C2_NUM		// Numero do Pedido de Compras
		...
		DbSelectArea('SC2')
		SC2->(DbSetOrder(1))
		If	( ! SC2->(DbSeek(xFilial('SC2') + cNumPed)) )
...
Private	_nQtdReg	:= 0,;		// Numero de registros para intruir a regua
		...
		DbSelectArea('SA2')
		SA2->(DbSetOrder(1))
		If	! SA2->(DbSeek(xFilial('SA2')+SC2->(C2_XFORNEC+C2_XLOJA)))
			Help('',1,'REGNOIS')
			Return .f.
		EndIf
		
		If MsgYesNo("Deseja Enviar Ordem de Fabricação por Email ?") 
			lEmail := .t.
		Endif
...
Private	oBrush		:= TBrush():New(,4),;
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³Monta query !³    //SC2.C2_CODPRF, 
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		cSELECT :=	'SC2.C2_FILIAL, SC2.C2_NUM, SC2.C2_EMISSAO, SC2.C2_XFORNEC, SC2.C2_XLOJA, SC2.C2_XCONDPG, SC2.C2_XCONTAC, '+;
					'SC2.C2_XPRSERV, SC2.C2_XOBSPC, SC2.C2_PRODUTO, SC2.C2_QUANT, SC2.C2_ITEM, SC2.C2_SEQUEN, SC2.C2_DATPRF, SC2.C2_UM, SC2.C2_LOCAL, SC2.C2_OBS '

		cFROM   :=	RetSqlName('SC2') + ' SC2 '

		cWHERE  :=	'SC2.D_E_L_E_T_ <>   '+CHR(39) + '*'            +CHR(39) + ' AND '+;
					'SC2.C2_FILIAL  =    '+CHR(39) + xFilial('SC2') +CHR(39) + ' AND '+;
					'SC2.C2_NUM     =    '+CHR(39) + cNumPed        +CHR(39) 

		cORDER  :=	'SC2.C2_FILIAL, SC2.C2_ITEM '
		...
		TcSetField('TRA','C2_EMISSAO','D')
		TcSetField('TRA','C2_DATPRF','D')
		...
		cTipoSC2	:= 'PC' //IIF((SC2->C2_TIPO == 1 .OR. SC2->C2_TIPO == 3),"PC","AE")
		cComprador 	:= ''//UsrFullName(SC2->C2_USER)
		cAlter	 	:= ''
		cAprov	 	:= ''

      	cObServ := ''
		While 	TRA->( ! Eof() )
				SC2->(dbSetOrder(1), dbSeek(xFilial("SC2")+TRA->C2_NUM+TRA->C2_ITEM+TRA->C2_SEQUEN))
				...
				If	( lFlag )
					...
					oPrint:Say(0730,0430,SC2->C2_XCONTAC,oFont11)
					...
					oPrint:Say(0600,2000,SC2->C2_NUM,oFont18)
					oPrint:Say(0650,2000,Dtoc(SC2->C2_EMISSAO),oFont12n)
					lFlag := .f.
				EndIf
				...
				oPrint:Say(nLinha,0120,Right(TRA->C2_ITEM,2),oFont10n)
				cCodPro := ''
				cCodPro := TRA->C2_PRODUTO
				oPrint:Say(nLinha,1230,TRA->C2_UM,oFont10)
				oPrint:Say(nLinha,1310,DtoC(TRA->C2_DATPRF),oFont10n,,,,1)
			    oPrint:Say(nLinha,1600,Alltrim(TransForm(TRA->C2_QUANT,'@E 99,999,999')),oFont10n,,,,1)

				If	( lImpPrc )
					oPrint:Say(nLinha,1700,AllTrim(TransForm(TRA->C2_XPRSERV,'@E 999,999.999999')),oFont10n,,,,1)
					oPrint:Say(nLinha,1950,AllTrim(TransForm(0,'@E 99.9')),oFont10n,,,,1)
					oPrint:Say(nLinha,2150,AllTrim(TransForm(Round(TRA->C2_XPRSERV * TRA->C2_QUANT,2),'@E 999,999.99')),oFont10n,,,,1)
				EndIf

				SB1->(dbSetOrder(1), dbSeek(xFilial("SB1")+TRA->C2_PRODUTO))
				If !SB5->(dbSetOrder(1), dbSeek(xFilial("SB5")+TRA->C2_PRODUTO))
				...
				_nValMerc 		+= Round(TRA->C2_XPRSERV * TRA->C2_QUANT,2)
				_nValIPI		+= 0.00
				_nValDesc		+= 0.00
				_nTotAcr		+= 0.00
				_nTotSeg		+= 0.00
...
/*				If	( Empty(TRA->C2_TES) )
					_nTotIcmsRet	+= TRA->C2_ICMSRET
				Else
					DbSelectArea('SF4')
					SF4->(DbSetOrder(1))
					If	SF4->(DbSeek(xFilial('SF4') + TRA->C2_TES))
  ...
  */
				...
				cObserv += AllTrim(TRA->C2_OBS)+' '+AllTrim(SC2->C2_XOBSPC)
...
//		oPrint:FillRect({nLinha,1390,nLinha+80,2300},oBrush)
        ...
        */
		...
		If !Empty(SC2->C2_XCONDPG)
			If SE4->(dbSetOrder(1), dbSeek(xFilial("SE4")+SC2->C2_XCONDPG))
...
Static Function xCabec()
		...
		oPrint:Say(0400,0770,OemToAnsi('Ordem de Fabricação'),oFont22)
Return
...
oPrint:Say(2670,0100,'NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras. ',oFont12n)
oPrint:Say(2710,0100,'Prezado Fornecedor, informamos que a partir de 01 de maio de 2017, efetuaremos os pagamentos apenas ',oFont12n)
oPrint:Say(2750,0100,'às terças e quintas-feiras, somente através de boletos bancários. Agradecemos a compreensão. ',oFont12n)
oPrint:Say(2830,0100,'***TODOS OS MATERIAIS DEVEM SER IDENTIFICADOS COM ETIQUETA APROPRIADA, CONTENDO O CÓDIGO E DESCRIÇÃO',oFont12n)
oPrint:Say(2870,0100,'DO PRODUTO (OS MESMOS QUE ESTÃO NA ORDEM DE FABRICAÇÃO OU PEDIDO DE COMPRA), QUANTIDADE, NOTA FISCAL ',oFont12n)
oPrint:Say(2910,0100,'E IDENTIFICAÇÃO DO FORNECEDOR. MATERIAL SUJEITO A DEVOLUÇÃO CASO HAJA INCONSISTÊNCIA NA IDENTIFICAÇÃO.',oFont12n)
...
Local	aRegs   := {},;
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		aAdd(aRegs,{cPerg,'01','Numero da O.F.     ?','Numero do Pedido   ?','Numero do Pedido   ?','mv_ch1','C', 6,0,0,'G','','mv_par01','','','','SC7'})
		aAdd(aRegs,{cPerg,'02','Imprime precos     ?','Imprime precos     ?','Imprime precos     ?','mv_ch2','N', 1,0,1,'C','','mv_par02',OemToAnsi('Não'),'Sim','',''})
		aAdd(aRegs,{cPerg,'03','Observações'         ,'Observações         ','Observações         ','mv_ch3','C',70,0,1,'G','','mv_par03','','','',''})
		aAdd(aRegs,{cPerg,'04','                    ','                    ','                    ','mv_ch4','C',70,0,1,'G','','mv_par04','','','',''})
		aAdd(aRegs,{cPerg,'05','                    ','                    ','                    ','mv_ch5','C',70,0,0,'G','','mv_par05','','','',''})
...
mCorpo += 'Site: www.metalacre.com.br - Email: nfe@metalacre.com '+Chr(13)+Chr(10)
...
Private cDe 	:= GetNewPar("MV_MFROMCP",'suprimentos@metalacre.com') //Trim(GetMV("MV_RELACNT"))
Private cPass    := GetNewPar("MV_SFROMCP",'Mtl1234@') //Trim(GetMV("MV_RELPSW"))  // 
