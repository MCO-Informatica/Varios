...
Private limite       := 220
Private tamanho      := "G"
Private nomeprog     := "RCarmaq" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo        := 18
...
Private CONTFL       := 01
Private m_pag        := 01
Private wnrel        := "RCarmaq" // Coloque aqui o nome do arquivo usado para impressao em disco
...
cQuery:= " SELECT    C2_DATPRI, C2_DATPRF, ISNULL(C6_ENTREG,C2_DATPRF) C6_ENTREG, C2_PRODUTO, ISNULL(C6_PRODUTO,C2_PRODUTO) C6_PRODUTO, B1_GRUPO, B1_DESC, BM_DESC, ISNULL(C5_CLIENTE,C2_CLI) C5_CLIENTE, ISNULL(C5_LOJACLI,C2_LOJA) C5_LOJACLI, A1_REGIAO, ISNULL(C6_NUM,C2_NUM) C6_NUM, ISNULL(C6_ITEM,C2_ITEM) C6_ITEM, ISNULL(C6_PEDCLI,'') C6_PEDCLI, ISNULL(C6_NUMOP,C2_NUM) C6_NUMOP, ISNULL(C6_ITEMOP,C2_ITEM) C6_ITEMOP,    " +CRLF
cQuery+= "        ISNULL(C6_QTDVEN,C2_QUANT) C6_QTDVEN, ISNULL(C6_QTDENT,C2_QUJE) C6_QTDENT, A1_CGC, A1_NOME, A1_END, A1_EST, A1_BAIRRO, A1_MUN, BM_CAPDIA, C2_DATAJF, ISNULL(C2_QUJE,0.00) C2_QUJE, ISNULL((SELECT TOP 1 GA_DESCOPC FROM " + RetSqlName("SGA") + " GA WHERE GA_GROPC = LEFT(C6_OPC,2) AND GA_OPC = SUBSTRING(C6_OPC,4,4) AND GA.D_E_L_E_T_<>'*'  ),'') OPCIONAL     " +CRLF
cQuery+= " FROM " + RetSqlName("SC2") +  " C2   " +CRLF
cQuery+= " INNER JOIN " + RetSqlName("SB1") + " B1 ON C2_PRODUTO=B1_COD AND B1.D_E_L_E_T_<>'*'  AND B1_TIPO='PA'  AND C2_DATPRF BETWEEN '" + DToS(MV_PAR05) + "' AND '" + DToS(MV_PAR06) + "'    " +CRLF
If !Empty(MV_PAR03)
	cQuery+= " AND B1_GRUPO = '" + MV_PAR03 + "'   " +CRLF
EndIf
//cQuery+= " INNER JOIN " + RetSqlName("SBM") + " BM ON BM_FILIAL = '" + xFilial('SBM')  + "'  AND B1_GRUPO=BM_GRUPO AND BM.D_E_L_E_T_<>'*'      " +CRLF
...
cQuery+= " LEFT  JOIN " + RetSqlName("SC6") + " C6 ON C6_FILIAL = '" + xFilial('SC6')  + "'  AND C2_PEDIDO=C6_NUM  AND C2_ITEMPV=C6_ITEM AND C6.D_E_L_E_T_<>'*'   AND C6_PRODUTO BETWEEN '" + MV_PAR01 + "' AND '" + MV_PAR02 + "'      " +CRLF
cQuery+= " LEFT  JOIN " + RetSqlName("SC5") + " C5 ON C5_FILIAL = '" + xFilial('SC5')  + "'  AND C5_NUM= C6_NUM AND C5.D_E_L_E_T_<>'*' AND C6_ENTREG BETWEEN '" + DToS(MV_PAR05) + "' AND '" + DToS(MV_PAR06) + "'    " +CRLF
cQuery+= " LEFT  JOIN " + RetSqlName("SA1") + " A1 ON A1_COD = CASE WHEN C5_CLIENTE IS NULL THEN C2_CLI WHEN C5_CLIENTE IS NOT NULL THEN C5_CLIENTE END AND A1_LOJA= CASE WHEN C5_LOJACLI IS NULL THEN C2_LOJA WHEN C5_LOJACLI IS NOT NULL THEN C5_LOJACLI END AND A1.D_E_L_E_T_<>'*' " +CRLF
cQuery+= " WHERE    C2_FILIAL='"+XFilial("SC2")+"' AND C2.D_E_L_E_T_<>'*'  AND C2_QUANT-C2_QUJE>0 " +CRLF

If MV_PAR04==3
	cQuery+= " ORDER BY 3, B1_GRUPO   " +CRLF
Else
	cQuery+= " ORDER BY 3, 4   " +CRLF
EndIf

MemoWrite('C:\qry\rcarmaq_qry' + dtos(ddatabase) + '.txt',cQuery)
...
If MV_PAR04 == 1 // RelatÛrio Detalhado
	...
	While !TRB->(EOF())
		...
		If !(TRB->C6_PRODUTO >= MV_PAR01 .And. TRB->C6_PRODUTO <= MV_PAR02)
			TRB->(dbSkip(1));Loop
		Endif
		
		If TRB->C6_QTDENT >= TRB->C6_QTDVEN
			TRB->(dbSkip(1));Loop
		Endif
		...
		@nLin,135 PSAY TRB->C6_QTDVEN - TRB->C2_QUJE  Picture "@E 999,999,999"
		...
		nQAProd += TRB->C6_QTDVEN - TRB->C2_QUJE
		
		nQPDia += TRB->C6_QTDVEN
		nQADia += TRB->C6_QTDVEN - TRB->C2_QUJE
...
ElseIf MV_PAR04 == 3
	...
	While !EOF()
		...
		ElseIf AllTrim(TRB->B1_GRUPO) $ "605/617"  //Familia 22 e 31
			While !TRB->(EOF()) .AND. cGrpAnt == TRB->B1_GRUPO
				nQAP22   += TRB->C6_QTDVEN - TRB->C2_QUJE
...
cQuery:= " SELECT ISNULL((SUM(C2_QUANT)-SUM(C2_QUJE)),0) QTD_OCUP  " +CRLF
...
cQuery+= " INNER JOIN "+RetSqlName("SC2")+" C2 ON C2_FILIAL='"+XFilial("SC2")+"' AND C2_PRODUTO=B1_COD AND C2.D_E_L_E_T_<>'*'   " +CRLF
cQuery+= " LEFT  JOIN " + RetSqlName("SC6") + " C6 ON C6_FILIAL = '" + xFilial('SC6')  + "'  AND C2_PEDIDO=C6_NUM  AND C2_ITEMPV=C6_ITEM AND C6.D_E_L_E_T_<>'*'   AND C6_PRODUTO BETWEEN '" + MV_PAR01 + "' AND '" + MV_PAR02 + "'      " +CRLF
...
MemoWrite('C:\qry\rcarmaq_qrygrp' + dtos(ddatabase) + '.txt',cQuery)
