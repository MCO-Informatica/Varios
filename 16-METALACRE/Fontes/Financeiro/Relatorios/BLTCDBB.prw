...
±±³Programa  ³ BLTCDBB  ³ Autor ³ Luiz Alberto           ³ Data ³ 15/04/14 ³±±
...
If Empty(Len(aDnfBol))
	lAutoEMA := .F.
Else
	MV_PAR22 := PadR(GetNewPar("MV_BOLBCO","001"),3)
	MV_PAR23 := PadR(GetNewPar("MV_BOLAGE","3222"),5)
	MV_PAR24 := PadR(GetNewPar("MV_BOLCON","101517"),10)
	MV_PAR25 := PadR(GetNewPar("MV_BOLSUB","BOL"),3)
...
If !SEE->(dbSetOrder(1), dbSeek(xFilial("SEE")+MV_PAR22+MV_PAR23+MV_PAR24+MV_PAR25))
	MsgStop("Parametros de Banco Nâo Localizado !")
	Return
Endif
...
DbSelectArea("SA6")
DbSetOrder(1)
DbSeek(xFilial("SA6")+MV_PAR22+MV_PAR23+MV_PAR24,.T.)
...
If !Empty(Len(aDnfBol))
	...
	For nI := 1 To Len(aDnfBol)
		...
		SA1->(dbSetOrder(1), DbSeek(xFilial("SA1")+SE1->E1_CLIENTE+SE1->E1_LOJA,.T.))
		...
		aCB_RN_NN    := Ret_cBarra( SE1->E1_PREFIXO	,SE1->E1_NUM	,SE1->E1_PARCELA	,SE1->E1_TIPO	,;
							       ...
							       cNroDoc		,(SE1->E1_SALDO-nVlrAbat)	, "17"	,"9"	)
		DbSelectArea("SE1")
...
oPrint:Say  (0084,100,"Banco do Brasil S.A.",oFont11 )	    // [2]Nome do Banco
...
oPrint:Say  (nRow2+0644,100,"Banco do Brasil S.A.",oFont11 ) //aDadosBanco[2],oFont11 )		// [2]Nome do Banco
...
oPrint:Say  (nRow2+0765,400 ,"Após, atualize o boleto no site bb.com.br.",oFont10)
...
//nCol := 1810+(374-(len(cString)*22))
oPrint:Say  (nRow2+0850,1810,cString,oFont11c)
...
cString := (aCB_RN_NN[3])+'-'+Modulo11(aCB_RN_NN[3])
nCol := 1810+(374-(len(cString)*22))
oPrint:Say  (nRow2+0940,1810,cString,oFont11c)
...
oPrint:Say  (nRow3+1934,100,"Banco do Brasil S.A.",oFont11) //aDadosBanco[2],oFont11 )		// 	[2]Nome do Banco
...
oPrint:Say  (nRow3+2055,400 ,"Após, atualize o boleto no site bb.com.br.",oFont10)
...
nCol 	 := 1810+(374-(len(cString)*22))
oPrint:Say  (nRow3+2230,1810,cString,oFont11c)
...
MSBAR("INT25",24.5,2,aCB_RN_NN[1],oPrint,.F.,Nil,Nil,0.025,1.5,Nil,Nil,"A",.F.) ///19
...
//   SE1->E1_NUMBCO 	:=	aCB_RN_NN[3]  // Nosso número (Ver fórmula para calculo)
 //SE1->E1_PORTADO := "001"
//   SE1->E1_HIST := "BOLETO BB GERADO"                                      s
//MsUnlock()
...
Else
   cNosso     := ALLTRIM(SEE->EE_CODEMP)+Right(AllTrim(NossoNum()),10)
   nNum       := cNosso              
...
If nValor > 0
	cFatorValor  := u_fator1()+Strzero(nValor*100,10)
Else
	cFatorValor  := u_fator1()+strzero(SE1->E1_SALDO*100,10)
EndIf
...
Local Auxi := 0, sumdig := 0
Local aBasecalc := { 4,3,2,9,8,7,6,5,4,3,2,9,8,7,6,5,4,3,2,9,8,7,6,5,4,3,2,9,8,7,6,5,4,3,2,9,8,7,6,5,4,3,2,9}
Local nMult     := 0
Local nD        := 0      
Local nE        := 0      
Local aMult     := {}
