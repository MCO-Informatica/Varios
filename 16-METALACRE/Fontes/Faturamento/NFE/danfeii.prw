...
// Especifico Metalacre - 3L Systems
...
If GetNewPar("MV_BOLIMP",.T.)
	If Len(aDnfBBBol) > 0                     
		MsAguarde({|| U_BLTCDBB(aDnfBBBol) },"Processando Boletos Banco do Brasil...","Aguarde... ")
	Endif
	If Len(aDnfCXBol) > 0                     
		MsAguarde({|| U_BOLCAX(aDnfCXBol) },"Processando Boletos Caixa Economica...","Aguarde... ")
	Endif
Endif

If Len(aDnfCer) > 0                     
	MsAguarde({|| U_XQIPR(aDnfCer) },"Processando Certificados...","Aguarde... ")
Endif
RestArea(aArea)
Return lRet
				...
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						Else
							...
							// Especifico Metalacre - 3L Systems - Titulos de Boletos
							
							If SE1->(dbSetOrder(1), dbSeek(xFilial("SE1")+SF2->F2_SERIE+SF2->F2_DOC))
								While SE1->(!Eof()) .And. SE1->E1_FILIAL == xFilial("SE1") .And. SE1->E1_NUM == SF2->F2_DOC .And. SE1->E1_PREFIXO == SF2->F2_SERIE .And. SE1->E1_CLIENTE == SF2->F2_CLIENTE .And. SE1->E1_LOJA == SF2->F2_LOJA
									If AllTrim(SE1->E1_TIPO) == 'NF'
										If AllTrim(SE1->E1_NATUREZ) $ GetNewPar("MV_BOLBB","101003*101004*101012") .And. Empty(SE1->E1_NUMBCO) .And. Empty(SE1->E1_BAIXA)
											AAdd(aDnfBBBol,{SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,SE1->E1_CLIENTE,SE1->E1_LOJA})
										ElseIf AllTrim(SE1->E1_NATUREZ) $ GetNewPar("MV_BOLCX","101013") .And. Empty(SE1->E1_NUMBCO) .And. Empty(SE1->E1_BAIXA)
											AAdd(aDnfCXBol,{SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,SE1->E1_CLIENTE,SE1->E1_LOJA})
										Endif
									Endif
							...
							// Especifico Metalacre - 3L Systems - Certificados
							
							If SF2->F2_TIPO == 'N' .And. !Empty(SF2->F2_DUPL)
								If SD2->(dbSetOrder(3), dbSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA))
									While SD2->(!Eof()) .And. SD2->D2_FILIAL == xFilial("SD2") .And. SD2->D2_DOC == SF2->F2_DOC .And. SD2->D2_SERIE == SF2->F2_SERIE .And. SD2->D2_CLIENTE == SF2->F2_CLIENTE .And. SD2->D2_LOJA == SF2->F2_LOJA
										If SC6->(dbSetOrder(1), dbSeek(xFilial("SC6")+SD2->D2_PEDIDO+SD2->D2_ITEMPV))
											If QPK->(dbSetOrder(1), dbSeek(xFilial("QPK")+SC6->C6_NUMOP+SC6->C6_ITEMOP+'001'))
												AAdd(aDnfCer,QPK->(Recno()))
											Endif
										Endif
