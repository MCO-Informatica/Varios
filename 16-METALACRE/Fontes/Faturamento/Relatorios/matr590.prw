...
Local oValor1 := Nil //Recebe a função do totalizador TB_VALOR1 para ser executada de acordo com a devoluçao 
Local oValor2 := Nil	//Recebe a função do totalizador TB_VALOR2 para ser executada de acordo com a devoluçao
Local oValor3 := Nil	//Recebe a função do totalizador TB_VALOR3 para ser executada de acordo com a devoluçao
	...
	While !oReport:Cancel() .And. !(cNomArq)->(Eof())
		...
		While Eval(bChave)
			...
			If (cNomArq)->TB_VALOR4 <> 0 .Or. (cNomArq)->TB_VALOR5 <> 0 .Or. (cNomArq)->TB_VALOR6 <> 0
				...
				//Imprime os totalizadores no final da seção
			...
			//Caso não haja devolução, ao finalizar a seção imprimir o totalizador
...
While !Eof() .And. xFilial()=SF2->F2_FILIAL 
	...
	If !Empty(SA1->A1_IDCWEB)	// Se For Cliente Venda WEB SEALBAG Então Pula
		lProcessa := .F.
	EndIf	                             
