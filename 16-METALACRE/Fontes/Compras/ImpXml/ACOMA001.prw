...
aAdd(aFM_Param,{"MV_FM_DXML","Diretorio onde será salvo os arquivos XML"                       ,'C', fX6Ret("MV_FM_DXML","\data\impxml\"                       )  })
aAdd(aFM_Param,{"MV_FMCPOP3","POP3 do servidor de e-mails pra baixar os XML."                  ,'C', fX6Ret("MV_FMCPOP3","pop.seudominio.com.br"               )  })
aAdd(aFM_Param,{"MV_FMCMAIL","E-MAIL da conta que receberá os XML que serao usados na rotina"  ,'C', fX6Ret("MV_FMCMAIL","protheus@seudominio.com.br"          )  })
aAdd(aFM_Param,{"MV_FMCPASS","SENHA da conta de e-mail que receberá os XML"                    ,'C', fX6Ret("MV_FMCPASS","senhaemail"                          )  })
...
aAdd(aFM_Param,{"MV_FM_MAIL","E-MAIL das pessoas que receberao o WF de confirmacao ou crítica" ,'C', fX6Ret("MV_FM_MAIL",""              )  })//lalberto@3lsystems.com.br;
...
aAdd(aFM_Param,{"MV_XMLDML" ,"Efetua a Exclusão dos Emails Lidos no Servidor ?"                ,'L', fX6Ret("MV_XMLDML" ,".T."                                 )  })
aAdd(aFM_Param,{"MV_FWSCONS","Valida XML via WebService Totvs"                                 ,'L', fX6Ret("MV_FWSCONS",".f."                                 )  })
aAdd(aFM_Param,{"MV_XMLZDC" ,"Numero da Nota Com Zeros à Esquerda"                             ,'L', fX6Ret("MV_FWSCONS",".T."                                 )  })
	...
	For x := 1 To Len(oItens)
		...
		If lVldProdut
			If lEhFornec
				...
				Else
					lVldProdut := .F.
					cVldMensag := "O produto ("+cXmlPrdCod+") do Forncecedor nao está cadastrado na tabela de relacionamento de produtos [SA5]!"
					fGravaMsg(nRecTab1,98," ",cVldMensag)
			...
			Else
				...
				Else
					lVldProdut := .F.
					cVldMensag := "O produto ("+cXmlPrdCod+") do Cliente nao está cadastrado na tabela de relacionamento de produtos [SA7]!"
					fGravaMsg(nRecTab1,98," ",cVldMensag)
		...
		If lVldProdut
			...
			If SB1->(!dbSeek(xFilial("SB1")+cCodProdut))
				lVldProdut := .F.
				cVldMensag := "O produto ("+cCodProdut+") informado na rotina de produto x "+IIf(lEhFornec,"fornecedor","cliente")+" nao existe no cadastro de produtos ou está com cadastro incompleto!"
				fGravaMsg(nRecTab1,98," ",cVldMensag)
			EndIf
		EndIf
		...
		If lVldProdut .And. !lEhFornec
			If Upper(AllTrim(SB1->B1_UM)) <> Upper(AllTrim(cXmlUniMed))
				lVldProdut := .F.
				cVldMensag := "O produto ("+cCodProdut+") está com Unidade de Medida Diferente no cadastro de produtos!"
				fGravaMsg(nRecTab1,98," ",cVldMensag)
		...
		If lVldProdut .And. !lEhFornec
			If Upper(AllTrim(SB1->B1_POSIPI)) <> Upper(AllTrim(Str(Val(cXmlPrdNcm))))
				lVldProdut := .F.
				cVldMensag := "O produto ("+cCodProdut+") está com NCM Diferente no cadastro de produtos!"
				fGravaMsg(nRecTab1,98," ",cVldMensag)                             
...
If lProc .And. Len(aItens) > 0
	...
	//If !lRetorno
//		MsgAlert("Falha No Lançamento do Documento de Entrada !")
//		Return .f.
//	Endif
	...
	For x:=1 To nProcessa
		...
		If !Empty(oLinkGd01:aCols[x][nPos1Ped]) .And. !Empty(oLinkGd01:aCols[x][nPos1Itm])
			...
			If oLinkGd01:aCols[x][nPos1Itm]<>'ZZZZ'	// Trata-se de 1 x 1
				...
				// Se Tiver Relacao com Pedidos de Compras então Pega o Centro de Custos
			...
			ElseIf oLinkGd01:aCols[x][nPos1Itm]=='ZZZZ'	// Trata-se de 1 x Varios Pedidos
				...
				For nPed := 1 To Len(aPedidos)-1
					...
					If nPed == 1	// Se For o Primeiro Pedido então altera a quantidade do item da nota
...
If lOk 
	For nPed := 1 To Len(aItPed)
		If AllTrim(oLinkGd01:aCols[nLinGD1][nPos1UM]) != AllTrim(aItPed[nPed,5])                                                 
			lOk     := MsgYesNo("ATENÇÃO: A unidade de medida do produto selecionado está diferente ( XML x PC ) !, Continua assim Mesmo ?")	
			Exit
		Endif
	Next
EndIf
...
If lOk 
	For nPed := 1 To Len(aItPed)
		If AllTrim(oLinkGd01:aCols[nLinGD1][nPos1Prc]) != AllTrim(aItPed[nPed,6])
			lOk     := MsgYesNo("ATENÇÃO: O preco unitario do produto selecionado está diferente ( XML x PC ) !, Continua assim Mesmo ?")	
			Exit
		Endif
	Next
EndIf
...
While lLoop
	...
	If Empty(cCadPrdCod) .And. !Empty(cXmlPrdBar)	// Se não encontrou nenhum relacionamento então tenta localização por codigo de barras e Sugere
...
//CFOP de Operacao com Exterior (inicia por 3 ou 7) e UF destinatário = EX
...
While SQL->(!Eof())
	...
	//Verificar se CNPJ Destinatario está cadastrado no sigamat, pois caso nao encontrado, foi uma emissao errado do fornecedor
	//Destinatario
	If lRet
	...
	//Verificar se chave nfe está ativa no sefaz, se existe e se nao foi cancelada
	If lRet
...
Else
	lRet := .T.
	cMsgCod := "999"
	cMsgDsc := 'Consulta Chave SPED Desativada, Parâmetro MV_FWSCONS'
Endif
...
Do Case
	Case nMsg == 1
		cMens := "CNPJ do Emitende nao está cadastrado no sistema (SA1)"

	Case nMsg == 2
		cMens := "CNPJ do Emitende nao está cadastrado no sistema (SA2)"

	Case nMsg == 3
		cMens := "CNPJ do Emitende nao localizado no XML"

	Case nMsg == 4
		cMens := "CNPJ do Destinatário nao está cadastrada no sistema (SIGAMAT)"
	...
	Case nMsg == 15
		cMens := "Xml Já Lançado no Sistema - Verifique!"
...
//Caso retorno maior que 0(zero) e porque nfe já foi registrada
If TMP->(!Eof())
	Do Case
		Case TMP->QTD == 0
			lRet := .T.
...
For i:=1 to Len(aDir)
	...
	If Empty(cAviso) .AND. Empty(cErro)
		...
		If !lOk .And. Type('oXML:_PROCEVENTONFE') == "U" .Or. cCnpjD=='RECUSADO'
			If !lEhJob                                        
				MsAguarde({||Inkey(2)}, "", "Atencao o XML " + cFile + " Nao Pertence à Empresa como Destinatario ! Arquivo Movido Para Pasta Recusados !", .T.)
			Endif
...
*-----------------------------------------------------------*
Local cToMail := AllTrim(SuperGetMV( "MV_FM_MAIL", , "lalberto@3lsystems.com.br;"              )) //E-mail dos usuários que serao notificados quando uma NF for gerada
Local cToFull := ""
Local cAttach   := ""
Local aMsg      := {}
Local aUsrMail := {}
...
If lProc
	...
	If lProc
		...
		If lProc
			...
			If !lRetorno
				MsgAlert("Lançamento Nota Cte Cancelado !")
				Return .f.
			Else
				IF lMSErroAuto 
					lProc := .F.
...
±±ºPrograma  ³NOVO2     ºAutor  ³Microsiga           º Data ³  10/27/11   º±±
