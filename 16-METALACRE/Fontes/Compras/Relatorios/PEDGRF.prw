...
Private	oBrush		:= TBrush():New(,4),;
		...
		While 	TRA->( ! Eof() )
				...
				If	( lPrintDesTab )
					...
					oPrint:Say(nLinha+10,1500,OemToAnsi('Qtde'),oFont12n)
				...
				_cObsMemo += SC7->C7_OBSM

				nLinCod := MlCount(AllTrim(cCodPro),10)

				_nLinhas := MlCount(AllTrim(cDesc),45)

				_nLinObs := MlCount(_cObsItem,45)
				_nLinMem := MlCount(_cObsMemo,45)
				...
				For _nT := 1 To Max(_nLinhas,nLinCod)
        			...
        			Endif
					If _nT <= _nLinhas .And. !Empty(Capital(MemoLine(cDesc,45,_nT)))
				...
				For __nI := 1 To _nLinObs
					oPrint:Say(nLinha,0450,Capital(MemoLine(_cObsItem,40,__nI)),oFont10,,0)
					nLinha+=40
				Next
...
*/
		...
		For _nT := 1 To _nLinMem
			oPrint:Say(nLinha,0500,Capital(MemoLine(_cObsMemo,100,_nT)),oFont12n,,0)
			nLinha+=60
			xVerPag()
		Next
	/*
...
oPrint:Say(2630,0100,'NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras. ',oFont12n)
oPrint:Say(2670,0100,'Prezado Fornecedor, informamos que a partir de 01 de maio de 2017, efetuaremos os pagamentos apenas ',oFont12n)
oPrint:Say(2710,0100,'às terças e quintas-feiras, somente através de boletos bancários. Agradecemos a compreensão. ',oFont12n)
oPrint:Say(2750,0100,'***TODOS OS MATERIAIS DEVEM SER IDENTIFICADOS COM ETIQUETA APROPRIADA, CONTENDO O CÓDIGO E DESCRIÇÃO',oFont12n)
oPrint:Say(2790,0100,'DO PRODUTO (OS MESMOS QUE ESTÃO NA ORDEM DE FABRICAÇÃO OU PEDIDO DE COMPRA), QUANTIDADE, NOTA FISCAL ',oFont12n)
oPrint:Say(2830,0100,'E IDENTIFICAÇÃO DO FORNECEDOR. ',oFont12n)
oPrint:Say(2870,0100,'*** SOLICITAMOS A TODOS OS FORNECEDORES DE COMPONENTES QUE ENVIEM OS CERTIFICADOS DE QUALIDADE A CADA LOTE FORNECIDO.',oFont12n)
oPrint:Say(2910,0100,'MATERIAL SUJEITO A DEVOLUÇÃO CASO AS SOLICITAÇÕES ACIMA NÃO SEJAM ATENDIDAS.',oFont12n)
