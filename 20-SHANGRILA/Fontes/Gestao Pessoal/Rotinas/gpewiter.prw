...
User Function GpeWriter()
	...
	Private cCadastro	:= "Integração OpenOffice Writer"
...
Static Function GPE2Writer(lExport,aInfo,cArqWord,lImpress,cArqSaida,nCopias,cPrinter,cOrientation,cError,cCRLF,lPreview)
	...
	Default cArqWord        := AllTrim( MV_PAR25 )

	Default lImpress        := ( MV_PAR28 == 1 )

	Default cArqSaida       := ''

	Default nCopias               := Max( MV_PAR23 , 1 )
	...
	nWait                   := IF( lImpress , 1 , 1 )

	nSleep                  := IF( lImpress , .5 , .5 )
