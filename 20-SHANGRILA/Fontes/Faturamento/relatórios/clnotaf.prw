...
//////////////////////////////////////////////////////////////////////////////
//                                                                          //
//   Programa ...: CLNOTAF                            Modulo : SIGAFAT      //
//                                                                          //
...
If Mv_Par04 == 2                       // Dados da Fatura
   ...
   If xDUPLICATAS == .T.
      ...
      //
      If !Empty(xLinDup01)
         @ 18, 010 PSAY xLinDup01
      Endif
      //
      If !Empty(xLinDup02)
         @ 18, 045 PSAY xLinDup02
      Endif
      //
      If !Empty(xLinDup03)
         @ 19, 010 PSAY xLinDup03
      Endif
      //
      If !Empty(xLinDup04)
         @ 19, 045 PSAY xLinDup04
      Endif
      //
   Endif
   //
...
For A := 1 To Len(xNATOP)
    //
    @ 08, 001+xCOLUNA PSAY Substr(Substr(xNATOP[A], 5, Len(Rtrim(xNATOP[A])) - 4), 1, 20) + ;
...
Next
//
@ 08, 025 PSAY xCODCFO                      // C.F.O.
//
@ 11, 002 PSAY Substr(xNOME_CLI, 1, 40)     // Nome do Cliente
//
//If Mv_Par04 == 1                            // NF de Entrada
...
If !Empty(xCGC_CLI)                         // C.G.C. ou C.P.F.
   ...
   If Len(Alltrim(xCGC_CLI)) == 11
      @ 11, 051 PSAY xCGC_CLI Picture "@R 999.999.999-99"          // C.P.F.
   Else
      @ 11, 050 PSAY xCGC_CLI Picture "@R 99.999.999/9999-99"      // C.G.C.
   Endif
   //
Else
   @ 11, 049 PSAY " "
Endif
//
@ 11, 071 PSAY xEMISSAO                // Data da Emissao do Documento
@ 13, 002 PSAY SubStr(xEND_CLI,1,38)   // Endereco
@ 13, 041 PSAY Substr(xBAIRRO, 1, 16)  // Bairro
@ 13, 060 PSAY Transform(xCEP_CLI, "@R 99999-999")
@ 13, 071 PSAY xEMISSAO //dDataBase               // Data da Saida
//
@ 15, 002 PSAY substr(xMUN_CLI,1,26)                // Municipio
@ 15, 031 PSAY Alltrim(xDDD_CLI)+"-"+Substr(xTEL_CLI, 1, 13) // + " - " + xFAX_CLI // Telefone/FAX
@ 15, 046 PSAY xEST_CLI                // U.F.
@ 15, 051 PSAY xINSC_CLI               // Insc. Estadual
@ 15, 072 PSAY " "                     // Reservado p/Hora da Saida
//
ClDuplic()                     // Impressao de Duplicatas
//
@ 21, 002 PSAY Chr(15) + Substr(xENDCOB, 1, 70) + "   " + Substr(xBAICOB, 1, 15) +;  //Mudanca do Campo a1_endecob para 70 posicoes, era 40 posicoes
...
For I := 1 To nTamDet
      ...
      If I <= Len(xCOD_PRO)
         ...
         If xTIPO # "I"
            ...
            //@ nLin, 055 PSAY xCLFISCAL[1]
            @ nLin, 062 PSAY SB1->B1_CLASFIS        // xCOD_FIS[xORDIMP]
            @ nLin, 066 PSAY SB1->B1_ORIGEM         // xCOD_TRIB[xORDIMP]
...
Endif
//
//nLin := 48 // Mudei de 47 p/ 48 a pedido de Adriana
//nLin := 45 	// Mudei de 48 p/ 45 a pedido de Adriana
// Alterado para Imprimir mensagens logo após a impressão dos itens
// a pedido de Adriana
nLin := nLin + 1
//      
xMensage2 := ""
...
//
nLin := nLin + 2
@ nLin, 002 PSAY Chr(15) + Alltrim(Substr(xEND_TRANSP, 1, 30))+iif(!Empty(xBAI_TRANSP),"/",".")+Substr(Alltrim(xBAI_TRANSP),1,20) + Chr(18)  // Endereco Transp. // Original 34
@ nLin, 038+11 PSAY Substr(xMUN_TRANSP,1,40) // Municipio/Bairro
@ nLin, 060+13 PSAY xEST_TRANSP                     // U.F.
// @ nLin, 066+17 PSAY Chr(15) + xINSC_TRANSP + Chr(18)          // Inscricao Estadual
// Não imprimir IE conforme solicitação Sra.Adriana
...
nLin := nLin + 3
For I := 1 To Len(xCLAS_FIS)
	@ nLin, 37 PSAY Chr(15) + xCOD_FIS[I]
	@ nLin, 40 PSAY Substr(xCLAS_FIS[I],1,4)+"."+Substr(xCLAS_FIS[I],5,2)+"."+Substr(xCLAS_FIS[I],7,2) + Chr(18)
	nLin := nLin + 1
Next
...
If Mv_Par04 == 2
   //
   nLin := 67
   @ nLin, 004 PSAY xC5_VEND1          // Vendedor
   @ nLin, 014 PSAY xPEDCLI            // Numero do Pedido
   @ nLin, 028 PSAY xC5_NUM            // Numero do Pedido
   //
Endif
//
nLin := 69
