...
TRCell():New(oSection1,'B1_DESC'	,cAliasSB1	,STR0028				,/*Picture*/,20			,/*lPixel*/,/*{|| code-block de impressao }*/)
...
TRCell():New(oSection1,'PERQUANT'	,'   '		,STR0051+CRLF+STR0052	,cPictVFim	,cTamVFim	,/*lPixel*/,/*{|| code-block de impressao }*/,,,"RIGHT")
...
Local cDifQuant := ''
Local cDifValor := ''
Local cPictQtde := PesqPict('SB7','B7_QUANT')
Local cPictQFim := PesqPict("SB2",'B2_QFIM')
Local cPictVFim := PesqPict("SB2",'B2_VFIM1')
Local oBreak
...
#IFDEF TOP
	...
	ElseIf nOrdem == 5 //-- Por Local
		If lVeic
			cOrderBy += " B1_FILIAL, B1_LOCPAD, B1_CODITE"
		Else
			cOrderBy += " B1_FILIAL, B1_LOCPAD, B1_COD"
		EndIf	
	EndIf
	cOrderBy += "%"
			...
			If lImprime .Or. mv_par14 == 3
				...
				For nX:=1 to Len(aRegInv)
					...
					If nX == 1
						...
						oSection1:Cell('PERQUANT'	):SetValue((nTotal/aSaldo[1])*100)
					Else	
				...
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				//³Impressao da linha de total do produto                      ³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ

				dbSelectArea(cAliasSB7)

				cTotQtde	:= Transform(nTotal								,cPictQtde)
				cTotQtData	:= Transform(aSaldo[1]							,cPictQFim)
				cDifQuant	:= Transform(nTotal-aSaldo[1]					,cPictQtde)
				cDifValor	:= Transform((nTotal-aSaldo[1])*aCM[mv_par10]	,cPictVFim)

				oReport:PrintText(STR0045		,oReport:Row()	,oSection1:Cell('B7_DOC'	):ColPos())
				oReport:PrintText(cTotQtde		,oReport:Row()	,oSection1:Cell('B7_QUANT'	):ColPos()-24)
				oReport:PrintText(cTotQtData	,oReport:Row()	,oSection1:Cell('QUANTDATA'	):ColPos()-24)
				oReport:PrintText(cDifQuant		,oReport:Row()	,oSection1:Cell('DIFQUANT'	):ColPos()-24)
				oReport:PrintText(cDifValor		,				,oSection1:Cell('DIFVALOR'	):ColPos()-24)
				...
				EndDo
		   		...
		   		If nSB7Cnt > 0
					...
					If lImprime .Or. mv_par14 == 3
						...
						oReport:PrintText(STR0045		,oReport:Row()	,oSection1:Cell('B7_DOC'	):ColPos())
						oReport:PrintText(cTotQtde		,oReport:Row()	,oSection1:Cell('B7_QUANT'	):ColPos())
						oReport:PrintText(cTotQtData	,oReport:Row()	,oSection1:Cell('QUANTDATA'	):ColPos())
						oReport:PrintText(cDifQuant		,oReport:Row()	,oSection1:Cell('DIFQUANT'	):ColPos())
						oReport:PrintText(cDifValor		,				,oSection1:Cell('DIFVALOR'	):ColPos())
                    EndIf
...
//ALTERADO BRUNO 24-09-14
local nQtdInvent := 0
local nQtdDtInv  := 0 
local nPercAcura := 0
//TERMINO BRUNO 24-09-14
Private	lLocCQ	:=.T.
...
If mv_par11 == 1
	If mv_par13 == 1
        Cabec1 := 'CODIGO          DESCRICAO                LOTE       SUB    LOCALIZACAO     NUMERO DE SERIE      TP GRP  UM AL DOCUMENTO            QUANTIDADE         QTD NA DATA   _____________DIFERENCA______________          PERCENTUAL '
        Cabec2 := '                                                    LOTE                                                                         INVENTARIADA       DO INVENTARIO          QUANTIDADE              VALOR          ACURACIDADE'
	...
	Else
        Cabec1 := 'CODIGO          DESCRICAO                LOTE       SUB    TP GRP  UM AL DOCUMENTO            QUANTIDADE         QTD NA DATA   _____________DIFERENCA______________          PERCENTUAL '
        Cabec2 := '                                                    LOTE                                    INVENTARIADA       DO INVENTARIO          QUANTIDADE              VALOR          ACURACIDADE'
...
Else
	If mv_par13 == 1
        Cabec1 := 'CODIGO          DESCRICAO                LOCALIZACAO     NUMERO DE SERIE      TP GRP  UM AL DOCUMENTO            QUANTIDADE         QTD NA DATA  _______________DIFERENCA_____________          PERCENTUAL '
        Cabec2 := '                                                                                                               INVENTARIADA       DO INVENTARIO          QUANTIDADE              VALOR          ACURACIDADE'
	...
	Else
        Cabec1 := 'CODIGO          DESCRICAO                TP GRP  UM AL DOCUMENTO            QUANTIDADE         QTD NA DATA  _______________DIFERENCA_____________          PERCENTUAL '
        Cabec2 := '                                                                          INVENTARIADA       DO INVENTARIO          QUANTIDADE              VALOR          ACURACIDADE'
		...
		Do While !(cAliasSB7)->(Eof()) .And. (cAliasSB7)->(B7_FILIAL+DtoS(B7_DATA)+B7_COD+B7_LOCAL) == xFilial('SB7')+DtoS(mv_par03)+cProduto+cLocal
			...
			If lImprime .Or. mv_par14 == 3
				...
				For nX:=1 to Len(aRegInv)
					...
					//ALTERADO BRUNO 24-09-14
					//nQtdInvent += val(strTran(aRegInv[nX,12],",","."))
					//TERMINO BRUNO 24-09-14
					
					//ALTERADO MATHEUS 09/01/2014
					cAux := strTran(aRegInv[nX,12],".","")
					nQtdInvent += val(strTran(cAux,",","."))
					//FIM
				...
				If nSB7Cnt > 0
					...
					If mv_par11 == 1 
						...
						@ Li,If(mv_par13==1,207,170) + nCOL1 PSAY Transform(((nTotal/aSaldo[1])*100), (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 15)))
					Else
						...
						@ Li,If(mv_par13==1,190,152) + nCOL1 PSAY Transform(((nTotal/aSaldo[1])*100), (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 15)))
					EndIf
...
//ALTERADO BRUNO 24-09-14
Li++
//@ Li,016 PSAY STR0020 // 'TOTAL DAS DIFERENCAS EM VALOR .............'
@ Li,010 PSAY STR0020 // 'TOTAL DAS DIFERENCAS EM VALOR .............'
@ Li,If(mv_par13==1,111,074) PSAY Transform(nQtdInvent	, (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 15)))
@ Li,If(mv_par13==1,131,094) PSAY Transform(nQtdDtInv	, (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 15)))

//ALTERADO BRUNO 24-09-14
nPercAcura += ((nQtdInvent/nQtdDtInv) * 100)
//TERMINO BRUNO 24-09-14
					

@ Li,If(mv_par13==1,190,152) PSAY Transform(nPercAcura	, (cAliasSB2)->(PesqPict("SB2",'B2_VFIM1', 15)))+"%" 
...
Local cPerg := Padr("MTR285",Len(SX1->X1_GRUPO))

PutSx1(cPerg, '14' ,'Listar Produtos ', 'Muestra Productos ', 'Show Products ',	'mv_che', 'N', 1, 0, 3, 'C', '', '', '', '', 'mv_par14','Com Diferenças',;
