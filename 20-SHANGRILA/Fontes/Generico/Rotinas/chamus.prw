...
ฑฑบUso       ณ Grupo Rech Tratores                                        บฑฑ
...
Local	_cSmtpSrv 	:= AllTrim(GetMv('MV_WFSMTPS')),;
		...
		_cSmtpError	:= '',;
		_lOk		:= .f.,;
		_cTitulo 	:= OemToAnsi('[GRUPO SHANGRI-Lม] CHAMADO: '+cNumCham),;
		_cTo		:= cTo,;
		_cFrom		:= _cMailTec,;
		_cMensagem	:= '',;
		_lReturn	:= .f.
		...
		_cMensagem	+= OemToAnsi('Grupo Shangri-lแ') + CHR_NOTNEGRITO + CHR_ENTER
...
Local	cStatus	:= ''
		...
		oPrint:Say(nLin+20,700,'GRUPO SHANGRI-Lม',oFont16)
