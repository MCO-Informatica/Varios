...
If !lImpDir .or. MV_PAR04 == 0 /* Caso impressão de DANFE seja realizada via AutoDistMail */
	...
	While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
		...
		If (cAliasSF3)->(Eof())
			...
			Next nX
			//*Limpa pasta de Certficados - Especifico Prozyn
			/*	aFiles		:= Directory('\CERTIFIQ\*.*','D')
				...
				For nX := 1 to nLenNotas
					...
					U_CERTQIP('','','',1,aNotas[nX][5],MV_PAR03,aNotas[nX][5])
					aFiles		:= Directory('\CERTIFIQ\*.PDF','D')
					aAttachment	:= {}
					If Len(aFiles) > 0
						// If MsgYesNo("Deseja enviar por e-mail, os certificados de análise referentes à nota " + Alltrim(aNotas[nx][5]) + " para o cliente " + cCliente +"?")
							For nI := 1 to Len(aFiles)
								aAdd(aAttachment,'\CERTIFIQ\'+aFiles[nI][1])
							Next nI
							...
							RestArea(aArea)*
						// EndIf
					EndIf */ // Fim Da customização Especifico Prozyn
				// Next nX
			aNotas := {}
