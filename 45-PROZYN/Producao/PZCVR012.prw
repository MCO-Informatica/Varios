...
ฑฑบFuncao    ณPZCVR012		บAutor  ณMicrosiga	     บ Data ณ  21/11/2019 บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณImpressao do novo Rotulo							 	      บฑฑ
ฑฑบ          ณ								                              บฑฑ
...
Static Function ImpLayout(oPrint, aParam, cArqTmp, nEtiPos, nQtdEtiq, cCodEtiq)
	...
	If Upper('Branca') $ Upper(Trim(cNomImp))
		...
		If len(cProduto) > 36
			oPrint:SayAlign(50, 0000, 	cProduto, oFnt32n, 800 ,0090,,2,1)//Descri็ใo do produto
		Else
			oPrint:SayAlign(80, 0000, 	cProduto, oFnt32n, 800 ,0060,,2,0)//Descri็ใo do produto
		EndIf
	Else
	oPrint:SayAlign(80, 0000, GetDXImp("1", cArqTmp, aParam)	, oFnt32n, 800 ,0060,,2,0)//Descri็ใo do produto
	EndIf
	...
	oPrint:SayAlign(130, 275,GetLabel("4", aParam)     			, oFnt10 	, 800,0060,,0,1)//Fabrica็ใo
	oPrint:SayAlign(140, 275,DTOC(STOD((cArqTmp)->C2_EMISSAO))	, oFnt22n 	, 800,0060,,0,1)//Data de fabrica็ใo
	...
	nLinha += 30
	oPrint:SayAlign(nLinha, 0,GetLabel("6", aParam)								, oFnt12n 	, 800,0060,,0,1)//Descri็ใo
	oPrint:SayAlign(nLinha, 85,GetDXImp("3", cArqTmp, aParam)					, oFnt12 	, 800,0060,,0,1)//Dados da descri็ใo

	nLinha += nEntrLin
	oPrint:SayAlign(nLinha, 0,GetLabel("7", aParam)     								, oFnt12n 	, 800,0060,,0,1)//Composi็ใo

	//Dados da composi็ใo - Quebra de linha
	For nX := 1 To MLCount(Alltrim((cArqTmp)->ZO_DESC),70)
		oPrint:SayAlign(nLinha, 85,MemoLine(Alltrim((cArqTmp)->ZO_DESC),70,nX)	, oFnt12 	, 800,0060,,0,1)//Descri็ใo da composi็ใo
		nLinha += nEntrLin
		lPulaLin := .T.
	Next
	...
	If !Empty((cArqTmp)->B1_HALAL)
		...
		oPrint:SayAlign(nLinha, 105,Alltrim((cArqTmp)->B1_HALAL)		, oFnt12n 	, 800,0060,,0,1)//Descri็ใo do certificado HALAL
	...
	+GetDXImp("5", cArqTmp, aParam)) 	, oFnt12n 	, 800,0060,,0,1)//Descri็ใo dos alergicos / Gluten
