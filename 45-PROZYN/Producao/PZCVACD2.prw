...
Static Function ProcOrdSep(aDadosSel, cOperador, lGerAglut, lGerAglEmb)
	...
	//Verifica se gera ordem de produção aglutinada
	If lGerAglut
		For nX := 1 To Len(aDadosSel)
			If aDadosSel[nX][MARK]
			...
			Do While AllTrim(CB7->CB7_OP) == AllTrim(_cOP) .and. !EoF()
				...
				Do While CB8->CB8_ORDSEP == CB7->CB7_ORDSEP .and. !EoF()
					...
					If lMsErroAuto
					   ...
					   U_ENVMAIL(_cRemet, _cDest, 'Erro de geração de Empenho - OS: '+CB7->CB7_ORDSEP+' OP:'+CB7->CB7_OP , _cErro)				
					   //MostraErro()
					   Aviso("Geração de OS","Erro na geração de empenho para a Ordem de Separação. Contate o Administrador!",{"Ok"},3)
					EndIf
...
*/
	...
	While (cArqTmp)->(!Eof())
		...
		If (cArqTmp)->B1_QE != 0
			...
			//Atualiza a ordem de separação Embalagem
			If (cArqTmp)->B1_TIPO == 'EM'
...
Static Function GrvSZT(cProd, cOp, cArmz, nQtdOri, cLote, cSubLote, nQtdEmb, cEndLocz,;
	...
	SZT->ZT_ORDEMB	:= cOrdSepEmb			//Ordem de Separação de Embalagem
	SZT->ZT_ORDEMB2	:= ""					//Ordem de Separação aglutinado de Embalagem
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFuncao    ³GerEmbAglu	ºAutor  ³Microsiga	     º Data ³  04/03/2020 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Gera ordem de separação aglutinada						  º±±
±±º          ³												              º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Static Function GerEmbAglu(cOps, cOperador)
	...
	While (cArqTmp)->(!Eof())
		...
		//Gravação dos itens da ordem de separação aglutinada 
		GrvCB8(cOrdEmb,;			//Ordem de separação 
		(cArqTmp)->ZT_PROD,;		//Produto 
		(cArqTmp)->ZT_LOCAL,;		//Armazem 
