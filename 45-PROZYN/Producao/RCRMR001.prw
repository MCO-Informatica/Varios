...
//	oPrn:SayAlign( _nLin + (_nEspPad/2), 0005, "Confirmação da Proposta " + AllTrim(_aCab[06]), oFont01, 0800-0005,0060,,2,0)
	oPrn:SayAlign( _nLin + (_nEspPad/2), 0005, "Confirmação da Proposta ", oFont01, 0800-0005,0060,,2,0)	
...
Static Function ImpLogo()
	...
	If lMv_Logod
		oPrn:SayBitmap(082,500,cLogoD,0170,95)
	Else
		oPrn:SayBitmap(082,500,cLogo ,0170,95)
	EndIf
...
AADD(aRegs,{cPerg,"01","Proposta:"	,"","","mv_ch1","C",TamSx3("ADY_CODIGO")[01],TamSx3("ADY_CODIGO")[02],0,"G","","MV_PAR01","","","","","","","","","","","","","","","","","","","","","","","","","ADY","","","",""})
...
Static Function Selecao()
	...
	_cQry += "INNER JOIN " + RetSqlName("ADY") + " ADY " + _cEnter
	_cQry += "ON ADY.ADY_CODIGO = SCJ.CJ_PROPOST " + _cEnter	
	...
	_cQry += "AND ADY.ADY_CODIGO='" + MV_PAR01 + "' " + _cEnter
...
//		EndIf
		...
		AAdd(_aItem,Transform((_cAliasTmp)->CK_PRCVEN/*-MaFisRet(Len(_aItens)+1,"IT_VALICM")-MaFisRet(Len(_aItens)+1,"IT_VALPIS")-MaFisRet(Len(_aItens)+1,"IT_VALCOF")*/,PesqPict("SCK","CK_PRCVEN")))
		AAdd(_aItem,_cMoeda)
		AAdd(_aItem,AllTrim(Str(MaFisRet(Len(_aItens)+1,"IT_ALIQICM")))+ "%")  //ICMS
		AAdd(_aItem,AllTrim(Str(MaFisRet(Len(_aItens)+1,"IT_ALIQPIS")+MaFisRet(Len(_aItens)+1,"IT_ALIQCOF")))+ "%")  //PIS+COFINS
		AAdd(_aItem,Transform((_cAliasTmp)->CK_PRCVEN+MaFisRet(Len(_aItens)+1,"IT_VALIPI")+MaFisRet(Len(_aItens)+1,"IT_VALSOL")+MaFisRet(Len(_aItens)+1,"IT_VALICM")+MaFisRet(Len(_aItens)+1,"IT_VALPIS")+MaFisRet(Len(_aItens)+1,"IT_VALCOF"),PesqPict("SCK","CK_PRCVEN"))) //Preço unitário com impostos
		AAdd(_aItem,AllTrim(Str(MaFisRet(Len(_aItens)+1,"IT_ALIQIPI")))+ "%")  //IPI
		AAdd(_aItem,(_cAliasTmp)->B1_EMBALAG)
		AAdd(_aItem,DTOC(STOD((_cAliasTmp)->CK_ENTREG)))
//		AAdd(_aItem,DTOC(STOD((_cAliasTmp)->CK_DTEXPED)))
		AAdd(_aItem,DTOC(STOD("20160826")))		
		AAdd(_aItens,_aItem)
...
Static Function ImpItens()
	...
	oPrn:SayAlign( _nLin+5 , 0300, "Preço unitário sem impostos (net)", oFont03, 0400-0300,0060,,2,0)
	...
	oPrn:Box( _nLin, 0535, _nLin + (_nEspPad*3), 0605, "-4")
	oPrn:SayAlign( _nLin+5 , 0535, "Preço unitário com impostos (bruto)", oFont03, 0605-0535,0060,,2,0)
	...
	For _nItem := 1 To Len(_aItens)
		...
		oPrn:Box( _nLin, 0535, _nLin + (_nEspPad*_nQuebLin), 0605, "-4")
