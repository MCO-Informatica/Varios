...
// Aviso customizado para a PROZYN. - DANIEL - 081217
_nOpcUm := Aviso("Unidade de medida", "Selecione a unidade de medida para a nota fiscal:" + CHR(13) + CHR(10) + CHR(13) + CHR(10) + cNota, {"1 U.M.", "2 U.M."})
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			ElseIf FunName() == "SPEDNFSE"
			    ...
			    aadd(aDest,(AllTrim(SA1->A1_EMAIL)+IIF(!Empty(SA1->A1_EMAIL),";","")+"xml@prozyn.com.br"))
			...
			Else
				...
				#IFDEF TOP
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_PRC2UM, D2_QTSEGUM //Campos de customização D2_PRC2UM, D2_QTSEGUM Prozyn
					...
					Else
						BeginSql Alias cAliasSD2
							SELECT D2_PRC2UM, D2_QTSEGUM, D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_ITEM,D2_CF,
							D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
							D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
...
//////Inicio customização Prozyn adicionar a mensagem do dolar e ped compra no dados adicionais ---- Isaque
					
					if _nVez == 1 
						_cNumPCom :=SC5->C5_NUMPCOM
					
						IF SC5->C5_TXREF <= 1 .and. SC5->C5_MOEDA <> 1 
							dbSelectArea("SM2")
   							dbSetOrder(1) //Data
							If dbSeek(dtos(SF2->F2_EMISSAO))		
								If SC5->C5_MOEDA == 2
									_nTxDolar := SM2->M2_MOEDA2
								ElseIF SC5->C5_MOEDA == 3
									_nTxDolar := SM2->M2_MOEDA3	
								ElseIF SC5->C5_MOEDA == 4
									_nTxDolar := SM2->M2_MOEDA4
								EndIf     
						...
						IF SC5->C5_TXREF > 1 .and. SC5->C5_MOEDA <> 1 
							_nTxDolar :=SC5->C5_TXREF       
            	        EndIF

						If !empty(_cNumPCOM)
							cMensCli += "Nr. Ordem de Compras: "+_cNumPCOM +" "											
						EndIF

						IF _nTxDolar > 0   
								_nTxDolar:= Alltrim(Transform(_nTxDolar,"@E 99.9999"))
							If SC5->C5_MOEDA == 2
						   		cMensCli += "Taxa do Dolar utilizada: "+_nTxDolar +" "
							ElseIf SC5->C5_MOEDA == 4
								cMensCli += "Taxa do Euro utilizada: "+_nTxDolar +" "	
					    	EndIF
						EndIF
						_nVez ++
					Endif
                    //////fim customização Prozyn ---- Isaque
						dbSelectArea("SC6")
						dbSetOrder(1)
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
								cMensCli += " "
							EndIf
							If !Empty(SA1->A1_NREDUZ) .And. !(" NOME FANTASIA: " $ cMensCli)
								If	(SC5->C5_TIPO == 'B') .Or. (SC5->C5_TIPO == 'D')
								cMensCli += " NOME FANTASIA: "+trim(Posicione("SA2",1,xFilial("SA2")+SC5->C5_CLIENTE+SC5->C5_LOJACLI,"A2_NREDUZ"))
								Else
								cMensCli += " NOME FANTASIA: "+trim(Posicione("SA1",1,xFilial("SA1")+SC5->C5_CLIENTE+SC5->C5_LOJACLI,"A1_NREDUZ")) 
								EndIf								
							EndIf
							If !Empty(SA1->A1_ENDENT) .AND. !Empty(SA1->A1_BAIRROE) .AND. !Empty(SA1->A1_CEPE) .AND. !Empty(SA1->A1_MUNE) .AND. !Empty(SA1->A1_EST) .And. !(" ENDEREÇO DE ENTREGA: " $ cMensCli)
								cMensCli += " ENDEREÇO DE ENTREGA: "+AllTrim(SA1->A1_ENDENT)+" BAIRRO: "+AllTrim(SA1->A1_BAIRROE)+" CEP: "+AllTrim(SA1->A1_CEPE)+" CIDADE: "+AllTrim(SA1->A1_MUNE)+"-"+AllTrim(SA1->A1_EST)
							EndIf
							If !Empty(SC5->C5_REDESP) .And. !(" TRANSP. REDESPACHO: " $ cMensCli)
								cMensCli += " TRANSP. REDESPACHO: "+Posicione("SA4",1,xFilial("SA4")+SC5->C5_REDESP,"A4_NOME")
							EndIf							
					...
					EndIf
						...
						// Aviso customizado para a PROZYN. Osmair - 03/09/2018
							If !Empty((cAliasSD2)->D2_LOTECTL)
								cDescProd += " LOTE: " + ALLTRIM((cAliasSD2)->D2_LOTECTL)
							EndIf        
		            ...
		            //Prozyn - roberta - 19/10/2016
		            //Se o produto estiver cadastrado na SA5 ou SA7 pegar o codigo e descrição de lá
		         	If SC5->C5_TIPO == "N" 
				         //--A7_FILIAL + A7_CLIENTE + A7_LOJA + A7_PRODUTO
				         SA7->(dbSetOrder(1))
				         If SA7->(MsSeek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD)))
				         	cCodProd  := If( !Empty(SA7->A7_CODCLI) ,SA7->A7_CODCLI ,cCodProd)
				            cDescProd := If( !Empty(SA7->A7_DESCCLI),AllTrim(cDescProd) + " " + AllTrim(SA7->A7_DESCCLI),cDescProd)
				         EndIf
					ElseIf SC5->C5_TIPO == "B"
				      	//--A5_FILIAL + A5_FORNECE + A5_LOJA + A5_PRODUTO
				         SA5->(dbSetOrder(1)) 	         
				         If SA5->(MsSeek( xFilial("SA5") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD))) 
				         	cCodProd  := If( !Empty(SA5->A5_CODPRF),SA5->A5_CODPRF,cCodProd)
				            cDescProd := If( !Empty(SA5->A5_DESREF),AllTrim(cDescProd) + " " + AllTrim(SA5->A5_DESREF),cDescProd)
				         EndIf
			      	EndIf
		            //Prozyn - roberta - 19/10/2016 - termina aqui
				 	  ...
				 	  // O array aProd possui customizacoes especificas para atender a PROZYN.    
						If SF2->F2_DESCZFR > 0
							nUnitario := (cAliasSD2)->D2_PRUNIT
						Else                                   
							nUnitario := Iif(_nOpcUm == 1, IIF(!(cAliasSD2)->D2_TIPO$"IP", IIf(!(cAliasSD2)->D2_TIPO $ "D",(cAliasSD2)->D2_PRCVEN+(nDesconto/(cAliasSD2)->D2_QUANT)+(NoRound((cAliasSD2)->D2_DESCZFR/(cAliasSD2)->D2_QUANT,2)), (cAliasSD2)->D2_PRCVEN),0), IIF(!(cAliasSD2)->D2_TIPO$"IP", IIf(!(cAliasSD2)->D2_TIPO $ "D",(cAliasSD2)->D2_PRC2UM+(nDesconto/(cAliasSD2)->D2_QTSEGUM)+(NoRound((cAliasSD2)->D2_DESCZFR/(cAliasSD2)->D2_QTSEGUM,2)), (cAliasSD2)->D2_PRC2UM),0))
						EndIf
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							SB1->B1_EX_NCM,;
							cD2Cfop,;
							Iif(_nOpcUm == 2, SB1->B1_SEGUM, SB1->B1_UM),;//Customização Prozyn
							Iif(_nOpcUm == 2, (cAliasSD2)->D2_QTSEGUM, (cAliasSD2)->D2_QUANT),;//Customização Prozyn,;
							...
							Iif(_nOpcUm == 2, SB1->B1_SEGUM, retUn2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), SB5->B5_UMDIPI, SB1->B1_UM ) ),;//Customização Prozyn
							Iif(_nOpcUm == 2, (cAliasSD2)->D2_QTSEGUM, retQtd2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), SB5->B5_CONVDIP, (cAliasSD2)->D2_QUANT )),;//Customização Prozyn
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							...
							cCodlan,; //aprod[44]  Código de Benefcio Fiscal na UF aplicado ao item .
				...
				EndIf
				//-------------------------------------------------------------------------------------------------------------------------------------------
				//Adicionado por Ricardo Nisiyama - 26/12/2016
				If AllTrim(SA1->A1_ENDENT) <> ''
					cMensCli += "End Entrega: "+SA1->A1_ENDENT+", "+SA1->A1_BAIRROE+", "+SA1->A1_MUNE+" - "+SA1->A1_ESTE+", "+SC5->C5_OBSCLI+", " //Alterado 24/05/2017
				Else
					If !Empty(Alltrim(SC5->C5_OBSCLI))
						cMensCli += SC5->C5_OBSCLI
					Endif
				Endif				
				//Tratamento implementado para atender a ICMS/PR 2017 (Decreto 7.871/2017) 
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_VUNIT,D1_CLASFIS,D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_LOTECTL,D1_LOTEFOR, //Adicionado campo D1_LOTEFOR - Prozyn
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				Endif
	   			...
	   			//-------------------------------------------------------------------------------------------------------------------------------------------
	   			//Customizado para Prozyn - 08/11/2016
	   			If !Empty(Alltrim(SF1->F1_OBSERV)) .and. nLoop == 1
	   				cMensCli += SF1->F1_OBSERV
	   				nLoop := 0 
	   			Endif
	   			//------------------------------------------------------------------------------------------------------------------------------------------- 
				//Verifica se existe Template DCL
		       ...
		       EndIf   		
				...
				//Prozyn - Derik - 11/11/2016
	            //Para as notas de entrada em formulario proprio na danfe, em frente a descricao do produto deve sair Lote 
				If (cAliasSD1)->D1_FORMUL = "S"
					cDescProd += " - " + (cAliasSD1)->D1_LOTEFOR
				EndIf
				//Termina aqui	 
    			...
    			If (cAliasSD1)->D1_TIPO == "N" 
					//--A5_FILIAL + A5_FORNECE + A5_LOJA + A5_PRODUTO
					SA5->(dbSetOrder(1)) 	         
					If SA5->(MsSeek( xFilial("SA5") + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) ))  
			         	cCodProd  := If( !Empty(SA5->A5_CODPRF),SA5->A5_CODPRF,cCodProd )
			            cDescProd := If( !Empty(SA5->A5_DESREF),SA5->A5_DESREF,cDescProd)
					EndIf
				ElseIf (cAliasSD1)->D1_TIPO == "B"
			        //--A7_FILIAL + A7_CLIENTE + A7_LOJA + A7_PRODUTO
					SA7->(dbSetOrder(1)) 	         
					If SA7->(MsSeek( xFilial("SA7") + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) ))  
			         	cCodProd  := If( !Empty(SA7->A7_CODCLI) ,SA7->A7_CODCLI ,cCodProd )
			            cDescProd := If( !Empty(SA7->A7_DESCCLI),SA7->A1_DESCCLI,cDescProd)
					EndIf
				EndIf
	            //Prozyn - roberta - 19/10/2016 - termina aqui
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					(cAliasSD1)->D1_CF,;
					Iif(_nOpcUm == 2, SB1->B1_SEGUM, SB1->B1_UM),;//Customização Prozyn
					Iif(_nOpcUm == 2, (cAliasSD1)->D1_QTSEGUM, (cAliasSD1)->D1_QUANT),;//Customização Prozyn,;
					...
					Iif(_nOpcUm == 2, SB1->B1_SEGUM, retUn2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD1)->D1_CF), SB5->B5_UMDIPI, SB1->B1_UM )),;//Customização Prozyn
					Iif(_nOpcUm == 2, (cAliasSD1)->D1_QTSEGUM, retQtd2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD1)->D1_CF), SB5->B5_CONVDIP, (cAliasSD1)->D1_QUANT )),;//Customização Prozyn
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
				...
				While !CD2->(Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
					...
					Do Case
						...
						//³de calculo do ICMS ST para notas fiscais de devolucao³
						//³Verifica o parametro MV_ICSTDEV                      ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
If !Empty(aNota)
	...
	//Customização Prozyn - Gustavo Gonzalez
	aProdAgrup		:= {}
 	aICMSAgrup		:= {}
 	aIPIAgrup		:= {}
 	aPISAgrup		:= {}
 	aCOFINSAgrup	:= {}
	aICMSZFMAgrup	:= {}
	fAgrupaItens(aProd,aProdAgrup,aICMS,aICMSAgrup,aIPI,aIPIAgrup,aPIS,aPISAgrup,aCOFINS,aCOFINSAgrup,cTipo,aICMSZFM,aICMSZFMAgrup)
	aProd 		:= aProdAgrup
	aICMS 		:= aICMSAgrup
	aIPI 		:= aIPIAgrup
	aPIS 		:= aPISAgrup
	aCOFINS 	:= aCOFINSAgrup
	aICMSZFM	:= aICMSZFMAgrup
	//
	...
	cString += "</infNFe>"
EndIf
MemoWrite('\xmlteste\xml_PROZYN.xml', EncodeUTF8(cString))
//cString := ''
...
//Rotina especifica Prozyn

Static Function fAgrupaItens(aProd,aProdAgrup,aICMS,aICMSAgrup,aIPI,aIPIAgrup,aPIS,aPISAgrup,aCOFINS,aCOFINSAgrup,cTipo,aICMSZFM,aICMSZFMAgrup)

	Local nQuant:=nTotal:=nQuant2:=nValFre:=nSeguro:=nDesconto:=nBCICMS:=0

	Local nVTICMS:=nQTICMS:=nBCIPI := 0     // Base Calculo IPI [6]
	Local nVTIPI:=nQTIPI := 0     // QTrib IPI        [7]
	Local nVTICMSZFM := 0		//ICMS Zona Franca de Manaus [1]
	...
	if cTipo="1" // nf saida
		ASort(aProd,,,{|a,b| a[2]+a[19] < b[2]+b[19]})
	endif
	...
	do while wX <= Len(aProd)
		...
		if len(aICMS) > 0
			If Len(aICMS[aProd[wX][1]]) > 0
				nBCICMS+=aICMS[aProd[wX][1]][5] 
		...
		if len(aICMSZFM) > 0
			If Len(aICMSZFM[aProd[wX][1]]) > 0
				nVTICMSZFM +=aICMSZFM[aProd[wX][1]][1]
			EndIf
		endif

		if 	  (cTipo="1" .and. wX < Len(aProd).and.;
				aProd[wX][2] == aProd[wX+1][2] .and.;
				aProd[wX][19] == aProd[wX+1][19] .and.;
				aProd[wX][20] == aProd[wX+1][20])
			wX++
			loop
		endif

		For nZ := 1 to Len(aProd[wX])
			If nZ == 1
				aadd(aProdAgrup,{})
				aadd(aProdAgrup[Len(aProdAgrup)],Len(aProdAgrup))
			ElseIf nZ == 9
				aadd(aProdAgrup[Len(aProdAgrup)],nQuant)
			ElseIf nZ == 10
				aadd(aProdAgrup[Len(aProdAgrup)],nTotal)
			ElseIf nZ == 12
				aadd(aProdAgrup[Len(aProdAgrup)],nQuant2)
			ElseIf nZ == 13
				aadd(aProdAgrup[Len(aProdAgrup)],nValFre)
			ElseIf nZ == 14
				aadd(aProdAgrup[Len(aProdAgrup)],nSeguro)
			ElseIf nZ == 15
				aadd(aProdAgrup[Len(aProdAgrup)],nDesconto)
			Else
				aadd(aProdAgrup[Len(aProdAgrup)],aProd[wX][nZ])
			EndIf
		Next nZ		

		if len(aICMS) > 0
			If Len(aICMS[aProd[wX][1]]) > 0
				For nZ := 1 to Len(aICMS[aProd[wX][1]])
					If nZ == 1
						aadd(aICMSAgrup,{})
						aadd(aICMSAgrup[Len(aICMSAgrup)],aICMS[aProd[wX][1]][nZ])
					ElseIf nZ == 5
						aadd(aICMSAgrup[Len(aICMSAgrup)],nBCICMS)
					ElseIf nZ == 7
						aadd(aICMSAgrup[Len(aICMSAgrup)],nVTICMS)
					ElseIf nZ == 9
						aadd(aICMSAgrup[Len(aICMSAgrup)],nQTICMS)
					Else
						aadd(aICMSAgrup[Len(aICMSAgrup)],aICMS[aProd[wX][1]][nZ])
					EndIf
				Next nZ
		...
		if len(aIPI) > 0
			If Len(aIPI[aProd[wX][1]]) > 0 
				For nZ := 1 to Len(aIPI[aProd[wX][1]])
					If nZ == 1
						aadd(aIPIAgrup,{})
						aadd(aIPIAgrup[Len(aIPIAgrup)],aIPI[aProd[wX][1]][nZ])
					ElseIf nZ == 6
						aadd(aIPIAgrup[Len(aIPIAgrup)],nBCIPI)
					ElseIf nZ == 7
						aadd(aIPIAgrup[Len(aIPIAgrup)],nQTIPI)
					ElseIf nZ == 10
						aadd(aIPIAgrup[Len(aIPIAgrup)],nVTIPI)
					Else
						aadd(aIPIAgrup[Len(aIPIAgrup)],aIPI[aProd[wX][1]][nZ])
					EndIf
				Next nZ
		...
		if len(aPIS) > 0
			If Len(aPIS[aProd[wX][1]]) > 0 
				For nZ := 1 to Len(aPIS[aProd[wX][1]])
					If nZ == 1
						aadd(aPISAgrup,{})
						aadd(aPISAgrup[Len(aPISAgrup)],aPIS[aProd[wX][1]][nZ])
					ElseIf nZ == 2
						aadd(aPISAgrup[Len(aPISAgrup)],nBCPIS)
					ElseIf nZ == 4
						aadd(aPISAgrup[Len(aPISAgrup)],nVTPIS)
					ElseIf nZ == 5
						aadd(aPISAgrup[Len(aPISAgrup)],nQTPIS)
					Else
						aadd(aPISAgrup[Len(aPISAgrup)],aPIS[aProd[wX][1]][nZ])
					EndIf
				Next nZ
		...
		if len(aCOFINS) > 0
			If Len(aCOFINS[aProd[wX][1]]) > 0
				For nZ := 1 to Len(aCOFINS[aProd[wX][1]])
					If nZ == 1
						aadd(aCOFINSAgrup,{})
						aadd(aCOFINSAgrup[Len(aCOFINSAgrup)],aCOFINS[aProd[wX][1]][nZ])
					ElseIf nZ == 2
						aadd(aCOFINSAgrup[Len(aCOFINSAgrup)],nBCCOFINS)
					ElseIf nZ == 4
						aadd(aCOFINSAgrup[Len(aCOFINSAgrup)],nVTCOFINS)
					ElseIf nZ == 5
						aadd(aCOFINSAgrup[Len(aCOFINSAgrup)],nQTCOFINS)
					Else
						aadd(aCOFINSAgrup[Len(aCOFINSAgrup)],aCOFINS[aProd[wX][1]][nZ])
					EndIf
				Next nZ
		...
		if len(aICMSZFM) > 0
			If Len(aICMSZFM[aProd[wX][1]]) > 0
				For nZ := 1 to Len(aICMSZFM[aProd[wX][1]])
					If nZ == 1
						aadd(aICMSZFMAgrup,{})
						aadd(aICMSZFMAgrup[Len(aICMSZFMAgrup)],nVTICMSZFM)
					Else
						aadd(aICMSZFMAgrup[Len(aICMSZFMAgrup)],aICMSZFM[aProd[wX][1]][nZ])
					EndIf
				Next nZ
			Else 
				aadd(aICMSZFMAgrup,{})
