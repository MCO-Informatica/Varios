...
PRIVATE aRotina := { { STR0001						, "AxPesqui"  	, 0 , 1},;	//"Pesquisar"
							 ...
							 { 'Planilha Inventario', "U_PLANX30"	, 0 , 3},;	   //"Relatorio"
...
MsgRun('Efetuando pesquisas de estoque / leituras efetuadas','Aguarde..',{|| PLANX30x()})
Return .T.

/*\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\*/
Static Function PLANX30x  
Local _nI        
...
Do While !EoF()      
   ...
   If Empty(_nPos)                            
      ...
      For _nI := 1 To Len(_aLeituras)
          ...
          aAdd(_aExcel[_nTam], 0 - _aExcel[_nTam,6])   						//Diferenca perante ao estoque
      Next  
      If _nTam > 0
...
_cArqXls   := U_ListExc(_aExcel2, _aCabec, _cTitulo) //, _aCores)
...
If mv_par11 == 1
   ...
   Do While !EoF()
      ...
      DbSelectArea('SB7')                       
      DbSetOrder(1)
      Conout('Pesquisa B7 - Produto: '+SB1->B1_COD)

      If !DbSeek(xFilial('SB7')+DtoS(dDataBase)+TMPB2->B2_COD+TMPB2->B2_LOCAL)
         Conout('Incluindo B7 Zerado - Produto: '+SB1->B1_COD)
         RecLock('SB7',.T.)
         Replace B7_FILIAL  With xFilial('SB7')
...
/*\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\*/
User Function ListExc(_aLista, _aCab, _cTitulo, _aCores)   
...
User Function MestrexOper(_cOperCBA)
