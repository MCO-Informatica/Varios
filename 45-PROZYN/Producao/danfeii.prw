...
#DEFINE MAXVALORC  020                                                // Máximo de caracteres por linha de valores numéricos
	...
	If !lImpDir
		...
		#IFDEF TOP
			...
			If MV_PAR04==1
				...
				If !Empty(MV_PAR07) .Or. !Empty(MV_PAR08)//Prozyn - Linha adicionada por Adriano em 06/03/2017
					cWhere += " AND (SF3.F3_EMISSAO >= '"+ SubStr(DTOS(MV_PAR07),1,4) + SubStr(DTOS(MV_PAR07),5,2) + SubStr(DTOS(MV_PAR07),7,2) + "' AND SF3.F3_EMISSAO <= '"+ SubStr(DTOS(MV_PAR08),1,4) + SubStr(DTOS(MV_PAR08),5,2) + SubStr(DTOS(MV_PAR08),7,2) + "')"//Prozyn - Linha adicionada por Adriano em 06/03/2017
				EndIF//Prozyn - Linha adicionada por Adriano em 06/03/2017
			...
			BeginSql Alias cAliasSF3
				...
				SF3.%notdel%
				ORDER BY SF3.F3_NFISCAL //Prozyn - Linha adicionada por Adriano em 06/03/2017
			EndSql
		...
		While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
			...
			If (cAliasSF3)->(Eof())
				...
				//Limpa pasta de Certficados
				aFiles		:= Directory('\CERTIFIQ\*.*','D')
				For nI := 1 to Len(aFiles)
					Ferase('\CERTIFIQ\'+aFiles[nI][1])
				Next nI
				
				//Limpa pasta de Certficados
				aFiles		:= Directory('\Certificados de Analise\*.*','D')
				For nI := 1 to Len(aFiles)
					Ferase('\Certificados de Analise\'+aFiles[nI][1])
				Next nI

				nX := 0
				For nX := 1 to nLenNotas
					//Gerar Certificados
					// cCliente	:= Alltrim(Posicione('SA1',1,xFilial('SA1')+SC5->C5_CLIENTE+SC5->C5_LOJACLI,'A1_NREDUZ'))
					cAssunto	:= 'Certificados de Análise Prozyn - NF: ' + Alltrim(aNotas[nx][4]) + ' ' + Alltrim(aNotas[nx][5])
					// Processa( {|| U_CERTQIP('','','',1,aNotas[nX][5],MV_PAR03,aNotas[nX][5]) },"Aguarde" ,"Gerando Certificado...")
					cFile := U_CERTIF2("","","",1,aNotas[nX][5],MV_PAR03)
					// aFiles		:= Directory('\Certificados de Analise\*.PDF','D')
					aAttachment	:= {}
					aAdd(aAttachment,'\Certificados de Analise\'+cFile+".pdf")
					cMensagem	:= MemoRead('\CERTIFIQ\MODELO_EMAIL\modelo_certifiq.htm') 
					cTO := SuperGetMV("MV_MAILCQ",,"qualidade@prozyn.com.br;expedicao@prozyn.com.br") //2022/07/05 - Massao
					///cTo := "qualidade@prozyn.com.br"
					U_zEnvMail(cTo, cAssunto, cMensagem, aAttachment, .f., .t., .t.)
