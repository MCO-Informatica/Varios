...
ùùùFuncao	 ù EMP650   ù Autor ù Adriano Leonardo    ù Data ù 07/07/2016 ùùù
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?ùù
ùùùDesc.     ù Ponto de entrada na geraùùo de empenhos, utilizado para    ùùù
ùùùDesc.     ù manipular as quantidades de acordo com a atividade enzimù- ùùù
ùùùDesc.     ù tica do produto e forùar lote quando a chamada for da roti-ùùù
ùùùDesc.     ù na RPCPE003.                                               ùùù
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?ùù
ùùùUso       ù Especùfico para a empresa Prozyn               			  ùùù
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?ùù
...
User Function EMP650()
	...
	/*ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?
	//ù Posicùes do aCols para manipulaùùo                                                                                      ù
	//ù aCols[n,x] - Onde o n e o  nùmero da linha  e x pode ser:                                                               ù
	//ù [01] Cùdigo do Produto a ser empenhado                                                                                  ù
	//ù [02] Quantidade do empenho                                                                                              ù
	//ù [03] Almoxarifado do empenho                                                                                            ù
	//ù [04] Sequùncia do componente na estrutura (Campo G1_TRT)                                                                ù
	//ù [05] Sub-Lote utilizado no empenho (Somente deve ser preenchido se o produto utilizar rastreabilidade do tipo "S")      ù
	//ù [06] Lote utilizado no empenho (Somente deve ser preenchido se o produto utilizar rastreabilidade)                      ù
	//ù [07] Data de validade do Lote (Somente deve ser preenchido se o  produto utilizar rastreabilidade)                      ù
	//ù [08] Localizaùùo utilizada no empenho (Somente deve ser preenchido se o produto utilizar controle de localizaùùo fùsica)ù
	//ù [09] Nùmero de Sùrie (Somente deve ser preenchido se o produto utilizar controle de localizaùùo fùsica)                 ù
	//ù [10] 1a. Unidade de Medida do Produto                                                                                   ù
	//ù [11] Quantidade do Empenho na 2a. Unidade de Medida                                                                     ù
	//ù [12] 2a. Unidade de Medida do Produto                                                                                   ù
	//ù [13] Coluna com valor lùgico que indica se a linha estù deletada (.T.) ou nùo (.F.)                                     ù
	//ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù*/
	If FunName() == "RPCPE004" //Rotina de reprocessamento de produtos
		...
		If Type("_aEmp650P")=="A"
			...
			For _nCont := 1 To Len(_aEmp650P)
				...
				//ùIgnorar produtos do tipo MOD ù
				...
				Else
					...
					If _aEmp650P[_nCont,03] == '2' //Somente considera enzima
						...
						//Faùo uma consulta no banco de dados para retornar todos os lotes disponùveis do enzima para empenho na produùùo, considerando o conceito FEFO (First Expired First Out)
						...
						_cQuery += "AND SB8.B8_LOCAL NOT IN ('98','00','92') "
						...
						if _aEmp650P[_nCont,04]<=(_cAlias)->Z1_ATIVIDA
							...
							While (_cAlias)->(!EOF())
								...
								If (_cAlias)->B8_SLDDISP >= _nQtdAux
									...
									_aColsIte[Len(_aColsIte)] := .F. //Flag para linha nùo deletada no aCols


									//Verifico o array de linhas deletadas, caso a linha em questùo esteja deletada, restauro a mesma
								...
								Else
									...
									_aColsIte[aScan(aHeader,{|x|Alltrim(x[2]) == "D4_DTVALID"	})]	:= STOD((_cAlias)->B8_DTVALID)
									_aColsIte[aScan(aHeader,{|x|Alltrim(x[2]) == "D4_LOCAL"		})]	:= (_cAlias)->B8_LOCAL
									_aColsIte[Len(aCols)] := .F. //Flag para linha nùo deletada no aCols
							...
							//Atualizo a descriùùo do produto no empenho
					...
					Else
						...
						_cQry += "AND SB8.B8_LOCAL NOT IN ('98','00','92') "
						_cQry += "AND SB8.B8_DTVALID>='" + DTOS(dDataBase) + "' "
						_cQry += "ORDER BY B8_DTVALID "
		...
		//Segunda etapa, cùlculo da quantidade total da OP
		...
		//Terceira etapa, cùlculo do diluente
		For _nItens := 1 To Len(aCols)
			...
			If dbSeek(xFilial("SG1")+SC2->C2_PRODUTO+_cProduto+_cSequen)
				If SG1->G1_TIPO == '1' //Diluente
					...
					//Percentual de rateio do diluente (para casos em que hù mais de um diluente na fùrmula)
					...
					If aCols[_nItens,aScan(aHeader,{|x|Alltrim(x[2]) == "D4_QUANT"	})] < 0
						...
						// LFSS - 14/08/2017 - 182 - Caluculo de diluente Negativando 						
	...
	//Rotina que Limpa os Empenhos automùticos duplicados - CR-Valdimari Martins - 09/02/2017
	_nTamAcols := Len(aCols) //Preservo o tamanho original do aCols, porque as quebras de lote, geraram itens adicionais, mas nùo interferirùo no cùlculo
	...
	//Primeira etapa, cùlculo da atividade enzimùtica
	...
	For _nItens := 1 To _nTamAcols
		...
		If dbSeek(xFilial("SG1")+SC2->C2_PRODUTO+_cProduto+_cSequen) 
			If SG1->G1_TIPO == '2' //Somente considera enzima
				...
				if SG1->G1_ATIVIDA<=(_cAlias)->Z1_ATIVIDA
					...
					While (_cAlias)->(!EOF()) 
						...
						If Type("_aEmp650")=="A"
							...
							If Len(_aEmp650)>0
								//Verifico se a OP em questùo existe no array
								If aScan(_aEmp650,{|x|Alltrim(x[1]) == AllTrim(SC2->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD))})>0
									//Verifico se o componente corrente possui prioridade de lote para essa ordem de produùùo
									If _aEmp650[aScan(_aEmp650,{|x|Alltrim(x[1]) == AllTrim(SC2->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD))}),1] == SC2->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD) .And. _aEmp650[aScan(_aEmp650,{|x|Alltrim(x[1]) == AllTrim(SC2->(C2_NUM+C2_ITEM+C2_SEQUEN+C2_ITEMGRD))}),2] == SG1->G1_COMP
										...
										//Defino o lote priorizado no empenho
										...
										aCols[_nItens][Len(aCols[_nItens])] := .F. //Flag para linha nùo deletada no aCols
										...
										//Deleto do array o empenho jù utilizado
				...
				Else
					_lAbort2 := .T.
					Aviso("Atenùùo","Produto sem saldo em estoque: "+Alltrim(_cProduto)+".",{"Ok"},2)
		...
		If SG1->G1_TIPO == '2' .and._nQtdAux > 0 
			aCols[_nItens,aScan(aHeader,{|x|Alltrim(x[2]) == "D4_QUANT"		})] := 0 
			//	MsgStop("Falha no cùlculo dos empenhos para Enzima, Sem Saldo para essa quantidade de Enzima!" + " Verificar o Produto: "+ SD4->D4_COD + " Lote: "+ SD4->D4_LOTECTL ,_cRotina+"_002")
			...
			//FA
		EndIf      
		//Restauro o posicionamento real dentro do laùo de repetiùùo (For)
		_nItens := _nBkpIt
	Next
		...
		If dbSeek(xFilial("SG1")+SC2->C2_PRODUTO+_cProduto+_cSequen)
			If SG1->G1_TIPO == '1' //Diluente
				...
				If _nQtdDil > 0 .And. !IsEstrut(SC2->C2_PRODUTO, aCols[_nItens,aScan(aHeader,{|x|Alltrim(x[2]) == "G1_COMP"	})])
					...
					_cQryDil2 += " AND SB8.B8_LOCAL NOT IN ('98','00','92') "+CRLF
					...
					_cQryDil2 += " ORDER BY GI_PRODALT, B8_DTVALID "+CRLF
	...
	For _nCont := 1 To Len(aCols)
		If aScan(_aDelProd,aCols[_nCont,aScan(aHeader,{|x|Alltrim(x[2]) == "G1_COMP"})])==0 .And. aCols[_nCont,aScan(aHeader,{|x|Alltrim(x[2]) == "D4_QUANT"})]>0
			AAdd(_aCols2,aClone(aCols[_nCont]))
		EndIf
	Next
...
Static Function EstonarOP()
	...
	*/
	...
	If MsgYesNo("Enzima(s) com problema no calculo enzimatico, deseja excluir a OP ?","COMA001E_A")  
		_lAbort := .t.
		MsExecAuto({|x,Y| Mata650(x,Y)},aMata650,5) //Exclusùo da OP
	Else 
		sc2->(RecLock('SC2',.F.))
...
Static Function ShowErr(cProd,cLot,nQtd,NSaldo,cTip) 
	...
	If( cTip == 2 /* Calculo de diluente  - Negativando */, "Falha no cùlculo dos empenhos para diluente, para essa quantidade base o diluente ficaria negativo! [Calculo de Diluente  - Negativando]!" ,;
	If( cTip == 3 /* Calculo Enzimatico */ 				, "Falha no cùlculo dos empenhos para Enzima ("+alltrim(SB1->B1_COD)+'-'+alltrim(SB1->B1_DESC)+"), Lote ("+Alltrim(cLot)+"), Qtd.Aux:"+alltrim(str(nQtd))+"! [Calculo Enzimatico]! " ,;
	If( cTip == 5 /* Erro no diluente */					, "Falha no cùlculo dos empenhos para diluente, informe ao Administrador do sistema! [Erro no diluente]!" ,;
	If( cTip == 6 /* Falha empenho */						, "Falha no cùlculo dos empenhos, informe ao Administrador do sistema! [Falha Empenho] !","Erro")))))                                                                                 
...
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?ùù
ùùùPrograma  ùIsEstrut	  ùAutor  ùMicrosiga	      ù Data ù  30/08/2019ùùù
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?ùù
ùùùDesc.     ùVerifica se o produto existe na estrutura                   ùùù
ùùù          ù                                                            ùùù
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?ùù
ùùùUso       ù 	                                                          ùùù
ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù?ùù
