...
For nZ := 1 To nLenDet
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
		...
		aadd(aItens,{;
			...
			"",;
			"",;
			iif(SUBSTR(oDet[nX]:_Prod:_CFOP:TEXT,1,1)="7" .and. nLinhDF=1, oDet[nX]:_Prod:_ucom:TEXT, ""),;
   			iif(SUBSTR(oDet[nX]:_Prod:_CFOP:TEXT,1,1)="7" .and. nLinhDF=1, oDet[nX]:_Prod:_qcom:TEXT, ""),;
		    iif(SUBSTR(oDet[nX]:_Prod:_CFOP:TEXT,1,1)="7" .and. nLinhDF=1, oDet[nX]:_Prod:_vuncom:TEXT, ""),;
			SubStr(aAux[3], 1, PosQuebrVal(aAux[3])),;
			SubStr(aAux[4], 1, PosQuebrVal(aAux[4])),;
			SubStr(aAux[5], 1, PosQuebrVal(aAux[5])),;
			SubStr(aAux[6], 1, PosQuebrVal(aAux[6])),;
...
If lMv_Logod
	...
	cLogoD := "\imagens\DANFE" + cDescLogo + ".bmp"/*GetSrvProfString("Startpath","") + "DANFE" + cDescLogo + ".BMP"*/
	If !File(cLogoD)
		cLogoD	:= "\imagens\DANFE" + cEmpAnt + ".bmp"/*GetSrvProfString("Startpath","") + "DANFE" + cEmpAnt + ".BMP"*/
		If !File(cLogoD)
			lMv_Logod := .F.
		EndIf
	EndIf
