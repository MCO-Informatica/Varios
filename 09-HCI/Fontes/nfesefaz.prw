	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							...
							// customizado
							IF lInfCom == .F.                                                                                                                                                              
								cMensCli += AllTrim(SC5->C5_MENNOTA) + Iif(!Empty(SC5->C5_XOBSABA),(" - " + AllTrim(SC5->C5_XOBSABA)),"") + " - " + AllTrim("Pedido Interno:" + SC5->C5_NUM)+IIF(FieldPos("C6_NUMFI") > 0,IIF(TRIM(C6_NUMFI)="","...","-"+C6_NUMFI),"") + IIF(!Empty(SC5->C5_XCOMPRA)," - Comprador:" + SC5->C5_XCOMPRA,"") + Iif(SC6->C6_CLI $ _cCliPetrob, IIF(!Empty(SC6->C6_NUMPCOM)," - Pedido Cliente:" + SC6->C6_NUMPCOM,""), IIF(!Empty(SC6->C6_PV)," - Pedido Cliente:" + SC6->C6_PV,""))
								lInfCom := .T.
							EndIF		
							// customizado
							EndIf
						...
						EndIf
					// customizado
					//Alterado 16/07/2015 (BZO) - Conforme analise de envio do XML para a Petrobras, para a impressão da TAG xPed e xItemPed
					
					cComDes:= ""

					If SC6->C6_CLI $ _cCliPetrob
						IF Len(AllTrim(SC6->C6_NUMPCOM))>0 
					      cComDes	:=	"-Ped.Cli. "+Alltrim(SC6->C6_NUMPCOM)
						EndIF
						
						IF Len(AllTrim(SC6->C6_ITEMPC))>0 
						   cComDes	:=	cComdes+"-Item: "+AllTrim(SC6->C6_ITEMPC)
						EndIF
					Else
						IF Len(AllTrim(SC6->C6_PV))>0 
					      cComDes	:=	"-Ref. "+Alltrim(SC6->C6_PV)
						ELSE 
						  IF Len(AllTrim(SC5->C5_COTCLI))>0 
						    cComDes	:=	"-Ref. "+Alltrim(SC5->C5_COTCLI)
						  EndIF
						EndIF
						IF Len(AllTrim(SC6->C6_ITEMCLI))>0 
						   cComDes	:=	cComdes+"-Item: "+AllTrim(SC6->C6_ITEMCLI)
						EndIF 
					EndIf
					
					IF Len(AllTrim(SC6->C6_NUMREF))>0 
					   cComDes	:=	cComdes+"-Cod.: "+AllTrim(SC6->C6_NUMREF) 
					EndIF 
                    
					IF SC6->C6_CLI='000068'
					   cComDes := cComDes+" - PV:"+AllTrim(SC6->C6_NUM)+"/"+AllTrim(SC6->C6_ITEM)  
					ENDIF
					// customizado
...
//						cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI) 
						cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)+AllTrim(cComDes) // customizado 
...
//						If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3-4-5-6-8" .And. cCSTrib $ "00-10-20-30-40-41-50-51-60-70-90")						
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							SB1->B1_EX_NCM,;
							cD2Cfop,;
							IIF((cAliasSD2)->D2_CLIENTE='000503' .OR. (cAliasSD2)->D2_CLIENTE='007938' .OR. (cAliasSD2)->D2_CLIENTE='000479',IIF(SB1->B1_UM="PC","UN",SB1->B1_UM),SB1->B1_UM),;
							(cAliasSD2)->D2_QUANT,;
							...
							retUn2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), SB5->B5_UMDIPI, IIF((cAliasSD2)->D2_CLIENTE='000503' .OR. (cAliasSD2)->D2_CLIENTE='007938' .OR. (cAliasSD2)->D2_CLIENTE='000479',IIF(SB1->B1_UM="PC","UN",SB1->B1_UM),SB1->B1_UM)),;
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				// customizado
				If SF1->(FieldPos("F1_OBSNOTA"))>0
					If !AllTrim(SF1->F1_OBSNOTA) $ cMensCli
						If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
							cMensCli += " "
						EndIf
						cMensCli += AllTrim(SF1->F1_OBSNOTA)
					EndIf
				EndIf
