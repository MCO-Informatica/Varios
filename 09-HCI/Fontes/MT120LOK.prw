...
±±º Programa    ³ MT120LOK ³ Ponto de entrada para complementar a inicialização do item daº±±
...
IF ACOLS[N,LEN(ACOLS[N])]=.T.
  ...
  IF MsSeek(xFilial("SZK")+"OC"+cOc+substring(aCols[n, nIt],2,3))
    MsgInfo("Exclusao de Item de Pedido nao permitido... Itens amarrados com OC/OS existentes. Elimine a Amarração para solucao do problema")
    LRET:=.F.
  ENDIF
ENDIF
/*
