...
Função para gravação do campo Memo e gravação na tabela temporária.
...
User Function fGrvTrb(_nOpc)
	...
	lOCAL cQuery		:= ""
	Local _cMsg			:= ""
	Local _cAssunto		:= "Aviso de Orçamento - Nº " + SZQ->ZQ_NUMERO
	Local _cPara		:= ""
				...
				For nJ := 1 To Len(_aCampos)
					...
					ElseIf _aCampos[NJ] == "ZQ_XHISTOR"
						IF !EMPTY(M->ZQ_XOBS)
							...
							_cAssunto		:= "Aviso de Follow Up - Nº " + SZQ->ZQ_NUMERO
							...
							For _nI	:= 1 To Len(_aEmailHst)
								...
								_cMsg	+= '		<p>'+CRLF
								_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Foi realizado a atualização do histórico do Follow Up: </p></FONT>'+CRLF 
								...
								_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Histórico: ' + MSMM(SZQ->ZQ_XHISTOR) + '.</p></FONT>'+CRLF
		...
		Else
			...
			Else
				...
				If !Empty(SZQ->ZQ_CONTATO)
					...
					_cMsg	+= '	<body> '+CRLF
					_cMsg	+= '		<hr size=2 width=100% align=center>'+CRLF
					_cMsg	+= '		<FONT SIZE = "5" COLOR = "#CD2626"><B><center>Orçamento - ' + Alltrim(SZQ->ZQ_NUMERO) + '</center></B></FONT>'+CRLF
					_cMsg	+= '		<hr size=2 width=100% align=center>'+CRLF
					_cMsg	+= '		<p>'+CRLF
					_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Prezado(a) ' + _cVend +',</p></FONT>'+CRLF
					_cMsg	+= '		<p>'+CRLF
					_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Recebemos uma consulta de preço: </p></FONT>'+CRLF 
					_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Cliente: ' + POSICIONE("SA1",1,xFilial("SA1") + SZQ->ZQ_CLIENTE + SZQ->ZQ_LOJA,"A1_NOME") + '.</p></FONT>'+CRLF
					_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Contato: ' + POSICIONE("SU5",1,xFilial("SU5") + SZQ->ZQ_CONTATO,"U5_CONTAT") + '.</p></FONT>'+CRLF
					_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Status = Em Execução </p></FONT>'+CRLF
					_cMsg	+= '		<FONT COLOR = "#CD2626"><p>** Enviaremos outra notificação assim que o orçamento for finalizado.</p></FONT>'+CRLF
					_cMsg	+= '		<FONT COLOR = "#CD2626"><p>Grato(a).</p></FONT>'+CRLF
					_cMsg	+= '		<hr size=2 width=100% align=center>'+CRLF
					_cMsg	+= '	</body>'+CRLF
					_cMsg	+= '</html>'+CRLF
...
//-------------------------------------------------------------------
/*/{Protheus.doc} fTOkZQ
Função para gravação do campo Memo.
