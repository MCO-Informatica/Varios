...
EndIf
	...
	cWhere := "% AND  ZBZ.ZBZ_DTRECB >='"+cDtIni+"' AND  ZBZ.ZBZ_DTRECB<='"+cDtFim+"' AND ZBZ.ZBZ_MAIL IN "+cEspecP
	...
	BeginSql Alias cZBZ
		...
		SELECT *
			...
			WHERE ZBZ.%notdel%
			%Exp:cWhere%
    		ORDER BY ZBZ_FILIAL,ZBZ_DTRECB
	EndSql      
...
If (cZBZ)->(EOF())
	@nLin,00 PSAY "Nenhum registro foi encontrado com os parâmetros selecionados."+(cZBZ)->ZBZ_FILIAL
Else
	...
	While (cZBZ)->(!EOF())
		...
		cFilProc := (cZBZ)->ZBZ_FILIAL
		@nLin,00 PSAY "Filial - "+(cZBZ)->ZBZ_FILIAL
		nLin ++    
		@nLin,nCol + 00 PSAY cLinCab  
		nLin ++    

		While (cZBZ)->ZBZ_FILIAL == cFilProc .And. (cZBZ)->(!EOF())
			...
			nScan := AScan(aStat, {|x| x[1] == (cZBZ)->ZBZ_MAIL}) 
	        If nScan <= 1  
				nScan := 1
			EndIf
			cEspecNf := ""
			If (cZBZ)->ZBZ_MODELO =="55"
				cEspecNf := "NF-e"
			ElseIf (cZBZ)->ZBZ_MODELO =="57"
				cEspecNf := "CT-e"	
			EndIf
					
		    @nLin,nCol + 00 PSAY cEspecNf
			@nLin,nCol + 10 PSAY (cZBZ)->ZBZ_SERIE
		    @nLin,nCol + 17 PSAY (cZBZ)->ZBZ_NOTA
		    @nLin,nCol + 30 PSAY ConvDate(1 , (cZBZ)->ZBZ_DTNFE)
		    @nLin,nCol + 40 PSAY ConvDate(1 , (cZBZ)->ZBZ_DTRECB)
...
PutSx1(cPerg,"05","Status E-mail   "	,"Status E-mail   "				,"Status E-mail   "				,"mv_ch5","C",20,0,0,"G","",   "","","","mv_par05","","","","","","","","","","","","","","","","",aHelpPor,aHelpPor,aHelpPor)
