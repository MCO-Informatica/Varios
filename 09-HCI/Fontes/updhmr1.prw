...
/*/
//--------------------------------------------------------------------
User Function UPDHMR1( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDHMR1" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDHMR1" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC2'																	, .T. }, ; //X3_ARQUIVO
	{ 'D9'																	, .T. }, ; //X3_ORDEM
	{ 'C2_XRETCOM'															, .T. }, ; //X3_CAMPO
	{ 'C'																		, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Retrab/Comer'															, .T. }, ; //X3_TITULO
	{ 'Retrab/Comer'															, .T. }, ; //X3_TITSPA
	{ 'Retrab/Comer'															, .T. }, ; //X3_TITENG
	{ 'Retrabalho/Comercializ.'												, .T. }, ; //X3_DESCRIC
	{ 'Retrabalho/Comercializ.'												, .T. }, ; //X3_DESCSPA
	{ 'Retrabalho/Comercializ.'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																		, .T. }, ; //X3_VALID
	...
	{ ''																		, .T. }, ; //X3_OBRIGAT
	{ ''																		, .T. }, ; //X3_VLDUSER
	{ 'C=Comercializacao;R=Retrabalho'										, .T. }, ; //X3_CBOX
	{ 'C=Comercializacao;R=Retrabalho'										, .T. }, ; //X3_CBOXSPA
	{ 'C=Comercializacao;R=Retrabalho'										, .T. }, ; //X3_CBOXENG
	{ ''																		, .T. }, ; //X3_PICTVAR
	{ ''																		, .T. }, ; //X3_WHEN
	{ ''																		, .T. }, ; //X3_INIBRW
	{ ''																		, .T. }, ; //X3_GRPSXG
...
aAdd( aSX3, { ;
	{ 'SG2'																	, .T. }, ; //X3_ARQUIVO
	{ '43'																	, .T. }, ; //X3_ORDEM
	{ 'G2_XDESDOC'															, .T. }, ; //X3_CAMPO
	{ 'C'																		, .T. }, ; //X3_TIPO
	{ 40																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Desen/Doc'																, .T. }, ; //X3_TITULO
	{ 'Desen/Doc'																, .T. }, ; //X3_TITSPA
	{ 'Desen/Doc'																, .T. }, ; //X3_TITENG
	{ 'Desenho/Documento'														, .T. }, ; //X3_DESCRIC
	{ 'Desenho/Documento'														, .T. }, ; //X3_DESCSPA
	{ 'Desenho/Documento'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																		, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Informa se a Ordem de produção é' )
aAdd( aHlpPor, 'Retrabalho ou Comercialização.' )
aHlpEng := {}
aHlpSpa := {}

PutHelp( "PC2_XRETCOM", aHlpPor, aHlpEng, aHlpSpa, .T. )
AutoGrLog( "Atualizado o Help do campo " + "C2_XRETCOM" )

//
// Helps Tabela SG2
//
aHlpPor := {}
aAdd( aHlpPor, 'Informa o Desenho/Documento da operação.' )
aHlpEng := {}
aHlpSpa := {}

PutHelp( "PG2_XDESDOC", aHlpPor, aHlpEng, aHlpSpa, .T. )
AutoGrLog( "Atualizado o Help do campo " + "G2_XDESDOC" )
