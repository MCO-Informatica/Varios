...
/*/
//--------------------------------------------------------------------
User Function UPDCARMAR( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDCARMAR" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDCARMAR" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Tipo PE '															, .T. }, ; //X3_TITSPA
	{ 'Tipo PE '															, .T. }, ; //X3_TITENG
	{ 'Tipo PE Espec '														, .T. }, ; //X3_DESCRIC
	{ 'Tipo PE Espec '														, .T. }, ; //X3_DESCSPA
	{ 'Tipo PE Espec '														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ ''																	, .T. }, ; //X3_VLDUSER
	{ 'D=Dias;M=Mes;A=Ano'													, .T. }, ; //X3_CBOX
	{ 'D=Dias;M=Mes;A=Ano'													, .T. }, ; //X3_CBOXSPA
	{ 'D=Dias;M=Mes;A=Ano'													, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
...
aAdd( aSX3, { ;
	...
	{ 3																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tempo PE '															, .T. }, ; //X3_TITULO
	{ 'Tempo PE '															, .T. }, ; //X3_TITSPA
	{ 'Tempo PE '															, .T. }, ; //X3_TITENG
	{ 'Tempo PE Espec'														, .T. }, ; //X3_DESCRIC
	{ 'Tempo PE Espec'														, .T. }, ; //X3_DESCSPA
	{ 'Tempo PE Espec'														, .T. }, ; //X3_DESCENG
	{ '@E 999'																, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX6, { ;
	...
	'ES_CARSCGR'															, ; //X6_VAR
	'C'																		, ; //X6_TIPO
	'Grupo de Produtos para Regra do PE Espe'								, ; //X6_DESCRIC
	''																		, ; //X6_DSCSPA
	''																		, ; //X6_DSCENG
	'cifico (SC na Carga Maquina)'											, ; //X6_DESC1
	''																		, ; //X6_DSCSPA1
	''																		, ; //X6_DSCENG1
	''																		, ; //X6_DESC2
	''																		, ; //X6_DSCSPA2
...
aAdd( aSX6, { ;
	...
	'ES_CARSCTP'															, ; //X6_VAR
	'C'																		, ; //X6_TIPO
	'Tipo  de Produtos para Regra do PE Espe'								, ; //X6_DESCRIC
	''																		, ; //X6_DSCSPA
	''																		, ; //X6_DSCENG
	'cifico (SC na Carga Maquina)'											, ; //X6_DESC1
	''																		, ; //X6_DSCSPA1
