...
Static _lAcrDSa	:= .F. //variavel customizada para indicar o acrescimo ou não de um dia na data de saida
/*/
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//Customizado
		_cxEmailC    :=  AllTrim(POSICIONE("SA3", 1, xFilial("SA3") + SF2->F2_VEND1, "A3_EMAIL"))
		//Fim Customizado 
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³Busca dados do ISS                                                      ³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			...
			// Customizado - Pega e-mail vendedor
			_cxEmailC    :=  Alltrim(POSICIONE("SA3", 1, xFilial("SA3") + SF2->F2_VEND1, "A3_EMAIL"))
			// Fim Customizado
			...
			Else
				...
				aadd(aTransp,AllTrim(SA4->A4_CGC))
 			    ...
 			    // Customizado - Adicionado telefone nome da Transportadora
				aadd( aTransp,Alltrim(SA4->A4_NOME)+" - "+Alltrim(SA4->A4_TEL) ) // aadd(aTransp,SA4->A4_NOME)
                //Fim Customizacao
					If (SA4->A4_TPTRANS <> "3")
			...
			/*
			//Customizado Inicio
			_cEspeci1 	:= Posicione("SC5",1,xFilial("SC5")+Posicione("SD2",3,xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA,"D2_PEDIDO"),"C5_ESPECI1")
			_nVolume1 	:= Posicione("SC5",1,xFilial("SC5")+Posicione("SD2",3,xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA,"D2_PEDIDO"),"C5_VOLUME1")
			...
			If _lVolume
				aadd(aEspVol,{ _cEspeci1, _nVolume1 , _nPesoLiq , _nPesoBru}) 
				_lVolume := .f.
			EndIf
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					If dbSeek(xFilial("SE4")+SC5->C5_CONDPAG,.F.)
						If !("Cond. Pagto.: " + Alltrim(SE4->E4_DESCRI) + Space(80-len("Cond. Pagto.: "+Alltrim(SE4->E4_DESCRI))) $ cMensCli)
							cMensCli += "Cond. Pagto.: " + Alltrim(SE4->E4_DESCRI) + Space(80-len("Cond. Pagto.: "+Alltrim(SE4->E4_DESCRI)))
						EndIf
					Else
						If !("Ver Cond. Pagto.: " + Alltrim(SC5->C5_CONDPAG) + Space(80-len("Ver Cond. Pagto.: "+Alltrim(SC5->C5_CONDPAG))) $ cMensCli)
							cMensCli += "Ver Cond. Pagto.: " + Alltrim(SC5->C5_CONDPAG) + Space(80-len("Ver Cond. Pagto.: "+Alltrim(SC5->C5_CONDPAG)))
						EndIf
					EndIf
									
					//----> INICIO DO BLOCO IMPLEMENTADO EM 04/02/2011 POR RICARDO CORREA DE SOUZA (MVG CONSULTORIA)
					If !("Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA + Space(80-len("Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA)) $ cMensCli)
						cMensCli += "Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA + Space(80-len("Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA))
					EndIf


					If !("Vendedor 1: " + Alltrim(SC5->C5_VEND1) + Space(80-len("Vendedor 1: "+Alltrim(SC5->C5_VEND1))) $ cMensCli)
						cMensCli += "Vendedor 1: " + Alltrim(SC5->C5_VEND1) + Space(80-len("Vendedor 1: "+Alltrim(SC5->C5_VEND1)))
					EndIf

					If !("Pedido Interno: " + Alltrim(SC5->C5_NUM) + Space(80-len("Pedido Interno: "+Alltrim(SC5->C5_NUM))) $ cMensCli)
						cMensCli += "Pedido Interno: " + Alltrim(SC5->C5_NUM) + Space(80-len("Pedido Interno: "+Alltrim(SC5->C5_NUM)))
					EndIf

					If !(Empty(SC6->C6_PEDCLI) .And. !"Pedido Cliente: " + Alltrim(SC6->C6_PEDCLI) + Space(80-len("Pedido Cliente: "+Alltrim(SC6->C6_PEDCLI))) $ cMensCli)
						cMensCli += "Pedido Cliente: " + Alltrim(SC6->C6_PEDCLI) + Space(80-len("Pedido Cliente: "+Alltrim(SC6->C6_PEDCLI)))
					EndIf
					
					//26/09/2016, corrigido redundancia de mensagem (CTP)
					//If !(Empty(SC5->C5_MENNOTA) .And. !Alltrim(SC5->C5_MENNOTA)+Space(80-len(Alltrim(SC5->C5_MENNOTA))) $ cMensCli)
					If !Empty(SC5->C5_MENNOTA) .And. !Alltrim(SC5->C5_MENNOTA) $ cMensCli
						cMensCli += Alltrim(SC5->C5_MENNOTA)+Space(80-len(Alltrim(SC5->C5_MENNOTA)))
					EndIf
					
					//----> OBSERVACOES CLIENTE (FATURAMENTO) - RICARDO SOUZA 30/07/2018
					If !Empty(SA1->A1_DHFATOB) .And. !Alltrim(SA1->A1_DHFATOB) $ cMensCli
						cMensCli += Alltrim(SA1->A1_DHFATOB)+Space(80-len(Alltrim(SA1->A1_DHFATOB)))
					EndIf

					//----> COLOCANDO MENSAGEM AUTOMÁTICA QUANDO O PRODUTO CONTÉM NÚMERO DE SÉRIE. - LUCAS BAÍA 01/09/2022
					IF SB5->B5_XNUMSER == "S"
						IF !" O serviço de assistência técnica  e instalação do equipamento coberto pela Garantia será prestado por representantes autorizados, em até um raio de 100 KM do ponto de venda da assistência técnica mais próxima. As despesas com viagem, transporte, estadia e alimentação dos técnicos serão arcadas pelo cliente que estiver situado fora da área de atuação das empresas credenciadas." $ cMensCli
						cMensCli += " O serviço de assistência técnica  e instalação do equipamento coberto pela Garantia será prestado por representantes autorizados, em até um raio de 100 KM do ponto de venda da assistência técnica mais próxima. As despesas com viagem, transporte, estadia e alimentação dos técnicos serão arcadas pelo cliente que estiver situado fora da área de atuação das empresas credenciadas."+Space(80-len("O serviço de assistência técnica  e instalação do equipamento coberto pela Garantia será prestado por representantes autorizados, em até um raio de 100 KM do ponto de venda da assistência técnica mais próxima. As despesas com viagem, transporte, estadia e alimentação dos técnicos serão arcadas pelo cliente que estiver situado fora da área de atuação das empresas credenciadas."))
						ENDIF
					ENDIF


					//27/09, indica o acrescimo de 1 dia na data de saída, atualiza variavel static para ser
					//utilizada posteriormente na formação da tag <dhsai>
					if SC5->C5_XDSAIDA <> '2'
					...
					//Customizacao DayHome - 16/06/2015 - Solicitado por Damaris/Larissa
					//Edmar Mendes do Prado (Ethosx Consultoria)
					If Len((cAliasSD2)->D2_NFORI) > 0 .And. !Empty((cAliasSD2)->D2_NFORI) .And. Alltrim((cAliasSD2)->D2_TIPO) == "I"
						If !" Nota Complementar de ICMS/ST da NFe:" $ cMensCli
							cMensCli += "Nota Complementar de ICMS/ST da NFe:" + Alltrim((cAliasSD2)->D2_NFORI) + "-" + Alltrim((cAliasSD2)->D2_SERIORI) + Space(80-len("Nota Complementar de ICMS/ST da NFe:" + Alltrim((cAliasSD2)->D2_NFORI) + "-" + Alltrim((cAliasSD2)->D2_SERIORI)))
						EndIf
					EndIf

					If !" Confira a mercadoria e a quantidade de volumes no ato da entrega."+Space(80-len("Confira a mercadoria e a quantidade de volumes no ato da entrega."))$ cMensCli
						cMensCli += " Confira a mercadoria e a quantidade de volumes no ato da entrega."+Space(80-len("Confira a mercadoria e a quantidade de volumes no ato da entrega."))
					EndIf
					
					If !" Nao aceitaremos reclamacoes posteriores."+Space(80-len("Nao aceitaremos reclamacoes posteriores."))$ cMensCli
						cMensCli += " Nao aceitaremos reclamacoes posteriores."+Space(80-len("Nao aceitaremos reclamacoes posteriores."))
					EndIf

					If !Empty(SA1->A1_ENDENT) .And. (SA1->A1_ENDENT) <> (SA1->A1_END)
						If !" Local Entrega: " + Alltrim(SA1->A1_ENDENT) + Space(80-len("Local Entrega: "+Alltrim(SA1->A1_ENDENT))) $ cMensCli
							cMensCli += "Local Entrega: " + Alltrim(SA1->A1_ENDENT) + Space(80-len("Local Entrega: "+Alltrim(SA1->A1_ENDENT)))
						EndIf

						If !"               " + Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE) + Space(80-len("               "+Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE))) $ cMensCli
							cMensCli += "               " + Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE) + Space(80-len("               "+Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE)))
						EndIf
					EndIf
							
					If !Empty(SA1->A1_ENDCOB) .And. (SA1->A1_ENDCOB) <> (SA1->A1_END)
						If !" Local Cobranca: " + Alltrim(SA1->A1_ENDCOB) + Space(80-len("Local Cobranca: "+Alltrim(SA1->A1_ENDCOB))) $ cMensCli
							cMensCli += "Local Cobranca: " + Alltrim(SA1->A1_ENDCOB) + Space(80-len("Local Cobranca: "+Alltrim(SA1->A1_ENDCOB)))
						EndIf

						If !"                " + Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC) + Space(80-len("                "+Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC))) $ cMensCli
							cMensCli += "                " + Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC) + Space(80-len("                "+Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC)))
						EndIf
					...
					//Fim Customização
						...
						If (cAliasSD2)->D2_FILIAL$"01"
							cDescProd := IIF(!Empty(SB1->B1_DESCATA),SB1->B1_DESCATA,SB1->B1_DESC) 
						Else
							cDescProd := SB1->B1_DESC 
						EndIf
 
						
						// INICIO DAYHOME - CUSTOMIZADO - FONTANELLI	
						cQuery := "SELECT Z4_NUMSERI FROM "+RetSqlName("SZ4")+" "
						cQuery += " WHERE Z4_FILIAL  = '"+xFilial("SZ4")+"' "
						cQuery += "   AND Z4_NOTA    = '"+(cAliasSD2)->D2_DOC+"' "
						cQuery += "   AND Z4_SERIE   = '"+(cAliasSD2)->D2_SERIE+"' "
						cQuery += "   AND Z4_CLIENTE = '"+(cAliasSD2)->D2_CLIENTE+"' "
						cQuery += "   AND Z4_LOJA    = '"+(cAliasSD2)->D2_LOJA+"' "
						cQuery += "   AND Z4_NUMPED  = '"+(cAliasSD2)->D2_PEDIDO+"' "
						cQuery += "   AND Z4_CODIGO  = '"+(cAliasSD2)->D2_COD+"' "
						cQuery += "   AND D_E_L_E_T_ = '' "
						dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),"QrySZ4",.T.,.T.)
						dbSelectArea("QrySZ4")
						QrySZ4->(dbGoTop())
						if !Empty(QrySZ4->Z4_NUMSERI)
							cDescProd += ", SERIAL: " +alltrim(QrySZ4->Z4_NUMSERI)   
						Endif
						QrySZ4->(DbCloseArea())
						// FIM DAYHOME - CUSTOMIZADO - FONTANELLI
						...
						aadd(aProd,	{Len(aProd)+1,;
							cCodProd,;
						    Iif(!Empty(SB1->B1_EAN13),SB1->B1_EAN13,Iif(!Empty(SB1->B1_UPC12),SB1->B1_UPC12,"")),;//CUSTOMIZAÇÃO DAYHOME PARA O CODIGO DE BARRA PEGAR DO B1_EAN13
							cDescProd,;
						    ...
						    SB1->B1_UMRES,;
							(cAliasSD2)->D2_QUANT,;
							IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+0/*nDesconto*/+(cAliasSD2)->D2_DESCZFR,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),; // Customizado
							SB1->B1_UMRES,;
							(cAliasSD2)->D2_QUANT,;
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							...
							Iif(!Empty(SB1->B1_EAN13),SB1->B1_EAN13,Iif(!Empty(SB1->B1_UPC12),SB1->B1_UPC12,"")),;//aprod[45]  Código de barra da segunda unidade de medida.
							Iif(!Empty(SB1->B1_EAN13),SB1->B1_EAN13,Iif(!Empty(SB1->B1_UPC12),SB1->B1_UPC12,"")),;
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !Empty(SF1->F1_NDI) .AND. !"DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI)+Space(80-len(Alltrim("DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI))))$cMensCli
					cMensCli += "DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI)+Space(80-len(Alltrim("DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI))))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 1)
				If !Empty(SF1->F1_X_OBS1) .AND. !Alltrim(SF1->F1_X_OBS1)+Space(80-len(Alltrim(SF1->F1_X_OBS1)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS1)+Space(80-len(Alltrim(SF1->F1_X_OBS1)))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 2)
				If !Empty(SF1->F1_X_OBS2) .AND. !Alltrim(SF1->F1_X_OBS2)+Space(80-len(Alltrim(SF1->F1_X_OBS2)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS2)+Space(80-len(Alltrim(SF1->F1_X_OBS2)))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 3)
				If !Empty(SF1->F1_X_OBS3) .AND. !Alltrim(SF1->F1_X_OBS3)+Space(80-len(Alltrim(SF1->F1_X_OBS3)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS3)+Space(80-len(Alltrim(SF1->F1_X_OBS3)))
				EndIf
				
				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 4)
				If !Empty(SF1->F1_X_OBS4) .AND. !Alltrim(SF1->F1_X_OBS4)+Space(80-len(Alltrim(SF1->F1_X_OBS4)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS4)+Space(80-len(Alltrim(SF1->F1_X_OBS4)))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 5)
				If !Empty(SF1->F1_X_OBS5) .AND. !Alltrim(SF1->F1_X_OBS5)+Space(80-len(Alltrim(SF1->F1_X_OBS5)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS5)+Space(80-len(Alltrim(SF1->F1_X_OBS5)))
				EndIf
				//Fim Customizado
				//Tratamento para nota sobre Cupom 
				DbSelectArea("SFT")
...
If cVeramb >= "3.10"
	...
	//26/09/2016, acrescenta um dia na data de saída para as notas fiscais, tratar o data dia util (CTP)
	if SubStr(aProd[1][7],1,1) >= "5"
		
		IF _lAcrDSa
		
			cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(Lastday(aNota[03]+1,3))+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00")))
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	if !lBrinde
		...
		xxx_eMAIL := AllTrim(aDest[16])
		
		if EMPTY(xxx_eMAIL) // VAZIO
			if !EMPTY(cMailTrans)
			   xxx_eMAIL := cMailTrans
			endif
	    else // NÃO VAZIO
			if !EMPTY(cMailTrans)
			   xxx_eMAIL := xxx_eMAIL + ";" + cMailTrans
			endif
		endif
		...
		if EMPTY(xxx_eMAIL) // VAZIO
			...
			if !EMPTY(_cxEmailC)
			   xxx_eMAIL := xxx_eMAIL + ";" + _cxEmailC
			endif
		endif
		
		cString += NfeTag('<EMAIL>',ConvType(xxx_eMAIL,250))
...
If  !lIssQn    
	...
	If Len(aIcmsST)>0	
		...
		Else
			cString += '<motDesICMS>'+ConvType(0)+'</motDesICMS>' 
			cString		+= NfeTag('<vICMSDeson>',ConvType(0,15,2))
			cMotDesICMS:= ConvType(0)
		EndIf
...
If !lIssQn
	...
	If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
		...
		// MCInfotec		
		// Ricardo Tavares
		// Alterado para receber o Valor de Base de Cálculo do COFINS ao invéis
		// do Valor de Base de Cálculo Total
		// Conforme solicitação do Sr. Rafael do Financeiro
		// 03/11/2014
	//	cString += '<vBC>'      +ConvType(aDI[17][03],15,2)+'</vBC>'
		If Len(aCOFINSST)=0
			nValCof:=0.00
			cString += '<vBC>'      +ConvType(nValCof,15,2)+'</vBC>'
		Else
			cString += '<vBC>'      +ConvType(aCOFINS[02],15,2)+'</vBC>'
		EndIf
		//Fim Customização
