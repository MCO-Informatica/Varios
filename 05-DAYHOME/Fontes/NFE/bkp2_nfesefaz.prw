	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					If dbSeek(xFilial("SE4")+SC5->C5_CONDPAG,.F.)
						If !("Cond. Pagto.: " + Alltrim(SE4->E4_DESCRI) + Space(88-len("Cond. Pagto.: "+Alltrim(SE4->E4_DESCRI))) $ cMensCli)
							cMensCli += "Cond. Pagto.: " + Alltrim(SE4->E4_DESCRI) + Space(88-len("Cond. Pagto.: "+Alltrim(SE4->E4_DESCRI)))
						EndIf
					Else
						If !("Ver Cond. Pagto.: " + Alltrim(SC5->C5_CONDPAG) + Space(88-len("Ver Cond. Pagto.: "+Alltrim(SC5->C5_CONDPAG))) $ cMensCli)
							cMensCli += "Ver Cond. Pagto.: " + Alltrim(SC5->C5_CONDPAG) + Space(88-len("Ver Cond. Pagto.: "+Alltrim(SC5->C5_CONDPAG)))
						EndIf
					EndIf
									
					//----> INICIO DO BLOCO IMPLEMENTADO EM 04/02/2011 POR RICARDO CORREA DE SOUZA (MVG CONSULTORIA)
					If !("Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA + Space(88-len("Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA)) $ cMensCli)
						cMensCli += "Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA + Space(88-len("Codigo Cliente: " + (cAliasSD2)->D2_CLIENTE+"/"+(cAliasSD2)->D2_LOJA))
					EndIf


					If !("Vendedor 1: " + Alltrim(SC5->C5_VEND1) + Space(88-len("Vendedor 1: "+Alltrim(SC5->C5_VEND1))) $ cMensCli)
						cMensCli += "Vendedor 1: " + Alltrim(SC5->C5_VEND1) + Space(88-len("Vendedor 1: "+Alltrim(SC5->C5_VEND1)))
					EndIf

					If !("Pedido Interno: " + Alltrim(SC5->C5_NUM) + Space(88-len("Pedido Interno: "+Alltrim(SC5->C5_NUM))) $ cMensCli)
						cMensCli += "Pedido Interno: " + Alltrim(SC5->C5_NUM) + Space(88-len("Pedido Interno: "+Alltrim(SC5->C5_NUM)))
					EndIf

					If !(Empty(SC6->C6_PEDCLI) .And. !"Pedido Cliente: " + Alltrim(SC6->C6_PEDCLI) + Space(88-len("Pedido Cliente: "+Alltrim(SC6->C6_PEDCLI))) $ cMensCli)
						cMensCli += "Pedido Cliente: " + Alltrim(SC6->C6_PEDCLI) + Space(88-len("Pedido Cliente: "+Alltrim(SC6->C6_PEDCLI)))
					EndIf
					
					//26/09/2016, corrigido redundancia de mensagem (CTP)
					//If !(Empty(SC5->C5_MENNOTA) .And. !Alltrim(SC5->C5_MENNOTA)+Space(88-len(Alltrim(SC5->C5_MENNOTA))) $ cMensCli)
					If !Empty(SC5->C5_MENNOTA) .And. !Alltrim(SC5->C5_MENNOTA) $ cMensCli
						cMensCli += Alltrim(SC5->C5_MENNOTA)+Space(88-len(Alltrim(SC5->C5_MENNOTA)))
					EndIf
					
					//----> OBSERVACOES CLIENTE (FATURAMENTO) - RICARDO SOUZA 30/07/2018
					If !Empty(SA1->A1_DHFATOB) .And. !Alltrim(SA1->A1_DHFATOB) $ cMensCli
						cMensCli += Alltrim(SA1->A1_DHFATOB)+Space(88-len(Alltrim(SA1->A1_DHFATOB)))
					EndIf
					...
					If Len((cAliasSD2)->D2_NFORI) > 0 .And. !Empty((cAliasSD2)->D2_NFORI) .And. Alltrim((cAliasSD2)->D2_TIPO) == "I"
						If !"Nota Complementar de ICMS/ST da NFe:" $ cMensCli
							cMensCli += "Nota Complementar de ICMS/ST da NFe:" + Alltrim((cAliasSD2)->D2_NFORI) + "-" + Alltrim((cAliasSD2)->D2_SERIORI) + Space(88-len("Nota Complementar de ICMS/ST da NFe:" + Alltrim((cAliasSD2)->D2_NFORI) + "-" + Alltrim((cAliasSD2)->D2_SERIORI)))
						EndIf
					EndIf

					If !"Confira a mercadoria e a quantidade de volumes no ato da entrega."+Space(88-len("Confira a mercadoria e a quantidade de volumes no ato da entrega."))$ cMensCli
						cMensCli += "Confira a mercadoria e a quantidade de volumes no ato da entrega."+Space(88-len("Confira a mercadoria e a quantidade de volumes no ato da entrega."))
					EndIf
					
					If !"Nao aceitaremos reclamacoes posteriores."+Space(88-len("Nao aceitaremos reclamacoes posteriores."))$ cMensCli
						cMensCli += "Nao aceitaremos reclamacoes posteriores."+Space(88-len("Nao aceitaremos reclamacoes posteriores."))
					EndIf

					If !Empty(SA1->A1_ENDENT) .And. (SA1->A1_ENDENT) <> (SA1->A1_END)
						If !"Local Entrega: " + Alltrim(SA1->A1_ENDENT) + Space(88-len("Local Entrega: "+Alltrim(SA1->A1_ENDENT))) $ cMensCli
							cMensCli += "Local Entrega: " + Alltrim(SA1->A1_ENDENT) + Space(88-len("Local Entrega: "+Alltrim(SA1->A1_ENDENT)))
						EndIf

						If !"               " + Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE) + Space(88-len("               "+Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE))) $ cMensCli
							cMensCli += "               " + Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE) + Space(88-len("               "+Alltrim(SA1->A1_BAIRROE)+" - "+Alltrim(SA1->A1_MUNE)+" - "+Alltrim(SA1->A1_ESTE)+" - "+Alltrim(SA1->A1_CEPE)))
						EndIf
					EndIf
							
					If !Empty(SA1->A1_ENDCOB) .And. (SA1->A1_ENDCOB) <> (SA1->A1_END)
						If !"Local Cobranca: " + Alltrim(SA1->A1_ENDCOB) + Space(88-len("Local Cobranca: "+Alltrim(SA1->A1_ENDCOB))) $ cMensCli
							cMensCli += "Local Cobranca: " + Alltrim(SA1->A1_ENDCOB) + Space(88-len("Local Cobranca: "+Alltrim(SA1->A1_ENDCOB)))
						EndIf

						If !"                " + Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC) + Space(88-len("                "+Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC))) $ cMensCli
							cMensCli += "                " + Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC) + Space(88-len("                "+Alltrim(SA1->A1_BAIRROC)+" - "+Alltrim(SA1->A1_MUNC)+" - "+Alltrim(SA1->A1_ESTC)+" - "+Alltrim(SA1->A1_CEPC)))
						EndIf
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !Empty(SF1->F1_NDI) .AND. !"DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI)+Space(88-len(Alltrim("DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI))))$cMensCli
					cMensCli += "DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI)+Space(88-len(Alltrim("DI: "+Alltrim(SF1->F1_NDI)+" DATA: "+Dtoc(SF1->F1_DTDI))))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 1)
				If !Empty(SF1->F1_X_OBS1) .AND. !Alltrim(SF1->F1_X_OBS1)+Space(88-len(Alltrim(SF1->F1_X_OBS1)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS1)+Space(88-len(Alltrim(SF1->F1_X_OBS1)))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 2)
				If !Empty(SF1->F1_X_OBS2) .AND. !Alltrim(SF1->F1_X_OBS2)+Space(88-len(Alltrim(SF1->F1_X_OBS2)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS2)+Space(88-len(Alltrim(SF1->F1_X_OBS2)))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 3)
				If !Empty(SF1->F1_X_OBS3) .AND. !Alltrim(SF1->F1_X_OBS3)+Space(88-len(Alltrim(SF1->F1_X_OBS3)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS3)+Space(88-len(Alltrim(SF1->F1_X_OBS3)))
				EndIf
				
				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 4)
				If !Empty(SF1->F1_X_OBS4) .AND. !Alltrim(SF1->F1_X_OBS4)+Space(88-len(Alltrim(SF1->F1_X_OBS4)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS4)+Space(88-len(Alltrim(SF1->F1_X_OBS4)))
				EndIf

				//----> NOTA FISCAL IMPORTACAO (OBSERVACAO 5)
				If !Empty(SF1->F1_X_OBS5) .AND. !Alltrim(SF1->F1_X_OBS5)+Space(88-len(Alltrim(SF1->F1_X_OBS5)))$cMensCli
					cMensCli += Alltrim(SF1->F1_X_OBS5)+Space(88-len(Alltrim(SF1->F1_X_OBS5)))
				EndIf
				//Fim Customizado
				//Tratamento para nota sobre Cupom 
				DbSelectArea("SFT")
