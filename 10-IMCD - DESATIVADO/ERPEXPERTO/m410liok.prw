...
*/
	...
	IF CEMPANT <> '04'
		...
		nAliqICMS	:= (aCOLS[n , GDFieldPos( 'C6_XICMEST', aHeader)] )
		nAliqPCC	:= (aCOLS[n , GDFieldPos( 'C6_XPISCOF', aHeader)])
		cCliente 	:= M->C5_CLIENTE
		cLoja 		:= M->C5_LOJACLI
		cTipoCli	:= M->C5_TIPOCLI
		lValid := .T.

		TESTEIMP(cCliente, cLoja, cTipoCli, n,cProduto,cTES,nQTDVEN,nPRCVEN,nAliqICMS, nAliqPCC, lValid)
	...
	if lValid
		IF _nAliqIcm <> nAliqICMS
			MSGALERT("Aliquota de ICMS Informada "+TRANSFORM(nAliqICMS,"@E 99.99") +;
				" É Diferente do Calculado (Planilha) "+TRANSFORM(_nAliqIcm,"@E 99.99") ,"ITEM "+TRANSFORM(nLin,"@E 9999"))
		ENDIF
