	...
	DbGoTop()
		...
		dbSetOrder(3)	
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				cMoeda:=Alltrim(str(SF2->F2_MOEDA))
				DbSelectArea("SYF")
				DbSetOrder(7)
				...
				Else
					//aadd(aPed,"UYU")                                   //5
					Aviso( "FISA057","Moneda no encontrada. Actualice la tabla de Monedas (SYF).",{"OK"},3)
					Return()
				EndIf
		...
		If DbSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
			...
			If SF1->F1_TIPO $ "DB|N|C"  .Or. Alltrim(SF1->F1_ESPECIE)=="NCI" .Or. Alltrim(SF1->F1_ESPECIE)=="NCC"
				...
				While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
					...
					cMoeda:=Alltrim(str(SF1->F1_MOEDA))
					DbSelectArea("SYF")
					DbSetOrder(7)
					If(SYF->(Dbseek(xFilial("SYF")+cMoeda)))              //5
						aadd(aPed,SYF->YF_MOEDA)
...
Static Function GeraXML(aCabNf,aDest,aPed,aProd,cTipo,cMensCli,nTkBruto)
	...
	Local aRetMntIV  	:= {}
	Local aNfOri	 	:= {}
	Local aConX		:= StrTokArr (GETMV("MV_URUCONX"),",")
	Local aTempArea	:= {}
	Local aDadosCae	:= {}
	...
	if cTipoCli == "3"  .and.  cTipoEx == "1" 		//Extrangeiro e compra fora do pais
		If alltrim(aCabNF[10]) == "RFN"
			cString +="<ns0:eRem_Exp>"
		Else
			cString +="<ns0:eFact_Exp>"
		Endif
	ElseIf cTipoCli == "3"  .and.  cTipoEx == "2" 	//Extrangeiro e compra dentro do pais
		If alltrim(aCabNF[10]) == "RFN"
			cString +="<ns0:eRem>"
		else
			cString +="<ns0:eTck>"
	...
	If alltrim(aCabNF[10]) == "RFN"
		...
		aCabNF[1] := Alltrim(aCabNF[1])
		nTam := LEN(aCabNF[1])
		cString+="<ns0:Serie>"+Alltrim(SubStr(aCabNF[1],nTam))+"</ns0:Serie>"
	Endif
	...
	if cTipoCli == "3" .AND. cTipoEx == "1"
		...
		cString+="<ns0:ClauVenta>"+aPed[11]+"</ns0:ClauVenta>"
		cString+="<ns0:ModVenta>1</ns0:ModVenta>"
		cString+="<ns0:ViaTransp>9</ns0:ViaTransp>"
	...
	For Nx := 1 to Len(aProd)
		...
		cString+="<ns0:NomItem>"+Alltrim(aProd[nX][21])+"</ns0:NomItem>"
		...
		If alltrim(aCabNF[10]) <> "RFN"
			...
			cString+="<ns0:DescuentoMonto>"+Convtype(aProd[nX][18],15,2)+"</ns0:DescuentoMonto>"
	...
	If (cEspeci $ "NCC|NCI|NDE|NCP|NF") .And. (cTipo <> "1")	//Comprobantes de entrada
				...
				If (!empty(aProd[nY][12]) .And. !empty(aprod[nY][11])) .Or. (!empty (cNfSerPe) .And. !empty (cNfOriPe))
					...
					If CompRef(aprod) <= 40 .And. (!Empty(cNfOri) .Or. !Empty(cNfOriPe))
						...
						if SF2->(dbSeek(xFilial("SF2")+cNfOri+cSerOri))
							if ASCAN(aNfaProd, { |x| x[1] == ALLTRIM(SF2->F2_CAEE) } ) == 0
								...
								cRefNoElec += Alltrim(SF2->F2_SERIE) + "-" + left(AllTrim(Str(int(val(SF2->F2_DOC)))),90)
								...
								AADD(aNfaProd,{aprod[nY][11],aprod[nY][12]})
		...
		cString+="</ns0:Referencia>"
	ElseIf (cEspeci $ "NDC|NDI|NCE|NDP|NF")		//Comprobantes de salida
		cString+="<ns0:Referencia>"
		For nY :=1 to len (aProd)
			cNfOri   :=(aprod[nY][11])
			cSerOri  :=(aprod[nY][12])
...
Static Function RetCodPais(cPais,lNomePais)
	...
	If DbSeek(xFilial("SYA")+cPais)
		if lNomePais
			cCod := alltrim(SYA->YA_DESCR)
		else
			cCod := Substr(SYA->YA_SGLLOC,1,2)
		endif
	Endif
...
Static Function RetDadosCae(cSerie,cNumero,cEspecie)
	...
	If SFP->(FieldPos("FP_CAI")) > 0
		...
		If (SFP->(DbSeek(xFilial("SFP") + cFilAnt + cSerie)))
			...
			While SFP->FP_FILIAL+SFP->FP_FILUSO+alltrim(SFP->FP_SERIE) == xFilial("SFP")+cFilAnt+cSerie
