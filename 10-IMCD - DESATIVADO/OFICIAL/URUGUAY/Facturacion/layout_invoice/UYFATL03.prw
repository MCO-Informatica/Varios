...
@type function
/*/
User Function UYFATL03( cAlias, nReg, nOpcx )
Local oReport
Local cPergunta		:= "UYFATL03"
Local lExec			:= .T.
Local aArea			:= GetArea()
...
BeginSQL Alias cAliasQry
	...
	,C5_BANCO ,C5_XNUMAGE,C5_XNUMCON, C5_XFRETE, C5_XSEGURO 
	...
	LEFT JOIN
	(
	SELECT C5_NUM, C5_EMISSAO, C5_XVIA, C5_XINCO, C5_USERLGI ,C5_BANCO ,C5_XNUMAGE,C5_XNUMCON, C5_XFRETE, C5_XSEGURO
	FROM %Table:SC5%
	...
	While !(cAliasQry)->(Eof())
		...
		If cF2Doc != (cAliasQry)->F2_DOC .Or. cF2Serie != (cAliasQry)->F2_SERIE .Or. cF2Clie != (cAliasQry)->F2_CLIENTE .Or. cF2Loja != (cAliasQry)->F2_LOJA
			If lFirstRec
				lFirstRec := .F.
			Else
				RectLotes(oReport, .T., nF2Moeda, nNFTotal, nFrete, nSeguro, cIncoTer)
				MakeFooter(oReport)
				oReport:EndPage()
			EndIf
			...
			cIncoTer	:= (cAliasQry)->C5_XINCO
			nFrete		:= (cAliasQry)->C5_XFRETE
			nSeguro		:= (cAliasQry)->C5_XSEGURO
			nNFPage		:= 1
		...
		oReport:Say(oReport:Row(), MARGEM_ESQUERDA_BOX_GERAL + 20, Transform( (cAliasQry)->D2_QUANT, PesqPict("SD2", "D2_QUANT") ), oFontSt01)
		oReport:Say(oReport:Row(), MARGEM_ESQUERDA_BOX_GERAL + 280, PadC( AllTrim( (cAliasQry)->B1_UM ), 4), oFontSt01)
...
Local oFontSt04	:= TFont():New('Arial',,-10,,.T.)
...
oReport:Say(nRow, MARGEM_ESQUERDA_BOX_GERAL + 200, AllTrim((cAliasQry)->A1_NOME), oFontSt04)
oReport:Say(nRow - 5, nHalfPage + 100, "SHIP TO:", oFontSt01)
oReport:Say(nRow, nHalfPage + 300, cNomeEnt, oFontSt04)
nRow += nLinSize
oReport:Say(nRow, MARGEM_ESQUERDA_BOX_GERAL + 200, AllTrim((cAliasQry)->A1_END), oFontSt04)
oReport:Say(nRow, nHalfPage + 300, cEndEnt, oFontSt04)
nRow += nLinSize
oReport:Say(nRow, MARGEM_ESQUERDA_BOX_GERAL + 200, AllTrim((cAliasQry)->A1_MUN), oFontSt04)
oReport:Say(nRow, nHalfPage + 300, cMunEnt, oFontSt04)
nRow += nLinSize
oReport:Say(nRow, MARGEM_ESQUERDA_BOX_GERAL + 200, AllTrim((cAliasQry)->PAIS_CLI), oFontSt04)
oReport:Say(nRow, nHalfPage + 300, cPaisEnt, oFontSt04)
nRow += nLinSize
oReport:Say(nRow, MARGEM_ESQUERDA_BOX_GERAL + 200, "Doc. Id.: " + AllTrim((cAliasQry)->RUT_CLI), oFontSt04)
oReport:Say(nRow, nHalfPage + 300, "Doc. Id.: " + cRUTEnt, oFontSt04)
nRow += nLinSize
oReport:Say(nRow, MARGEM_ESQUERDA_BOX_GERAL + 200, "CP: " + AllTrim((cAliasQry)->A1_CEP), oFontSt04)
oReport:Say(nRow, nHalfPage + 300, "CP: " + cCEPEnt, oFontSt04)
Return
...
oReport:Line(oReport:Row() + 40, MARGEM_ESQUERDA_BOX_GERAL + 278,  MARGEM_INFERIOR_BOX_ITEMS, MARGEM_ESQUERDA_BOX_GERAL + 275 )
...
@type function
/*/
Static Function RectLotes(oReport, lPrintTotal, nMoeda, nTotal, nFrete, nSeguro, cIncoTer)
...
Local nRowFimBox	:= MARGEM_INFERIOR_BOX_GERAL - 250 //120
Local nRowTopBox	:= MARGEM_INFERIOR_BOX_ITEMS
Local nColLin1		:= MARGEM_ESQUERDA_BOX_GERAL + 650
Local nColLin2		:= MARGEM_ESQUERDA_BOX_GERAL + 1550 //1920
...
//oReport:Line(nRowFimBox, nColLin1-100,  MARGEM_INFERIOR_BOX_GERAL, nColLin1-100 )
...
If lPrintTotal
	...
	nPos2 := 250
	nPos3 := 350
	oReport:Say(nRowFimBox + 010, nColLin2 + nPos1	, "FCA VALUE", oFontSt04)
	oReport:Say(nRowFimBox + 010, nColLin2 + nPos2	, cMoeda, oFontSt04)
	oReport:Say(nRowFimBox + 010, nColLin2 + nPos3	, Transform(nTotal-(nFrete+nSeguro), PesqPict("SF2", "F2_VALBRUT")), oFontSt02)

	oReport:Say(nRowFimBox + 060, nColLin2 + nPos1	, "Int Freight", oFontSt04)
	oReport:Say(nRowFimBox + 060, nColLin2 + nPos2	, cMoeda, oFontSt04)
	oReport:Say(nRowFimBox + 060, nColLin2 + nPos3	, Transform(nFrete, PesqPict("SF2", "F2_VALBRUT")), oFontSt02)

	oReport:Say(nRowFimBox + 110, nColLin2 + nPos1	, "Int Insurance", oFontSt04)
	oReport:Say(nRowFimBox + 110, nColLin2 + nPos2	, cMoeda, oFontSt04)
	oReport:Say(nRowFimBox + 110, nColLin2 + nPos3	, Transform(nSeguro, PesqPict("SF2", "F2_VALBRUT")), oFontSt02)

	oReport:Say(nRowFimBox + 165, nColLin2 + nPos1	, cIncoTer+" Value", oFontSt04)
	oReport:Say(nRowFimBox + 165, nColLin2 + nPos2	, cMoeda, oFontSt04)
	oReport:Say(nRowFimBox + 165, nColLin2 + nPos3	, Transform(nTotal, PesqPict("SF2", "F2_VALBRUT")), oFontSt02)
...
oReport:Say(nRowFimBox + 40, MARGEM_ESQUERDA_BOX_GERAL + 800 /*890*/, '"Thank You For Your Order!"', oFontSt03)
...
Else
	...
	If lNewIntent
		If lImpAuto
			U_UYFatL03(cAlias, (cAlias)->(Recno()))
		Else
			U_UYFatL03()
		EndIf
	EndIf
...
If ValType(oReport) == "O"
	...
	If !Empty((cAliasQry)->C5_USERLGI)
		...
		If !Empty(cUsrPV)
			oReport:Say(MARGEM_INFERIOR_BOX_GERAL - 200, MARGEM_ESQUERDA_BOX_GERAL + 100, PadC(AllTrim(cUsrPV),30), oFontSt02)
			oReport:Say(MARGEM_INFERIOR_BOX_GERAL - 150, MARGEM_ESQUERDA_BOX_GERAL + 100, PadC("FOREING TRADE",30), oFontSt02)
		EndIf
	EndIf
EndIf
