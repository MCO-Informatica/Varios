...
±±³Fun‡…o    ³ REST006  ³ Autor ³ Marcos V. Ferreira    ³ Data ³ 16/06/06 ³±±
...
±±³Richard     ³12/05/10³      ³ Regras de Confidencialidade              ³±±
...
User Function REST006()
	...
	//oLogTXT := EPLOGTXT():NEW( UPPER(ALLTRIM(FUNNAME())) , "REST006" , __cUserID )

	Private lAdmVendas, cGrpAcesso

	cGrpAcesso := ""
	lAdmVendas := (__cUserId $ GetMv("MV_TABPR94"))		// Parametro que tras os usuarios de Adm.Vendas que podem ver todas as informacoes

	If ! lAdmVendas
		cGrpAcesso := GrpsAcesso(__cUserId)
	Endif
...
Static Function ReportDef()
	...
	//³                                                                        ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oReport:= TReport():New("REST006",STR0001,"MTR260", {|oReport| ReportPrint(oReport,aOrdem,cAliasTRB)},STR0002+" "+STR0003+" "+STR0004) //"Relacao da Posicao do Estoque"
...
Static Function MontaTrab(oReport,nOrdem,cAliasTRB,oSection)
	...
	If ! lAdmVendas
		...
		//³Inclui regra de confidencialidade³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		cWhereB1 += " AND (SB1.B1_SEGMENT = ' ' OR (SB1.B1_SEGMENT <> ' ' AND SB1.B1_SEGMENT IN " + FormatIn(cGrpAcesso,"/")+"))"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³REST006R3 ³ Autor ³ Eveli Morasco         ³ Data ³ 01/03/93 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Posicao dos Estoques                                       ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ Generico                                                   ³±±
...
Static Function R260Select( lEnd )
	...
	oTmpTable := FWTemporaryTable():New(cFileTRB)  
	oTmpTable:SetFields(aCampos) 
	oTmpTable:AddIndex("1", cIndxKEY)
	oTmpTable:Create()
	...
	If ! lAdmVendas
		...
		//³Inclui regra de confidencialidade³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		cWhereB1 += " AND SB1.B1_SEGMENT IN " + FormatIn(cGrpAcesso,"/")
...
Static Function GrpsAcesso(cCodUser)
	...
	cSql += "    VEN.A3_GRPVEN <> ' '	AND " 
	cSql += "    GER.A3_COD = VEN.A3_GEREN "
	cSql += "WHERE "
	cSql += "  GER.A3_CODUSR = '" + cCodUser + "' AND "
	cSql += "  GER.A3_GRPVEN <> ' ' " 
	cSql += "GROUP BY "
	cSql += "  VEN.A3_GRPVEN "
