...
Private cCadastro := "Conciliação DDA"
...
If !FDdaInUse() .and. !lAutomato 
	Alert ("O tratamento para DDA - Débito Direto Autorizado não está corretamente implementado."+CHR(13)+;  
...
pergunte("FIN260",.F.)
If !lAutomato
	aADD(aSays,"Esta rotina tem como objetivo a conciliação das informações importadas através de") 
...
If nReconc == 9   // Se não reconciliado
...
If !lAutomato
	...
	If Empty(FIG->FIG_DDASE2)
		MsgAlert("Registro não conciliado.") 
		lRet := .F.
	Endif
	...
	If lRet
		...
		If !MsSeek(aDadosSe2[1]+aDadosSe2[2]+aDadosSe2[3]+aDadosSe2[4]+aDadosSe2[5]+aDadosSe2[6]+aDadosSe2[7])
			MsgAlert("Registro de Contas a pagar não encontrado. Não será possivel cancelar esta conciliação.") 
			lRet := .F.
		Endif
	Endif
	...
	If lRet .and. SE2->E2_SALDO == 0
		MsgAlert ("O titulo relacionado a esta conciliação se encontra totalmente baixado."+CHR(13)+"Não será possivel o cancelamento da conciliação.") 
		lRet := .F.
	Endif
Endif
...
If lRet
	...
	If nOpcA == 1
		...
		If !Empty(SE2->E2_IDCNAB) .and. !lAutomato
			If !MsgYesNo("Este título possui IDCNAB preenchido e pode ter sido enviado para pagamento via CNAB. Confirma cancelamento?","Atenção")
				lRet := .F.
			Endif
		Endif
...
If !U_IMC260VF()
	Help(" ",1,"NOALTER",,"Este registro não permite alterações. Apenas titulos com dados a verificar podem sofrer alteração. Vide Legenda.",1,0)
	Return .T.
Endif
...
Aadd(aLegenda, {"DISABLE", "DDA Não Conciliado"})
...
Aadd(aLegenda, {"DISABLE"		, "Não Conciliado"})	//"Não Conciliado"
