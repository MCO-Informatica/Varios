...
wnrel:="IMCD139"            //Nome Default do relatorio em Disco
...
PRIVATE 	cInSE1 := 'E1_FILIAL=="'+xFilial("SE1")+'".And.'
cInSE1 += 'E1_CLIENTE>="'+mv_par01+'".and.E1_CLIENTE<="'+mv_par02+'".And.'
cInSE1 += 'E1_PREFIXO>="'+mv_par03+'".and.E1_PREFIXO<="'+mv_par04+'".And.'
cInSE1 += 'E1_NUM>="'+mv_par05+'".and.E1_NUM<="'+mv_par06+'".And.'
cInSE1 += 'DTOS(E1_VENCREA)>="'+DTOS(mv_par09)+'".and.DTOS(E1_VENCREA)<="'+DTOS(mv_par10)+'".And.'
cInSE1 += 'E1_NFELETR>="'+mv_par07+'".and.E1_NFELETR<="'+mv_par08+'".And.'
cInSE1 += 'DTOS(E1_EMISSAO)>="'+DTOS(mv_par13)+'".and.DTOS(E1_EMISSAO)<="'+DTOS(mv_par14)+'"'
...
aItens :={}
aCabExp := {"Prefixo","Numero","Parcela","nfes","Cliente","Loja","Nome Do Cliente","Tipo",;
"Natureza","Data de Emissao","Vencimento","Venc. Real",	"Banco","Situacao","Vlr Original","Segmento"}
...
While !Eof() .and. M0_CODIGO == cEmpAnt .and. IIf( lFWCodFil, FWGETCODFILIAL, SM0->M0_CODFIL ) <= cFilAte
	...
	Elseif nOrdem = 4
		...
		#ELSE
			...
			IndRegua("SE1",cIndexSe1,cChaveSe1,,cInSE1,OemToAnsi(STR0022))
	...
	While &cCond1 .and. !Eof() .and. lContinua .and. E1_FILIAL == xFilial("SE1")
		...
		While &cCond2==cCarAnt .and. !Eof() .and. lContinua .and. E1_FILIAL == xFilial("SE1")
			...
			IF li > 58
				nAtuSM0 := SM0->(Recno())
				SM0->(dbGoto(nRegSM0))
				cabec(titulo,cabec1,cabec2,nomeprog,tamanho,nComp)
				SM0->(dbGoTo(nAtuSM0))
			EndIF
			...
			cChvSD2	:= XFILIAL("SD2")+SE1->E1_NUM+SE1->E1_PREFIXO + SE1->E1_CLIENTE+SE1->E1_LOJA
			
			dbSelectArea("SE1")
			
			cSegmento := bscPedido(cChvSD2)
			
			aadd(aItens, {SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,SE1->E1_NFELETR,;
			SE1->E1_CLIENTE ,SE1->E1_LOJA ,SubStr(SA1->A1_NREDUZ,1,17),SE1->E1_TIPO,;
			SE1->E1_NATUREZ,SE1->E1_EMISSAO,SE1->E1_VENCTO,SE1->E1_VENCREA,;
			SE1->E1_PORTADO,SE1->E1_SITUACA	,nVlrOrig,cSegmento} )
...
Else
	DlgToExcel({ {"ARRAY", "Posicao dos Titulos a Receber", aCabExp, aItens} })
Endif
aCabExp := aCabExp[1]
...
STATIC FUNCTION BSCPEDIDO(cChvSD2)
Local cGrpVen := ''
Local aArea2 := GetArea()

dbSelectArea("SD2")
SD2->(DbSetOrder(3)) //D2_FILIAL+D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA
SD2->(DbSeek(cChvSD2 , .F.))

dbSelectArea("SC5")
SC5->(DbSetOrder(1))
If SC5->(DbSeek(xFilial("SC5") +  SD2->D2_PEDIDO, .F.))
	
	if EMPTY(SC5->C5_GRPVEN )
		DbSelectArea("ACY")
		DbSetOrder(1)
		If DbSeek(xFilial("ACY")+SC5->C5_GRPVEN)
			cGrpVen    := SC5->C5_GRPVEN+" - " +ACY->ACY_DESCRI
		ENDIF
	ELSE
		
		SA1->(DbSetOrder(1))
		If SA1->(DbSeek(xFilial("SA1") + SD2->D2_CLIENTE+SD2->D2_LOJA, .F.))
			
			DbSelectArea("ACY")
			DbSetOrder(1)
			If DbSeek(xFilial("ACY")+SA1->A1_GRPVEN)
				cGrpVen    := SA1->A1_GRPVEN+" - " +ACY->ACY_DESCRI
			ENDIF
		ENDIF
