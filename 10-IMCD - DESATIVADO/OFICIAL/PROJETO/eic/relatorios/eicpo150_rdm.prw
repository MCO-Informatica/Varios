...
#DEFINE LITERAL_PAGINA             IF( nIdioma == INGLES, "Page: "             , "Pßgina: ")
#DEFINE LITERAL_FORNECEDOR         IF( nIdioma == INGLES, "SUPPLIER........: " , "FORNECEDOR......: ")
#DEFINE LITERAL_ENDERECO           IF( nIdioma == INGLES, "ADDRESS.........: " , "ENDEREÃO........: ")
...
#DEFINE LITERAL_DESCRICAO          IF( nIdioma == INGLES, "Description"        , "DescriþÒo")
#DEFINE LITERAL_FABRICANTE         IF( nIdioma == INGLES, "Manufacturer"       , "Fabricante")
#DEFINE LITERAL_PRECO_UNITARIO1    IF( nIdioma == INGLES, "Unit"               , "Preþo")
#DEFINE LITERAL_PRECO_UNITARIO2    IF( nIdioma == INGLES, "Price"              , "Unitßrio")
...
#DEFINE LITERAL_OBSERVACOES        IF( nIdioma == INGLES, "REMARKS"            , "OBSERVAÃiES")
...
cCadastro := "Seleção de Purchase Order"
nPagina:=0
...
dbSelectArea("SW2")  
//set filter to  SW2->W2_CONAPRO == "L"    // FILTRO NO BROWSER PARA PO APROVADOS   // JUNIOR CARVALHO 
...
Local aRotina :=  { { "Pesquisar","AxPesqui"  , 0 , 1},; //
                    { "Emissao","Eval(bFuncao)", 0 , 0}} //"Emissao"###"ReEmissao"
...
If W2_CONAPRO <> "L"
	...
	AVISO('PEDIDO BLOQUEADO', 'O pedido '+W2_PO_NUM+', ainda não foi Liberado', { "OK" }, 1) // JUNIOR CARVALHO 
...
@  8,10 TO 48,80 TITLE "Selecione o Idioma"
...
IF nMarcados == 0
   MsgInfo("NÒo existem registros marcados para a impressÒo !","AtenþÒo") //"NÒo existem registros marcados para a impressÒo !"###"AtenþÒo"

ELSE
   dbSelectArea("SW2")

   AVPRINT oPrn NAME "EmissÒo do Pedido"
      ...
      AVPAGE
		...
		//³CHAMADA DA ROTINA PARA IMPRESSÃO DO RELATORIO DO EIC                        ³
		//³TINHA COLOCADO UMA PERGUNTA PARA SABER SE IMPRIMIA O RELATORIO ANTIGO OU NÃO³
		//³MAIS A VANESSA SOLICITOU QUE DEIXA-SE O NOVO SEMPRE                         ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ

         //if aviso("Versão de Relatorio","Voce deseja imprimir a Versão Nova ou Versão Original do Relatorio",{"Nova","Original"})==2
         	Processa({|X| lEnd := X, PO150Det() })
         //else
         //	Processa({|X| lEnd := X, U_IMCD150(aMarcados,nIdioma) })
         //endif
...
IF GetMv("MV_ID_EMPR") == 'S'
   c2EndSM0 := c2EndSM0 +IF( !EMPTY(SM0->M0_BAIRCOB), ALLTRIM(SM0->M0_BAIRCOB)+' - ', "" )
...
cComprador := Alltrim(SY1->Y1_NOME) //JUNIOR CARVALHO
...
IF GetMv("MV_ID_EMPR") == 'S'
   oSend( oPrn, "Say", Linha    , 1150, ALLTRIM(SM0->M0_NOMECOM)  , aFontes:TIMES_NEW_ROMAN_12_NEGRITO,,,, 2 )
//   Linha:=Linha+150
...
oSend( oPrn, "Say", Linha:= Linha+50, 1150, 'Tel. +55 '+LEFT(ALLTRIM(SM0->M0_TEL),9)+'-'+RIGHT(ALLTRIM(SM0->M0_TEL),4), aFontes:TIMES_NEW_ROMAN_12,,,, 2 )
...
oSend( oPrn, "Say",  Linha, 630, ALLTRIM(SA2->A2_NOME)+ Space(2) + Alltrim(IF(EICLOJA(), LITERAL_STORE /*"Loja:"*/ + Alltrim(SA2->A2_LOJA),"")) , aFontes:TIMES_NEW_ROMAN_12 ) //FDR - 06/01/12 // JUNIOR CARVALHO
Linha := Linha+50
...
cDescrItem := SW3->W3_COD_I +" "+MSMM(IF( nIdioma==INGLES, SB1->B1_DESC_I, SB1->B1_DESC_P ),36) +' NCM:'+SB1->B1_POSIPI
...
If Len(Alltrim(cNomeFantasia)) > 50
	...
	oSend( oPrn, "Say",  Linha,1450, LEFT(AllTrim(SA2->A2_NREDUZ),len(cNomeFantasia)/2) ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 ) //FDR - 06/01/12 //JUNIOR CARVALHO
//	oSend( oPrn, "Say",  Linha,1450, LEFT(AllTrim(SA2->A2_NREDUZ),len(cNomeFantasia)/2) + Space(2) + IF(EICLOJA(), LITERAL_STORE /*"Loja:"*/ + Alltrim(SA2->A2_LOJA),"") ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 ) //FDR - 06/01/12
    ...
    oSend( oPrn, "Say",  Linha,1450, AllTrim(SA2->A2_NREDUZ)  ,aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 ) //JUNIOR CARVALHO
...
oSend( oPrn, "Say",  Linha, 065, cComprador , aFontes:COURIER_08_NEGRITO ) //Junior Carvalho 26/06/2014
