...
User Function IMP_PROD()    // U_IMP_SALDO()
...
IF !bSair
	...
	@ 020, 006 RADIO oRadMenu1 VAR nEscolha ITEMS "Alterar/incluir Produtos","Fornecedor","Clientes","Saldos de Estoque","Saldo a Pagar","Saldo a Receber","Lotes de Estoque","Pedidos de Venda em Aberto" SIZE 159, 130 OF oDlg COLOR 0, 16777215 PIXEL
...
AAdd( aMensagem, OemToAnsi("Para alterar a tabela de Produto, o arquivo deverá conter campos da SB1"))
...
For nI:=1 to  Len(aDados)
	...
	dbSelectArea("SB1")
	dbSetOrder(1)
	IF MsSeek(xFilial("SB1")+aDados[nI,1])     //comentado para incluir
		IF nEscolha <> 8
			aExecAuto := {}
			...
			Begin Transaction
			IF cTipo == 'B1'       // Produto
				MSExecAuto({|x,y| MATA010(x,y)},aExecAuto,4) //incluir
			ELSEIF cTipo == 'A2'   // Fornecedor
				MSExecAuto({|x,y| MATA020(x,y)},aExecAuto,3)
			ELSEIF cTipo == 'A1'   //Cliente
				MSExecAuto({|x,y| MATA030(x,y)},aExecAuto,4) //alteração
