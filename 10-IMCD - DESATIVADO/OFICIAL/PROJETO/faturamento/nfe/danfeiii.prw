...
User Function DANFE_P1(	cIdEnt	, cVal1		, cVal2, oDanfe,;
						oSetup	, lIsLoja	, lAutomato, lJob, nEntSai)
...
if lJob
	DANFEProc(@oDanfe, .F., cIDEnt, Nil, Nil, @lExistNFe, lIsLoja,lAutomato, lJob, nEntSai)
elseif lAutomato
...
Static Function DanfeProc(	oDanfe	, lEnd		, cIdEnt	, cVal1,;
							cVal2	, lExistNfe	, lIsLoja	, lAutomato, lJob, nEntSai)
...
If lContinua
	...
	ElseIf  lImpDir
		...
		If !lIsLoja .AND. !Empty(cNaoAut) .and. !lAutomato .and. !lJob
			Aviso("SPED","As seguintes notas não foram autorizadas: "+CRLF+CRLF+cNaoAut,{"Ok"},3)
		EndIf
    EndIf
	...
	If ValType(oTransp:_Vol) == "A"
		...
		If  Type("oTransp:_Vol[1]:_PesoB") <>"U"
			For nX := 1 to len(oTransp:_Vol)
				if !empty(XmlChildEx(oTransp:_Vol[nX], "_PESOB"))
					nPesoB += Val(oTransp:_Vol[nX]:_PesoB:TEXT)
				endif
		...
		If Type("oTransp:_Vol[1]:_PesoL") <>"U"
			For nX := 1 to len(oTransp:_Vol)
				if !empty(XmlChildEx(oTransp:_Vol[nX], "_PESOL"))
					nPesoL += Val(oTransp:_Vol[nX]:_PesoL:TEXT)
				ENDIF
			next 
...
//	nPerDesc	:= ((nDesc*100)/nVTotal)
	...
	nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, MAXCODPRD)+1
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]);
	       ...
	       .Or. !Empty(aAux[11])
		nMaxCod := MaxCod(cAuxItem, MAXCODPRD)+1
...
For nY := 1 To nLenItens
	...
	Else
		...
		nAuxH2 := len(aAux[1][14][nY]) + (aColProd[11][1] + ((aColProd[11][2] - aColProd[11][1]) - RetTamTex(aAux[1][11][nY], oFont07:oFont, oDanfe)))
		oDanfe:Say(nLinha+nAjustaPro, nAuxH2, aAux[1][11][nY], oFont07:oFont)
...
Else
	If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
		...
		oDanfe:Say(nLine+120,nBaseTxt, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
		If xFilial("SD2") == "03"
			oDanfe:Say(nLine+130,nBaseTxt, "Licença PF nº 200802338-3",oFont08N:oFont)
		Else
			oDanfe:Say(nLine+130,nBaseTxt, "Licença PF nº 00024016-8",oFont08N:oFont)
		Endif
	...
	Else
		...
		If xFilial("SD2") == "03"
			oDanfe:Say(nLine+120,nBaseTxt, "Licença PF nº 200802338-3",oFont08N:oFont)
		Else
			oDanfe:Say(nLine+120,nBaseTxt, "Licença PF nº 00024016-8",oFont08N:oFont)
		Endif
...
If nFolha == 1   
	...
	If nMAXITEM <> 0
		...
		oDanfe:Box(nLine+454+nAjustaPro, nAux2, nLine+461+nAjustaPro, nAux2 + aTamCol[15] + aTamCol[16])
...
If nFolha ==1
	...
	For nX := 1 To Min(nLenMensagens, MAXMSG)
  		...
  		nLin:= nLin+10
	Next
	oDanfe:Say(MAXBOXV+7,nBaseTxt,"Versão Março/2020",oFont08:oFont)
ElseIf nFolha == 2
	...
	oDanfe:Say(MAXBOXV+7,nBaseTxt,"Versão Marco/2020",oFont08:oFont)	
EndIf	
Return() 
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	if !oPrinter:lInJob
		...
		aTamCol := {45, 69, 39, 19, 23, 19, 33, 41, 45, 39, 46,40,51,42,34,30}
	endif
	...
	aTamCol := {56, 155, 40, 20, 30, 20, 40, 60, 60, 45, 40, 53,45, 45,30,30}
