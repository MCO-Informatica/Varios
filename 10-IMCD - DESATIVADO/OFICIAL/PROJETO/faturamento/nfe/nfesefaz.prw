					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,D2_LOCAL,D2_DTVALID,
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						*/
						...
						ElseIf !EMPTY((cAliasSD2)->D2_LOTECTL)
							aadd(aLote,{(cAliasSD2)->D2_DTVALID,(cAliasSD2)->D2_QUANT,Posicione("SB8" , 3 , xFilial("SB8") + (cAliasSD2)->D2_COD + (cAliasSD2)->D2_LOCAL + (cAliasSD2)->D2_LOTECTL , "B8_DFABRIC"),STOD((cAliasSD2)->D2_DTVALID),""})
						Else
							aadd(aLote,{})
	   					EndIf	
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_VUNIT,D1_CLASFIS,D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_LOTECTL,D1_DTVALID,D1_DFABRIC,
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				*/				
				...
				ElseIf !Empty(aMed) .And. !Empty(aMed[1][1])
					aadd(aLote,{CD7->CD7_LOTE,CD7->CD7_QTDLOT,CD7->CD7_FABRIC,CD7->CD7_VALID,""})
				ElseIf !EMPTY((cAliasSD1)->D1_LOTECTL)
					aadd(aLote,{(cAliasSD1)->D1_LOTECTL,(cAliasSD1)->D1_QUANT,STOD((cAliasSD1)->D1_DFABRIC),STOD((cAliasSD1)->D1_DTVALID),""})
				Else
					aadd(aLote,{})
	   			EndIf	
...
IF lPe01Nfe  
	...
	if cEmpAnt == '02'
		...
		aNfVinc,aDetPag,aObsCont,aPedido, aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,;	
		aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,aComb,aCsosn,aPedCom,aICMSZFM,aFCI,;
		aAgrPis,aAgrCofins,aICMUFDest,aIPIDevol,aItemVinc,aLote	}
		aParam := ExecBlock("PE02NFESEFAZ",.F.,.F.,aParam)
	else
	...
	//	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,aProcRef}
		aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,aPedido,aLote }

		if cEmpAnt == '04'
			
			aParam := ExecBlock("PE04NFESEFAZ",.F.,.F.,aParam)		
	...
	/*
		...
		cSql := "SELECT * FROM "+RetSqlName("SWV")+" SWV WHERE WV_LOTE = '"+aProd[nCount][19]+"' AND SWV.D_E_L_E_T_ =' ' AND WV_FILIAL = '"+xFilial("SWV")+"' "
		If Select("QWV") > 0
			QWV->(DbCloseArea())
		EndIf

		TcQuery ChangeQuery(cSql) New Alias "QWV"

		If !Empty(QWV->WV_LOTE)
	    aadd(aLote, {QWV->WV_LOTE,QWV->WV_QTDE,Substr(QWV->WV_DTFABR,1,4) + "-" + Substr(QWV->WV_DTFABR,5,2) + "-" + Substr(QWV->WV_DTFABR,7,2),Substr(QWV->WV_DT_VALI,1,4) + "-" + Substr(QWV->WV_DT_VALI,5,2) + "-" + Substr(QWV->WV_DT_VALI,7,2),""})
		...
		If !Len(aLote) > 0
			...
			For nCount := 1 to Len(aProd)
			cSql := "SELECT * FROM "+RetSqlName("SB8")+" SB8 WHERE B8_LOTECTL = '"+aProd[nCount][19]+"' AND SB8.D_E_L_E_T_ =' ' AND B8_FILIAL = '"+xFilial("SB8")+"' AND B8_PRODUTO = '"+aProd[nCount][2]+"' "
			If Select("QB8") > 0
				QB8->(DbCloseArea())
			EndIf

			TcQuery ChangeQuery(cSql) New Alias "QB8"

			If !Empty(QB8->B8_LOTECTL)
				aadd(aLote,{QB8->B8_LOTECTL,aProd[nCount][9],Substr(QB8->B8_DFABRIC,1,4) + "-" + Substr(QB8->B8_DFABRIC,5,2) + "-" + Substr(QB8->B8_DFABRIC,7,2),Substr(QB8->B8_DTVALID,1,4) + "-" + Substr(QB8->B8_DTVALID,5,2) + "-" + Substr(QB8->B8_DTVALID,7,2),""})
	...
	*/
	...
	If ( Len(aParam) >= 5 )
		...
		if cEmpAnt == '02'		
			...
			aFCI := aParam[41]
			aAgrPis := aParam[42]
			aAgrCofins := aParam[43]
			aICMUFDest := aParam[44]
			aIPIDevol := aParam[45]
			aItemVinc := aParam[46]
			aLote := aParam[47]
...
If	cVeramb >= "4.00" .and. !empty(aLote)
    If  !empty(aLote[1])
		...
		//cString += '<dFab>'+ConvType(StrZero(Year(STOD(aLote[04])),4))+'-'+StrZero(Month(STOD(aLote[04])),2)+'-'+StrZero(Day(STOD(aLote[04])),2) +'</dFab>'
		//cString += '<dVal>'+ConvType(StrZero(Year(STOD(aLote[03])),4))+'-'+StrZero(Month(STOD(aLote[03])),2)+'-'+StrZero(Day(STOD(aLote[03])),2) +'</dVal>'
