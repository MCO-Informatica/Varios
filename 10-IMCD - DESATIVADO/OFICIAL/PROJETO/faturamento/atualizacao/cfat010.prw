...
Local nRegSA1 := SA1->(Recno())
	...
	if cEmpant == '01'
		CFAT010R( cProduto )
	elseif cEmpant == '02' .or. cEmpant == '04'
		U_CFAT010A( cProduto )
	else
		Alert("Rotina não configurada para essa empresa")
	Endif
...
Itens->( DbCloseArea() )
	...
	//		nReserva += QtdEmPedidos( aCols[i, 1], aCols[i, 3] )
	//		nVenc	 += QtdVencidas( aCols[i, 1], aCols[i, 3] )
	//		nEmTerc	 += QtdEmTerc( aCols[i, 1], aCols[i, 3] )
	//		nDeTerc	 += QtdDeTerc( aCols[i, 1], aCols[i, 3] )
	Next
...
/*
nReserva := QtdEmPedidos( cProduto )
nVenc    := QtdVencidas( cProduto )
nEmTerc  := QtdEmTerc( cProduto )
nDeTerc  := QtdDeTerc( cProduto )
nSaldo   := nAtu - nEmp - nReserva
*/
...
@080,160 BUTTON "Ordens Produção" SIZE 50,10 OF oDlg PIXEL ACTION C010OrdPro(ACOLS[oLstBox:nAt, 1],ACOLS[oLstBox:nAt, 3])
@080,220 BUTTON "Lotes" SIZE 50,10 OF oDlg PIXEL ACTION C010Lotes(ACOLS[oLstBox:nAt, 1],ACOLS[oLstBox:nAt, 3])
@080,280 BUTTON "Fechar" SIZE 50,10 OF oDlg PIXEL ACTION (lRet := .T.,oDlg:End())
...
Static Function C010PedCom(cProduto,nSalPedi, cLocal)
	...
	Do While !eof()
		...
		_d := U__ctod( (cArqQry)->C7_DATPRF )
		cNumPO := 	U_BUSCAPO((cArqQry)->C7_FILIAL , (cArqQry)->C7_NUM  )
		...
		aadd(_aAux, cNumPO  )   // Nr. Proc. Importação
...
//o if abaixo eh para nao dar erro de execucao no ListBox, se o Select nao retornar nenhum registro
	...
	aHeader2 := {"Fornecedor","Entrega","Chegada","Quantidade"         ,"Nr Pedido","Nr.Processo EIC"}
	aPicture := {"@!"        ,"@D"     ,"@D"     ,"@E 999,999,999.9999","@!"       ,"@!"}

	EnchoiceBar(oDlg,{||oDlg:End(),Nil},{||oDlg:End()},,)

	//Parametros RDListBox(lin,col,compr,alt, ,  , tamanho das colunas no grid)
	oLstBoxPC := RDListBox(5,.80,360,130, aCols, aHeader2,{60,30,30,30,40,30,40},aPicture,.F. , {||VERHISTPO(cProduto)} )
	//oLstBoxPC:BlDblClick := {|| ( ABREPO(oLstBoxPC:nAt,aCols),oLstBoxPC:Refresh() ) }
...
Static Function C010PedVen(cProduto, cLocal)
	...
	Do While !eof()
		_aAux := {}
		SC5->( dbSeek( xFilial("SC5") + (cArqQry)->C6_NUM ) )
		if SC5->C5_X_CANC == "C"
			DbSkip()
			loop
		endif
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Static Function C010OrdPro(cProduto, cLocal)
	Local cArqQry, lQuery, cQuery
	Local _aAux    := {}
	Local aHeader2 := {}
	Local oLstBox, oDlg
	...
	cArqQry := "QUERYSC2"
	lQuery  := .T.
	cQuery  := "SELECT C2_NUM, C2_ITEM, C2_QUANT, C2_QUJE, C2_DATPRF "
	cQuery  += "FROM "+RetSqlName("SC2")+" QUERYSC2 "
	cQuery  += "WHERE C2_FILIAL='"+xFilial("SC2")+"' AND "
	cQuery  += "C2_PRODUTO='"+cProduto+"' AND "
	cQuery  += "C2_LOCAL='"+cLocal+"' AND "
	...
	Do While !eof()
		_aAux := {}
		aadd(_aAux, (cArqQry)->C2_NUM )
		aadd(_aAux, (cArqQry)->C2_ITEM )
		aadd(_aAux, (cArqQry)->C2_QUANT - (cArqQry)->C2_QUJE )
		aadd(_aAux, STOD( (cArqQry)->C2_DATPRF ) )
...
//o if abaixo eh para nao dar erro de execucao no ListBox, se o Select nao retornar nenhum registro
	if len(acols) == 0
		aadd(aCols,{space(07),space(06),0.00,Stod(" ")})
	endif

	DEFINE MSDIALOG oDlg FROM  0,0 TO 400,500 TITLE ( "Ordens de Produção" ) PIXEL
	@ 035,05 TO 060,240  LABEL '' OF oDlg 	PIXEL
	@ 040,10 SAY "Código"     SIZE  21,7 OF oDlg PIXEL
	@ 040,40 SAY cProduto     SIZE  49,8 OF oDlg PIXEL COLOR CLR_BLUE
	@ 050,10 SAY "Descrição"  SIZE  32,7 OF oDlg PIXEL
	@ 050,40 SAY Posicione("SB1",1,xFilial("SB1") + cProduto,"B1_DESC") SIZE 140,8 OF oDlg PIXEL COLOR CLR_BLUE

	aHeader2 := {"OP", "ITEM","Quantidade" ,"Data de Disponibilidade"         }
	aPicture := {"@!", "@!"  ,"@E 999,999,999.9999","@D" }

	EnchoiceBar(oDlg,{||oDlg:End(),Nil},{||oDlg:End()},,)

	//Parametros RDListBox(lin,col,compr,alt, ,  , tamanho das colunas no grid)
	oLstBox := RDListBox(5,1,240,120, aCols, aHeader2,{40,40,80,10},aPicture)
	ACTIVATE MSDIALOG oDlg CENTERED
...
Static Function C010Lotes(cProduto, cLocal)
	...
	lQuery  := .T.
	cQuery  := "SELECT B8_FILIAL, B8_PRODUTO, B8_LOCAL, B8_DATA, B8_DTVALID, B8_SALDO, B8_LOTECTL, B8_DFABRIC, B8_XOBS "
	...
	Do While !eof()
		...
		aadd(_aAux, stod((cArqQry)->B8_DFABRIC ) )
		aadd(_aAux, stod((cArqQry)->B8_DTVALID) )
		aadd(_aAux, alltrim((cArqQry)->B8_XOBS))
...
//o if abaixo eh para nao dar erro de execucao no ListBox, se o Select nao retornar nenhum registro
	...
	aHeader2 := {"Filial","Saldo","Lote","Data Entrada","Data Fabricação","Data Validade", "Observacao"   }
...
User FUNCTION BUSCAPO(cFilPC, cNumPn)

	Local aArea		:= GetArea()
	Local cRet		:= ' '
	Local cAliasPO	:= GETNEXTALIAS()

	BeginSql alias cAliasPO
	
	SELECT W6_HAWB, W2_PO_NUM  FROM %Table:SW2% SW2 
	LEFT JOIN  %Table:SW6% SW6 ON 
	W6_PO_NUM = W2_PO_NUM
	AND W6_FILIAL = W2_FILIAL
	AND SW6.%NotDel%
	WHERE W2_PO_SIGA = %Exp:cNumPn%
	AND W2_FILIAL = %Exp:cFilPC%
	AND SW2.%NotDel%
	...
	While (cAliasPO)->(!Eof())
		...
		cRet := (cAliasPO)->W2_PO_NUM //IIF( !EMPTY( (cAliasPO)->W6_HAWB),(cAliasPO)->W6_HAWB, 'PO - '+ (cAliasPO)->W2_PO_NUM  )

		(cAliasPO)->( dbSkip())

	Enddo

	If Select(cAliasPO) > 0
		(cAliasPO)->(DbCloseArea())
	EndIf
...
Static Function VERHISTPO(cProduto)
	Local oDlgHIST
	Local cMemoPO := ""
	Local cMemoEmb := ""
	Local cMemo := ""
	...
	cPO := ACOLS[ oLstBoxPC:nAt, 6 ]
	xTitulo := ('ETA - Pedido ' + cPO )

	IF !EMPTY(cPO)

		DEFINE MSDIALOG oDlgHIST FROM  0,0 TO 400,600 TITLE xTitulo PIXEL
		...
		While SW3->(!Eof()) .AND.;
				SW3->W3_FILIAL == cFilAnt .AND. ;
				SW3->W3_PO_NUM == cPO

			If SW3->W3_SEQ #0 .OR. SW3->W3_COD_I <> cProduto
				SW3->(dbSkip())
				LOOP
			Endif

			cMemoPO += Alltrim(SW3->W3_PO_NUM)	+ " PREV EMBARQUE - " + DTOC(SW3->W3_DT_EMB)+ " PREV ENTREGA - " +DTOC(SW3->W3_DT_ENTR)
		...
		dbSelectArea("SW6")
		dbSetOrder(1)
		SW6->(dbSeek(xFilial()+cPO))
		While SW6->(!Eof()) .AND.;
				SW6->W6_FILIAL == cFilAnt .AND. ;
				SW6->W6_HAWB == Avkey(CPO,"W6_HAWB")

			cMemoEmb += Alltrim(SW6->W6_HAWB)	+ " PREV EMBARQUE - "+  DTOC(SW6->W6_DT_ETD)
			cMemoEmb += " PREV ENTREGA - " +DTOC(SW6->W6_DT_ETA)+CHR(13)+CHR(10)

			SW6->(dbSkip())
		Enddo
		
		cMemo := IIF( EMPTY(cMemoPO),"","PO"+CHR(13)+CHR(10)+cMemoPO +CHR(13)+CHR(10))
		cMemo += CHR(13)+CHR(10)
		cMemo += IIF( EMPTY(cMemoEmb),"","EMBARQUE"+CHR(13)+CHR(10)+cMemoEmb )

		@ 05,10 SAY cMemo SIZE 300,400 OF oDlgHIST PIXEL

		//EnchoiceBar(oDlgHIST,{||oDlgHIST:End(),Nil},{||oDlgHIST:End()},,)
		@300,250 BUTTON "Fechar" SIZE 50,10 OF oDlgHIST PIXEL ACTION (oDlgHIST:End())
