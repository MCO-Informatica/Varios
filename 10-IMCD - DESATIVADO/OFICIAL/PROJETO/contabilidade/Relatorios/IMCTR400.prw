...
//-------------------------------------------------------------------
/*/{Protheus.doc} IMCTR400
Razão contábil(CTBR400) customizado para a IMCD. 
Adicionados os campos de usuário inclusão/alteração e inclusão do 
lançamento automático ou manual. 
...
*/
User Function IMCTR400(cContaIni, cContaFim, dDataIni, dDataFim, cMoeda, cSaldos,;
...
If lOk
	If lImpRazR4
		...
		IMCR400R4(aCtbMoeda,lCusto,lItem,lCLVL,@cArqTmp,aSelFil,lTodasFil, lProcSld, cTmpProc )
		//ConOut( " FIM Impressao    : "+ Time())
	Else
		IMCR400R3( cContaIni, cContaFim, dDataIni, dDataFim, cMoeda, cSaldos,;
					cBook, lCusto, cCustoIni, cCustoFim, lItem, cItemIni, cItemFim,;
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function IMCR400R4(aCtbMoeda,lCusto,lItem,lCLVL,cArqTmp,aSelFil,lTodasFil, lProcSld, cTmpProc  )
Default lProcSld := .F.
Default cTmpProc := ""
...
//---------------------------------------------------------------
//INICIO CUSTOMIZAÇÃO IMCD
//---------------------------------------------------------------
//Adição de campos customizados para o relatório para auditoria.
//----------------------------------------------------------------
TRCell():New(oSection2,"XROTINA"	,"cArqTmp","MANUAL/AUTOMATICO",/*Picture*/,nTamCLVL,/*lPixel*/,{|| posRegCT2("CT2_ROTINA") },"RIGHT",,"RIGHT",,,.F.)// "INCLUSAO MANUAL/AUTOMATICO"
TRCell():New(oSection2,"XUSERGI"	,"cArqTmp","USUARIO INCLUSAO",/*Picture*/,nTamCLVL,/*lPixel*/,{|| posRegCT2("CT2_USERGI")},"RIGHT",,"RIGHT",,,.F.)// "USUARIO INCLUSAO"
TRCell():New(oSection2,"XUSERGA"	,"cArqTmp","USUARIO ALTERACAO",/*Picture*/,nTamCLVL,/*lPixel*/,{||posRegCT2("CT2_USERGA") },"RIGHT",,"RIGHT",,,.F.)// "USUARIO ALTERACAO"
//------------------------
//FIM CUSTOMIZAÇÃO IMCD
//------------------------
...
oSection2:Cell("XROTINA"):lHeaderSize	:= .F.
oSection2:Cell("XUSERGI"):lHeaderSize	:= .F.
oSection2:Cell("XUSERGA"):lHeaderSize	:= .F.
...
oSection1_1:Cell("SALDOANT"):SetBlock( {|| STR0033 + AsString(ValorCTB(aSaldoAnt[6],,,TAM_VALOR,nDecimais,.T.,cPicture,cNormal,,,,,,lPrintZero,.F.)) } )//"SALDO ANTERIOR: "
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function IMCR400R3(	cContaIni, cContaFim, dDataIni, dDataFim, cMoeda, cSaldos,;
					cBook, lCusto, cCustoIni, cCustoFim, lItem, cItemIni, cItemFim,;
					lClVl, cClvlIni, cClvlFim,lSaltLin,cMoedaDesc,aSelFil )
...
RptStatus({|lEnd| IMC400Imp(@lEnd,wnRel,cString,aSetOfBook,lCusto,lItem,lCLVL,;
...
/*/
Static Function IMC400Imp(lEnd,WnRel,cString,aSetOfBook,lCusto,lItem,lCLVL,lAnalitico,Titulo,nTamlinha,aCtbMoeda,nTamConta,aSelFil)
...
If lNumAsto .and. FunName() $ "CTBR400|CTBR410"
	IF Type("nTipoRel") = "U"
		IF FunName() == "CTBR400"
			nTipoRel := MV_PAR08
		Else
...
If FWIsInCallStack("CTBR400") .OR. lAutomR400
...
If ( nTipoRel <> 3 )
	...
	If FunName() == "CTBR400" .OR. IsInCallStack("CT400Imp")
		...
		cProcCT2 += "	ORDER BY CT2_FILIAL, CARQTMP.CONTA, CT2_DATA, CT2_LOTE, CT2_SBLOTE, CT2_DOC, CT2_LINHA, CT2_SEQHIS "+ CRLF
		...
		cProcCT2 += "		IF  @cCT2_FILIAL = @cFilial "+ CRLF
		cProcCT2 += "			AND @cConta = @cContaHist " + CRLF
		cProcCT2 += "			AND @cCT2_DATA = @cData "+ CRLF
		cProcCT2 += "			AND @cCT2_LOTE = @cLote "+ CRLF
		cProcCT2 += "			AND @cCT2_SBLOTE = @cSubLote "+ CRLF
		cProcCT2 += "			AND @cCT2_DOC = @cDoc " + CRLF
		cProcCT2 += "			AND @cCT2_SEQLAN = @cSeqLan " + CRLF

		cProcCT2 += "		BEGIN

		If ( "MSSQL" $ Upper(TCGetDb()) )
			cProcCT2 += "			SELECT @cHistCT2 = RTRIM(@cHistCT2) || ' ' || RTRIM(@cCT2_HIST)  " + CRLF //Radu: Colocar uma quebra de linha aqui para cada concatenação
		Else
			cProcCT2 += "			SELECT @cHistCT2 = TRIM(@cHistCT2) || ' ' || TRIM(@cCT2_HIST)  " + CRLF
		EndIf

		cProcCT2 += "		END " + CRLF	//Finaliza o If @cCT2_FILIAL = @cFilial AND...
		cProcCT2 += "		ELSE " + CRLF
		cProcCT2 += "		BEGIN " + CRLF
		...
		cProcCT2 += "			SET " + CRLF
		cProcCT2 += "				HISTORICO = @cHistCT2 " + CRLF
		cProcCT2 += "			WHERE " + CRLF
		cProcCT2 += "				FILIAL = @cFilial " + CRLF
		cProcCT2 += "				AND CONTA = @cContaHist " + CRLF
		cProcCT2 += "				AND DATAL = @cData " + CRLF
		cProcCT2 += "				AND LOTE = @cLote " + CRLF
		cProcCT2 += "				AND SUBLOTE = @cSubLote " + CRLF
		cProcCT2 += "				AND SEQLAN = @cSeqLan " + CRLF
		cProcCT2 += "				AND DOC = @cDoc " + CRLF
		cProcCT2 += "				AND D_E_L_E_T_ = '' " + CRLF
		...
		cProcCT2 += "		IF @cCT2_SEQHIS = '001' BEGIN " + CRLF
		...
		cProcCT2 += "		END " + CRLF	//Fim If  @cCT2_SEQHIS = '001'...
...
//-------------------------------------------------------------------
/*/{Protheus.doc} posRegCT2
Posiciona na CT2 e retorna o valor do campo desejado.
@author  marcio.katsumata
@since   06/11/2019
@version 1.0
...
/*/
//-------------------------------------------------------------------
static function posRegCT2(cCampo)

	local aAreaCT2 as array      //area da tabela CT2
	local cRetValue as character //retorno da função

	aAreaCT2 := CT2->(getArea())
	cRetValue:= ""

	//--------------------------------------------
	//Posicina na CT2 para retornar as informações
	//necessárias
	//---------------------------------------------
	dbSelectArea("CT2")
	CT2->(dbGoTo(cArqTMP->RECNOCT2))


	DO CASE 
		CASE cCampo == "CT2_USERGI"
			cRetValue := FWLeUserlg("CT2_USERGI", 1)
		CASE cCampo == "CT2_USERGA"
			cRetValue := FWLeUserlg("CT2_USERGA", 1)
		CASE cCampo == "CT2_ROTINA"
			cRetValue := iif(alltrim(CT2->CT2_ROTINA) == "CTBA102",;
							"MANUAL","AUTOMATICO")
		OTHERWISE
			cRetValue := CT2->&(cCampo)
	ENDCASE
	
	
	//Restaura area da CT2
	restArea(aAreaCT2)

	//Realiza limpeza de array
	aSize(aAreaCT2,0)
