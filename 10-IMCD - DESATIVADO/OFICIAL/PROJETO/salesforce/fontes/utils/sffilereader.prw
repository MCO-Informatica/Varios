...
CLASS SFFILEREADER FROM LONGNAMECLASS
...
METHOD New( cFName , cFSep , nFBuffer ) CLASS SFFILEREADER
	DEFAULT cFSep := CHAR(10)
...
METHOD Open( iFMode ) CLASS SFFILEREADER
	DEFAULT iFMode := 0
	::_CleanLastErr()
	If ::nHnd != -1
	...
	fSeek(::nHnd,0)
Return .T.
METHOD Close() CLASS SFFILEREADER
	::_CleanLastErr()
	If ::nHnd == -1
	...
	::_PosBuffer := 0
	Return .T.
	METHOD ReadLine( /*@*/ cReadLine ) CLASS SFFILEREADER
	Local cTmp := ''
	Local cBuffer
	Local nRPos
	Local nRead
...
METHOD GetError() CLASS SFFILEREADER
Return ::nFerror

METHOD GetOSError() CLASS SFFILEREADER
Return ::nOSError

METHOD GetErrorStr() CLASS SFFILEREADER
Return ::cFerrorStr

METHOD GetFSize() CLASS SFFILEREADER
Return ::nFSize

METHOD _SetError(nCode,cStr) CLASS SFFILEREADER
	::nFerror := nCode
	::cFerrorStr := cStr
Return

METHOD _SetOSError(nCode,cStr,nOsError) CLASS SFFILEREADER
	::nFerror := nCode
	::cFerrorStr := cStr
	::nOsError := nOsError
Return

METHOD _CleanLastErr() CLASS SFFILEREADER
	::nFerror := 0
	::cFerrorStr := ''
	::nOsError := 0
Return

user function sffilereader()
