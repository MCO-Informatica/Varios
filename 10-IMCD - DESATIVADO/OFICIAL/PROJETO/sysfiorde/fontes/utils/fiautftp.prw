	...
	if  wizardP12()
		...
		if !empty(cUserName) .and. !empty(cPass)
			...
			if MemoWrite(cArquivo, cLine)
				aviso ("FIAUTFTP", "Arquivo de autenticação gravado com sucesso", {"OK"}, 1)
			else
...
Static Function wizardP12()
	...
	DEFINE DIALOG oDlg TITLE 'Wizard de Configuração Arquivo Autenticação FTP Sysfiorde' PIXEL
	oDlg:nWidth := 600
	oDlg:nHeight := 630
...
static function cria_pg1 (oPanel)
    ...
    @ 20, 15 SAY oSay1 VAR "Esta rotina tem por objetivo realizar a configuração do arquivo de autenticação do FTP Sysfiorde" OF oPanel PIXEL	
