			...
			If !Empty((cAliasSD2)->D2_NFORI) 
				If (cAliasSD2)->D2_TIPO $ "DBN"
					...
					If DbSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						aadd(aNfVinc,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",U_RetSM0(,,"M0_CGC"),SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",U_RetSM0(,,"M0_CGC"),SA2->A2_CGC)),U_RetSM0(,,"M0_ESTCOB"),SF1->F1_ESPECIE})
					EndIf
				Else
					aOldReg  := SD2->(GetArea())
					...
					If DbSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						aadd(aNfVinc,{SF2->F2_EMISSAO,SD2->D2_SERIE,SD2->D2_DOC,U_RetSM0(,,"M0_CGC"),U_RetSM0(,,"M0_ESTCOB"),SF2->F2_ESPECIE})
					EndIf
					RestArea(aOldReg)
					RestArea(aOldReg2)
				EndIf
				...
				If !Empty((cAliasSD1)->D1_NFORI) 
					...
					Else					
						...
						If DbSeek(xFilial("SD2")+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_COD+(cAliasSD1)->D1_ITEMORI)
							...
							aadd(aNfVinc,{SD2->D2_EMISSAO,SD2->D2_SERIE,SD2->D2_DOC,U_RetSM0(,,"M0_CGC"),U_RetSM0(,,"M0_ESTCOB"),SF2->F2_ESPECIE})
...
If cTipo == "1" .And. !Empty(cMotCancela)
	cString := u_NFseM102( U_RetSM0(,,"M0_CODMUN"), cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela )[1]
EndIf	
...
cAssinatura += StrZero(Val(U_RetSM0(,,"M0_INSCM")),11) 
cAssinatura += "NF   "  
...
Local cString := ""
Local cImPrestador:=AllTrim(U_RetSM0(,,"M0_INSCM"))
Local cEmail:=""  
...
cString += '<CPFCNPJPrestador>'+AllTrim(U_RetSM0(,,"M0_CGC"))+'</CPFCNPJPrestador>'
cString += '<RazaoSocialPrestador>'+AllTrim(U_RetSM0(,,"M0_NOMECOM"))+'</RazaoSocialPrestador>' 
If cCodMun $ '4303905-4307906' //Campo Bom-RS  - Farroupilha-RS
	cString += '<LogradouroPrestador>'+AllTrim(U_RetSM0(,,"M0_ENDCOB"))+'</LogradouroPrestador>' 
	cString += '<NumeroEnderecoPrestador>'+Substr(AllTrim(U_RetSM0(,,"M0_ENDCOB")),RAT(",",AllTrim(U_RetSM0(,,"M0_ENDCOB")))+1,5)+'</NumeroEnderecoPrestador>' 
	cString += '<BairroPrestador>'+AllTrim(U_RetSM0(,,"M0_BAIRCOB"))+'</BairroPrestador>' 
Endif
If !cCodMun $ '4303905-4307906' //Campo Bom-RS  - Farroupilha-RS
	cString += '<NFantasiaPrestador>'+IIF(lUsaGesEmp,FWFilialName(),Alltrim(U_RetSM0(,,"M0_NOME")))+'</NFantasiaPrestador>'
	cString += '<CidadePrestador>'+AllTrim(U_RetSM0(,,"M0_CODMUN"))+'</CidadePrestador>'
EndIf
cString += '<UFPrestador>'+AllTrim(U_RetSM0(,,"M0_ESTCOB"))+'</UFPrestador>'  
...
If Len( cCodTom ) <= 5 .And. (!(cCodTom$'99999').Or. (cCodTom$'99999' .And. U_RetSM0(,,"M0_CODMUN") == '3550308'))
...
EndIf
cString += '<DDDPrestador>'+AllTrim(Str(FisGetTel(U_RetSM0(,,"M0_TEL"))[2],3))+'</DDDPrestador>'
cString += '<TelefonePrestador>'+AllTrim(Str(FisGetTel(U_RetSM0(,,"M0_TEL"))[3],15))+'</TelefonePrestador>'
