...
If TRepInUse()
	...
	oReport:PrintDialog()
Else
	Return U_XFRVS3R3() // Executa versão anterior do fonte
Endif
...
TRCell():New(oSection2,"DAD" 	    ,      , "DES/ABA/DEC", Tm(0,13)					, 10 ,/*lPixel*/,/*CodeBlock*/,"RIGHT",,"RIGHT") 
TRCell():New(oSection2,"JMA" 	    ,      , "JUR/MUL/ACR", Tm(0,13)					, 10 ,/*lPixel*/,/*CodeBlock*/,"RIGHT",,"RIGHT")
TRCell():New(oSection2,"VLRBAIXA"	,      , STR0049	, Tm(0,13) 						, 18 ,/*lPixel*/,/*CodeBlock*/,"RIGHT",,"RIGHT") //"Valor Baixado"
...
TRCell():New(oSection4,"TXTTOTAL"	,       , STR0055	,								, 54 ,.T.,/*CodeBlock*/) //"Totais"
TRCell():New(oSection4,"VALOR"	   	,		, STR0040 	, PesqPict("SE1","E1_VALOR")	, 15 ,.T.,/*CodeBlock*/,"RIGHT",,"RIGHT")//"Valor Original"
...
TRCell():New(oSection4,"DAD" 	, 	   		, "DES/ABA/DEC", Tm(0,13)               ,13,.T.,/*CodeBlock*/,"RIGHT",,"RIGHT") //"Abatimentos"
TRCell():New(oSection4,"JMA" 		,       , "JUR/MUL/ACR", Tm(0,13)               ,10  ,.T.,/*CodeBlock*/,"RIGHT",,"RIGHT") //"Juros"
TRCell():New(oSection4,"VLRBAIXA"	,    	, STR0049	, Tm(0,19)                  ,16  ,.T.,/*CodeBlock*/,"RIGHT",,"RIGHT") //"Valor Baixado"
TRCell():New(oSection4,"SALDO" 		, 	   	, STR0051	, Tm(0,nTamVal)				,19  ,.T.,/*CodeBlock*/,"RIGHT",,"RIGHT")	 //"Saldo Atual"
...
TRCell():New(oSectionSA,"TXTSA"	   	,     ,"SALDOANTERIOR", 						    , 20,/*lPixel*/,/*CodeBlock*/) //"Valor Original"
TRCell():New(oSectionSA,"VALOR"	   	,     ,"Valor Original"	, Tm(0,19)				    , 17,/*lPixel*/,/*CodeBlock*/,"RIGHT",,"RIGHT") //"Valor Original"
TRCell():New(oSectionSA,"VLRBAIXA"	,     ,"Valor Baixa"	, Tm(0,19)				    , 86,/*lPixel*/,/*CodeBlock*/,"RIGHT",,"RIGHT") //"Valor Original"
TRCell():New(oSectionSA,"SALDO"		,     ,"SALDO"			, Tm(0,nTamVal)				, 8,/*lPixel*/,/*CodeBlock*/,"RIGHT",,"RIGHT") //"Valor Baixado"
...
oSection3:Cell("DAD"	 	):SetBlock( { || If (!lTotGer, nTit2+nTit3+nTit11, nTot2  ) } )
oSection3:Cell("JMA"	 	):SetBlock( { || If (!lTotGer, nTit4+nTit5+nTit10, nTot4  ) } )
...
oSection4:Cell("DAD"	 	):SetBlock( { || nTotFil2 } )				
oSection4:Cell("JMA"	 	):SetBlock( { || nTotFil4 } )
...
oSectionSA:Cell("VALOR"		):SetBlock({||nValorA})
oSectionSA:Cell("VLRBAIXA"	):SetBlock({||nBaixaA})
oSectionSA:Cell("SALDO"		):SetBlock({||nSaldoA})
   ...
   If aSM0[nInc][1] == cEmpAnt .AND. (aSM0[nInc][2] >= cFilDe .and. aSM0[nInc][2] <= cFilAte)
		...
		#IFDEF TOP
			If TcSrvType() != "AS/400"
				...
				cQuery += " AND SE1.E1_EMISSAO between '20140101' AND '" + DTOS(mv_par06) + "'" //DANILO ALVES TESTE
		...
		While SE1->(!Eof()) .And. lContinua .And. &cCond1 .And. !oReport:Cancel()
			...
			If ( ABS(nTit1)+ABS(nTit2)+ABS(nTit3)+ABS(nTit4)+ABS(nTit5)+;
				...
				nTot1  += nTit1
				nTot2  += nTit2+nTit3+nTit11
				nTot3  += nTit3
				nTot4  += nTit4+nTit5+nTit10
				nTot5  += nTit5
				nTot6  += nTit6
				nTot7  += nTit7
				nTot8  += nTit8
				...
				nTotFil1  += nTit1
				nTotFil2  += nTit2+nTit3+nTit11
				nTotFil3  += nTit3
				nTotFil4  += nTit4+nTit5+nTit10
				nTotFil5  += nTit5
				nTotFil6  += nTit6
				nTotFil7  += nTit7
				nTotFil8  += nTit8			
