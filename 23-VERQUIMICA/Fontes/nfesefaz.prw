...
Private lVqPolicia  := .F.
Private lVqExercito := .F.
Private cMensVerq   := ""
Private _cMsgEic	:= ""   
Private aVqCPCli	:= {}				//Danilo Busso 18/08/2016
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				/*======   INICIO   ======*/
				/*|||||| VERQUIMICA ||||||*/
				aAareaDy3 := DY3->(GetArea())
				aAareaSa1 := SA1->(GetArea())
				aAareaSa4 := SA4->(GetArea())
				aAareaSb1 := SB1->(GetArea())
				aAareaSb5 := SB5->(GetArea())
				aAareaSc6 := SC6->(GetArea())
				aAareaSd2 := SD2->(GetArea())
				...
				cVerqxPed := "PEDIDO CLIENTE: "
				lVerqxPed := .F. 
				If SD2->(DbSeek(xFilial("SD2")+SF2->F2_DOC))
					While !SD2->(Eof()) .And. SD2->(D2_FILIAL+D2_DOC) == xFilial("SD2")+SF2->F2_DOC
						If SB1->(DbSeek(xFilial("SB1")+SD2->D2_COD))
							If SB5->(DbSeek(xFilial("SB5")+SD2->D2_COD))
								...
								If !Empty(SB5->B5_ONU) .And. !Empty(SB5->B5_ITEM)
									If DY3->(DbSeek(xFilial("DY3")+SB5->(B5_ONU+B5_ITEM)))
										cMensVerq += "ONU: " + AllTrim(DY3->DY3_DESCRI) + " " + AllTrim(DY3->DY3_CLASSE) + " " + AllTrim(DY3->DY3_NRISCO) + " " + AllTrim(DY3->DY3_GRPEMB) + ". "
									EndIf
								EndIf
						...
						If SC6->(DbSeek(xFilial("SC6")+SD2->(D2_PEDIDO+D2_ITEMPV)))   
							If !Empty(SC6->C6_NUMPCOM)
								lVerqxPed := .T. 
								cVerqxPed += AllTrim(SC6->C6_NUMPCOM)
								If !Empty(SC6->C6_ITEMPC)
									cVerqxPed += "/"+AllTrim(SC6->C6_ITEMPC)
								EndIf
								cVerqxPed += " - "
							EndIf
						EndIf
				...
				/*   RETIRADO A PEDIDO DE FERNANDA LEMES em 31/05/2017 
				If lVqPolicia
					cVqPfNumLi := GetMv("VQ_PFNUMLI")
					dVqPfDatLi := GetMv("VQ_PFDATLI")
					cMensVerq += "NUM.LIC.PF.CLIENTE:"           + AllTrim(SA1->A1_VQ_LIPF) + " ATE " + dToc(SA1->A1_VQ_DLPF) + ". "
					cMensVerq += "NUM.LIC.PF.VERQ.:"             + AllTrim(cVqPfNumLi)      + " ATE " + dToc(dVqPfDatLi)      + ". "
					cMensVerq += "NUM.LIC.PF.TRANSPORTADORA:"    + AllTrim(SA4->A4_VQ_LIPF) + " ATE " + dToc(SA4->A4_VQ_DLPF) + ". "
				EndIf 
				*/
				If lVqExercito
					cMensVerq += "NUM.LIC.EXERC.CLIENTE:"        + AllTrim(SA1->A1_VQ_LIEX) + " ATE " + dToc(SA1->A1_VQ_DLEX) + ". "
					cMensVerq += "NUM.LIC.EXERC.TRANSPORTADORA:" + AllTrim(SA4->A4_VQ_LIEX) + " ATE " + dToc(SA4->A4_VQ_DLEX) + ". "
				EndIf
			
				DY3->(RestArea(aAareaDy3))
				SA1->(RestArea(aAareaSa1))
				SA4->(RestArea(aAareaSa4))
				SB1->(RestArea(aAareaSb1))
				SB5->(RestArea(aAareaSb5))
				SC6->(RestArea(aAareaSc6))
				SD2->(RestArea(aAareaSd2))
				/*======    FIM     ======*/
				/*|||||| VERQUIMICA ||||||*/
				    dbSelectArea("SA1")
					dbSetOrder(1)
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_CLASFIS,D2_PRCVEN,D2_VQ_QTDE,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
					...
					If lContinua
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							...
							If !Empty(cFieldMsg) .and. SC5->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SC5->"+cFieldMsg))
								cMensCli := alltrim(&("SC5->"+cFieldMsg))
							Else
								cMensCli += AllTrim(SC5->C5_MENNOTA)+AllTrim(SC5->C5_MENNOT1)+AllTrim(SC5->C5_MENNOT2)+AllTrim(SC5->C5_MENNOT3)
							EndIf
						...
						If SC6->(FieldPos("C6_VQCPCLI")) > 0
							If !Empty(SC6->C6_VQCPCLI)
								aadd(aVqCPCli,{SC6->C6_VQCPCLI})
							Else
								aadd(aVqCPCli,{}) 
							EndIf
						Else
						...
						||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
						||+---------------------------------------+------------------------------+-----------------+||
						||| Customizacao: 2a Unidade de medida    | Autor: Celso Ferrone Martins | Data 15/08/2014 |||
						||+---------------------------------------+------------------------------+-----------------+||
						||| Descricao: Verifica a tabela SA7 (Produto x Cliente) para utilizar a unidade de medida |||
						|||            do cliente					                                               |||
						||+----------------------------------------------------------------------------------------+||
						||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
						==============================================================================================
						*/
						...
						cVerqUm   := SB1->B1_UM
						nD2Quant  := (cAliasSD2)->D2_QUANT
						//If !SC5->C5_TIPO $ "B/D" .And. !Empty(SC6->C6_VQ_UM)
						If !Empty(SC6->C6_VQ_UM)
							If SC6->C6_UM != SC6->C6_VQ_UM
								cVerqUm   := SB1->B1_SEGUM
								If SB1->B1_TIPCONV == "D"
									nD2Quant := nD2Quant / SB1->B1_CONV
								Else
									nD2Quant := nD2Quant * SB1->B1_CONV
								EndIf
								nD2Quant := Round(nD2Quant,2)
							EndIf
						EndIf
                    	cDescCom := ""
						If !Empty(SB5->B5_ONU) .And. !Empty(SB5->B5_ITEM)
							If DY3->(DbSeek(xFilial("DY3")+SB5->(B5_ONU+B5_ITEM)))
								cDescCom += "ONU " + ;
								AllTrim(DY3->DY3_ONU   ) + ", " + ;
								AllTrim(DY3->DY3_DESCRI) + ", " + ;
								AllTrim(DY3->DY3_CLASSE) + ", " + ;
								AllTrim(DY3->DY3_NRISCO) + ", " + ;
								AllTrim(DY3->DY3_GRPEMB) + ". "
							EndIf
						EndIf
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							cVerqUm,;//IIF(Empty(SB5->B5_UMDIPI),SB1->B1_UM,SB5->B5_UMDIPI),;
							nD2Quant,;//IIF(Empty(SB5->B5_CONVDIP),(cAliasSD2)->D2_QUANT,SB5->B5_CONVDIP*(cAliasSD2)->D2_QUANT),;
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
							...
							cDescCom+cInfAdic,;//Informacoes adicionais do produto(B5_DESCNFE)
							nDescZF,;
							(cAliasSD2)->D2_TES,;
							...
							(cAliasSD2)->D2_PRCVEN,;	 //aProd[50]
							(cAliasSD2)->D2_VQ_QTDE,;    //aProd[51]
							})
		...
		Else				
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_VQ_VSEG,D1_QTSEGUM,D1_VUNIT,D1_CLASFIS,D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_LOTECTL, 
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					SF4->F4_ART274,; //aprod[48]
					0,;  //aprod[49]   nValLeite
					(cAliasSD1)->D1_VUNIT,;	 //aProd[50] 
					(cAliasSD1)->D1_QTSEGUM,; //aProd[51]
					})
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		If nLenaIpi > 0
			...
			EndIf
		EndIf
		//Danilo Busso 13/01/2017 - Adicionado a verificação do tipo de nota para saber se vai passar o aVqCpCli como parametro.
		if cTipo == "1"
			cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
						...
						aAgrPis[nX],aAgrCofins[nX],nIcmsDif,aICMUFDest[nX],@nvFCPUFDest,@nvICMSUFDest,@nvICMSUFRemet,cAmbiente,aIPIDevol[nX],@nvBCUFDest, aItemVinc[nX], @npFCPUFDest,@npICMSUFDest,@npICMSInter,@npICMSIntP,aLote[nX],@cMensDifal,aVqCpCli[nX])
		ELSE
			cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
						aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],aAnfI[nX],cTipo,cVerAmb, aComb[Nx],;
						@cMensFis,aCsosn[Nx],aPedCom[nX],aNota,aICMSZFM[nX],aDest,cIpiCst,aFCI[nX],lIcmDevol,@nVicmsDeson,@nVIcmDif,cMunPres,;
...
If !Empty(aIEST) 
	/*ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	  ...
	  ³ acordo PROTOCOLO ICMS CONSELHO NACIONAL DE POLÝTICA FAZENDÝRIA - CONFAZ Nº 191 DE 11.12.2009³ 
...
IF Alltrim(aProd[11]) <> "L"
cString += '<qTrib>' + ConvType(aProd[12], 15, 4) + '</qTrib>'
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],21,8))+'</vUnTrib>'
ELSE
cString += '<qTrib>' + ConvType(aProd[51], 15, 5) + '</qTrib>'
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],16,5))+'</vUnTrib>'
ENDIF
...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÝÄÄÄÄÄÄÄÝÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÝÄÄÄÄÄÄÝÄÄÄÄÄÄÄÄÄÄ´±±
...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ SIGAFIS                                                           ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÝÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
