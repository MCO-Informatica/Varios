...
Do Case
	...
	Case _cTipo == "PT000"
         ...
         ElseIf  SEA->EA_MODELO == "18"     // Posicao 018 a 019: Identificacao do Tributo  03-Darf Simples
            _cRetorno := "03"                          
         ElseIf SEA->EA_MODELO == "22" .or. SEA->EA_MODELO == "91"     // Posicao 018 a 019: Identificacao do Tributo  05-ICMS
            _cRetorno := "05"                          
    ...
    Case _cTipo == "PT001"	                           
         ...
         If SEA->EA_MODELO == "16"    
           ...
           // _cRetorno := "02"                         
            ...
            _cRetorno += If(!Empty(SE2->E2_XCODPAG),STRZERO(Val(SE2->E2_XCODPAG),4),STRZERO(Val(SE2->E2_XCODPAG),4))
           ...
           EndIf
            ...
            Else
               _cRetorno += Subs(U_RetSM0(,,"M0_CGC"),1,14)
            EndIf
            ...
            If !Empty(SE2->E2_XCNPJC)
               ...
               EndIf
            Else
               _cRetorno += Subs(U_RetSM0(,,"M0_NOMECOM"),1,30)
            EndIf                                                                      
  ...
  //  Dados FGTS  
         ELSEIf SEA->EA_MODELO == "35"    
           ...
           EndIf
            ...
            // Posicao 087 a 102: IDENTIFICACAO FGTS   
            _cRetorno += Strzero(val(SE2->E2_XIDFGTS),16) 

            // Posicao 103 a 111: LACRE CONECTIVIDADE SOCIAL
            _cRetorno += Strzero(val(SE2->E2_XLACRE),9)                               
            
            // Posicao 112 a 113: DG LACRE             
            _cRetorno += Strzero(val(SE2->E2_XDGLACR),2)         
            
            // Posicao 114 a 143: NOME RAZAO DO CONTRIBUINTE        
            _cRetorno += Subs(U_RetSM0(,,"M0_NOMECOM"),1,30)          

            // Posicao 144 a 151: DATA DO PAGAMENTO
            _cRetorno += GravaData(DATE(),.F.,5)
         ...
         ElseIf (SEA->EA_MODELO == "17")   
            ...
            _cRetorno +=  STRZERO(Val(SE2->E2_XCODPAG),4)  
            ...
            If !Empty(SE2->E2_XCONTR)
               ...
               _cRetorno += Strzero(Val(SE2->E2_XCNPJC),14)
           Else
                _cRetorno += Strzero(Val(U_RetSM0(,,"M0_CGC")),14)
           EndIf
	...
	Case _cTipo == "PR000"		
		...
		_cRetorno = iif(se2->e2_tipo<>"FGT",Strzero((SE2->E2_SALDO+SE2->E2_ACRESC-SE2->E2_DECRESC)*100,14),repl("0",14))


    Otherwise  //  Parametro não existente
      
	 MsgAlert('Não foi encontrado o Parametro '+ _cTipo + "."+;
                  'Solicite à informática para ver o fonte PAGAR341, ou o arquivo de configuração do CNAB.')
