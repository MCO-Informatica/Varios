...
User Function DBRCOTV(cMod)      
	...
	Local cOper   := SUA->UA_OPER // 1 - Faturamento 2 - Cotação
	Local cNumOrc := SUA->UA_NUM	
	Local cNumPed := SUA->UA_NUMSC5
	Local nOpc    := 0

	Local cTexto  := "Selecione a opção que deseja abaixo - Cotação:" + cNumOrc + " - Pedido:" + cNumPed + "              
	Private cPerg:= Padr("COTVEND",10)  

	If(ISINCALLSTACK("TMKA271")) 

		nOpc := Aviso("Impressão Cotação/Pedido", cTexto,{"Pedido","Orçamento","Fechar"}, 2)

		If nOpc = 2 
			If !Empty(cNumOrc)  
				DBRELCOTV(cNumOrc,cNumPed,nOpc)
			Else
				Alert("Não foi possível imprimir essa cotação/orçamento")  
			Endif     
		...
		Elseif nOpc = 1
			If !Empty(cNumPed)
				DBRELCOTV(cNumOrc,cNumPed,nOpc) 
			Else
				Alert("Não é possivel imprimir o Pedido de Vendas")
			Endif
		Endif   
	EndIf	
...
Static Function DBRELCOTV(cNumOrc,cNumPV,nOpc)              
	...
	If nOpc = 1  // Validação para determinar se é Pedido de Vendas ou Orçamento
		lPv := .T.
	Endif 
	...
	If SUA->(DbSeek(xFilial("SUA")+cCodOrc))    
		...
		If SA1->(DbSeek(xFilial("SA1")+SUA->(UA_CLIENTE+UA_LOJA)))
			...
			Else
				oPrint:Say(nLinha1+500,100,"Conforme negociação, enviamos a confirmação do seu pedido de compras para fornecimento do(s) produto(s) abaixo:", oFont11n)
			Endif  		
			...
			If SUB->(DbSeek(xFilial("SUB")+SUA->UA_NUM))
				...
				IF !EMPTY(SUA->UA_CODOBS)
					...
					If !Empty(AllTrim(cObs))
						...
						Else
							oPrint:Say(nLinha4,100,"OBSERVAÇÕES DO PEDIDO: " + cObs,oFont11n)
						Endif    
					EndIf
				EndIf
				nLinha4 := nLinha4 + 100
				...
				Else
					oPrint:Say(nLinha5, 100, "Agradecemos pelo pedido e colocamo-nos a sua disposição.", oFont11n)
				Endif  
...
Static Function FrasesPersonalizadas()
	...
	Else
		Aadd(aRet, "* Esta é a confirmação do seu pedido de compras e qualquer dúvida e/ou divergência, por favor formalizar.")
		Aadd(aRet, "* Caso não recebamos qualquer notificação, consideraremos o pedido em questão confirmado.")
	Endif  
