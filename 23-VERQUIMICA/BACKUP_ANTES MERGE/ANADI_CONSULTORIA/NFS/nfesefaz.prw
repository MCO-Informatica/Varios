		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					(SF2->(FieldPos("F2_CLIREM"))<>0 .And. SF2->(FieldPos("F2_LOJAREM"))<>0 .And. !Empty(SF2->F2_CLIREM + SF2->F2_LOJAREM) .And. (SF2->F2_CLIREM+SF2->F2_LOJAREM <> SF2->F2_CLIENT+SF2->F2_LOJENT .OR. cTpOper == "1" ))
					
						If !Empty(SF2->F2_CLIREM + SF2->F2_LOJAREM)//verifica se existe cliente remessa preenchido
...
if !empty( cString )
	for nX := 129 to 255
		if EncodeUTF8( Chr(nX) ) == nil
			cString := StrTran( cString, Chr( nX ), "." )
		endif
	next
endif
return cString