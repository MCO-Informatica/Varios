...
||| Descricao |                                                            |||
||+-----------+------------------------------------------------------------+||
||| Alteracao | Fabio Oliveira			| Data: 24/01/2019 |||
||+-----------+------------------------------------------------------------+||
||| Uso       |                                                            |||
||+-----------+------------------------------------------------------------+||
||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
		...
		If !GdDeleted(nX,aHeader,aCols)
			If !Empty(aCols[nX][nPosProd])
				...
				*/
				...
				nPesoTBr := nPesoTBr + (aRetCusto[4] * (aCols[nx][nPQtdVer] / aCols[nx][nPosCapa])) 
			EndIf      
		EndIf
	Next Nx         
	M->UA_PESOB := nPesoTot + nPesoTBr 
...
User Function fCfmCusto(aCfmParam)
    ...
    // Para cliente EX, não tem fator de conversão do Cliente
    // Luiz Dalloste - Anadi consultoria - 05/09/2019 
	IF SA1->A1_EST == "EX"
		nZ03ValCal := Round(((nZ03ValCal*nFatPrd)),6)
	Else
...
User Function fCfmCD(aCfmParam)
	...
	nZ03CustoE := Z03->Z03_CUSTOE //( nReComEm + ((nReComEm*nIpiEm)/100) ) * (nBasCalc/nCapacid)
	nZ03FreEnt := nParamFre       //Round(((nFreteTon*(((nBasCalc/nCapacid)*nPesoEm)+nBasCalcKg))/nBasCalc),2) // Alterar esse campo
	nZ03Custo1 := Round(nReComMp+nFreteMp+nZ03CustoE+((nReComMp*nPercExt)/100),2) // VL REF. + FRETC + EMBALAGEM
	...
	nZ03Margem  := &("nMargemD")
	nValExtr   := (Round((nReComMp*nPercExt)/100,2))
	nCalc01    := (nReComMp*(1-nIcmsMp/100))+nFreteMp+nValExtr+nZ03FreEnt  // VL REF. - IMCS + FRETC + vl extra + FRETEV 
	nCalc02    := (nReComMp+nFreteMp+nZ03CustoE+nZ03FreEnt)*((nPercPis+nPercCof)/100) // VALOR PIS+COF
	nCalc03    := nZ03CustoE*((100-nIcmsEm)/100)   // CUSTO EMBALAGEM - ICMS 
	nCalc04    := 100-nIcmsMp-nPercPis-nPercCof-nPercIR-(nCusOpera+nZ03Margem)  // PERCENCUAL MARGEM LIQUIDA
	nCalc05    := ((nCAlc01-nCalc02+nCalc03)/nCalc04)*100  //  
	nZ03Markup := nZ03Custo1/nCalc05
	nZ03Markup := Round(nZ03Markup,4)
