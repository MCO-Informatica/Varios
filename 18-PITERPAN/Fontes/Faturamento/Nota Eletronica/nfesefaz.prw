...
Local _cod_m1 := space(3)
Local _cod_m2 := space(3)
Local _nomeVend := ""
Local _xtes := {}
Local _xestado := {}
Local _ftes := {}
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				cAux := GetNewPar("MV_MSGDAN",.F.)
						cmensfis += " - " + cAux + " - "
						//Final do customizacao da msg via parametro MV_MSGDAN
						If !empty(_cod_m1)
							dbSelectArea("SM4")
							dbSetOrder(1)
							dbSeek(XFilial("SM4")+_cod_m1)
							xMENSAGEM := SM4->M4_FORMULA
							_cAux := xMENSAGEM
							cMensfis += _cAux + " - "
						Endif
						...
						If !Empty(_nomeVend)
							cMensfis += "Vendedor: " + alltrim(_nomeVend) + " - "
						Endif
						cMensFis += "Telefone da Transportadora: " + SA4->A4_TEL  // MSG CUSTOMIZADA - Mauricio
						For _ix := 1 to len(_ftes)
							If _ix <= len(_ftes)
								if _ftes[_ix] $ "834/835" .and. ALLTRIM(_xestado) == "SP"
									dbSelectArea("SM4")
									dbSetOrder(1)
									dbSeek(XFilial("SM4")+"251")
									_xmensagem := SM4->M4_FORMULA
									cmensfis += _xmensagem + " - "
									_ix := len(_ftes)+1
								Endif
							Endif
