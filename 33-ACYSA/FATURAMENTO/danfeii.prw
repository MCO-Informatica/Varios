...
nForTo		:=	Len(cStrAux)/26
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*26)+1),26), oFont12N:oFont )
	nLinCalc+=10
Next nX
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(058,001,cLogoD,094,045)
	Else
		oDanfe:SayBitmap(058,001,cLogo,094,045)
	EndIF
Endif
