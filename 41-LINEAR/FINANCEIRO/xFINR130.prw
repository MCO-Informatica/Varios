...
If FindFunction("TRepInUse") .And. TRepInUse()
 	...
 	oReport:PrintDialog()
Else
	U_xR3FINR130() // Executa versão anterior do fonte
Endif
...
TRCell():New(oSection1,"BANCO",,"Banco",,nTamBan,.F.,,,,,,,.F.)  //"Bco St"
...
Local nGem 			:= 0
Local aFiliais 		:= {}
Local lTotFil		:= (mv_par21 == 1 .And. SM0->(Reccount()) > 1)	// Totaliza e quebra por filial     MARCO
Local aAreaSE5
	...
	*/ 
	...
	#IFDEF TOP
		...
		If ( mv_par41 == 1 ) .and. lVerCmpFil
			...
			*/
			...
			cQuery += " 			WHERE SE1.E1_FILIAL = '"+xFilial('SE5')+"' AND"
			...
			*/
			...
			Else
				...
				Else
					cQuery += " AND E1_FILORIG " + cQryFilSE1
				Endif
			Endif
			/* 
			GESTAO - fim
	...
	While !Eof() .And. SE1->E1_FILIAL == cFilSE1 .And. &cCond1
		...
		While !Eof() .And. SE1->E1_FILIAL == cFilSE1 .And. &cCond2 == cCarAnt
			...
			If ! SE1->E1_TIPO $ MVABATIM	+"/"+MVFUABT
				If ! (SE1->E1_TIPO $ MVRECANT+"/"+MV_CRNEG) .And. ;
                    ...
                    //Busca as informações para alimentar a array aTitImp utilizando a função F130RETIMP     PTO CRITICO
	                dbSelectArea("__SE1")
			   		dbSetOrder(2)
		   ...
		   //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			...
			aDados[VL_ORIG] := Round(NoRound(xMoeda(SE1->E1_VALOR,SE1->E1_MOEDA,mv_par15,SE1->E1_EMISSAO,ndecs+1,Iif(MV_PAR39==2,Iif(!Empty(SE1->E1_TXMOEDA),SE1->E1_TXMOEDA,RecMoeda(SE1->E1_EMISSAO,SE1->E1_MOEDA)),0))* If((SE1->E1_TIPO$MVABATIM +"/"+MVFUABT+"/"+MV_CRNEG+"/"+MVRECANT+"/"+MVABATIM) .OR. lAbatIMPBx, -1,1),nDecs+1),nDecs)
			aDados[VL_NOMINAL] :=0
			aDados[VL_CORRIG]:=0    
			aDados[VL_VENCIDO]:=0
		...
		IF nTit5 > 0 .And. nOrdem != 2 .And. nOrdem != 10 .And. mv_par19 == 2 //1 = Analitico - 2 = Sintetico
			U_xRSubTot130(nTit0,nTit1,nTit2,nTit3,nTit4,nOrdem,cCarAnt,nTotJur,nDecs,oReport,,oSection2)
		Endif
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function xRSubTot130(nTit0,nTit1,nTit2,nTit3,nTit4,nOrdem,cCarAnt,nTotJur,nDecs,oReport,aDados,oSection)
...
While !Eof() .and. M0_CODIGO == cEmpAnt .and. IIf( lFWCodFil, FWGETCODFILIAL, SM0->M0_CODFIL ) <= cFilAte
	...
	While &cCond1 .and. !Eof() .and. lContinua .and. SE1->E1_FILIAL == xFilial("SE1")
		...
		While &cCond2==cCarAnt .and. !Eof() .and. lContinua .and. SE1->E1_FILIAL == xFilial("SE1")
			...
			EndIF
			If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
				@li,185 PSAY SubStr(SE1->E1_HIST,1,16)+ ;
		...
		IF nTit5 > 0 .and. nOrdem != 2 .and. nOrdem != 10 
			U_xSubTot130(nTit0,nTit1,nTit2,nTit3,nTit4,nOrdem,cCarAnt,nTotJur,nDecs)
			If mv_par19 == 1 //1 = Analitico - 2 = Sintetico
				Li++
			EndIf
		Endif
	...
	If mv_par21 == 1 .and. SM0->(Reccount()) > 1 
		If lTemMov 		
			xImpFil130(nTotFil0,nTotFil1,nTotFil2,nTotFil3,nTotFil4,nTotFiltit,nTotFilJ,nDecs,nOrdem)
		EndIf
	Endif
	lTemMov := .F.
	Store 0 To nTotFil0,nTotFil1,nTotFil2,nTotFil3,nTotFil4,nTotFilTit,nTotFilJ
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function xSubTot130(nTit0,nTit1,nTit2,nTit3,nTit4,nOrdem,cCarAnt,nTotJur,nDecs)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
STATIC Function xImpFil130(nTotFil0,nTotFil1,nTotFil2,nTotFil3,nTotFil4,nTotFilTit,nTotFilJ,nDecs,nOrdem)
...
If SX1->(DbSeek(PadR("FIN130",Len(SX1->X1_GRUPO))+"21"))
	aHelpPor := {'Selecione a opção "Sim" para que a','geração do relatório considere as','filiais a serem informadas nos','parâmetros seguintes, ou "Não",','caso contrário. Esta pergunta não terá','efeito em ambientes TOPCONNECT /','TOTVSDBACCESS.'}
...
If SX1->(DbSeek(PadR("FIN130",Len(SX1->X1_GRUPO))+"22"))
	aHelpPor := {'Informe o código inicial do intervalo de',' filiais da sua empresa a serem',' consideradas na geração do relatório,',' quando o parâmetro anterior "Considera',' Filiais Abaixo?" estiver "Sim". Esta',' pergunta não terá efeito em ambientes',' TOPCONNECT / TOTVSDBACCESS.'}
	aHelpSpa := {'Digite el codigo inicial del intervalo','de sucursales de su empresa que se debe','considerar en la generacion del informe,','cuando el parametro anterior "¿Considera',' Siguientes Sucursales?" este "Si". Esta',' pregunta no tendra efecto en el',' entorno TOPCONNECT/TOTVSDBACCES.'} 
	aHelpEng := {'Enter the initial code of your company´s',' branches interval to be considered in',' the report generation when the previous',' parameters ("Consider branches below?")','is marked with "Yes". This question does',' not work','in TOPCONNECT/TOTVSDBACCESS',' environments'}
	PutSX1Help("P.FIN13022.",aHelpPor,aHelpEng,aHelpSpa,.T.) 
Endif
If SX1->(DbSeek(PadR("FIN130",Len(SX1->X1_GRUPO))+"23"))
	aHelpPor := {'Informe o código final do intervalo de','filiais da sua empresa a serem','consideradas na geração do relatório,','quando o parâmetro anterior "Considera','Filiais Abaixo?" estiver "Sim". Esta','pergunta não terá efeito em ambientes','TOPCONNECT / TOTVSDBACCESS.'} 
	aHelpSpa := {'Digite el codigo final del intervalo de','sucursales de su empresa que se debe','considerar en la generacion del informe,','cuando el parametro anterior "¿Considera','Siguientes Sucursales?" este "Si". Esta','pregunta no tendra efecto en el entorno','TOPCONNECT/TOTVSDBACCES.'}
	aHelpEng := {'Enter the final code of your company´s','branches interval to be considered in','the report generation when the previous','parameters ("Consider branches below?")','is marked with "Yes". This question does','not work in TOPCONNECT/TOTVSDBACCESS.','environments.'}
...
If !(SX1->(DBSeek(PadR("FIN130",Len(SX1->X1_GRUPO))+"42")))
	...
	PutSx1( "FIN130", "42", "Seleciona Filiais?" ,"¿Selecciona sucursales?" ,"Select Branches?","mv_chW","N",1,0,2,"C","","","","S","mv_par42","Sim","Si ","Yes","","Nao","No","No","","","","","","","","","",aHelpPor,aHelpEng,aHelpSpa)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function Fr130Cond(cTipos)
Local lRet := .T.
Local dDtContab 
Local dDtDigDis
Local aArea		:= getArea()
