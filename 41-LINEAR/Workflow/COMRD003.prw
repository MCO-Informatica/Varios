...
// Removido valida็ใo do superior, desta forma TODAS a SCs serใo enviadas para o aprovador
//If ! Empty(cSuperior)
//RecLock("SC1",.F.)
//C1_CODAPRO := cSuperior // CODIGO DO APROVADOR
//MsUnlock()
...
// FUNCAO ABAIXO FOI RETIRADA ATE DESCOBRIR O QUE ESTA CAUSANDO ERRO COM OS PRODUTOS QUE NAO SAO DA SPIDER
// THIAGO - SUPERTECH 06/10/11
...
//Local cMailSup	:= "paulo@linciplas.com.br" //UsrRetMail(cAprov)
Local cMailSup 		:= "compras@linciplas.com.br" //UsrRetMail(cAprov)
Local cCopia 		:= "thiago@stch.com.br;"
Local cCopia2		:= "pcpspider@spidercapacetes.com.br;nikolas@spidercapacetes.com.br"
...
If nRec > 0
	...
	oProcess:cBCC     	:= cCopia+IIF(xFilial("SC1")=='02',cCopia2,"")
...
cQuery += " FROM "+RetSqlName("SC1")+" AS C7"
...
If nRec > 0
	...
	cQuery2 := " SELECT *"
	cQuery2 += " FROM "+RetSqlName("SB3")+" AS B3"
	...
	cQuery3 := " SELECT TOP 5 * " //C1_NUM, C1_ITEM, C1_FORNECE, C1_LOJA, A2_NOME, C1_QUANT, C1_UM, C1_PRECO, C1_TOTAL, C1_EMISSAO, C1_DATPRF, C1_COND, C1_QUJE, C1_RESIDUO"
	cQuery3 += " FROM "+RetSqlName("SC1")+" AS C1"
	cQuery3 += " INNER JOIN SA2010 AS A2 ON A2_COD = C1_FORNECE AND A2_LOJA = C1_LOJA"
	cQuery3 += " WHERE C1_FILIAL = '"+TRB->C1_FILIAL+"' AND C1_PRODUTO = '"+TRB->C1_PRODUTO+"'"
