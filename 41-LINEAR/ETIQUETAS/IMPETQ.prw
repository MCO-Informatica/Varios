...
Else
		RptStatus({|| Run04Report(Cabec1,Cabec2,Titulo,nLin) },Titulo)
EndIf
...
Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE == _cDoc+_cSerie
	...
	Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA == SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA
		...
		dbSeek(xFilial("SC9")+SD2->D2_CLIENTE+SD2->D2_LOJA+SD2->D2_PEDIDO+SD2->D2_ITEMPV)
		Do While !Eof() .And. SD2->D2_CLIENTE+SD2->D2_LOJA+SD2->D2_PEDIDO+SD2->D2_ITEMPV == C9_CLIENTE+C9_LOJA+C9_PEDIDO+C9_ITEM .And. xFilial("SC9") == SC9->C9_FILIAL
		...
		If cFilAnt$"0103"
			oPrn:Say(0060, 0250, "LINEAR PLASTICOS"       , oFtGrande )
		Else
...
//		oPrn:Say(0450, 1980, "Rev.3.0 - 11/08/08"                       , oFtPequeno)
		...
		oPrn:Say(0150, 0060, "CÓD.FORNECEDOR:"             , oFtNormal )
		...
		oPrn:Say(0980, 0750, "LOTE DE FABRICAÇÃO:"                , oFtNormal )
		oPrn:Say(0980, 1780, "DATA DE EXPEDIÇÃO:"                 , oFtNormal )
		...
		oPrn:Say(1330, 0060, "No.SÉRIE SEQUENCIAL (S):"    , oFtNormal )
		...
		oPrn:Say(1500, 1400,"PÇS"                          , oFtGigante)
		...
		If !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA == SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA
			...
			If cFilAnt$"0103"
				oPrn:Say(1810, 0250, "LINEAR PLASTICO"       , oFtGrande )
			Else
...
//			oPrn:Say(2200, 1980, "Rev.3.0 - 11/08/08"                       , oFtPequeno)
			...
			oPrn:Say(1900, 0060, "CÓD.FORNECEDOR:"             , oFtNormal )
		    ...
		    oPrn:Say(2730, 0750, "LOTE DE FABRICAÇÃO:"         , oFtNormal )
		    oPrn:Say(2730, 1780, "DATA DE EXPEDIÇÃO:"          , oFtNormal )
		    ...
		    oPrn:Say(3080, 0060, "No.SÉRIE SEQUENCIAL (S):"    , oFtNormal )
			...
			oPrn:Say(3250, 1400,"PÇS"                          , oFtGigante)
...
Private _cTurno    := Space(10)
Private _cOperador := Space(16)
...
If !(SA1->A1_COD == "001741") .And. !(SA1->A1_COD == "001760")
	SetRegua(RecCount())
	...
	Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE == _cDoc+_cSerie
		...
		Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA == SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA
			...
			For _nI := 1  To  _nQtdEtq
				...
				If SM0->M0_CODFIL == "01" .Or. SM0->M0_CODFIL == "04"
					MSCBSAY(083,135,"H SILVA (LINCIPLAS) - GRU / SP" ,"R","B","027")
				Else
...
//              MSCBSAYBAR(060,167,StrZero(_nQtdVen,3)               ,"R","MB07",7.86,.F.,.F.,.F.,,,)
                MSCBSAYBAR(060,167,ALLTRIM(Str(_nQtdVen))            ,"R","MB02",7.86,.F.,.F.,.F.,,,)
...
Static Function Run04Report(Cabec1,Cabec2,Titulo,nLin)
Local _cDoc   := "000000000"
Local _cDoc01 := "         "
Local _cSerie := ""
Local _cSer01 := ""
...
Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE == _cDoc+_cSerie
	...
	Do While !Eof() .And. SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA == SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA
		...
		oPrn:Say(0070, 1050, "TO: "                        , oFtNormal )
//		oPrn:SayBitmap(0160,1060,cPath+"logomae.BMP",0375,0315)
		oPrn:Say(0150, 1450, "TRIMTEC LTDA / INTERTRIM LTDA"      , oFtGrande )
		oPrn:Say(0210, 1450, "AV. HENRY NESTLE, 2600"  , oFtNormal )
		oPrn:Say(0260, 1450, "BAIRRO: GAMA"                     , oFtNormal )
		oPrn:Say(0310, 1450, "CEP: 12286-160"                           , oFtNormal )
		oPrn:Say(0360, 1450, "CAÇAPAVA - SP"                            , oFtNormal )
		oPrn:Say(0410, 1450, "BRASIL"                                   , oFtNormal )
//		oPrn:Say(0450, 1980, "Rev.3.0 - 11/08/08"                       , oFtPequeno)
		...
		oPrn:Say(0980, 0060, "QUANTIDADE (Q):"                    , oFtNormal )
		oPrn:Say(0980, 0750, "LOTE DE FABRICAÇÃO:"                , oFtNormal )
		oPrn:Say(0980, 1780, "FIFO:"                              , oFtNormal )
		oPrn:Say(1070, 0180, STR(SD2->D2_QUANT)                   , oFtGigante)
		MSBAR3("CODE128",10.0,1.5,"Q"+ALLTRIM(STR(SD2->D2_QUANT)) ,oPrn,.F.,Nil,.T.,0.025,0.70,.F.,Nil,"A",.F.)
		If Empty(MV_PAR04)
            oPrn:Say(1120, 0850, AllTrim(_cLote01)+AllTrim(_cLote02)   , oFtGigante)
