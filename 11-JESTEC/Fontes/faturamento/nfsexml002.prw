    ...
    If cTipo == "1"
        ...
        If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
                ...
                If IntTms()
                    ...
                    Else
                        If lSC5 .And. SC5->(FieldPos("C5_MUNPRES")) > 0 .And. !Empty(SC5->C5_MUNPRES)
                            cMunPrest := UFCodIBGE2(SC5->C5_ESTPRES)+SC5->C5_MUNPRES
                            cDescMunP := SC5->C5_ESTPRES+SC5->C5_DESCMUN
                        Else
            ...
            IF SF3->F3_ISSMAT > 0
                ...
                cMensCli += "MATERIAIS: "+Transform(_nMat,"@E 99.99")+"% R$ "+AllTrim(Transform(SF3->F3_ISSMAT,"@E 999,999.99")) +" - SERVICOS: "+Transform(_nSer,"@E 99.99")+"% R$ "+AllTrim(Transform(SF2->F2_VALBRUT - SF3->F3_ISSMAT,"@E 999,999.99")) + Space(1) + Chr(124)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
    ...
    If !Empty(aNota)
        ...
        cString += NFSEItem2(cCodMun,aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aDest, aNota,aTotal,aRetido,aRetServ,aDeducao,cMunPrest,cDescMunP,cMenscli,cF4Agreg,lNFeDesc,cDescrNFSe,cDiscrNFSe,@nDescon) //----> BLOCO CUSTOMIZADO - RICARDO SOUZA - MVG CONSULTORIA - 12/08/2013
...
Static Function NFSEItem2(cCodMun,aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ, aDest, aNota, aTotal,aRetido,aRetServ,aDeducao,cMunPrest,cDescMunP,cMensCli,cF4Agreg,lNFeDesc,cDescrNFSe,cDiscrNFSe,nDescon)  //----> BLOCO CUSTOMIZADO - RICARDO SOUZA - MVG CONSULTORIA - 12/08/2013
...
//Outras retenções, sera colocado o valor 0 (zero), pois atualmente nao existe valor de Outras retencoes
    ...
    If !lNFeDesc
        cString += '<DescricaoRPS>'+AllTrim(cMensCli)+'</DescricaoRPS>' //CUSTOMIZADO POR FELIPE VALENCA 20121029
...
//informar o CPF/CNPJ do Intermediário
    ...
    For Nx := 1 to Len(aProd)
        ...
        Else
            cXml += '<ItemListaServico>'+ConvType2(aProd[Nx][24],5)+'</ItemListaServico>'
        Endif
        If cCodMun $ "5201108-4104808-5103403-3524709-3300704-4313409-1400100" // Anápolis-GO, Cascavel-PR, Cuiabá-MT, Boa Vista-RR
        ...
        EndIf
        If !lNFeDesc
            cXml += '<DiscriminacaoServico>'+AllTrim(cMensCli)+'</DiscriminacaoServico>' //CUSTOMIZADO POR FELIPE VALENCA MVG CONSULTORIA 20121029
