...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
    ...
    Local lRecIrrf  := .T.
    Local lMvred		:= SuperGetMV("MV_NFSERED",,.F.)
    Local aNota     := {}
    Local aDupl     := {}
    Local aDest     := {}
    Local aEntrega  := {}
    ...
    // INICIO DE CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
    Local _nRetPis := 0
    Local _nAliqPis:= 0
    Local _nRetCof := 0
    Local _nAliqCof:= 0
    ...
    Local _cTarefSC6:= ""
    Local _cCodIss  := ""
    Local _nMat 	:= 0 	// Customização Sergio Junior MVG Consultoria - 07/11/2013
    Local _nSer 	:= 0    // Customização Sergio Junior MVG Consultoria - 07/11/2013
// FINAL DE CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
    ...
    Default cMotCancela := IIf(ValType(cMotCancela) == "U","",cMotCancela)
    ...
    If cTipo == "1" .And. Empty(cMotCancela)
        ...
        If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
            ...
            aadd(aRetido,{"PIS",0,nRetPis,SED->ED_PERCPIS,0,aRetPis})
            _nRetPis := nRetPis // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
            _nAliqPis:= SED->ED_PERCPIS // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
            aadd(aRetido,{"COFINS",0,nRetCof,SED->ED_PERCCOF,0,aRetCof})
            _nRetCof := nRetCof // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
            _nAliqCof:= SED->ED_PERCCOF // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
            aadd(aRetido,{"CSLL",0,nRetCsl,SED->ED_PERCCSL,0,aRetCsl})
            _nRetCsl := nRetCsl // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
            _nAliqCsl:= SED->ED_PERCCSL // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012

            If SF2->(FieldPos("F2_VALIRRF"))<>0 .and. SF2->F2_VALIRRF>0
                aadd(aRetido,{"IRRF",SF2->F2_BASEIRR,SF2->F2_VALIRRF,SED->ED_PERCIRF,0,aRetIRR})
                _nRetIRRF := SF2->F2_VALIRRF // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
                _nAliqIR := SED->ED_PERCIRF // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
            EndIf
            If SF2->(FieldPos("F2_BASEINS"))<>0 .and. SF2->F2_BASEINS>0
                aadd(aRetido,{"INSS",SF2->F2_BASEINS,SF2->F2_VALINSS,SED->ED_PERCINS,0,aRetIns})
                _nRetINSS := SF2->F2_VALINSS // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
                _nAliqINS := SED->ED_PERCINS // CUSTOMIZACAO POR FELIPE VALENCA MVG CONSULTORIA - 08/03/2012
            EndIf
                                                                ...
                                                                If(!Empty(cNumMatOb),aadd(aConstr,(cNumMatOb)),aadd(aConstr,"") ) // Numero de Mtricula para Obra
                                                                    If(!Empty(cNumEncap),aadd(aConstr,(cNumEncap)),aadd(aConstr,"") ) // NumeroEncapsulamento
                                                                        ...
                                                                        While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
                                                                            ...
                                                                            // INICIO DE CUSTOMIZACAO FELIPE VALENCA MVG CONSULTORIA 20120411
                                                                            _cProjSC6 := SC6->C6_PROJPMS
                                                                            _cTarefSC6:= SC6->C6_TASKPMS
                                                                            _cCodIss  := SC6->C6_CODISS
                                                                            // FIM DE CUSTOMIZACAO FELIPE VALENCA MVG CONSULTORIA 20120411
                                                                            cFieldMsg := GetNewPar("MV_CMPUSR","")
                                                                            ...
                                                                            mSQL += " WHERE AFC_PROJET='"+_cProjSC6+"' AND D_E_L_E_T_<>'*' AND AFC_FILIAL='"+xFilial ("AFC")+"' "
                                                                            ...
                                                                            _cTarefa := Posicione("AF9",1,xFilial("AF9")+_cProjSC6+_cRevisa+_cTarefSC6,"AF9_EDTPAI")
                                                                            _cPai    := Posicione("AFC",1,xFilial("AFC")+_cProjSC6+_cRevisa+_cTarefa,"AFC_EDTPAI")
                                                                            ...
                                                                            If SC6->(FieldPos("C6_TPDEDUZ")) > 0 .And. !Empty(SC6->C6_TPDEDUZ)
                                                                                ...
                                                                                //----> INICIO DO DESCRITIVO DOS SERVICOS PARA O CLIENTE NEXTEL
                                                                                If SC5->C5_XTIPONF$"C"
                                                                                    cMensCli += If(FindFunction('CleanSpecChar'),CleanSpecChar(Transform((cAliasSD2)->D2_QUANT,"@E 999")),Transform((cAliasSD2)->D2_QUANT,"@E 999"))+Space(1)
                                                                                    cMensCli += If(FindFunction('CleanSpecChar'),CleanSpecChar(SB1->B1_UM),SB1->B1_UM)+Space(1)
                                                                                    cMensCli += If(FindFunction('CleanSpecChar'),CleanSpecChar(Transform((cAliasSD2)->D2_PRCVEN,"@E 999,999.99")),Transform((cAliasSD2)->D2_PRCVEN,"@E 999,999.99"))+Space(1)
                                                                                    cMensCli += If(FindFunction('CleanSpecChar'),CleanSpecChar(Transform((cAliasSD2)->D2_TOTAL,"@E 999,999.99")),Transform((cAliasSD2)->D2_TOTAL,"@E 999,999.99"))+Space(5)
                                                                                    cMensCli += If(FindFunction('CleanSpecChar'),CleanSpecChar(Subs(SC6->C6_DESCRI,1,60)),Subs(SC6->C6_DESCRI,1,60)) + Space(1) + Chr(124)
                                                                                Else

                                                                                    If !AllTrim(SC5->C5_XMENNOT) $ cMensCli
                                                                                        cMensCli +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SC5->C5_XMENNOT)),AllTrim(SC5->C5_XMENNOT))+ Space(1) + Chr(124)
                                                                                    EndIf

                                                                                EndIf
                                                                                //----> FINAL DO DESCRITIVO DOS SERVICOS PARA O CLIENTE NEXTEL
                                                                            ...
                                                                            cMensCli += "" + Space(1) + Chr(124)

                                                                            If Alltrim(_cCodIss) = '01023'
                                                                                cMensCli += "LC 116/03 LC 07.02 " + Space(1) + Chr(124)
                                                                            Elseif Alltrim(_cCodIss) = '01520'
                                                                                cMensCli += "LC 116/03 LC 07.01 " + Space(1) + Chr(124)
                                                                            Elseif Alltrim(_cCodIss) = '07285'
                                                                                cMensCli += "LC 116/03 LC 14.06 " + Space(1) + Chr(124)
                                                                            Elseif Alltrim(_cCodIss) = '01864'
                                                                                cMensCli += "LC 116/03 LC 07.19 " + Space(1) + Chr(124)
                                                                            Elseif Alltrim(_cCodIss) = '01031'
                                                                                cMensCli += "LC 116/03 LC 07.04 " + Space(1) + Chr(124)
                                                                            Elseif Alltrim(_cCodIss) = '01694'
                                                                                cMensCli += "LC 116/03 LC 07.03 " + Space(1) + Chr(124)
                                                                            Elseif Alltrim(_cCodIss) = '02151'
                                                                                cMensCli += "LC 116/03 LC 31.01	" + Space(1) + Chr(124)
                                                                            Endif

                                                                            //			cMensCli += Posicione("AF8",1,xFilial("AF8")+SC6->C6_PROJ,"AF8_DESCRI") + Space(1) + Chr(124)
                                                                            //			cMensCli += Posicione("AFC",1,xFilial("AFC")+SC6->(C6_PROJPMS+_cRevisa+_cPai),"AFC_DESCRI") + Space(1) + Chr(124)


                                                                            If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
                                                                                cMensCli +=If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SC5->C5_MENNOTA)),AllTrim(SC5->C5_MENNOTA))+ Space(1) + Chr(124)
                                                                            EndIf

                                                                            If !Empty(SC5->C5_MENPAD) .And. !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensFis
                                                                                cMensFis += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(FORMULA(SC5->C5_MENPAD))),AllTrim(FORMULA(SC5->C5_MENPAD)))+ Space(1) + Chr(124)
                                                                            EndIf

                                                                            If !Empty(SC5->C5_XPROTOC) //CUSTOMIZADO POR FELIPE VALENCA (MVG CONSULTORIA) 20121029
                                                                                cMensCli +=  "  PROTOCOLO: " + SC5->C5_XPROTOC + Space(1) + Chr(124) //CUSTOMIZADO POR FELIPE VALENCA (MVG CONSULTORIA) 20121029
                                                                            Endif //CUSTOMIZADO POR FELIPE VALENCA (MVG CONSULTORIA) 20121029

                                                                            cMensCli += "SITE: "+AllTrim(Posicione("AF8",1,xFilial("AF8")+_cProjSC6,"AF8_SITE")) + Space(1) + Chr(124)
                                                                            cMensCli += "LOCAL DA PRESTACAO: "+AllTrim(Posicione("AF8",1,xFilial("AF8")+_cProjSC6,"AF8_MUN"))+" - "+Posicione("AF8",1,xFilial("AF8")+_cProjSC6,"AF8_EST") + Space(1) + Chr(124)

                                                                            dbSelectArea("AF9")
                                                                            dbSetOrder(1)
                                                                            dbSeek(xFilial()+_cProjSC6,.f.)
                                                                            
                                                                            IF AF9->AF9_XPERMA < 100
                                                                                // CUSTOMIZACAO SERGIO JUNIOR (MVG CONSULTORIA) 20131107
                                                                                _nMat := AF9->AF9_XPERMA
                                                                                _nSer := 100-AF9->AF9_XPERMA
                                                                                cMensCli += "MATERIAIS: "+Transform(_nMat,"@E 99.99")+"% R$ "+AllTrim(Transform(SF2->F2_BASEINS,"@E 999,999.99")) +" - SERVICOS: "+Transform(_nSer,"@E 99.99")+"% R$ "+AllTrim(Transform(SF2->F2_VALBRUT - SF2->F2_BASEINS,"@E 999,999.99")) + Space(1) + Chr(124)
                                                                                //				cMensCli += "MATERIAIS: 50,00% R$ "+AllTrim(Transform(SF3->F3_ISSMAT,"@E 999,999.99")) +" - SERVICOS: 50,00% R$ "+AllTrim(Transform(SF2->F2_VALBRUT - SF3->F3_ISSMAT,"@E 999,999.99")) + Space(1) + Chr(124)
                                                                                // FIM CUSTOMIZAÇÃO
                                                                            Else
                                                                                cMensCli += "SERVICOS: 100,00% R$ "+AllTrim(Transform(SF2->F2_VALBRUT,"@E 999,999.99"))+ Space(1) + Chr(124)
                                                                            Endif

                                                                            If (Iif(SF3->F3_RECISS $ "1S",SF3->F3_VALICM,0)+_nRetPis+_nRetCof+_nRetCsl+_nRetIRRF+_nRetINSS) > 0
                                                                                cMensCli += Iif(SF3->F3_RECISS $ "1S","RETENCAO: ","")
                                                                                cMensCli += Iif(SF3->F3_RECISS $ "1S", "ISS: "+ Transform(SD2->D2_ALIQISS,"@E 999.99")+ " R$ "+ Transform(SF3->F3_VALICM,"@E 999,999,999.99"),"")+ Space(1) + Chr(124)+ Space(1) + Chr(124)
                                                                                //				Iif(_nRetPis > 0, " PIS: "+ Transform(_nAliqPis, "@E 999.99")+ " R$ "+ Transform(_nRetPis, "@E 999,999,999.99"),"")+;
                                                                                    //				Iif(_nRetCof > 0, " COFINS: "+ Transform(_nAliqCof, "@E 999.99")+ " R$ "+ Transform(_nRetCof, "@E 999,999,999.99"),"")+;
                                                                                    //				Iif(_nRetCsl > 0, " CSLL: "+ Transform(_nAliqCsl, "@E 999.99")+ " R$ "+ Transform(_nRetCsl, "@E 999,999,999.99"),"")+;
                                                                                    //				Iif(_nRetIRRF > 0, " IRRF: "+ Transform(_nAliqIR, "@E 999.99")+ " R$ "+ Transform(_nRetIRRF, "@E 999,999,999.99"),"")+;
                                                                                    //				Iif(_nRetINSS > 0, " INSS: "+ Transform(_nAliqINS, "@E 999.99")+ " R$ "+ Transform(_nRetINSS, "@E 999,999,999.99"),"")+ Space(1) + Chr(124)+ Space(1) + Chr(124)
                                                                            EndIf

                                                                            cMensCli += "VALOR LIQUIDO: " + Transform(SF2->F2_VALBRUT - (if(SF3->F3_RECISS $ "1S",SF3->F3_VALICM,0)+_nRetPis+_nRetCof+_nRetCsl+_nRetIRRF+_nRetINSS) ,"@E 999,999,999.99")+ Space(1) + Chr(124)

                                                                            dbSelectArea("SE1")
                                                                            dbSetOrder(1)
                                                                            If dbSeek(xFilial("SE1")+SF2->(F2_SERIE+F2_DOC),.f.)

                                                                                _nSaldo := 0

                                                                                While Eof() = .f. .And. SE1->(E1_PREFIXO+E1_NUM) == SF2->(F2_SERIE+F2_DOC)

                                                                                    If SE1->E1_TIPO $"NF "

                                                                                        cMensCli += "VENCIMENTO: "+ DtoC(SE1->E1_VENCTO)+ Space(5) + Transform(SE1->(E1_SALDO - E1_IRRF - E1_ISS - E1_INSS - E1_PIS - E1_COFINS - E1_CSLL),"@E 999,999,999.99") + Space(1) + Chr(124)
                                                                            ...
                                                                            If _nRetINSS > 0
                                                                                cMensCli += "OBS: A RETENCAO DO INSS SOMENTE INCIDIRA SOBRE OS SERVICOS PRESTADOS CONFORME ARTS. 149, 150 DA IN 20/2007."+ Space(1) + Chr(124) + Space(1) + Chr(124)
                                                                            Endif
                                                                        ...
                                                                        if !empty(aNota)
                                                                            if len (aProd) > 0
                                                                                ...
                                                                                cString	+= servicos( aProd, aISSQN, aRetido, cMensCli, lNFeDesc, cDiscrNFSe, aCST, aDest[22], if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ), cF4Agreg ,nDescon,cFntCtrb,@aLeiTrp,lRecIrrf )
...
static function substit( aNota )
    ...
    if !empty( allTrim( aNota[1] ) + allTrim( aNota[2] ) )

        cString	+= "<substituicao>"

        cString	+= "<serierps>" + allTrim(aNota[1]) + "</serierps>"
        cString	+= "<numerorps>" + allTrim(str( val(aNota[2]))) + "</numerorps>"
        cString	+= "<idnfse>" + aNota[1] + allTrim(aNota[2]) + "</idnfse>"
        cString	+= "<tipo>1</tipo>"
...
/*/
//-----------------------------------------------------------------------
static function servicos( aProd, aISSQN, aRetido, cMensCli, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf)
    ...
    for nX := 1 to len( aProd )
        ...
        ElseIf !lNFeDesc
            cString	+= "<discr>" + AllTrim(cMensCli)+ cCargaTrb + "</discr>"
        Else
